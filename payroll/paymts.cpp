...
static char Use_Rate_Accounts=FALSE;
static char job_rollsover_deduction=FALSE,EncumberPayroll=FALSE;
static char print_style=FALSE,DID_STUFF=FALSE;
...
static void okbpaymstr(MakeBrowseList *b,void *targetkey)
{
	...
	FINDRSCSETEDITABLE(b->mainrsrc,"BROWSE LIST",FALSE);
...
*/
	...
	if(!RDAstrcmp(errorlist->libs[0],"No Errors Detected") && errorlist->numlibs==1)
	{		
		FINDRSCSETEDITABLE(b->mainrsrc,"PRINT ERROR LIST",FALSE);
		FINDRSCSETSENSITIVE(b->mainrsrc,"PRINT ERROR LIST",FALSE);
	} else {
		FINDRSCSETSENSITIVE(b->mainrsrc,"PRINT ERROR LIST",TRUE);
		FINDRSCSETEDITABLE(b->mainrsrc,"PRINT ERROR LIST",TRUE);
	}
	...
	if(!RDAstrcmp(warnlist->libs[0],"No Warnings Detected") && warnlist->numlibs==1)
	{		
		FINDRSCSETEDITABLE(b->mainrsrc,"PRINT WARNING LIST",FALSE);
		FINDRSCSETSENSITIVE(b->mainrsrc,"PRINT WARNING LIST",FALSE);
	} else {
		FINDRSCSETEDITABLE(b->mainrsrc,"PRINT WARNING LIST",TRUE);
		FINDRSCSETSENSITIVE(b->mainrsrc,"PRINT WARNING LIST",TRUE);
	}
...
static void printlistformatted(RDA_PFILE *fp,APPlib *list)
{
	...
	for(x=0;x<list->numlibs;++x)
	{
		...
		sprintf(temp1,"%s",list->libs[x]);
		y=0;
		while(y<(RDAstrlen(temp1)))
		{
			if(print_style==FALSE)
			{
				if(y==0)
				{
			...
			++plines;
			if((print_style==FALSE && plines==58) || (print_style==TRUE && plines==76))
			{
				RDA_fprintf(fp,"\f");
				plines=3;	
			}
...
static void printerrorlist(RDArsrc *parent,RDA_PFILE *fp)
{
	...
	if(print_style==FALSE) w=80; else w=128;
	if(fp!=NULL)
	{
			date=GETCURRENTDATE();
			timex=GETCURRENTTIME();
			RDA_fprintf(fp,"%*sXPERT PAYROLL MAKE TRANSACTIONS ERROR LIST%*sDate: %*s\r\n",w/2-21,"",w/2-35,"",8,date);
			RDA_fprintf(fp,"%*sExecuted By: %s%*sTime: %s\r\n\r\n",
				((w-(13+RDAstrlen(USERLOGIN)))/2),
				"",USERLOGIN,
				(((w-(13+RDAstrlen(USERLOGIN)))/2)-14),"",
				timex);
...
void printerrorlistcb(RDArsrc *prsrc,RDArsrc *parent)
{
	...
	if(outdevice!=NULL)
	{
		...
		if(fp!=NULL)
		{
			...
			if(DID_STUFF)
			{
			temp=stralloc((mbl_paymstr->list->keyvalue!=NULL ? mbl_paymstr->list->keyvalue:""));
			unpad(temp);
			RDA_SetEnv("SELECTED_PAY_IDENTIFICATION",(temp!=NULL ? temp:""));
			args=APPlibNEW();
			addAPPlib(args,"PAYROLL");
			addAPPlib(args,"FINDITAUTO");
			Execute2Program("doreport",args);
			if(args!=NULL) freeapplib(args);
			RDA_UnSetEnv("SELECTED_PAY_IDENTIFICATION");
			RDA_SetEnv("PAY_IDENTIFICATION",(temp!=NULL ? temp:""));
			args=NULL;
			Execute2Program("payunmts",args);
			RDA_UnSetEnv("PAY_IDENTIFICATION");
			}
			DO_AUDIT=FALSE;
			FINDRSCSETEDITABLE(parent,"PRINT ERROR LIST",FALSE);
			FINDRSCSETSENSITIVE(parent,"PRINT ERROR LIST",FALSE);
		} else {
...
static void printwarnlist(RDArsrc *parent,RDA_PFILE *fp)
{
	...
	if(fp!=NULL)
	{
			date=GETCURRENTDATE();
			timex=GETCURRENTTIME();
			RDA_fprintf(fp,"%*sXPERT PAYROLL MAKE TRANSACTIONS WARNING LIST%*sDate: %*s\r\n",w/2-22,"",w/2-36,"",8,date);
			RDA_fprintf(fp,"%*sExecuted By: %s%*sTime: %s\r\n\r\n",
				((w-(13+RDAstrlen(USERLOGIN)))/2),
				"",USERLOGIN,
				(((w-(13+RDAstrlen(USERLOGIN)))/2)-14),"",
...
/*lint -e611 */
	addrfcbrsrc(mrsrc,"PRINT ERROR LIST",FALSE,print_list,(void *)printerrorlistcb);
	addrfcbrsrc(mrsrc,"PRINT WARNING LIST",FALSE,print_list,(void *)printwarnlistcb);
/*lint +e611 */
	mbl_paymstr=ADVSbrowse(paymstr,1,mrsrc,srsrc,drsrc,NULL,NULL,selectbpaymstr,quitbpaymstr,
