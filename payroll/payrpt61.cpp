/* PAYROLL's - Make (PAYSELA-CHAMBK) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
----------------------------------------------------------------------*/
	...
	sprintf(rptname,"PAYSELA-CHAMBK");
	t=Rmalloc(26+1);
	sprintf(t,"Select Payroll Audit Trail");
	desc=Rmalloc(101+1);
	sprintf(desc,"This output definition is used to provide an audit trail for Select Payroll for the City of Chamblee.");
	tmp=RDAreportNEWADV(rptname,t,desc,1,"",FALSE,TRUE,FALSE,TRUE,TRUE,"PAYROLL","PAYSELA1-CHAMB",FALSE,1,FALSE,"");
	if(t!=NULL) Rfree(t);
...
------------------------------------------------------------------------*/
	...
	t=Rmalloc(165+1);
	sprintf(t,"([PAYROLL][PAYJPRM][DELETEFLAG]=FALSE) AND ([PAYROLL][PAYJPRM][SOURCE USER]=[LOGIN IDENTIFICATION]) AND ([PAYROLL][PAYJPRM][PAY IDENTIFICATION]=[PAY IDENTIFICATION])");
	x=addsearchfileADV(tmp,"PAYROLL","PAYJPRM",0,NULL,NULL,"RDA PAYJPRM KEY",FALSE,t,NULL,1);
	if(x!=(-1))
	{
		s=tmp->search+(x-1);
		addsearchconfld(s,"PAY IDENTIFICATION","[PAY IDENTIFICATION]");
	...
	x=addsearchfileADV(tmp,"POSTRK","POSMSTR",1,"PAYROLL","PAYJPRM","POSMSTR",FALSE,NULL,NULL,1);
	if(x!=(-1))
	{
		s=tmp->search+(x-1);
...
------------------------------------------------------------------------*/
	...
	if(t!=NULL) Rfree(t);
	addvirtuallineADV(tmp,"PAY IDENTIFICATION",1,15,t,0,3);
...
RSNNL][PERDMG][LAST NAME]+\", \"+[PRSNNL][PERDMG][FIRST NAME]+\" \"+[PRSNNL][PERDMG][MIDDLE NAME])");
	...
	if(t!=NULL) Rfree(t);
	t=Rmalloc(630+1);
	sprintf(t,"IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"UNIT\") THEN RETURN_VALUE=0\nELSE IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"SALARY\") THEN RETURN_VALUE=0\nELSE IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"OVERTI\
ME-BASE\") THEN RETURN_VALUE=1 \nELSE IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"OVERTIME-ABOVE\") THEN RETURN_VALUE=2\nELSE IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"ADJUSTMENTS\") THEN RETURN_VALUE=3\nEL\
SE IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"LEAVE DOCKED\") THEN RETURN_VALUE=4\nELSE IF([POSTRK][POSRATE][RATE IDENTIFICATION]=\"SICK PAY\") THEN RETURN_VALUE=5\nELSE RETURN_VALUE=99\nFI FI FI FI FI FI F\
I");
...
NOWN???\" FI FI FI FI FI FI FI FI FI FI FI FI");
	addvirtuallineADV(tmp,"CALENDAR MONTH STRING",1,12,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(133+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"LEAVE DOCKED\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"LEAVE DOCK AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(125+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"UNIT\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"UNIT AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(127+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"SALARY\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"SALARY AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(135+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"OVERTIME-ABOVE\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"OVERTIME-ABOVE AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(134+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"OVERTIME-BASE\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"OVERTIME-BASE AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(132+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"ADJUSTMENTS\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"ADJUSTMENTS AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(129+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"SICK PAY\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"SICK PAY AMOUNT",2,20,t,0,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(133+1);
	sprintf(t,"IF([PAYROLL][PAYJPRM][RATE IDENTIFICATION]=\"VACATION PAY\") \nTHEN RETURN_VALUE=[PAYROLL][PAYJPRM][GROSS] \nELSE RETURN_VALUE=0 \nFI");
	addvirtuallineADV(tmp,"VACATION PAY AMOUNT",2,20,t,0,0);
...
----------------------------------------------------------------------*/
	...
	addaccumline(tmp,"TOTAL LEAVE DOCK AMOUNT",7,"[LEAVE DOCK AMOUNT]",0);
	addaccumline(tmp,"TOTAL SICK PAY AMOUNT",7,"[SICK PAY AMOUNT]",0);
	addaccumline(tmp,"TOTAL VACATION PAY AMOUNT",7,"[VACATION PAY AMOUNT]",0);
	addaccumline(tmp,"TOTAL UNIT AMOUNT",7,"[UNIT AMOUNT]",0);
	addaccumline(tmp,"TOTAL SALARY AMOUNT",7,"[SALARY AMOUNT]",0);
	addaccumline(tmp,"TOTAL OVERTIME-BASE AMOUNT",7,"[OVERTIME-BASE AMOUNT]",0);
	addaccumline(tmp,"TOTAL OVERTIME-ABOVE AMOUNT",7,"[OVERTIME-ABOVE AMOUNT]",0);
	addaccumline(tmp,"TOTAL ADJUSTMENTS AMOUNT",7,"[ADJUSTMENTS AMOUNT]",0);
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","          @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC    PAGE:@RRRRRRR@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","                                                   SELECT PAYROLL AUDIT TRAIL                                            DATE:@LLLLLLL@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","           @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC    TIME:@LLLLLLL@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[PAYROLL][PAYJPRM][PAY IDENTIFICATION]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][PAY IDENTIFICATION] HEADER","Pay ID: @LLLLLLLLLLLLLLLLLLLLL  Pay Date: @LLLLLLLLL  Description: @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL@@@",FALSE,NULL,6,pval);
	free_pvals(pval,6);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][PAY IDENTIFICATION] HEADER","Calendar Year-Mo.: @RRR -@R (@LLLLLLLLLLL )  Fiscal Year-Mo.: @RRR -@R (@LLLLLLLLLL )@@@",FALSE,NULL,9,pval);
	free_pvals(pval,9);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","  PersonnelID  Name                       Job  Pay Location  Position ID        Gross ID       Rate ID       Units           Gross @@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[PAYROLL][PAYJPRM][PERSONNEL IDENTIFICATION]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[PAYROLL][PAYJPRM][RATE IDENTIFICATION]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[PAYROLL][PAYJPRM][UNITS]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[PAYROLL][PAYJPRM][GROSS]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","  @LLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLLLLLL @LLL  @LLLLLLLLLLL @LLLLLLLLLLLLLLLLL @LLLLLLLLLLL   @LLLLLLLLLLLL @RRRRR.RR @,.RRRRRRRRRRRR@@",TRUE,"[FREQUENCY]=1",11,pval);
	free_pvals(pval,11);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","                                          @LLL               @LLLLLLLLLLLLLLLLL @LLLLLLLLLLL   @LLLLLLLLLLLL @RRRRR.RR @,.RRRRRRRRRRRR@@",TRUE,"[FREQUENCY]>1",8,pval);
	free_pvals(pval,8);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][PERSONNEL IDENTIFICATION] FOOTER","                                                                                                             --------- ---------------@@",TRUE,"([FREQUENCY]>1)",2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][JOB NUMBER] FOOTER","                                                                                                             --------- ---------------@@",TRUE,"([FREQUENCY]>1)",2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][PAY IDENTIFICATION] FOOTER","                                                                                                             --------- ---------------@@",TRUE,"([FREQUENCY]>1)",2,pval);
	free_pvals(pval,2);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL UNITS]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][PERSONNEL IDENTIFICATION] FOOTER","                                                  TOTAL PERSONNEL IDENTIFICATION: @LLLLLLLLLLLLLLLLLLLLLLLL  @RRRRR.RR @,.RRRRRRRRRRRR@@",TRUE,"([FREQUENCY]>1)",5,pval);
	free_pvals(pval,5);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][JOB NUMBER] FOOTER","                                                                                        TOTAL JOB #: @RRRRR  @RRRRR.RR @,.RRRRRRRRRRRR@@",TRUE,"([FREQUENCY]>1)",5,pval);
	free_pvals(pval,5);
	...
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPRM][PAY IDENTIFICATION] FOOTER","                                                 TOTAL PAY IDENTIFICATION: @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL  @RRRRR.RR @,.RRRRRRRRRRRR@@@",TRUE,"([FREQUENCY]>1)",6,pval);
	free_pvals(pval,6);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                                                                                                  TOTAL ALL  @RRRRR.RR @,.RRRRRRRRRRRR@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL SALARY AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Salary Amount:                      @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL UNIT AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Unit Amount:                        @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL OVERTIME-BASE AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Overtime-Base Amount:               @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL OVERTIME-ABOVE AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Overtime-Above Amount:              @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL ADJUSTMENTS AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Adjustments Amount:                 @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL LEAVE DOCK AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Leave Dock Amount:                  @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL SICK PAY AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Sick Pay Amount:                    @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL VACATION PAY AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                  Total Vacation Pay Amount:                @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                         TOTAL GROSS AMOUNT:                @,.RRRRRRRRRRRRRRRRRR@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
