/* payjdfm.c - Xpert Payroll Job Deduction Fiscal Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjdfm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjdfm.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_payjdfm=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjdfm(RDArsrc *,RDATData *);
...
void payjdfmm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjdfm,previous);
	...
	if(mbl_payjdfm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_payjdfm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_payjdfm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_payjdfm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_payjdfm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_payjdfm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJDFM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJDFM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJDFM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJDFM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjdfm!=NULL) mbl_payjdfm=NULL;
}
