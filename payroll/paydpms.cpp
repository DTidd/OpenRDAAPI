/* paydpms.c - Xpert Deduction Pay Master Browse */
/*lint -library */
#ifndef WIN32
#define __NAM__ "paydpms.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paydpms.exe"
#endif
#include <app.hpp>
...
static void paydpmsm(short,void *);
static void doexit(MakeBrowseList *),okfunc(MakeBrowseList *,void *);
static void new_record(RDArsrc *,MakeBrowseList *);
static void quit_record(RDArsrc *);
static MakeBrowseList *mbl_paydpms=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_paydpms(RDArsrc *);
...
static void browse_paydpms(RDArsrc *mtnrsrc)
{
	...
	mbl_paydpms=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYDPMS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYDPMS");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYDPMS",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYDPMS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_paydpms!=NULL) mbl_paydpms=NULL;
}
...
static void paydpmsm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_paydpms,NULL);
	...
	if(mbl_paydpms!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	APPmakescrn(mtnrsrc,FALSE,quit_record,NULL,(mbl_paydpms==NULL?TRUE:FALSE));
}
