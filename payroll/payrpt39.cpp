/* PAYROLL's - Make (VWCHECK-CCH) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
extern char *module;
void VWCHECK_CCH(void);
static void VWCHECK_CCH_RANGE_SCREEN(void);
...
------------------------------------------------------------------------*/
	...
	adddisplayADV(tmp,(double)8.500000,(double)0.000000,0,TRUE,1,"raddogdlp",10,1,0,NULL);
...
------------------------------------------------------------------------*/
	...
	t=Rmalloc(2+1);
	sprintf(t,"10");
	addvirtuallineADV(tmp,"SELECTED BODY COUNT",18,2,t,0,0);

	if(t!=NULL) Rfree(t);
	addvirtuallineADV(tmp,"MESSAGE #1",0,0,t,1,2);

	if(t!=NULL) Rfree(t);
	addvirtuallineADV(tmp,"MESSAGE #2",0,0,t,1,2);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(21+1);
	sprintf(t,"SPELL([CHECK AMOUNT])");
	addvirtuallineADV(tmp,"SPELLED TOTAL AMOUNT",0,0,t,1,0);
...
----------------------------------------------------------------------*/
	...
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","     XXX@@",TRUE,"[BODY COUNT]=0",2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","    Deduction ID                                                            Amount @@",TRUE,"[BODY COUNT]=0",2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","   @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL@@",TRUE,"[BODY COUNT]>0",3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","      Check Date      Control Number @@",TRUE,"[BODY COUNT]>0",2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","      @LLLLLLLL       @RRRRRRRRRR@@",TRUE,"[BODY COUNT]>0",4,pval);
	free_pvals(pval,4);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[SPELLED TOTAL AMOUNT]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","     @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL@@",TRUE,"([LAST VIRTUAL CHECK NUMBER]=[LAST CHECK NUMBER]) AND ([BODY COUNT]>0)",3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","     ******  VOID  *******  VOID *******  VOID  *******  VOID *******  VOID *******@@",TRUE,"(NOT([LAST VIRTUAL CHECK NUMBER]=[LAST CHECK NUMBER])) AND ([BODY COUNT]>0)",2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","                                               @LLLLLLL @RRRRRRRRRR @*,.RRRRRRRRR@@",TRUE,"([LAST VIRTUAL CHECK NUMBER]=[LAST CHECK NUMBER]) AND ([BODY COUNT]>0)",5,pval);
	free_pvals(pval,5);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","                                               @LLLLLLL @RRRRRRRRRR $*******0.00 @@",TRUE,"(NOT([LAST VIRTUAL CHECK NUMBER]=[LAST CHECK NUMBER])) AND ([BODY COUNT]>0)",4,pval);
	free_pvals(pval,4);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			VWCHECK_CCH_RANGE_SCREEN();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
/* PAYROLL's - Make (VWCHECK-CCH RANGE SCREEN) Screen */
static void VWCHECK_CCH_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","VWCHECK-CCH RANGE SCREEN");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"RANGE ON [PAYDDSV][CHECK NUMBER]","Range on Check Number","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","From:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"FROM [PAYDDSV][CHECK NUMBER]","From Check Number","","",0,6,0,"","","","");
		ADVaddwdgt(scrn,5,"","To:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"TO [PAYDDSV][CHECK NUMBER]","To Check Number","","",0,6,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"RANGE ON [PAYDDSV][VENDOR IDENTIFICATION]","Range on Vendor Identification","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","From:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"FROM [PAYDDSV][VENDOR IDENTIFICATION]","From Vendor Identification","","",0,20,0,"","","","");
		ADVaddwdgt(scrn,5,"","To:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"TO [PAYDDSV][VENDOR IDENTIFICATION]","To Vendor Identification","","",0,20,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"RANGE ON [PAYDDSV][FISCAL YEAR]","Range on Fiscal Year","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","From:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"FROM [PAYDDSV][FISCAL YEAR]","From Fiscal Year","","",0,4,0,"","","","");
		ADVaddwdgt(scrn,5,"","To:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"TO [PAYDDSV][FISCAL YEAR]","To Fiscal Year","","",0,4,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Selected Bank Identification:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"SELECTED BANK IDENTIFICATION","Selected Bank Identification","","",0,30,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Message #1:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"MESSAGE #1","Message #1","","",0,20,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Message #2:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"MESSAGE #2","Message #2","","",0,20,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
