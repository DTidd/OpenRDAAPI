/* paypfsm.c - Xpert Payroll Fiscal Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "paypfsm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paypfsm.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_paypfsm=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_paypfsm(RDArsrc *,RDATData *);
...
void paypfsmm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_paypfsm,previous);
	...
	if(mbl_paypfsm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_paypfsm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_paypfsm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_paypfsm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_paypfsm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_paypfsm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYPFSM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYPFSM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYPFSM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYPFSM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_paypfsm!=NULL) mbl_paypfsm=NULL;
}
