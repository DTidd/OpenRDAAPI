...
#ifndef WIN32
#define __NAM__ "paymts.2005.09.15.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paymts.2005.09.15.exe"
#endif
#include <app.hpp>
...
static PAYMTS *BlendedRates(PAYMTS *mts,char *perid,int jobno,char *payid,double jobgross,RDArsrc *diagrsrc,int fiscal_year)
{
	...
	if(!BlendExists(mts,perid,jobno,payid))
	{
		for(x=0,a=mts->a;x<mts->num;++x,++a)
		{
			AddAccountStruct(blend,a->accno,a->deftype,a->account,a->debit,a->credit,a->pdeftype,a->payable,a->cash,a->due_from,a->enc_code,a->res_code,a->prorate,a->percentage,a->amount,a->disenc);
		}
		return(blend);
	}
	ZERNRD(payjprm);
			...
			for(x=0,a=mts->a;x<mts->num;++x,++a)
			{
				...
				if(amount!=0)
				{
					if((x+1)<mts->num || total_rgross!=jobgross)
					{
						...
						AddAccountStruct(blend,a->accno,a->deftype,new_acc,new_deb,new_cred,a->pdeftype,new_pay,new_cash,new_due,new_enc,new_res,TRUE,p,0.0,a->disenc);
					} else {
						AddAccountStruct(blend,a->accno,a->deftype,new_acc,new_deb,new_cred,a->pdeftype,new_pay,new_cash,new_due,new_enc,new_res,FALSE,0.0,0.0,a->disenc);
					}
				}
	...
	if((jobgross-total_rgross)!=0.0)
	{
		...
		for(x=0,a=mts->a;x<mts->num;++x,++a)
		{
			...
			if(amount!=0)
			{
				...
				{
					p=(round(amount/jobgross*10000))/100;	
					AddAccountStruct(blend,a->accno,a->deftype,a->account,a->debit,a->credit,a->pdeftype,a->payable,a->cash,a->due_from,a->enc_code,a->res_code,TRUE,p,0.0,a->disenc);
				} else {
					AddAccountStruct(blend,a->accno,a->deftype,a->account,a->debit,a->credit,a->pdeftype,a->payable,a->cash,a->due_from,a->enc_code,a->res_code,FALSE,0.0,0.0,a->disenc);
				}
			}
		}
	}
...
/*
								prterr("acc->amount [%f] amount [%f] a_total [%f] total_amount [%f] gross [%f] x [%d] pmts->num [%d] ",acc->amount,amount,a_total,total_amount,gross,x,pmts->num);TRACE;
*/
								if(acc->amount<0.0) 
								{
									amount=acc->amount;
...
/*
										prterr("		amount [%f] total_amount [%f] ",amount,total_amount);TRACE;
*/
										if(amount!=0.0)
										{
											FINDFLDSETDOUBLE(paydtrn,"AMOUNT",amount);
