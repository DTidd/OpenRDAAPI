...
extern char *module;
void PAYDIST(void);
static void PAYDIST_RANGE_SCREEN(void);
...
----------------------------------------------------------------------*/
	...
	memset(stemp,0,101);
	rptname=Rmalloc(7+1);
	sprintf(rptname,"PAYDIST");
	t=Rmalloc(20+1);
	sprintf(t,"Payroll Distribution");
	desc=Rmalloc(130+1);
	sprintf(desc,"This output definition is used to print the Payroll Distribution report for verification before actually updating the database(s).");
	tmp=RDAreportNEWADV(rptname,t,desc,1,"",FALSE,TRUE,FALSE,TRUE,TRUE,"PAYROLL","PAYDIST1",TRUE,1,FALSE,"");
	if(t!=NULL) Rfree(t);
...
");
	...
	addvirtuallineADV(tmp,"REPORT TITLE",1,60,t,1,0);
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","               @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC        Date: @LLLLLLL@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[ACCOUNT CODE] HEADER","@LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL@@",TRUE,"([ACCOUNT TYPE]<2)",4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PERSONNEL IDENTIFICATION] FOOTER","   @LLLLLLLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL @RRRR                 @,.RRRRRRRRRRRRRRRRR@@",TRUE,"([LAST ACCOUNT TYPE]<2)",6,pval);
	free_pvals(pval,6);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[ACCOUNT CODE] FOOTER","  TOTAL @LLLLLLLLLLLLLLLLLLLLLLLLLLLL                                               @,.RRRRRRRRRRRRRRRRR@@",TRUE,"[LAST ACCOUNT TYPE]<2",4,pval);
	free_pvals(pval,4);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			PAYDIST_RANGE_SCREEN();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
/* PAYROLL's - Make (PAYDIST RANGE SCREEN) Screen */
static void PAYDIST_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","PAYDIST RANGE SCREEN");
	if(scrn!=NULL)
	{
		...
		ADVaddwdgt(scrn,9,"SHOW EXPENDITURE DETAIL","Show Expenditure Detail on Printout","","",0,0,0,"","","","");
