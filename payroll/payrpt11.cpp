/* PAYROLL's - Make (CHGMSCSAL1) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
----------------------------------------------------------------------*/
	...
	sprintf(rptname,"CHGMSCSAL1");
	t=Rmalloc(29+1);
	sprintf(t,"Change Miscellaneous Salaries");
	desc=Rmalloc(142+1);
	sprintf(desc,"This output definition is used to manage the Change of Miscellaneous Salaries.  This report actually updates the miscellaneous salary changes.");
	tmp=RDAreportNEWADV(rptname,t,desc,1,"",TRUE,FALSE,FALSE,FALSE,FALSE,"","",TRUE,0,TRUE,"");
	if(t!=NULL) Rfree(t);
...
----------------------------------------------------------------------*/
	...
	pvals->fieldname=stralloc("[PAYROLL][PAYJMST][MISC SALARY AMOUNT]");
	addimageline(tmp,"DETAIL LINES","WRITE [PAYROLL][PAYJMST]",TRUE,"([NEW SALARY]!=[PAYROLL][PAYJMST][MISC SALARY AMOUNT])",3,pval);
	free_pvals(pval,3);
