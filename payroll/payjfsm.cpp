/* payjfsm.c - Xpert Payroll Job Fiscal Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjfsm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjfsm.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_payjfsm=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjfsm(RDArsrc *,RDATData *);
...
void payjfsmm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjfsm,previous);
	...
	if(mbl_payjfsm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_payjfsm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_payjfsm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_payjfsm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_payjfsm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_payjfsm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJFSM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJFSM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJFSM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJFSM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjfsm!=NULL) mbl_payjfsm=NULL;
}
