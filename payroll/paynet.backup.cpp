...
#ifndef WIN32
#define __NAM__ "paynet.backup.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paynet.backup.exe"
#endif
#include <app.hpp>
...
void SetDJobBase(DeductionStruct *d,int jobno,double gross_amt,double ls)
{
	JobStruct *job=NULL;
	int x;
	...
	while(!ef)
	{
		...
		if(!deleteflag && active)
		{
			...
			if(!ADVEQLNRDsec(payddms,1,ComputeSubData,pass_struct)) 
			{
				...
				if((!deleteflag && active && strncmp(ddtype,"DIRECT DEP",10)) || (!deleteflag && active && ach && !strncmp(ddtype,"DIRECT DEP",10)))
				{
			...
			if(c->d!=NULL)
			{
				...
				} else if(!RDAstrcmp(filename,"LIFE INSURANCE SALARY"))
				{
					...
					if(x<c->num)
					{
						...
						{
							sprintf(stemp,"(%f)",d->life_insurance_salary);
						} else sprintf(stemp,"%f",d->life_insurance_salary);
						str_length=RDAstrlen(stemp);
						if(RDAstrlen(*tmp)<RDAstrlen(stemp))
						{
							memset(*tmp,0,RDAstrlen(*tmp)+1);
...
static void quitbpaymstr(MakeBrowseList *blist)
{
	...
	if(printed_warnlist==FALSE && warnlist->numlibs>0) 
	{
...
/* set files for expression */
						...
						if(!isEMPTY(d->rate_selection))
						{
							...
							{
								d->base+=gross_amt;
								SetDJobBase(d,jobno,gross_amt,ls);
...
/* Write PAYDPMS records */
					...
					{
						d->ratio=((RDAroundup(d->affect_base/d->annual_affect_base*1000000))/1000000);
					} else d->ratio=0;
					if(d->jobs!=NULL)
					{
						ctotal_net=0;
...
*/
						...
						for(y=0,job=d->jobs;y<d->numjobs;++y,++job)
						{
							...
							if(amt2!=0.0 && d->reduce_net==FALSE && !RDAstrcmp(dedtype,"REIMBURSEMENT"))
							{
					...
					if(!strncmp(dedtype,"DIRECT DEP",10))
					{
						...
						FINDFLDSETDOUBLE(paydpms,"GROSS",d->affect_base);
