/* payw2ss.c - Create Magnetic Media for W2's */
#ifndef WIN32
#define __NAM__ "payw2mm1996.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payw2mm1996.exe"
#endif
#include <app.hpp>
...
static void w2employer_header(RDArsrc *prtrsrc,PassableStruct *p,int cal_yr,short w2type)
{
	FILE *fp=NULL;
/*
	DBsort *sortfile=NULL;
	...
	{	
		...
		if(tran_ein!=NULL)
		{
			if(!strstr(tran_ein,"-"))
			{
				...
				for(x=0;x<RDAstrlen(tran_ein);++x)
				{
					if(tran_ein[x]!='-')
					{
...
/* begin Employer Header */
					...
					{
						w2employer_header(prtrsrc,p,cal_yr,w2type);
...
/* end Employer Header */
					...
					if(!ADVEQLNRDsec(perdmg,1,SCRNvirtualSubData,mainrsrc))
					{
						...
						fprintf(fp,"2W%-7s%-1s%-7s%-1s%-9s%-1s%-6s%-9s%-9s%-1s%-9s%-7d%-7s%-7s%-7s%-7s%-9s%-1s%-1s%-1s%-1s%-1s%-9s%-1s%-7s\r\n",box3_string," ",box7_string," ",box1_string," ",box4_string,box2_string,box11A_string," ",box11B_string,cont_no,box13C_string,"0000000",box9_string,box8_string,box12_string," ",(penplan==TRUE?"P":" ")," ",(defcomp==TRUE?"D":" ")," ",box13DEFGH_string," ",box10_string);
						if(report==TRUE)
						{
						...
						if(MakeSRecords==TRUE)
						{
						fprintf(fp,"2S%-12s%-4s%-9s%-9s%-2s%-4s%-4s%-5s%-2s%-9s%-8s%-10s%-1s%-5s%-9s%-7s%7d%-19s\r\n",(st1_ein!=NULL?st1_ein:""),strptperiod,"000000000","000000000","52",(!isEMPTY(stempdate)?stempdate:"    "),"    ","     ",(state_number_code!=NULL?state_number_code:""),st1_gross_string,st1_amount_string,"          ",(!isEMPTY(loc1_label)?"C":" "),(loc1_label!=NULL?loc1_label:""),loc1_gross_string,loc1_amount_string,cont_no," ");
/*
						fprintf(fp,"2S%-12s%-4s%-9s%-9s%-2s%-4s%-4s%-5s%-2s%-9s%-8s%-10s%-1s%-5s%-9s%-7s%7d%-19s\r\n",(st1_ein!=NULL?st1_ein:""),strptperiod,st1_gross_string,st1_gross_string,"52",(!isEMPTY(stempdate)?stempdate:"    "),"    ","     ",(state_number_code!=NULL?state_number_code:""),st1_gross_string,st1_amount_string,"          ",(!isEMPTY(loc1_label)?"C":" "),(loc1_label!=NULL?loc1_label:""),loc1_gross_string,loc1_amount_string,cont_no," ");
*/
						}
						if(report==TRUE)
						{
