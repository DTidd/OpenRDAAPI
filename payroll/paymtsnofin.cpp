...
#ifndef WIN32
#define __NAM__ "paymtsnofin.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paymtsnofin.exe"
#endif
#include <app.hpp>
...
/*
								amount=(round(jobgross*acc->percentage/100));
*/
								amount=acc->amount;
...
*/
	...
	if((tg+tb)!=tp)
	{
		...
		tpstr=famt(tp,20);
		temp1=Rmalloc(512);
		sprintf(temp1,"Error:  The total gross [%s] plus total benefits [%s] is unequal to total expenditures [%s].  There is a difference of [%s].  Please refer to the Payroll Undo Submenu for the FINDIT Reports.",tgstr,tbstr,tpstr,tdstr);
