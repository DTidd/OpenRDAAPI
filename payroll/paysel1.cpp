...
#ifndef WIN32
#define __NAM__ "paysel1.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paysel1.exe"
#endif
#include <app.hpp>
...
static short paytyp=(-1);
static short lvedck=(-1),lvedsc=(-1),earnings_style=0;
...
short PAYGLOBALSubData(char **tmp,PassableStruct *p,char *modulename)
{
	...
	if(PAYROLL_SETUP->manage_earnings==TRUE)
	{
	if(!RDAstrcmp(modulename,"EARNINGS AMT"))
	{
	...
	if(!RDAstrcmp(modulename,"PERIOD EARNINGS AMT"))
	{
...
void select_backend(DBsort *dbsort)
{
	...
	double job_earnings=0.0,earnings=0.0;
	RDArsrc *tmprsrc=NULL;
	RDATData *prev=NULL;
	...
	if(dbsort!=NULL)
	{
		...
		while(!ef)
		{
			...
			if(PAYROLL_SETUP->manage_earnings==TRUE)
			{
				FINDFLDGETDOUBLE(dbsort->fileno,"PERIOD EARNINGS",&earnings);
			}
	...
	double *total_gross,int seq,char *pay_id)
{
	...
	char *exprssn=NULL,*temp=NULL,dockcompleted=FALSE;
	int ljobno=0;
	RDATData *prev=NULL;
	...
	PaySelStruct *ps,char *select_expression,PassableStruct *p)
{
	...
	char *earningsamt=NULL,*period_earnings=NULL;
	int seq=0,x;
	RDAvirtual *v=NULL;
		...
		if(!testfile)
		{
			...
			if(!madetrans)
			{
				...
				if(!testfile)
				{
					...
					if(diagapps)
					{
						prterr("Skipped: Personnel Master record [PRSNNL][PERDMT] of Personnel Identification of [%s] because Record Not Found.",(perid==NULL?"":perid));KEYNRD(prsnnl,1);
					}
				}
			} else {
				if(diagapps)
...
static void test_selectpay(RDArsrc *mtnrsrc)
{
	...
	ZERNRD(paymstr);
	readwidget(mtnrsrc,"[PAYMSTR][PAY IDENTIFICATION]");
...
*/
	...
	if(fields!=NULL)
	{
		for(x=0,fldx=fields;x<nofields;++x,++fldx)
		{
			...
			} else nonlstmakefld(mainrsrc,paymstr,fldx,TRUE);
		}
	}
...
*/
	...
	p->a=NULL;
	p->p=NULL;
	if(ADVEQLNRDsec(bnkacc,1,PaySelSubData,p)) KEYNRD(bnkacc,1);
	if(paytyp!=(-1))
	{
		if(ADVEQLNRDsec(paytyp,1,PaySelSubData,p)) KEYNRD(paytyp,1);
	}
	if(p!=NULL) Rfree(p);
   ...
   RateStructs??????? */
	...
	switch(dowhich)
	{
		...
		case 2:
			...
			{
				seteditsinglefilersrc(t->fileno,1,mtnrsrc,TRUE,FALSE);
