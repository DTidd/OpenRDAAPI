/* PAYROLL's - Make (PAYMTSA-CCGK) Report */
...
----------------------------------------------------------------------*/
	...
	tmp=RDAreportNEWADV(rptname,t,desc,1,"",FALSE,TRUE,FALSE,TRUE,TRUE,"PAYROLL","PAYMTSA1-CCG",FALSE,1,FALSE,"");
	if(t!=NULL) Rfree(t);
...
------------------------------------------------------------------------*/
	...
	t=Rmalloc(220+1);
	sprintf(t,"([PAYROLL][PAYJTRN][SOURCE USER VERIFIED]=FALSE) AND ([PAYROLL][PAYJTRN][DELETEFLAG]=FALSE) AND ([PAYROLL][PAYJTRN][SOURCE USER]=[LOGIN IDENTIFICATION]) AND ([PAYROLL][PAYJTRN][PAY IDENTIFICATION]=[PAY\
 IDENTIFICATION])");
	x=addsearchfileADV(tmp,"PAYROLL","PAYJTRN",0,NULL,NULL,"RDA PAYJTRN KEY",FALSE,t,NULL,1);
	if(x!=(-1))
	{
		s=tmp->search+(x-1);
	...
	t=Rmalloc(220+1);
	sprintf(t,"([PAYROLL][PAYDTRN][DELETEFLAG]=FALSE) AND ([PAYROLL][PAYDTRN][SOURCE USER VERIFIED]=FALSE) AND ([PAYROLL][PAYDTRN][SOURCE USER]=[LOGIN IDENTIFICATION]) AND ([PAY IDENTIFICATION]=[PAYROLL][PAYDTRN][PAY\
 IDENTIFICATION])");
	x=addsearchfileADV(tmp,"PAYROLL","PAYDTRN",0,NULL,NULL,"RDA PAYDTRN KEY",FALSE,t,NULL,1);
	if(x!=(-1))
	{
		s=tmp->search+(x-1);
		addsearchconfld(s,"PAY IDENTIFICATION","[PAY IDENTIFICATION]");
