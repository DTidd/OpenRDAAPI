/* PAYROLL's - Make (941STATE-CHAMB) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
extern char *module;
void STATE941(void);
static void STATE941_RANGE_SCREEN(void);
...
----------------------------------------------------------------------*/
	...
	sprintf(rptname,"941STATE");
	t=Rmalloc(20+1);
	sprintf(t,"941 State Tax Report");
	desc=Rmalloc(81+1);
	sprintf(desc,"This output definition is used to print the standard 941 State Tax Report report.");
...
------------------------------------------------------------------------*/
	...
	t=Rmalloc(38+1);
	sprintf(t,"([PAYROLL][PAYPCSM][DELETEFLAG]=FALSE)");
	x=addsearchfileADV(tmp,"PAYROLL","PAYPCSM",1,"PAYROLL","PAYDCSM","PAYPCSM KEY",FALSE,t,NULL,1);
	if(x!=(-1))
	{
		s=tmp->search+(x-1);
		addsearchconfld(s,"PERSONNEL IDENTIFICATION","[PAYDCSM][PERSONNEL IDENTIFICATION]");
...
------------------------------------------------------------------------*/
	...
	if(t!=NULL) Rfree(t);
	addvirtuallineADV(tmp,"SELECTED QUARTER",7,1,t,0,2);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(244+1);
	sprintf(t,"IF([SELECTED QUARTER]=1) THEN RETURN_VALUE=\"FIRST QUARTER\"\nELSE IF([SELECTED QUARTER]=2) THEN RETURN_VALUE=\"SECOND QUARTER\"\nELSE IF([SELECTED QUARTER]=3) THEN RETURN_VALUE=\"THIRD QUARTER\"\nELSE RETURN_V\
ALUE=\"FOURTH QUARTER\" FI FI FI");
...
----------------------------------------------------------------------*/
	...
	addaccumline(tmp,"TOTAL QUARTER TO DATE EARNINGS",7,"[QUARTER TO DATE EARNINGS]",0);
	...
	addaccumline(tmp,"TOTAL QUARTER TO DATE SHELTERED",7,"[QUARTER TO DATE SHELTERED]",0);
	addaccumline(tmp,"TOTAL ANNUAL EARNINGS",7,"[ANNUAL CALENDAR PERSONNEL GROSS]",0);
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","                         @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC                             DATE: @LLLLLLL@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","                                            @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC                          ANNUAL@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","PERSONNEL ID    NAME                        EARNINGS           AMOUNT        SHELTERED        EARNINGS          AMOUNT       SHELTERED@@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDCSM][CALENDAR YEAR] HEADER","Calendar Year @LLL@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDCSM][DEDUCTION DESCRIPTION IDENTIFICATION] HEADER","    @LLLLLLLLLLLLLLLL @VVVVVVVVVVVVVVVVVVVVVV@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDCSM][DEDUCTION DESCRIPTION IDENTIFICATION] HEADER","@@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","@LLLLLLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLL @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR@@",FALSE,NULL,10,pval);
	free_pvals(pval,10);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDCSM][DEDUCTION DESCRIPTION IDENTIFICATION] FOOTER","                 TOTAL @LLLLLLLLLLLLLLL @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR @,.RRRRRRRRRRRR@@",FALSE,NULL,9,pval);
	free_pvals(pval,9);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			STATE941_RANGE_SCREEN();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
/* PAYROLL's - Make (941STATE-CHAMB RANGE SCREEN) Screen */

static void STATE941_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","941STATE RANGE SCREEN");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Selected Calendar Year:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"SELECTED CALENDAR YEAR","Selected Calendar Year","","",0,4,0,"","","","");
		ADVaddwdgt(scrn,5,"","Selected Quarter (1-4):","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"SELECTED QUARTER","Selected Quarter","","",0,1,0,"","","","");
