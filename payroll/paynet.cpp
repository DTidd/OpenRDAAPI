...
#ifndef WIN32
#define __NAM__ "paynet.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paynet.exe"
#endif
#include <app.hpp>
...
struct JobStructs
{
	...
	double net;
	double dirdep;
	double life_insurance_salary;
	double retirement_salary;
	double advanced_salary;
	char gross_set;
};
...
static char *module="PAYROLL",printed_errorlist=FALSE,printed_warnlist=FALSE;
static char Force_Warning=TRUE,Use_Select_Completed=FALSE,Skip_Negative_Net_Job_Error=FALSE;
static char did_payunnet=FALSE;
...
static short paydtyp=(-1),paydedm=(-1),paytyp=(-1),paygrp=(-1);
static short paydcsm=(-1),paydfsm=(-1),paydpms=(-1),paypcsm=(-1);
static short paypfsm=(-1),payjdpm=(-1),COMPUTE_METHOD=FALSE;
...
void xSeeDeductionStruct(DeductionStruct *d,int line,char *file)
{
	...
	if(d!=NULL)
	{
		...
		prterr("Computation Level [%d] Annual Base [%f] Annual Affect Base [%f] Base [%f] Affect Base [%f] Gross [%.0f] Amount [%.0f] Ratio [%.08f] Reduce Net [%s].\n",d->comp_level,d->annual_base,d->annual_affect_base,d->base,d->affect_base,d->gross,d->amount,d->ratio,(d->reduce_net==TRUE?"TRUE":"FALSE"));
		if(d->jobs!=NULL)
		{
...
static char DedCompareJobs(DeductionStruct *d,DeductionStruct *d1)
{
	JobStruct *job=NULL,*job1=NULL;
	int x,y=0;

	if(d->jobs!=NULL && d1->jobs!=NULL)
	{
		for(x=0,job=d->jobs;x<d->numjobs;++x,++job)
		{
			for(y=0,job1=d1->jobs;y<d1->numjobs;++y,++job1)
			{
				if(job->jobno==job1->jobno) return(TRUE);
			}
		}
	}
	return(FALSE);
}
void SetDJobBase(DeductionStruct *d,int jobno,double gross_amt,double ls,double rets,double advanced)
{
	JobStruct *job=NULL;
	int x;
	...
	if(d->jobs!=NULL)
	{
		...
		if(x<d->numjobs)
		{
			...
			job->net+=gross_amt;
			job->dirdep=0.0;
			if(job->life_insurance_salary==0)
			{
				job->life_insurance_salary=ls;
				d->life_insurance_salary+=ls;
			}
			if(job->retirement_salary==0)
			{
				job->retirement_salary=rets;
				d->retirement_salary+=rets;
			}
			if(job->advanced_salary==0)
			{
				job->advanced_salary=advanced;
				d->advanced_salary=advanced;
			}
		} else {
...
void xSeeComputeStruct(ComputeStruct *c,int line,char *file)
{
	...
	if(c!=NULL)
	{
		...
		for(x=0,job=c->job;x<c->numjob;++x,++job)
		{
			SeeJobStruct(job);
		}
...
static int addDeductionStruct(ComputeStruct *c,char *dedid,short comp_level,
	...
	char *affect_select,char reduce_net,char expensed)
{
	DeductionStruct *d;
	...
	d->life_insurance_salary=0.0;
	d->retirement_salary=0.0;
	d->advanced_salary=0.0;
	d->annual_base=0;
	d->ratio=0;
	++c->num;
	return(c->num);
...
static void SortComputeStruct(ComputeStruct *c)
{
	...
	for(x=0;x<c->num-1;++x) for(y=x+1;y<c->num;++y)
	{
		...
		if(d2->comp_level<d1->comp_level)
		{
			...
			expensed=d1->expensed;
			d1->expensed=d2->expensed;
			d2->expensed=expensed;
			tmp=d1->dedid;
			d1->dedid=d2->dedid;
			d2->dedid=tmp;
			tmp=d1->computation;
...
static ComputeStruct *GetPersonnelComputeStruct(PassableStruct *pass_struct,char *perid)
{
	...
	char reduce_net=FALSE,ach=FALSE,*ddtype=NULL;
	char *affect_select=NULL,expensed=FALSE;
	...
	while(!ef)
	{
		...
		FINDFLDGETCHAR(paydedm,"ACH ACTIVE",&ach);
		if(!deleteflag && active)
		{
			...
			if(!ADVEQLNRDsec(payddms,1,ComputeSubData,pass_struct)) 
			{
				...
				FINDFLDGETSTRING(payddms,"DEDUCTION TYPE",&ddtype);
				if((!deleteflag && active && strncmp(ddtype,"DIRECT D",8)) || (!deleteflag && active && ach && !strncmp(ddtype,"DIRECT D",8)))
				{
					...
					if(!ADVEQLNRDsec(paydtyp,1,ComputeSubData,pass_struct)) 
					{
						...
						if(!deleteflag)
						{
							...
							FINDFLDGETCHAR(payddms,"REDUCE NET",
								&reduce_net);
							FINDFLDGETCHAR(payddms,"EXPENSED",
								&expensed);
							...
							addDeductionStruct(tmp,dedid,comp_level,
								...
								affect_select,reduce_net,expensed);
						}
					}
				}
			}
			...
			if(c->d!=NULL)
			{
				...
				if(!RDAstrcmp(filename,"ANNUAL BASE"))
				{
					...
					if(x<c->num)
					{
						if(d->annual_base<0) 
						{
							sprintf(stemp,"(%.08f)",d->annual_base);
						} else sprintf(stemp,"%.08f",d->annual_base);
						str_length=RDAstrlen(stemp);
						if(RDAstrlen(*tmp)<RDAstrlen(stemp))
						{
							memset(*tmp,0,RDAstrlen(*tmp)+1);
				...
				} else if(!RDAstrcmp(filename,"ADVANCED SALARY"))
				{
					for(x=0,d=c->d;x<c->num;++x,++d)
					{
						if(!RDAstrcmp(d->dedid,modulename)) break;
					...
					if(x<c->num)
					{
						if(d->advanced_salary<0) 
						{
							sprintf(stemp,"(%.08f)",d->advanced_salary);
						} else sprintf(stemp,"%.08f",d->advanced_salary);
						str_length=RDAstrlen(stemp);
						if(RDAstrlen(*tmp)<RDAstrlen(stemp))
						{
							memset(*tmp,0,RDAstrlen(*tmp)+1);
				...
				} else if(!RDAstrcmp(filename,"LIFE INSURANCE SALARY"))
				{
					for(x=0,d=c->d;x<c->num;++x,++d)
					{
						if(!RDAstrcmp(d->dedid,modulename)) break;
					...
					if(x<c->num)
					{
						if(d->life_insurance_salary<0) 
						{
							sprintf(stemp,"(%.08f)",d->life_insurance_salary);
						} else sprintf(stemp,"%.08f",d->life_insurance_salary);
						str_length=RDAstrlen(stemp);
						if(RDAstrlen(*tmp)<RDAstrlen(stemp))
						{
							memset(*tmp,0,RDAstrlen(*tmp)+1);
				...
				} else if(!RDAstrcmp(filename,"RETIREMENT SALARY"))
				{
					for(x=0,d=c->d;x<c->num;++x,++d)
					{
						if(!RDAstrcmp(d->dedid,modulename)) break;
					...
					if(x<c->num)
					{
						if(d->retirement_salary<0) 
						{
							sprintf(stemp,"(%.08f)",d->retirement_salary);
						} else sprintf(stemp,"%.08f",d->retirement_salary);
						str_length=RDAstrlen(stemp);
						if(RDAstrlen(*tmp)<RDAstrlen(stemp))
						{
							memset(*tmp,0,RDAstrlen(*tmp)+1);
				...
				} else if(!RDAstrcmp(filename,"ANNUAL AFFECT BASE"))
				{
					...
					{
						memset(stemp,0,101);
						if(d->annual_affect_base<0) 
						{
							sprintf(stemp,"(%.08f)",d->annual_affect_base);
						} else sprintf(stemp,"%.08f",d->annual_affect_base);
						str_length=RDAstrlen(stemp);
						if(RDAstrlen(*tmp)<str_length)
						{
							memset(*tmp,0,RDAstrlen(*tmp)+1);
				...
				} else if(!RDAstrcmp(filename,"AFFECT BASE"))
				{
					...
					if(x<c->num)
					{
						...
						if(d->affect_base<0)
						{
							sprintf(stemp,"(%.08f)",d->affect_base);
						} else {
							sprintf(stemp,"%.08f",d->affect_base);
						}
						str_length=RDAstrlen(stemp);
	...
	if(y<p->r->num)
	{
		switch(v->type)
		{
			...
			case SDECIMALV:
				...
				{
					sprintf(stemp,"(%.08f)",*v->value.float_value);
				} else {
					sprintf(stemp,"%.08f",*v->value.float_value);
				}
				str_length=RDAstrlen(stemp);
	...
	int line,char *file)
{
	...
	if(jobno!=(-1))
	{
		...
		if(paygrp!=(-1))
		{
		COPYFIELD(payjmst,paygrp,"PAY GROUP");
		if(ADVEQLNRDsec(paygrp,1,ComputeSubData,pass_struct)) KEYNRD(paygrp,1);
		}
...
static void quitbpaymstr(MakeBrowseList *blist)
{
	...
	if(printed_errorlist==FALSE && errorlist->numlibs>0) 
	{
		...
		{
			ERRORDIALOG("COMPUTE ERRORS","There were errors detected in the compute deduction process.\nYou must print the error list out before you can exit this process.",NULL,FALSE);
			return;
		}
	}
	if(Force_Warning==TRUE && printed_warnlist==FALSE && warnlist->numlibs>0) 
	{
		...
		{
			ERRORDIALOG("COMPUTE WARNINGS","There were warnings detected in the compute deduction process.\nYou must print the warning list out before you can exit this process.",NULL,FALSE);
			return;
		}
	}
	...
	if((errorlist->numlibs!=1) || RDAstrcmp(errorlist->libs[0],"No Errors Detected"))
	{
		APPlib2SupportRequest("PAYROLL","paynet -> Compute Payroll",errorlist);
	}
	if(errorlist!=NULL) freeapplib(errorlist);
	if(warnlist!=NULL) freeapplib(warnlist);
	if(DO_AUDIT && PAYROLL_SETUP->paynet_audit && did_payunnet==FALSE)
	{
		if(!isEMPTY(payid))
		{
...
static void okbpaymstr(MakeBrowseList *b,void *targetkey)
{
	...
	char *curdate=NULL,*curtime=NULL,suv=FALSE,*e=NULL,*ptype=NULL;
	NRDfield *field=NULL;
	char OKval=FALSE,*lastrateid=NULL,select_completed=FALSE;
	ComputeStruct *CompStruct=NULL;
	DeductionStruct *d=NULL,*d1=NULL;
	double q,gross_amt=0,amt1=0,amt2=0,last_annual_base=0;
	double total_net=0;
	char *tmp=NULL,*bdate=NULL,*temp1=NULL;
	int x,y,jobno=0,z=0,w=0,avl=0,we=0;
	JobStruct *job=NULL,*job1=NULL,*job2=NULL;
	PassableStruct *pass_struct=NULL;
	RDATData *prev_file=NULL,*prev=NULL;
	char *dedtype=NULL,DID_STUFF=FALSE;
	double ls=0.0,tb=0.0,rets=0.0,total_gross_amt=0.0;
	double ad=0.0,advd=0.0,d_zero=0.0;
	double damt=0.0,pmegross=0.0;

	printed_errorlist=FALSE;
	printed_warnlist=FALSE;
	did_payunnet=FALSE;
	FINDRSCSETEDITABLE(b->mainrsrc,"PAYUNNET",FALSE);
	FINDRSCSETSENSITIVE(b->mainrsrc,"PAYUNNET",FALSE);
	updatersrc(b->mainrsrc,"PAYUNNET");
	ZERNRD(paymstr);
	...
	if(paygrp!=(-1))
	{
		addDFincvir(diagrsrc,module,"PAYGRP",NULL,paygrp);
	}
	pass_struct->r=diagrsrc;
	pass_struct->a=NULL;
	...
	FINDFLDGETSTRING(paymstr,"PAY IDENTIFICATION",&payid);
	FINDFLDGETSTRING(paymstr,"PAY TYPE",&ptype);
	ZERNRD(payppms);
	...
	while(!ef)
	{
		...
		FINDFLDGETCHAR(payppms,"SELECT COMPLETED",&select_completed);
		FINDFLDGETDOUBLE(payppms,"GROSS",&total_gross_amt);
		if(!benefits_computed && (select_completed || !Use_Select_Completed))
		{
...
/* Get Personnel's Gross Earnings (base) */
			...
			FINDFLDGETSTRING(prsnnl,"BIRTHDATE",&bdate);
			if(((isEMPTY(bdate)) || (RDAstrlen(bdate)<10)) && (Skip_Negative_Net_Job_Error==FALSE))
			{
				temp1=Rmalloc(RDAstrlen(perid)+100+21);
				sprintf(temp1,"NO OR INVALID BIRTH DATE: Personnel ID [%-30s]",(perid==NULL?"":perid));
				addAPPlib(warnlist,temp1);
...
*/
			...
			while(!ef1)
			{
				...
				if(suv)
				{
					...
					FINDFLDGETDOUBLE(payjmst,"LIFE INSURANCE SALARY",&ls);
					FINDFLDGETDOUBLE(payjmst,"RETIREMENT SALARY",&rets);
					FINDFLDGETDOUBLE(payjmst,"ADVANCE",&advd);
...
/* set files for expression */
						...
						if(!isEMPTY(d->rate_selection))
						{
							...
							{
								d->base+=gross_amt;
								SetDJobBase(d,jobno,gross_amt,ls,rets,advd);
					...
					if(diagapps && diageval)
					{
						prterr("DIAG Skipped PAYJPMS (Payroll Job Pay Master Summary) because of SOURCE USER VERIFED=FALSE in payid=[%s]  perid=[%s]  jobno=[%d].",payid,perid,jobno);
					}
					tmp=Rmalloc(RDAstrlen(perid)+RDAstrlen(payid)+100+21);
					sprintf(tmp,"INVALID JOB PAY MASTER: Source User Verified is FALSE for Pay ID=[%s], Personnel ID=[%s], Job=[%d].",payid,perid,jobno);
...
/* Compute DeductionStruct's */
			if(diagapps) SeeComputeStruct(CompStruct);
			deleteflag=FALSE;
			if(((CompStruct->num)<1) && RDAstrcmp(ptype,"NO DEDUCTIONS"))
			{        
...
/* sight change 04-02-2004 */
					if(d->annual_affect_base!=0.0 && d->affect_base!=0)
					{
...
/* HERE */
						...
						{
							job=d->jobs;
							set_jobrate_files_for_expression(payid,perid,job->jobno,NULL,diagrsrc,pass_struct);
						}
/* HERE */
					...
					if(d->jobs!=NULL)
					{
						...
						for(y=0,job=d->jobs;y<d->numjobs;++y,++job)
						{
							if(d->base==0.0)
							{
								...
								job->amount=0;
								job->gross_set=TRUE;
							} else if(job->affect_base==job->base)
							{
								job->gross=job->base;
								if(avl==0 && (y+1)==d->numjobs)
								{
									job->amount=d->amount-d->amount_used;
								} else {
									job->amount=round(((d->amount*job->net)/ctotal_net));
								}
								job->gross_set=TRUE;
							} else {
								++avl;
							...
							d->gross_used+=job->gross;
							d->amount_used+=job->amount;
						}
						for(y=0,job=d->jobs;y<d->numjobs && avl>0;++y,++job)
						{
							if(job->gross_set==FALSE)
							{
								if(avl>1)
								{
									if(d->base!=0)
									{
										job->gross=round(((d->affect_base*job->net)/ctotal_net));
										job->amount=round(((d->amount*job->net)/ctotal_net));
									} else { 
										job->gross=0;
										job->amount=0;
									...
									job->gross=d->affect_base-d->gross_used;
									job->amount=d->amount-d->amount_used;
								}
								d->gross_used+=job->gross;
								d->amount_used+=job->amount;
							...
							job=d->jobs;
							job->gross=d->affect_base;
							job->amount=d->amount;
							d->gross_used=d->affect_base;
							d->amount_used=d->amount;
							job->gross_set=TRUE;
						}
						} else { /* RDA Recommended  COMPUTE_METHOD==0 */
						FINDFLDGETSTRING(payddms,"DEDUCTION TYPE",&dedtype);
						for(y=0,job=d->jobs;y<d->numjobs;++y,++job)
						{
							if(d->numjobs==1)
							{
								job->gross=d->gross-d->gross_used;
								job->amount=d->amount-d->amount_used;
								job->gross_set=TRUE;
							} else if((y+1)<d->numjobs)
							{
								if(d->affect_base!=0)
								{
									if(((x+1)<CompStruct->num) && RDAstrcmp(dedtype,"DIRECT DEPOSIT"))
									{
										job->gross=round(((d->gross*job->affect_base)/d->affect_base));
										if(job->gross>job->affect_base) job->gross=job->affect_base;
										job->amount=round(((d->amount*job->affect_base)/d->affect_base));
										if(job->amount>job->net && job->net>0) job->amount=job->net;
									} else {
										job->gross=round(((d->gross*job->net)/ctotal_net));
										if(job->gross>job->affect_base) job->gross=job->affect_base;
										job->amount=round(((d->amount*job->net)/ctotal_net));
										if(job->amount>job->net && job->net>0) job->amount=job->net;
									}
						...
						if(diagapps)
						{
						prterr("DIAG Deduction: %s",(d->dedid!=NULL ? d->dedid:""));
						for(y=0,job=d->jobs;y<d->numjobs;++y,++job)
						{
							prterr("\tJob: %d Base %15.02f Affect Base: %15.02f Net %15.02f Gross: %15.02f Amount: %15.02f\r\n",job->jobno,job->base/100,job->affect_base/100,job->net/100,job->gross/100,job->amount/100); 
						}
						}
						for(y=0,job=d->jobs;y<d->numjobs;++y,++job)
						{
							...
							FINDFLDGETSTRING(payddms,"DEDUCTION TYPE",&dedtype);
							if((!strncmp(dedtype,"DIRECT DEP",10)) && (DD_NET_TYPE==0))
							{
								AddJobComputeStruct(CompStruct,job->jobno,0.0,(d->reduce_net ? amt2:0.0),amt2);
							} else if(!strncmp(dedtype,"DIRECT DEPOSIT",14))
							{
								AddJobComputeStruct(CompStruct,job->jobno,0.0,(d->reduce_net ? amt2:0.0),amt2);
							} else if(!RDAstrcmp(dedtype,"REIMBURSEMENT") || !RDAstrcmp(dedtype,"FRINGE-CASH"))
							{
								AddJobComputeStruct(CompStruct,job->jobno,0.0,amt2*(-1),0.0);
							} else {
								AddJobComputeStruct(CompStruct,job->jobno,0.0,(d->reduce_net ? amt2:0.0),0.0);
							}
							if(!strncmp(dedtype,"DIRECT DEP",10))
							{
								FINDFLDSETDOUBLE(payjdpm,"GROSS",amt2);
								FINDFLDSETDOUBLE(payjdpm,"SHELTERED",0.00);
								damt+=amt2;
							} else {
								FINDFLDSETDOUBLE(payjdpm,"GROSS",amt1);
								FINDFLDSETDOUBLE(payjdpm,"SHELTERED",((job->base-job->affect_base)));
							}
							FINDFLDSETDOUBLE(payjdpm,"AMOUNT",amt2);
							if(amt1!=0.0 || amt2!=0.0 || ((job->base-job->affect_base)!=0))
							{
								ADVWRTTRANSsec(payjdpm,0,NULL,prev_file,ComputeSubData,pass_struct);
							}
							if(d->expensed==FALSE && !RDAstrcmp(dedtype,"ADVANCE-REPAY"))
							{
								ZERNRD(paymstr);
								FINDFLDSETSTRING(paymstr,"PAY IDENTIFICATION",payid);
								if(ADVEQLNRDsec(paymstr,1,ComputeSubData,pass_struct)) 
								{
									prev=NULL;
									KEYNRD(paymstr,1);
								} else prev=RDATDataNEW(paymstr);
								FINDFLDGETDOUBLE(paymstr,"TOTAL ADVANCE REPAYMENTS",&tb);
								tb-=amt2;
								FINDFLDSETDOUBLE(paymstr,"TOTAL ADVANCE REPAYMENTS",tb);
								ADVWRTTRANSsec(paymstr,0,NULL,prev,ComputeSubData,pass_struct);
								if(prev!=NULL) FreeRDATData(prev);
							}
							if(d->reduce_net==FALSE && d->expensed==TRUE)
							{
								ZERNRD(paymstr);
								FINDFLDSETSTRING(paymstr,"PAY IDENTIFICATION",payid);
								if(ADVEQLNRDsec(paymstr,1,ComputeSubData,pass_struct)) 
								...
								tb+=amt2;
								FINDFLDSETDOUBLE(paymstr,"TOTAL BENEFITS",tb);
								ADVWRTTRANSsec(paymstr,0,NULL,prev,ComputeSubData,pass_struct);
								if(prev!=NULL) FreeRDATData(prev);
							}
							if(amt2!=0.0 && d->reduce_net==FALSE && (!RDAstrcmp(dedtype,"REIMBURSEMENT") || !RDAstrcmp(dedtype,"FRINGE-CASH")))
							{
							...
							} else if(amt2!=0.0 && d->reduce_net==TRUE) 
							{
								total_net=total_net-amt2;
								for(w=(x+1);w<CompStruct->num;++w)
								{
									...
									for(z=0,job1=d1->jobs;z<d1->numjobs;++z,++job1)
									{
										...
										{
											job1->net=job1->net-amt2;
											if(job1->net<0 && COMPUTE_METHOD==1)
											{
					...
					if(!strncmp(dedtype,"DIRECT DEP",10))
					{
						FINDFLDSETDOUBLE(paydpms,"GROSS",d->amount);
						FINDFLDSETDOUBLE(paydpms,"SHELTERED",(0.00));
					} else {
						FINDFLDSETDOUBLE(paydpms,"GROSS",d->gross);
						FINDFLDSETDOUBLE(paydpms,"SHELTERED",(d->base-d->affect_base));
					}
					...
					if(d->amount!=0.0 || d->gross!=0.0 || ((d->base-d->affect_base)!=0))
					{
					...
					if(!isEMPTY(d->affect_select))
					{
						for(y=x+1;y<CompStruct->num;++y)
						{
							...
							{
								prterr("DIAG EVALUATEdbl Affect Select for Deduction [%s].",(d->dedid!=NULL ? d->dedid:""));
							}
							if(DedCompareJobs(d,d1))
							{
							q=(round(EVALUATEdbl(d->affect_select,ComputeSubData,pass_struct)));
							} else q=0.0;
							d1->affect_base=d1->affect_base-q;
							if(d->base!=0.0) ad=round(q*d->annual_base/d->base);
								else ad=0.0;
/*
							d1->annual_affect_base=d1->annual_affect_base-(round(q*d->annual_base/d->base));
*/
							d1->annual_affect_base=d1->annual_affect_base-ad;
							if(diagapps)
							{
								prterr("DIAG Deduction [%s] Sheltering from [%s] by [%f].",(d->dedid!=NULL ? d->dedid:""),(d1->dedid!=NULL ? d1->dedid:""),ad/100);
							}
							if(q!=0.0)
							{
								for(z=0,job1=d1->jobs;z<d1->numjobs;++z,++job1)
								{
									for(we=0,job2=d->jobs;we<d->numjobs;++we,++job2)
									{
										if(job2->jobno==job1->jobno) break;
									}
									if(we<d->numjobs)
									{
/*
										job1->affect_base=(job1->affect_base-round(((q*job2->base/d->base))));
*/
										job1->affect_base=(job1->affect_base-round(((q*job2->affect_base/d->affect_base))));
									}
								}
							}
						}
						...
						d->gross!=d->gross_used)
					{
						if(diagapps==TRUE)
						{
							prterr("ERROR: PERSONNEL: %s DEDUCTION: %s AMOUNT %15.02f!=AMOUNT USED %15.02f  and/or GROSS %15.02f!=GROSS USED %15.02f ",(perid!=NULL ? perid:""),(d->dedid!=NULL ? d->dedid:""),d->amount/100,d->amount_used/100,d->gross/100,d->gross_used/100);TRACE;
						}
					} 
				}
			}
			...
			if(total_gross_amt==total_net && RDAstrcmp(ptype,"NO DEDUCTIONS"))
			{
				tmp=Rmalloc(RDAstrlen(perid)+200+42);
				sprintf(tmp,"VERIFY EMPLOYEE PAID DEDUCTIONS: Personnel ID [%-30s] Gross Amount [%20.02f] Net Amount [%20.02f]",(perid==NULL?"":perid),total_gross_amt/100,total_net/100);
				addAPPlib(warnlist,tmp);
				if(tmp!=NULL) Rfree(tmp);
			}
			...
			if(!ADVEQLNRDsec(payppms,1,ComputeSubData,pass_struct))
			{
				...
				FINDFLDSETDOUBLE(payppms,"NET",(total_net+damt));
				FINDFLDGETDOUBLE(payppms,"GROSS",&pmegross);
				if(pmegross==(total_net+damt))
				{
					FINDFLDSETCHAR(payppms,"COMPUTE COMPLETED",benefits_computed);
					FIELDCOPY(payppms,"BENEFITS COMPUTED DATE",payppms,"COMPUTE COMPLETED DATE");
					FIELDCOPY(payppms,"BENEFITS COMPUTED TIME",payppms,"COMPUTE COMPLETED TIME");
					FIELDCOPY(payppms,"BENEFITS COMPUTED BY",payppms,"COMPUTE COMPLETED BY");
				}
			...
			if(CompStruct!=NULL)
			{
				...
				if(CompStruct->job!=NULL && field!=NULL)
				{
					for(z=0,job1=CompStruct->job;z<CompStruct->numjob;++z,++job1)
					{
						...
						if((job1->net<d_zero) && (job1->gross>d_zero) && (Skip_Negative_Net_Job_Error==FALSE))
						{
							tmp=Rmalloc(RDAstrlen(perid)+205+21);
							sprintf(tmp,"INVALID JOB NET AMOUNT: Personnel ID [%-30s] Job# [%4d] Net Amount [%20.02f]",(perid==NULL?"":perid),job1->jobno,job1->net/100);
							addAPPlib(errorlist,tmp);
							if(tmp!=NULL) Rfree(tmp);
						}
					}
			...
			if(diagapps && diageval)
			{
				if(Use_Select_Completed==FALSE)
				{
					...
					prterr("DIAG Skipped PAYPPMS (Payroll Personnel Pay Master) because of BENEFITS COMPUTED=TRUE or SELECT COMPLETED=FALSE in payid=[%s]  perid=[%s].",payid,perid);
				}
			}
			if((Use_Select_Completed) && (select_completed==FALSE))
			{
				tmp=Rmalloc(RDAstrlen(perid)+RDAstrlen(payid)+150);
				sprintf(tmp,"INVALID PERSONNEL PAY MASTER: The Select Completed flag is FALSE for Pay ID=[%s], Personnel ID=[%s].",payid,perid);
				addAPPlib(errorlist,tmp);
				if(tmp!=NULL) Rfree(tmp);
			}
		}
	...
	if(DID_STUFF==FALSE)
	{
		ERRORDIALOG("NO RECORDS SELECTED!","No records were selected for Compute Payroll from the given criteria. Unless this is a \"No Deductions\" test payroll, please review the pay master status.",NULL,FALSE);
		prterr("No records were selected for Compute Payroll from the given criteria. Unless this is a \"No Deductions\" test payroll, please review the pay master status.");
	}
	...
	if(errorlist!=NULL)
	{
		...
		{		
			...
			FINDRSCSETEDITABLE(b->mainrsrc,"PAYUNNET",TRUE);
			FINDRSCSETSENSITIVE(b->mainrsrc,"PAYUNNET",TRUE);
		}
	}
	if(warnlist!=NULL)
	{
...
static void printerrorlist(RDArsrc *parent,RDA_PFILE *fp)
{
	...
	if(fp!=NULL)
	{
		...
		RDA_fprintf(fp,"%*sXPERT COMPUTE PAYROLL ERROR LIST          Date: %*s\r\n",24,"",8,date);
...
static void printwarnlist(RDArsrc *parent,RDA_PFILE *fp)
{
	...
	if(fp!=NULL)
	{
		date=GETCURRENTDATE();
		timex=GETCURRENTTIME();
		RDA_fprintf(fp,"%*sXPERT COMPUTE PAYROLL WARNING LIST        Date: %*s\r\n",22,"",8,date);
		RDA_fprintf(fp,"%*sExecuted By: %s%*sTime: %s\r\n\r\n",
			((80-(15+RDAstrlen(USERLOGIN)))/2),
			"",USERLOGIN,
			(((80-(15+RDAstrlen(USERLOGIN)))/2)-14),"",
...
static void payunnet(RDArsrc *r)
{
	APPlib *args=NULL;
...
#endif
{
	...
	FreeRDAGenericSetup(GSV);	
	GSV=RDAGenericSetupNew(module,"USE SELECT COMPLETED");
	if(getRDAGenericSetupbin(libx,GSV))
	{
		Use_Select_Completed=FALSE;
	} else {
		Use_Select_Completed=*GSV->value.string_value;
	}
	FreeRDAGenericSetup(GSV);	
	GSV=RDAGenericSetupNew(module,"SKIP ERROR ON NEGATIVE NET JOBS");
	if(getRDAGenericSetupbin(libx,GSV))
	{
		Skip_Negative_Net_Job_Error=FALSE;
	} else {
		Skip_Negative_Net_Job_Error=*GSV->value.string_value;
	}
	FreeRDAGenericSetup(GSV);	
	GSV=RDAGenericSetupNew(module,"COMPUTE METHOD");
	if(libx!=NULL) Rfree(libx);
	...
	if(getRDAGenericSetupbin(libx,GSV))
	{
		...
		COMPUTE_METHOD=*GSV->value.short_value;
	}
	FreeRDAGenericSetup(GSV);	
	...
	if((paygrp=APPOPNNRD(module,"PAYGRP",TRUE,TRUE))==(-1)) return;
...
/*lint +e611 */
	...
	addrfcbrsrc(mrsrc,"PAYUNNET",TRUE,payunnet,NULL);
	FINDRSCSETSENSITIVE(mrsrc,"PAYUNNET",FALSE);
	FINDRSCSETEDITABLE(mrsrc,"PAYUNNET",FALSE);
	ADVSbrowse(paymstr,1,mrsrc,srsrc,drsrc,NULL,NULL,selectbpaymstr,quitbpaymstr,
		okbpaymstr,NULL,NULL,TRUE,NULL,0,0,0,FALSE);
	RDAAPPMAINLOOP();
}
