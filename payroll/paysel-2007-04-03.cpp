...
#ifndef WIN32
#define __NAM__ "paysel-2007-04-03.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paysel-2007-04-03.exe"
#endif
#include <app.hpp>
	...
	double *total_gross,int seq,char *pay_id)
{
	...
	if(lvetrn!=(-1))
	{
		...
		while(!ef)
		{
			...
			if(dockvalue!=0.0 && dockcompleted==FALSE && deleteflag==FALSE && (tmpdock!=0.0 || *total_gross>0.0))
			{
				prev=RDATDataNEW(lvetrn);
				LOCNRDFILE(lvetrn);
...
void selectpay(RDArsrc *mainrsrc)
{
	...
	while(!ef)
	{
		...
		if(!testfile)
		{
			...
			if(!computed && payjpms_ok==TRUE && scompleted==FALSE)
			{
			...
			if(!madetrans)
			{
				...
				if(!testfile)	
				{
					...
					e=Rmalloc(512);
					sprintf(e,"Skipped: Personnel Master record [PRSNNL][PERDMT] of Personnel Identification of [%s] because Record Not Found.",(perid==NULL?"":perid));
					addAPPlib(errorlist,e);
					if(e!=NULL) Rfree(e);
				}
			} else {
