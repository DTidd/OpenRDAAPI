/* payppms.c - Xpert Payroll Personnel Pay Master Browse */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payppms.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payppms.exe"
#endif
#include <app.hpp>
...
static void payppmsm(short,void *);
static void doexit(MakeBrowseList *),okfunc(MakeBrowseList *,void *);
static void new_record(RDArsrc *,MakeBrowseList *);
static void quit_record(RDArsrc *);
static MakeBrowseList *mbl_payppms=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payppms(RDArsrc *);
...
static void browse_payppms(RDArsrc *mtnrsrc)
{
	...
	mbl_payppms=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYPPMS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYPPMS");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYPPMS",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYPPMS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payppms!=NULL) mbl_payppms=NULL;
}
...
static void payppmsm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payppms,NULL);
	...
	if(mbl_payppms!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	APPmakescrn(mtnrsrc,FALSE,quit_record,NULL,(mbl_payppms==NULL?TRUE:FALSE));
}
