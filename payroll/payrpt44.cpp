...
extern char *module;
void PAYVWCR(void);
static void PAYVWCR_RANGE_SCREEN(void);
...
----------------------------------------------------------------------*/
	...
	memset(stemp,0,101);
	rptname=Rmalloc(7+1);
	sprintf(rptname,"PAYVWCR");
	t=Rmalloc(41+1);
	sprintf(t,"Payroll Vendor Withholding Check Register");
	desc=Rmalloc(78+1);
	sprintf(desc,"This output definition is used to print the Vendor Withholding Check Register.");
...
------------------------------------------------------------------------*/
	...
	if(t!=NULL) Rfree(t);
	t=Rmalloc(86+1);
	sprintf(t,"IF([CHECK NUMBER]=[LAST CHECK NUMBER]) \nTHEN RETURN_VALUE=0\nELSE RETURN_VALUE=1 \nFI");
	addvirtuallineADV(tmp,"CHECK NUMBER COUNTER",8,10,t,1,0);
...
----------------------------------------------------------------------*/
	...
	addaccumline(tmp,"TOTAL NUMBER OF CHECKS",7,"[CHECK NUMBER COUNTER]",0);
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","            @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC       Page: @RRRRRRR@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","                             VENDOR WITHHOLDING CHECK REGISTER                          Date: @LLLLLLL@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","            @CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC       Time: @LLLLLLL@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","  Check No.    Check Date Vendor Ident.         Vendor Name                 Type                Amount @@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","  @RRRRRRRRRRR @LLLLLLLLL @LLLLLLLLLLLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLLLLLLL @LLLLLLLLLL @,.RRRRRRRRRRRR@@",TRUE,"([LAST VIRTUAL CHECK NUMBER]=[LAST CHECK NUMBER]) AND ([LAST MANUAL VOUCHER]=FALSE)",8,pval);
	free_pvals(pval,8);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","  @RRRRRRRRRRR @LLLLLLLLL @LLLLLLLLLLLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLLLLLLL @LLLLLLLLLL @,.RRRRRRRRRRRR@@",TRUE,"([LAST MANUAL VOUCHER]=TRUE) AND ([BODY COUNT]>0)",8,pval);
	free_pvals(pval,8);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][VIRTUAL CHECK NUMBER] FOOTER","  @RRRRRRRRRRR @LLLLLLLLL @LLLLLLLLLLLLLLLLLLLL @LLLLLLLLLL VOID VOID VOID  @LLLLLLLLLL $*********0.00 @@",TRUE,"(NOT([LAST VIRTUAL CHECK NUMBER]=[LAST CHECK NUMBER])) AND ([BODY COUNT]>0)",7,pval);
	free_pvals(pval,7);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL NUMBER OF CHECKS]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDSV][BANK IDENTIFICATION] FOOTER","Total Bank: @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL Total Number of Checks: ( @RRRRRR ) : @,.RRRRRRRRRRRR@@",FALSE,NULL,5,pval);
	free_pvals(pval,5);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			PAYVWCR_RANGE_SCREEN();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
/* PAYROLL's - Make (PAYVWCR RANGE SCREEN) Screen */
static void PAYVWCR_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","PAYVWCR RANGE SCREEN");
	if(scrn!=NULL)
	{
