/* payjdcm.c - Xpert Payroll Job Deduction Calendar Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjdcm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjdcm.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_payjdcm=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjdcm(RDArsrc *,RDATData *);
...
void payjdcmm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	if(fields!=NULL)
	{
		...
		{
			if(fldx->type==DOLLARS) edit_rsrc=(short)(editable &&
				(strstr(fldx->name," GROSS")==NULL) &&
				(strstr(fldx->name," AMOUNT")==NULL));
                        else edit_rsrc=editable;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjdcm,previous);
	...
	if(mbl_payjdcm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_payjdcm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_payjdcm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_payjdcm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_payjdcm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_payjdcm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJDCM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJDCM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJDCM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJDCM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjdcm!=NULL) mbl_payjdcm=NULL;
}
