/* payjdpm.c - Xpert Job Deduction Pay Master Browse */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjdpm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjdpm.exe"
#endif
#include <app.hpp>
...
static void payjdpmm(short,void *);
static void doexit(MakeBrowseList *),okfunc(MakeBrowseList *,void *);
static void new_record(RDArsrc *,MakeBrowseList *);
static void quit_record(RDArsrc *);
static MakeBrowseList *mbl_payjdpm=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjdpm(RDArsrc *);
...
static void browse_payjdpm(RDArsrc *mtnrsrc)
{
	...
	mbl_payjdpm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJDPM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJDPM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJDPM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJDPM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjdpm!=NULL) mbl_payjdpm=NULL;
}
...
static void payjdpmm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjdpm,NULL);
	...
	if(mbl_payjdpm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	APPmakescrn(mtnrsrc,FALSE,quit_record,NULL,(mbl_payjdpm==NULL?TRUE:FALSE));
}
