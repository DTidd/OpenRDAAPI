/* PAYROLL's - Make (CHGMSCSAL) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
extern char *module;
void CHGMSCSAL(void);
static void CHGMSCSAL_SEARCH_BROWSE(void);
static void CHGMSCSAL_BROWSE(void);
static void CHGMSCSAL_DEFINE_LIST(void);
static void CHGMSCSAL_MAINTAIN_SCREEN(void);
static void CHGMSCSAL_RANGE_SCREEN(void);
...
----------------------------------------------------------------------*/
	...
	memset(stemp,0,101);
	rptname=Rmalloc(9+1);
	sprintf(rptname,"CHGMSCSAL");
	t=Rmalloc(29+1);
	sprintf(t,"Change Miscellaneous Salaries");
	desc=Rmalloc(78+1);
	sprintf(desc,"This output definition is used to manage the Change of Miscellaneous Salaries.");
	tmp=RDAreportNEWADV(rptname,t,desc,1,"",TRUE,TRUE,FALSE,TRUE,TRUE,"PAYROLL","CHGMSCSAL1",TRUE,1,FALSE,"");
	if(t!=NULL) Rfree(t);
...
------------------------------------------------------------------------*/
	...
	t=Rmalloc(291+1);
	sprintf(t,"([PAYROLL][PAYJMST][DELETEFLAG]=FALSE) AND ([PAYROLL][PAYJMST][ACTIVE]=TRUE)\nAND (([PAYROLL][PAYJMST][GROSS IDENTIFICATION]!=\"TEACHER-BACHELOR\") AND\n([PAYROLL][PAYJMST][GROSS IDENTIFICATION]!=\"TEACHER-\
1/2 MASTER\") AND \n([PAYROLL][PAYJMST][GROSS IDENTIFICATION]!=\"TEACHER-MASTER\"))");
	...
	t=Rmalloc(99+1);
	sprintf(t,"([POSTRK][POSMSTR][DELETEFLAG]=FALSE) AND ([POSTRK][POSMSTR][CLASS IDENTIFICATION][1,1,3]!=\"SUB\")");
...
------------------------------------------------------------------------*/
	...
	if(t!=NULL) Rfree(t);
	addvirtuallineADV(tmp,"PERCENT INCREASE",20,5,t,0,2);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(59+1);
	sprintf(t,"([PAYROLL][PAYJMST][MISC SALARY AMOUNT]*[PERCENT INCREASE])");
	addvirtuallineADV(tmp,"NEW SALARY",2,20,t,0,0);
	...
	if(t!=NULL) Rfree(t);
	t=Rmalloc(139+1);
	sprintf(t,"IF([PAYROLL][PAYJMST][MISC SALARY AMOUNT]>0) THEN RETURN_VALUE=([NEW SALARY]/[PAYROLL][PAYJMST][MISC SALARY AMOUNT]) ELSE RETURN_VALUE=0 FI");
	addvirtuallineADV(tmp,"PERCENT CHANGE",20,6,t,1,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(202+1);
	sprintf(t,"IF([PAYROLL][PAYJMST][MISC SALARY AMOUNT]=0) THEN RETURN_VALUE=\"NEW\"\nELSE IF([NEW SALARY]>[PAYROLL][PAYJMST][MISC SALARY AMOUNT]) THEN \nRETURN_VALUE=\"INCREASE\" ELSE RETURN_VALUE=\"DECREASE\" FI FI");
	addvirtuallineADV(tmp,"CHANGE DESC",0,0,t,1,0);

	if(t!=NULL) Rfree(t);
	t=Rmalloc(47+1);
	sprintf(t,"([TOTAL NEW SALARY]-[TOTAL MISC SALARY AMOUNT])");
	addvirtuallineADV(tmp,"TOTAL INCREASE",2,20,t,1,0);
...
----------------------------------------------------------------------*/
	...
	addaccumline(tmp,"LAST SALARY AMOUNT",3,"[PAYROLL][PAYJMST][MISC SALARY AMOUNT]",0);
	addaccumline(tmp,"LAST NEW SALARY",3,"[NEW SALARY]",0);
	addaccumline(tmp,"TOTAL NEW SALARY",7,"[NEW SALARY]",0);
	addaccumline(tmp,"TOTAL MISC SALARY AMOUNT",7,"[PAYROLL][PAYJMST][MISC SALARY AMOUNT]",0);
	addaccumline(tmp,"AVERAGE PERCENTAGE CHANGE",0,"[PERCENT CHANGE]",0);
...
----------------------------------------------------------------------*/
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[NEW SALARY]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[PERCENT CHANGE]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[CHANGE DESC]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","@LLLLLLLLLLLLLLL @RRR @LLLLLLLLLLLLLLLLLLLLLLLLLLLLLL @,.RRRRRRRRR @,.RRRRRRRRR @RR.RR @VVV@@",FALSE,NULL,9,pval);
	free_pvals(pval,9);
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[TOTAL INCREASE]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[AVERAGE PERCENTAGE CHANGE]");
	pvals->fieldname=NULL;
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","TOTAL INCREASE OF @,.RRRRRRRRRRR AVERAGE PERCENTAGE CHANGED @RR.RR@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			CHGMSCSAL_BROWSE();
			CHGMSCSAL_DEFINE_LIST();
			CHGMSCSAL_MAINTAIN_SCREEN();
			CHGMSCSAL_RANGE_SCREEN();
			CHGMSCSAL_SEARCH_BROWSE();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
/* PAYROLL's - Make (CHGMSCSAL BROWSE) Screen */

static void CHGMSCSAL_BROWSE()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","CHGMSCSAL BROWSE");
	if(scrn!=NULL)
	{
		...
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,7,"BROWSE LIST","Browse List","","",15,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
...
/* PAYROLL's - Make (CHGMSCSAL DEFINE LIST) Screen */

static void CHGMSCSAL_DEFINE_LIST()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","CHGMSCSAL DEFINE LIST");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Enter the position and length for the following:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		...
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"DELETEFLAG POSITION","Delete Flag Position","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,0,"DELETEFLAG LENGTH","Delete Flag Length","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Personnel ID:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][PERSONNEL IDENTIFICATION] POSITION","Personnel ID Position","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][PERSONNEL IDENTIFICATION] LENGTH","Personnel Identification Length","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Job #:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		...
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][POSITION IDENTIFICATION] POSITION","Position ID Position","","",0,3,0,"","","","");
		...
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Class ID:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[POSTRK][POSMSTR][CLASS IDENTIFICATION] POSITION","Class Identification Position","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,0,"[POSTRK][POSMSTR][CLASS IDENTIFICATION] LENGTH","Class Identification Length","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Misc Salary:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][MISC SALARY AMOUNT] POSITION","Misc Salary Amount Position","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][MISC SALARY AMOUNT] LENGTH","Misc Salary Amount Length","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","New Salary:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[NEW SALARY] POSITION","New Salary Position","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,0,"[NEW SALARY] LENGTH","New Salary Length","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Location:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[POSTRK][POSMSTR][LOCATION IDENTIFICATION] POSITION","Location Identification Position","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,0,"[POSTRK][POSMSTR][LOCATION IDENTIFICATION] LENGTH","Location Identification Length","","",0,3,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,4,"","","","",0,0,0,"","","","");
...
/* PAYROLL's - Make (CHGMSCSAL MAINTAIN SCREEN) Screen */
static void CHGMSCSAL_MAINTAIN_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","CHGMSCSAL MAINTAIN SCREEN");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"DELETEFLAG","Skip this record","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Personnel ID:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][PERSONNEL IDENTIFICATION]","Personnel Identification","","",0,15,0,"","FALSE","","");
		ADVaddwdgt(scrn,5,"","Name:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PRSNNL][PERDMG][NAME]","Name","","",0,40,0,"","FALSE","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Job #:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][JOB NUMBER]","Job Number","","",0,4,0,"","FALSE","","");
		ADVaddwdgt(scrn,5,"","Position:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][POSITION IDENTIFICATION]","Position Identification","","",0,30,0,"","FALSE","","");
		ADVaddwdgt(scrn,5,"","Class:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[POSTRK][POSMSTR][CLASS IDENTIFICATION]","Class Identification","","",0,15,0,"","FALSE","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Location:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[POSTRK][POSMSTR][LOCATION IDENTIFICATION]","Location Identification","","",0,15,0,"","FALSE","","");
		ADVaddwdgt(scrn,5,"","Gross ID:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][GROSS IDENTIFICATION]","Gross Identification","","",0,15,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Original Salary:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[PAYROLL][PAYJMST][MISC SALARY AMOUNT]","Original Salary","","",0,20,0,"","FALSE","","");
		ADVaddwdgt(scrn,5,"","New Salary:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"[NEW SALARY]","New Salary","","",0,20,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
...
/* PAYROLL's - Make (CHGMSCSAL RANGE SCREEN) Screen */
static void CHGMSCSAL_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","CHGMSCSAL RANGE SCREEN");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		...
		ADVaddwdgt(scrn,5,"","From:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"FROM [PAYJMST][GROSS IDENTIFICATION]","From Gross Identification","","",0,30,0,"","","","");
		ADVaddwdgt(scrn,5,"","To:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"TO [PAYJMST][GROSS IDENTIFICATION]","To Gross Identification","","",0,30,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"RANGE ON [POSMSTR][CLASS IDENTIFICATION]","Range on Class ID's","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","From:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"FROM [POSMSTR][CLASS IDENTIFICATION]","From Class ID","","",0,15,0,"","","","");
		ADVaddwdgt(scrn,5,"","To:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"TO [POSMSTR][CLASS IDENTIFICATION]","To Class ID","","",0,15,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,5,"","Percentage Increase:","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,0,"PERCENT INCREASE","Percent Increase","","",0,6,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
...
/* PAYROLL's - Make (CHGMSCSAL SEARCH BROWSE) Screen */

static void CHGMSCSAL_SEARCH_BROWSE()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","CHGMSCSAL SEARCH BROWSE");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"SELECT DELETEFLAG FALSE","Select non-deleted records","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,9,"SELECT DELETEFLAG TRUE","Select deleted records","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,2,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,3,"","","","",0,0,0,"","","","");
		ADVaddwdgt(scrn,1,"","","","",0,0,0,"","","","");
