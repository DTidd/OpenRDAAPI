/* paysddn.c - Set Direct Deposit Numbers */
#ifndef WIN32
#define __NAM__ "paysddn.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paysddn.exe"
#endif
#include <app.hpp>
...
int start_check=1;
static char *check_date=NULL,DID_STUFF=FALSE,*bank_id=NULL;
...
void setcheck(RDArsrc *mainrsrc)
{
	...
	char *bankid1=NULL,*payid1=NULL,*pay_id=NULL;
	char *ded_type=NULL;
	...
	char cprinted=FALSE/*,*libx=NULL,*temp1=NULL*/;
	double amount=0;
	RDATData *prev_file=NULL;
	RDATData *prev=NULL;
	NRDfield *DDNET=NULL;

	DDNET=FLDNRD(payjpms,"DIRECT DEPOSIT NET");
	...
	if(sortfile->fileno<0) 
	{
		prterr("Error:  Cannot create sort file.  Please review security.");
		return;
	}
	...
	while(!ef)
	{
		...
		{
			FINDFLDGETDOUBLE(payjpms,"DIRECT DEPOSIT NET",&amount);
			if(amount!=0.0 || DDNET==NULL)
			{
...
*/
		...
		*/
		ERRORDIALOG("Set Direct Deposit Error","No direct deposit numbers were assigned for the selected pay identification.  Please try again.",NULL,FALSE);
	}
...
void check_warnings(RDArsrc *mainrsrc)
{
	...
	if(!isEMPTY(check_date))
	{
		...
		{
			ADVWARNINGDIALOG("WARNING DIALOG SCREEN","DD DATE'S YEAR","Warning: The year of the direct deposit date does not agree with the year of the system date. Are you sure you wish to continue?",setcheck,NULL,FALSE,1,mainrsrc,NULL,NULL);
			return;
		} else {
			setcheck(mainrsrc);
		}
...
*/
		...
		++start_check;
		FINDRSCSETINT(mainrsrc,"STARTING DIRECT DEPOSIT NUMBER",start_check);
		updatersrc(mainrsrc,"STARTING DIRECT DEPOSIT NUMBER");
/*
		if(temp1!=NULL) Rfree(temp1);
		if(libx!=NULL) Rfree(libx);
*/
...
static void deddate(RDArsrc *mainrsrc,NRDfield *field)
{
	char *dirdep_date=NULL;

	readwidget(mainrsrc,"DIRECT DEPOSIT DATE");
	FINDRSCGETSTRING(mainrsrc,"DIRECT DEPOSIT DATE",&dirdep_date);
	if(field!=NULL)
	{
		if(field->len==8)
		{
			if((RDAstrlen(dirdep_date))<8)
			{
				FINDRSCSETSENSITIVE(mainrsrc,"SELECT",FALSE);
				FINDRSCSETEDITABLE(mainrsrc,"SELECT",FALSE);
			} else {
				if(((atoi(dirdep_date))<1) || ((atoi(dirdep_date))>12) || ((atoi(dirdep_date+3))<1) || ((atoi(dirdep_date+3))>31))
				{
					FINDRSCSETSENSITIVE(mainrsrc,"SELECT",FALSE);
					FINDRSCSETEDITABLE(mainrsrc,"SELECT",FALSE);
				} else {
	...
	} else if((RDAstrlen(dirdep_date))<10)
	{
		FINDRSCSETSENSITIVE(mainrsrc,"SELECT",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"SELECT",FALSE);
	} else {
	...
	if(dirdep_date!=NULL) Rfree(dirdep_date);
}
static void paysddnResetGroupDefaults(RDArsrc *r)
{
...
#endif
{
	...
	char *payid=NULL,*dedtype=NULL;
	NRDfield *field=NULL;
	char *libx=NULL;
	RDAGenericSetup *gsv=NULL;

	if(InitializeSubsystems(argc,argv,module,"SET DIRECT DEPOSIT NUMBERS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
...
*/
	...
	gsv=RDAGenericSetupNew("PAYROLL","RECORD DD NUMBER");
	if(gsv!=NULL)
	{
	...
	if(gsv!=NULL) FreeRDAGenericSetup(gsv);
	if(libx!=NULL) Rfree(libx);
	mainrsrc=RDArsrcNEW(module,"SET DIRECT DEPOSIT NUMBERS");
	...
	if(payjdpm!=(-1)) file2rangersrc(payjdpm,mainrsrc);
	...
	addstdrsrc(mainrsrc,"STARTING DIRECT DEPOSIT NUMBER",LONGV,8,&start_check,TRUE);
	field=FLDNRD(payjpms,"DIRECT DEPOSIT DATE");
	if(field!=NULL)
	{
		if(field->len==8)
		{
			addstdrsrc(mainrsrc,"DIRECT DEPOSIT DATE",DATES,8,CURRENT_DATE,TRUE);
		} else {
			addstdrsrc(mainrsrc,"DIRECT DEPOSIT DATE",DATES,10,CURRENT_DATE10,TRUE);
		}
	} else addstdrsrc(mainrsrc,"DIRECT DEPOSIT DATE",DATES,10,CURRENT_DATE10,TRUE);
	FINDRSCSETFUNC(mainrsrc,"DIRECT DEPOSIT DATE",deddate,field);
	...
	FINDRSCGETSTRING(mainrsrc,"[PAYDTYP][DEDUCTION TYPE]",&dedtype);
	FINDFLDSETSTRING(paydtyp,"DEDUCTION TYPE",dedtype);
	if(ADVEQLNRDsec(paydtyp,1,SCRNvirtualSubData,mainrsrc)) KEYNRD(paydtyp,1);
	filerecord2rsrc(paydtyp,mainrsrc);
	if(payid!=NULL) Rfree(payid);
	if(dedtype!=NULL) Rfree(dedtype);
	...
	addbtnrsrc(mainrsrc,"RESET GROUP DEFAULTS",TRUE,paysddnResetGroupDefaults,NULL);
