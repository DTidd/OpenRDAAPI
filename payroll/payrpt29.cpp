/* PAYROLL's - Make (FICAEARNINGS) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
extern char *module;
void FICAEARNINGS(void);
static void FICAEARNINGS_RANGE_SCREEN(void);
...
ELSE RETURN_VALUE=\"DECEMBER\" FI FI FI FI FI FI FI FI FI FI FI");
	...
	if(t!=NULL) Rfree(t);
	t=Rmalloc(204+1);
	sprintf(t,"IF([SELECTED MONTH]<3) THEN RETURN_VALUE=\"FIRST\"\nELSE IF([SELECTED MONTH]<6) THEN RETURN_VALUE=\"SECOND\"\nELSE IF([SELECTED MONTH]<9) THEN RETURN_VALUE=\"THIRD\"\nELSE RETURN_VALUE=\"FOURTH\" FI FI FI");
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","                                          @CCCCCCCCCCCCCCCCCC       @CCCCCCCCCCCCCCCCCC           ANNUAL@@",FALSE,NULL,4,pval);
	free_pvals(pval,4);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","PERSONNEL ID    NAME                      GROSS        AMOUNT       GROSS        AMOUNT       GROSS      AMOUNT@@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDCSM][CALENDAR YEAR] HEADER","@@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"DETAIL LINES","@LLLLLLLLLLLLLLL @LLLLLLLLLLLLLLLLLLLLLLL @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR@@",FALSE,NULL,10,pval);
	free_pvals(pval,10);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYDDMS][DEDUCTION DESCRIPTION IDENTIFICATION] FOOTER","@@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                   TOTAL @LLLLLLLLLLLLLLL @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR @,.RRRRRRRRR@@",FALSE,NULL,9,pval);
	free_pvals(pval,9);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			FICAEARNINGS_RANGE_SCREEN();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
static void FICAEARNINGS_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","FICAEARNINGS RANGE SCREEN");
	if(scrn!=NULL)
	{
