/* payjpms.c - Xpert Job Pay Master Browse */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjpms.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjpms.exe"
#endif
#include <app.hpp>
...
static void payjpmsm(short,void *);
static void doexit(MakeBrowseList *),okfunc(MakeBrowseList *,void *);
static void new_record(RDArsrc *,MakeBrowseList *);
static void quit_record(RDArsrc *);
static MakeBrowseList *mbl_payjpms=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjpms(RDArsrc *);
...
static void browse_payjpms(RDArsrc *mtnrsrc)
{
	...
	mbl_payjpms=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJPMS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJPMS");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJPMS",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJPMS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjpms!=NULL) mbl_payjpms=NULL;
}
