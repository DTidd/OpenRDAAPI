/* paydcsm.c - Xpert Payroll Deduction Calendar Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "paydcsm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paydcsm.exe"
#endif
#include <app.hpp>
...
MakeBrowseList *mbl_paydcsm=NULL;
MaintainMaster *MASTER=NULL;
void browse_paydcsm(RDArsrc *,RDATData *);

void paydcsmm(short dowhich,void *targetkey)
{
	char *name=NULL;	
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_paydcsm,previous);
	...
	if(mbl_paydcsm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_paydcsm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_paydcsm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_paydcsm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_paydcsm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_paydcsm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYDCSM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYDCSM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYDCSM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE("PAYROLL","PAYDCSM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_paydcsm!=NULL) mbl_paydcsm=NULL;
}
