...
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjams.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjams.exe"
#endif
#include <app.hpp>
...
static void setDEFtypes1(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=(short)x;
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]",d);
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][DEBIT CODE]",d);
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][ENCUMBRANCE CODE]",d);
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][RESERVE CODE]",d);
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][CREDIT CODE]",d);
}
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&x);
	d1=(short)x;
	FINDRSCGETDEFTYPE(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]",&d2);
	if(d1!=d2)
	{
		readallwidgets(mtnrsrc);
		FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFINITION TYPE",d1);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][CREDIT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CREDIT CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][DEBIT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"DEBIT CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][ENCUMBRANCE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ENCUMBRANCE CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][RESERVE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"RESERVE CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]",d1);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][DEBIT CODE]",d1);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][ENCUMBRANCE CODE]",d1);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][RESERVE CODE]",d1);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][CREDIT CODE]",d1);
...
static void setPDEFtypes1(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=(short)x;
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][PAYABLE CODE]",d);
	FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][CASH CODE]",d);
}
static void setPDEFtypes(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	if(x>=mtn->PDEFtypes->numlibs)
	{
		...
		FINDRSCSETDEFTYPE(mtnrsrc,"PAYABLE CODE",0);
...
static void changePDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDRSCGETINT(mtnrsrc,"PAYROLL FUND DEFINITION TYPES",&x);
	d1=(short)x;
	FINDRSCGETDEFTYPE(mtnrsrc,"[PAYJAMS][CASH CODE]",&d2);
	if(d1!=d2)
	{
		readallwidgets(mtnrsrc);
		FINDFLDSETSHORT(MASTER->passkey->fileno,"PAYROLL FUND DEFINITION TYPE",d1);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][CASH CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CASH CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"PAYABLE CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"[PAYJAMS][PAYABLE CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][CASH CODE]",d1);
		FINDRSCSETDEFTYPE(mtnrsrc,"[PAYJAMS][PAYABLE CODE]",d1);
		computeallSCRNvirtuals(mtnrsrc);
		updateallrsrc(mtnrsrc);
	}
}
...
static short checkacct(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	readwidget(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]",&acctcode);
...
static short checkencumbrance(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&x);
	FINDRSCGETCHAR(mtnrsrc,"[PAYJAMS][DISENCUMBER]",&disenc);
	FINDFLDSETCHAR(MASTER->passkey->fileno,"DISENCUMBER",disenc);
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[PAYJAMS][ENCUMBRANCE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][ENCUMBRANCE CODE]",&acctcode);
...
static short chkencumbrancecode(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDFLDGETCHAR(MASTER->passkey->fileno,"[PAYJAMS][DISENCUMBER]",&disenc);
	if(disenc)
	{
...
static short checkreserve(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[PAYJAMS][RESERVE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][RESERVE CODE]",&acctcode);
...
static short checkdebit(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[PAYJAMS][DEBIT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][DEBIT CODE]",&acctcode);
...
static short checkpayable(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	readwidget(mtnrsrc,"[PAYJAMS][PAYABLE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][PAYABLE CODE]",&acctcode);
...
static short checkcash(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"PAYROLL FUND DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[PAYJAMS][CASH CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][CASH CODE]",&acctcode);
...
static short checkcredit(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[PAYJAMS][CREDIT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[PAYJAMS][CREDIT CODE]",&acctcode);
...
void payjamsm(short dowhich,void *targetkey)
{
		...
		for(x=0,fldx=fields;x<nofields;++x,++fldx)
		{
			...
			} else if(!RDAstrcmp(fldx->name,"PAYROLL FUND DEFINITION TYPE"))
			{
				...
				addlstrsrc(mtnrsrc,"PAYROLL FUND DEFINITION TYPES",&acctno,
					TRUE,changePDEFtype,mtn->PDEFtypes->numlibs,
	...
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][ACCOUNT CODE]",chkacctcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][DEBIT CODE]",chkdebitcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][ENCUMBRANCE CODE]",chkencumbrancecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][RESERVE CODE]",chkreservecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][PAYABLE CODE]",chkpayablecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][CASH CODE]",chkcashcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[PAYJAMS][CREDIT CODE]",chkcreditcodecb,mtn);
...
/* Put this here to get around the setting of the DEFTYPE not working 
  before the screen is made, orginally found this in finaje.c by DCT */
	list2file(mtnrsrc,mtn);
}
...
static void save_check(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCGETCHAR(mtnrsrc,"[PAYJAMS][DELETEFLAG]",&delflag);
	FINDRSCGETCHAR(mtnrsrc,"[PAYJAMS][DISENCUMBER]",&disenc);
	if(!delflag)
	{
		...
		if(!RDAstrcmp(mtn->DEFtypes->libs[seldeftype],"No Definitions Available"))
		{
			...
			if(checkacct(mtnrsrc,mtn,TRUE)==TRUE)
			{
				if(checkdebit(mtnrsrc,mtn,TRUE)==TRUE)
				{
					...
					{
						if(checkcredit(mtnrsrc,mtn,TRUE)==TRUE)
						{
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJAMS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJAMS");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJAMS",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJAMS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
