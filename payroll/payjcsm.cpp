/* payjcsm.c - Xpert Payroll Job Calendar Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjcsm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjcsm.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_payjcsm=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjcsm(RDArsrc *,RDATData *);
...
void payjcsmm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjcsm,previous);
	...
	if(mbl_payjcsm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_payjcsm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_payjcsm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_payjcsm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_payjcsm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_payjcsm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJCSM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJCSM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJCSM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJCSM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjcsm!=NULL) mbl_payjcsm=NULL;
}
