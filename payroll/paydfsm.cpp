/* paydfsm.c - Xpert Payroll Deduction Fiscal Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "paydfsm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paydfsm.exe"
#endif
#include <app.hpp>
...
MakeBrowseList *mbl_paydfsm=NULL;
MaintainMaster *MASTER=NULL;
void browse_paydfsm(RDArsrc *,RDATData *);

void paydfsmm(short dowhich,void *targetkey)
{
	char *name=NULL;	
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_paydfsm,previous);
	...
	if(mbl_paydfsm!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_paydfsm==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_paydfsm,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_paydfsm!=NULL) quit_record(mtnrsrc,previous);
...
void browse_paydfsm(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_paydfsm=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYDFSM")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYDFSM");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYDFSM",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE("PAYROLL","PAYDFSM",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_paydfsm!=NULL) mbl_paydfsm=NULL;
}
