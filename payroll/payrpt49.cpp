/* PAYROLL's - Make (PAYCKREG-ID) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","   Check Num      SSN          Employee ID   Name                                    Net Pay@@",FALSE,NULL,2,pval);
	free_pvals(pval,2);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"[PAYROLL][PAYJPMS][CHECK NUMBER] FOOTER","   @RRRRRRRRRRRR  @LLLLLLLLLL  @LLLLLLLLLLL  @LLLLLLLLLLLLLLLLLLLLLLLLLLLLL  @,.RRRRRRRRRRRR@@",FALSE,NULL,7,pval);
	free_pvals(pval,7);
