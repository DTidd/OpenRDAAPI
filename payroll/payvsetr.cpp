/* payvsetr.c - Set Vendor Withholding Electronic Transfer Numbers */
#ifndef WIN32
#define __NAM__ "payvsetr.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payvsetr.exe"
#endif
#include <app.hpp>
...
void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual(module,"VENDOR WITHHOLDING ELECTRONIC TRANSFER SORT SEQUENCE");
	if(d!=NULL)
	{
...
int voucher_count(RDArsrc *mainrsrc,short fileno,char *vendorid,char *venaddrid)
{
	int count=0;
	short ef;
...
*/
	...
	while(!ef)
	{
		...
		if(!RDAstrcmp(venid,vendorid) && !RDAstrcmp(addrid,venaddrid))
		{
			++count;
		} else {
			break;
...
void setcheck(RDArsrc *mainrsrc)
{
	...
	char *venaddrid=NULL,*last_venaddrid=NULL;
	...
	while(!ef)
	{
		...
		if(diagapps)
		{
			...
			FINDFLDGETINT(payddsv,"FISCAL YEAR",&fyear);
			FINDFLDGETINT(payddsv,"TRANSACTION NUMBER",&transno);
			prterr("Skipped the [PAYROLL][PAYDDSV] (Deduction Description Voucher) data file record Deduction Description Identification [%s], Fiscal Year [%d], and Transaction Number [%d] because of ELECTRONIC TRANSFER field value of FALSE .",
				(dedid==NULL?"":dedid),fyear,transno);
		}
		}
		} else {
	...
	if(SIZNRD(sortfile->fileno))
	{
		...
		if(sortfile2!=(-1))
		{
			...
			if(key!=NULL)
			{
				...
				ven_count=voucher_count(mainrsrc,sortfile2,vendorid,venaddrid);
				...
				while(!ef)
				{
					...
					FINDFLDGETSTRING(sortfile->fileno,
						"ADDRESS IDENTIFICATION",&venaddrid);
					if(RDAstrcmp(last_vendorid,vendorid) || RDAstrcmp(last_venaddrid,venaddrid))
					{
					...
					if(!ADVEQLNRDsec(payddsv,1,SCRNvirtualSubData,mainrsrc))
					{
						...
						FINDFLDSETINT(payddsv,"ELECTRONIC TRANSFER NUMBER",
							check_number);
						FINDFLDSETINT(payddsv,"VIRTUAL ELECTRONIC TRANSFER NUMBER",
							virt_chk_no);
						FINDFLDSETSTRING(payddsv,"ELECTRONIC TRANSFER DATE",
							check_date);
...
static void payvsetrResetGroupDefaults(RDArsrc *r)
{
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"SET VENDOR WITHHOLDING ELECTRONIC TRANSFER NUMBERS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	if(gsv!=NULL) FreeRDAGenericSetup(gsv);
	if(libx!=NULL) Rfree(libx);
	mainrsrc=RDArsrcNEW(module,"SET VENDOR WITHHOLDING ELECTRONIC TRANSFER NUMBERS");
	...
	ADVGET_SCREEN_VIRTUALS(mainrsrc,2,TRUE,TRUE);
	...
	field=FLDNRD(payddsv,"ELECTRONIC TRANSFER DATE");
	if(field!=NULL)
	{
		if(field->len==8)
		{
	...
	} else addstdrsrc(mainrsrc,"ELECTRONIC TRANSFER DATE",DATES,8,CURRENT_DATE,TRUE);
	FINDRSCSETFUNC(mainrsrc,"ELECTRONIC TRANSFER DATE",chkdate,field);
	...
	addbtnrsrc(mainrsrc,"RESET GROUP DEFAULTS",TRUE,payvsetrResetGroupDefaults,NULL);
	...
	if(banknum!=(-1)) bankidcb(mainrsrc);
