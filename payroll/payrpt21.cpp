/* PAYROLL's - Make (PAYDSTE1) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
----------------------------------------------------------------------*/
	...
	memset(stemp,0,101);
	rptname=Rmalloc(8+1);
	sprintf(rptname,"PAYDSTE1");
	t=Rmalloc(51+1);
	sprintf(t,"Payroll Encumbrance Distribution Updating Datafiles");
	desc=Rmalloc(175+1);
	sprintf(desc,"This output definition is used to actually update the database files with the Payroll Distribution Information.  This report is configured to also manage the dis-encumbrances.");
...
OUNT] \n     ELSE RETURN_VALUE=0 \n     FI \nFI");
	...
	if(t!=NULL) Rfree(t);
	t=Rmalloc(268+1);
	sprintf(t,"IF([WRITE COUNT]=6) \nTHEN IF([PRIMARY FILE NUMBER]=1) \n       THEN RETURN_VALUE=[PAYROLL][PAYJTRN][AMOUNT]*(-1) \n     ELSE IF([PRIMARY FILE NUMBER]=2) \n       THEN RETURN_VALUE=[PAYROLL][PAYDTRN][AMOUNT]*(-1) \n \
    ELSE RETURN_VALUE=0 \n     FI FI\nELSE RETURN_VALUE=0 FI");
...
----------------------------------------------------------------------*/
	...
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[LAST FISCAL PROCESSING MONTH]");
	...
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("2");
	pvals->fieldname=stralloc("[FINMGT][FINEACT][TYPE]");
	addimageline(tmp,"[ACCOUNT CODE] FOOTER","WRITE [FINMGT][FINEACT]",TRUE,"([LAST ACCOUNT TYPE]=0) AND ([TOTAL ENCUMBRANCE AMOUNT]!=0.0)",13,pval);
	free_pvals(pval,13);
	...
	pval=Rmalloc(59*sizeof(RDApval));
	pvals=pval;
	pvals->symbol_type=0;
	pvals->type=0;
	pvals->name=stralloc("[LAST DEFINITION TYPE]");
	...
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][PRE-YEAR UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][PRE-YEAR UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][PRE-YEAR DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][PRE-YEAR DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][PRE-YEAR DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINEYR][JANUARY UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JANUARY UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JANUARY UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINEYR][JANUARY UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JANUARY UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JANUARY UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINEYR][JANUARY DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JANUARY DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JANUARY DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINEYR][JANUARY DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JANUARY DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JANUARY DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][FEBRUARY UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][FEBRUARY UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][FEBRUARY DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][FEBRUARY DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][FEBRUARY DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINEYR][MARCH UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MARCH UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MARCH UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINEYR][MARCH UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MARCH UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MARCH UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINEYR][MARCH DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MARCH DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MARCH DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINEYR][MARCH DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MARCH DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MARCH DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINEYR][APRIL UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][APRIL UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][APRIL UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINEYR][APRIL UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][APRIL UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][APRIL UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINEYR][APRIL DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][APRIL DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][APRIL DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINEYR][APRIL DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][APRIL DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][APRIL DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINEYR][MAY UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MAY UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MAY UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINEYR][MAY UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MAY UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MAY UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINEYR][MAY DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MAY DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MAY DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINEYR][MAY DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][MAY DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][MAY DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINEYR][JUNE UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JUNE UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JUNE UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINEYR][JUNE UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JUNE UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JUNE UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINEYR][JUNE DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JUNE DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JUNE DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINEYR][JUNE DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JUNE DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JUNE DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINEYR][JULY UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JULY UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JULY UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINEYR][JULY UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JULY UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JULY UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINEYR][JULY DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JULY DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JULY DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINEYR][JULY DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][JULY DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][JULY DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINEYR][AUGUST UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][AUGUST UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][AUGUST UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINEYR][AUGUST UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][AUGUST UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][AUGUST UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINEYR][AUGUST DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][AUGUST DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][AUGUST DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINEYR][AUGUST DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][AUGUST DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][AUGUST DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][SEPTEMBER UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][SEPTEMBER UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][SEPTEMBER DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][SEPTEMBER DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][SEPTEMBER DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINEYR][OCTOBER UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][OCTOBER UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][OCTOBER UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINEYR][OCTOBER UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][OCTOBER UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][OCTOBER UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINEYR][OCTOBER DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][OCTOBER DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][OCTOBER DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINEYR][OCTOBER DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][OCTOBER DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][OCTOBER DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][NOVEMBER UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][NOVEMBER UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][NOVEMBER DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][NOVEMBER DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][NOVEMBER DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINEYR][DECEMBER UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][DECEMBER UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][DECEMBER UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINEYR][DECEMBER UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][DECEMBER UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][DECEMBER UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINEYR][DECEMBER DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][DECEMBER DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][DECEMBER DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINEYR][DECEMBER DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][DECEMBER DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][DECEMBER DISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINEYR][POST YEAR UNDISTRIBUTED EXPENDITURES]-[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][POST YEAR UNDISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][POST YEAR UNDISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINEYR][POST YEAR UNDISTRIBUTED ENCUMBRANCES]-[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][POST YEAR UNDISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][POST YEAR UNDISTRIBUTED ENCUMBRANCES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINEYR][POST YEAR DISTRIBUTED EXPENDITURES]+[TOTAL EXPENSED AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][POST YEAR DISTRIBUTED EXPENDITURES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][POST YEAR DISTRIBUTED EXPENDITURES]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINEYR][POST YEAR DISTRIBUTED ENCUMBRANCES]+[TOTAL ENCUMBRANCE AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINEYR][POST YEAR DISTRIBUTED ENCUMBRANCES] FI");
	pvals->fieldname=stralloc("[FINMGT][FINEYR][POST YEAR DISTRIBUTED ENCUMBRANCES]");
	addimageline(tmp,"[ACCOUNT CODE] FOOTER","WRITE [FINMGT][FINEYR]",TRUE,"[LAST ACCOUNT TYPE]=0",59,pval);
	free_pvals(pval,59);
	...
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][PRE-YEAR UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][PRE-YEAR DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINBYR][JANUARY UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JANUARY UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JANUARY UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINBYR][JANUARY DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JANUARY DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JANUARY DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][FEBRUARY UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][FEBRUARY DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINBYR][MARCH UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MARCH UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MARCH UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINBYR][MARCH DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MARCH DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MARCH DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINBYR][APRIL UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][APRIL UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][APRIL UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINBYR][APRIL DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][APRIL DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][APRIL DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINBYR][MAY UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MAY UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MAY UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINBYR][MAY DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MAY DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MAY DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINBYR][JUNE UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JUNE UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JUNE UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINBYR][JUNE DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JUNE DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JUNE DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINBYR][JULY UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JULY UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JULY UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINBYR][JULY DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JULY DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JULY DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINBYR][AUGUST UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][AUGUST UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][AUGUST UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINBYR][AUGUST DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][AUGUST DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][AUGUST DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][SEPTEMBER UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][SEPTEMBER DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINBYR][OCTOBER UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][OCTOBER UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][OCTOBER UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINBYR][OCTOBER DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][OCTOBER DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][OCTOBER DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][NOVEMBER UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][NOVEMBER DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINBYR][DECEMBER UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][DECEMBER UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][DECEMBER UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINBYR][DECEMBER DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][DECEMBER DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][DECEMBER DISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINBYR][POST YEAR UNDISTRIBUTED DEBITS]-[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][POST YEAR UNDISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][POST YEAR UNDISTRIBUTED DEBITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINBYR][POST YEAR DISTRIBUTED DEBITS]+[TOTAL DEBIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][POST YEAR DISTRIBUTED DEBITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][POST YEAR DISTRIBUTED DEBITS]");
	addimageline(tmp,"[ACCOUNT CODE] FOOTER","WRITE [FINMGT][FINBYR]",TRUE,"([LAST ACCOUNT TYPE]=2) AND ([TOTAL DEBIT AMOUNT]!=0)",31,pval);
	free_pvals(pval,31);
	...
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][PRE-YEAR UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=0) THEN RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][PRE-YEAR DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][PRE-YEAR DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINBYR][JANUARY UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JANUARY UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JANUARY UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=1) THEN RETURN_VALUE=[FINMGT][FINBYR][JANUARY DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JANUARY DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JANUARY DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][FEBRUARY UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=2) THEN RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][FEBRUARY DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][FEBRUARY DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINBYR][MARCH UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MARCH UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MARCH UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=3) THEN RETURN_VALUE=[FINMGT][FINBYR][MARCH DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MARCH DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MARCH DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINBYR][APRIL UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][APRIL UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][APRIL UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=4) THEN RETURN_VALUE=[FINMGT][FINBYR][APRIL DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][APRIL DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][APRIL DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINBYR][MAY UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MAY UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MAY UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=5) THEN RETURN_VALUE=[FINMGT][FINBYR][MAY DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][MAY DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][MAY DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINBYR][JUNE UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JUNE UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JUNE UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=6) THEN RETURN_VALUE=[FINMGT][FINBYR][JUNE DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JUNE DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JUNE DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINBYR][JULY UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JULY UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JULY UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=7) THEN RETURN_VALUE=[FINMGT][FINBYR][JULY DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][JULY DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][JULY DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINBYR][AUGUST UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][AUGUST UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][AUGUST UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=8) THEN RETURN_VALUE=[FINMGT][FINBYR][AUGUST DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][AUGUST DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][AUGUST DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][SEPTEMBER UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=9) THEN RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][SEPTEMBER DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][SEPTEMBER DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINBYR][OCTOBER UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][OCTOBER UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][OCTOBER UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=10) THEN RETURN_VALUE=[FINMGT][FINBYR][OCTOBER DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][OCTOBER DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][OCTOBER DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][NOVEMBER UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=11) THEN RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][NOVEMBER DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][NOVEMBER DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINBYR][DECEMBER UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][DECEMBER UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][DECEMBER UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=12) THEN RETURN_VALUE=[FINMGT][FINBYR][DECEMBER DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][DECEMBER DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][DECEMBER DISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINBYR][POST YEAR UNDISTRIBUTED CREDITS]-[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][POST YEAR UNDISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][POST YEAR UNDISTRIBUTED CREDITS]");
	++pvals;
	pvals->symbol_type=1;
	pvals->type=0;
	pvals->name=stralloc("IF([LAST FISCAL PROCESSING MONTH]=13) THEN RETURN_VALUE=[FINMGT][FINBYR][POST YEAR DISTRIBUTED CREDITS]+[TOTAL CREDIT AMOUNT] ELSE RETURN_VALUE=[FINMGT][FINBYR][POST YEAR DISTRIBUTED CREDITS] FI");
	pvals->fieldname=stralloc("[FINMGT][FINBYR][POST YEAR DISTRIBUTED CREDITS]");
	addimageline(tmp,"[ACCOUNT CODE] FOOTER","WRITE [FINMGT][FINBYR]",TRUE,"([LAST ACCOUNT TYPE]=2) AND ([TOTAL CREDIT AMOUNT]!=0)",31,pval);
	free_pvals(pval,31);
