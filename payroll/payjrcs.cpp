/* payjrcs.c - Xpert Payroll Job Rate Calendar Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjrcs.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjrcs.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_payjrcs=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjrcs(RDArsrc *,RDATData *);
...
void payjrcsm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	if(fields!=NULL)
	{
		for(x=0,fldx=fields;x<nofields;++x,++fldx)
		{
			...
			if(edit_rsrc)
			{
				...
				} else if(fldx->type==LONGV)
				{
					edit_rsrc=(short)(RDAstrcmp(fldx->name,"NUMBER OF CHECKS ISSUED"));
				}
			}
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjrcs,previous);
	...
	if(mbl_payjrcs!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_payjrcs==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_payjrcs,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_payjrcs!=NULL) quit_record(mtnrsrc,previous);
...
void browse_payjrcs(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_payjrcs=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJRCS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJRCS");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJRCS",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJRCS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjrcs!=NULL) mbl_payjrcs=NULL;
}
