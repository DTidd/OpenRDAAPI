/* payjrfs.c - Xpert Payroll Job Rate Fiscal Summary Master */
/*lint -library */
#ifndef WIN32
#define __NAM__ "payjrfs.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payjrfs.exe"
#endif
#include <app.hpp>
...
static MakeBrowseList *mbl_payjrfs=NULL;
static MaintainMaster *MASTER=NULL;
static void browse_payjrfs(RDArsrc *,RDATData *);
...
void payjrfsm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_payjrfs,previous);
	...
	if(mbl_payjrfs!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"SAVE NO UPDATE",FALSE);
	}
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_payjrfs==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_payjrfs,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_payjrfs!=NULL) quit_record(mtnrsrc,previous);
...
void browse_payjrfs(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_payjrfs=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"PAYROLL","MTN PAYJRFS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("PAYROLL","MTN PAYJRFS");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE);
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN PAYJRFS",MASTER->module);
		doexit(NULL);
	} else {
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"PAYJRFS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
...
void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_payjrfs!=NULL) mbl_payjrfs=NULL;
}
