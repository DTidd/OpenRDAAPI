/* PAYROLL's - Make (941FEDERAL-CHAM) Report */
#include <cstdio>
#include <rptgen.hpp>
/*COMPMEM 40960 */
...
extern char *module;
void FEDERAL941(void);
static void FEDERAL941_RANGE_SCREEN(void);
...
----------------------------------------------------------------------*/
	...
	sprintf(rptname,"941FEDERAL");
	t=Rmalloc(22+1);
	sprintf(t,"941 Federal Tax Report");
	desc=Rmalloc(108+1);
	sprintf(desc,"This output definition is used to print the standard 941 Federal Tax Report report.");
...
SE IF([SELECTED QUARTER]=3) THEN \n	RETURN_VALUE=[THIRD QUARTER CALENDAR PERSONNEL DEDUCTION AMOUNT]\nELSE RETURN_VALUE=[FOURTH QUARTER CALENDAR PERSONNEL DEDUCTION AMOUNT]\nFI FI FI");
	...
	if(t!=NULL) Rfree(t);
	t=Rmalloc(47+1);
	sprintf(t,"(\"941 FEDERAL REPORT (\"+[QUARTER NAME]+\")\")");
	addvirtuallineADV(tmp,"REPORT TITLE",0,50,t,1,0);
...
----------------------------------------------------------------------*/
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"PAGE HEADER","                                                            @CCCCCCCCCCCCC                                         ANNUAL@@",FALSE,NULL,3,pval);
	free_pvals(pval,3);
	...
	pvals->name=stralloc("LINEFEED");
	pvals->fieldname=NULL;
	addimageline(tmp,"REPORT FOOTER","                                  TOTAL @,.RRRRRRRRRRRR  @,.RRRRRRRRRRRR  @,.RRRRRRRRRRRR  @,.RRRRRRRRRRRR  @,.RRRRRRRRRRRR  @,.RRRRRRRRRRRR@@",FALSE,NULL,8,pval);
	free_pvals(pval,8);
	...
	if(tmp!=NULL)
	{
		if(WRTRDAreport(module,tmp))
		{
			...
			FEDERAL941_RANGE_SCREEN();
		}
		FreeRDAreport(tmp);
	}
	if(rptname!=NULL) Rfree(rptname);
}
/* PAYROLL's - Make (941FEDERAL-CHAM RANGE SCREEN) Screen */

static void FEDERAL941_RANGE_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
...
============================================================================*/
	...
	scrn=RDAscrnNEW("PAYROLL","941FEDERAL RANGE SCREEN");
	if(scrn!=NULL)
	{
