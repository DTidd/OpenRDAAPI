/* empabsm.c - Xpert Employee Absentee Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern MakeBrowseList *mbl_empabs;
extern void browse_empabs(RDArsrc *,RDATData *);
...
void empabsm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_empabs,previous);
	if(mbl_empabs!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,mbl_empabs==NULL?TRUE:FALSE);
}
static void set_appdefaults(RDArsrc *mtnrsrc,RDATData *previous,short update)
{
	...
	FINDRSCSETSTRING(mtnrsrc,"[EMPABS][ENTRY DATE]",date);
	FINDRSCSETSTRING(mtnrsrc,"[EMPABS][ENTRY TIME]",timex);
	FINDRSCSETSTRING(mtnrsrc,"[EMPABS][SOURCE USER]",USERLOGIN);
	if(update)
	{
...
static void seteditable(RDArsrc *mtnrsrc,RDATData *previous,short update)
{
	...
	FINDFLDGETCHAR(MASTER->passkey->fileno,"SOURCE USER VERIFIED",&closed);
	if(closed)
	{
		if(update)
		{
		...
		FINDRSCSETEDITABLE(mtnrsrc,"[EMPABS][DELETEFLAG]",TRUE);
		if(update) updatersrc(mtnrsrc,"[EMPABS][DELETEFLAG]");
	} else {
		if(update)
		{
			updateeditfilersrcsnokeys(MASTER->passkey->fileno,MASTER->passkey->keyno,mtnrsrc,TRUE);
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_empabs,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_empabs!=NULL)
		{
