/* finintera.c - Financial Management Inter-Bank Transfers (a)*/
#ifndef WIN32
#define __NAM__ "finintera.lnx"
#endif
#ifdef WIN32
#define __NAM__ "finintera.exe"
#endif
#include <app.h>
#include <stdlib.h>
#include <rdaglob.h>
...
static short filenum=0,yearnum=0,balnum=0,expnum=0,revnum=0,jrnlnum=0;
...
static void save_record()
{
	...
	if(FINDRSCGETDOUBLE(mainrsrc,"AMOUNT",&amount)) return;
	if(amount==0) return;	
...
void changeDEFtype()
{
	...
	while(!ef)
	{
		FINDFLDGETSHORT(balnum,"DEFINITION TYPE",
			&deftype);
...
void main(int argc,char **argv)
{
	...
	if((filenum=OPNNRDsec(module,"FININTRA",TRUE,TRUE,FALSE))==(-1))
	{
		ERRORDIALOG("Security Access Denied","Security Access denied to the [FININTRA] file or the file cannot otherwise be opened.\n",NULL,TRUE);
		ShutdownSecurity();
		RDAAPPMAINLOOP();
		quit_record();
		return;
	...
	jrnlnum=OPNNRDsec(module,"FINJRNL",TRUE,FALSE,FALSE);
	balnum=OPNNRDsec(module,"FINBACC",TRUE,FALSE,FALSE);
	mk_define_list(definelist,nofields,fields,nokeys,keys,NULL,filenum,1);
	...
	for(x=0,fldx=fields;x<nofields;++x,++fldx)
	{
		...
		{
		} else if(!RDAstrcmp(fldx->name,"CASHFROM CODE"))
		{
		} else if(!RDAstrcmp(fldx->name,"CASHTO CODE"))
		{
	...
	{
		addAPPlib(prevtrans,"No Intra-Fund Bank Transfers added this session");
	}
