		...
		case 0:
			switch(accttype1)
			{
				case REV_ACCT:
					...
					break;
				case BAL_ACCT:
					if((tmp=ADVCHECKACCT(byrnum,baccnum,acctcode1,accttype1,deftype1,yearid1,SCRNvirtualSubData,mtnrsrc))!=NULL)
					{
	...
	int selectedd,short update_list,short restore_accounts)
{
	...
	if(accttype1!=accttype2 || deftype1!=deftype2)
	{
		...
		{
				FINDRSCSETSENSITIVE(mtnrsrc,"[FINREC][DEBIT CODE]",FALSE);
				FINDRSCSETSENSITIVE(mtnrsrc,"[FINREC][CREDIT CODE]",FALSE);
        			FINDRSCSETEDITABLE(mtnrsrc,"[FINREC][DEBIT CODE]",FALSE);
        			FINDRSCSETEDITABLE(mtnrsrc,"[FINREC][CREDIT CODE]",FALSE);
		}
	}
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	int year=0;
	double amount=0.0,newamt=0.0,initamt=0.0;
				...
				if(dist==FALSE && delflag==FALSE)
				{
					...
					if(banknum!=(-1))
					{
						...
						if(depnum!=(-1))
						{
							...
							FINDFLDSETDOUBLE(depnum,"DEPOSIT AMOUNT",newamt);
							if(ADVWRTTRANSsec(depnum,1,"DEPOSIT NUMBER",NULL,SCRNvirtualSubData,mtnrsrc))
							{
								WARNINGDIALOG("WARNING DIALOG SCREEN","WRITE ERROR OCCURRED!"
									,"The write function failed when attempting to write a recordto [BNKREC][BNKDEP]"
									,NULL,NULL,TRUE,NULL);
							} else {
								ZERNRD(banknum);
								FINDFLDSETSTRING(banknum,"BANK IDENTIFICATION",bankid);
								LOCNRD(banknum);
								if(ADVEQLNRDsec(banknum,1,SCRNvirtualSubData,mtnrsrc))
								{
								} else {
									...
									if(ADVWRTTRANSsec(banknum,0,NULL,previous_bank,SCRNvirtualSubData,mtnrsrc))
									{
										WARNINGDIALOG("WARNING DIALOG SCREEN","WRITE ERROR OCCURRED!"
										,"The write function failed when attempting to write a record to [BNKREC][BNKACC]"
										,NULL,NULL,TRUE,NULL);
									}
							...
							WARNINGDIALOG("WARNING DIALOG SCREEN","CAN NOT FIND FILE!"
								,"The [BNKREC][BNKDEP] data file was not found.  Make sure that it exist and that the [BNKDEP.idx] and [BNKDEP.dat] have been made.  If so contact your istaller."
								,NULL,NULL,TRUE,NULL);
						}
					}
				}
			...
			if(dist==FALSE && delflag==FALSE)
			{
				...
				if(banknum!=(-1))
				{
					...
					if(depnum!=(-1))
					{
						...
						if(date!=NULL) { Rfree(date); date=NULL; }
					} else {
						WARNINGDIALOG("WARNING DIALOG SCREEN","CAN NOT FIND FILE!"
							,"The [BNKREC][BNKDEP] data file was not found.  Make sure that it exist and that the [BNKDEP.idx] and [BNKDEP.dat] have been made.  If so contact your istaller."
							,NULL,NULL,TRUE,NULL);
