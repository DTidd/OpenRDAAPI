/* fineactm.c - Expenditure Activity Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern short eyrnum;
extern MakeBrowseList *mbl_fineact;
extern void browse_fineact(RDArsrc *,mtnstruct *);
...
void fineactm(short,void *);
char editable=TRUE;

void fineactm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_fineact,mtn);
	if(mbl_fineact!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_fineact==NULL?TRUE:FALSE));
}
static void set_appdefaults(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCSETSTRING(mtnrsrc,"[FINEACT][SOURCE USER]",USERLOGIN);
	FINDRSCSETSTRING(mtnrsrc,"[FINEACT][SOURCE MODULE]",MASTER->module);
	FINDRSCSETSTRING(mtnrsrc,"[FINEACT][SOURCE PROCESS]",MASTER->name);
	FINDRSCSETSTRING(mtnrsrc,"[FINEACT][ENTRY DATE]",date);
	FINDRSCSETSTRING(mtnrsrc,"[FINEACT][ENTRY TIME]",timex);
	if(update)
	{
...
static void seteditable(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCGETCHAR(mtnrsrc,"[FINEACT][POSTED]",&dist);
	if(dist)
	{
		if(update)
		{
		...
		FINDRSCSETEDITABLE(mtnrsrc,"[FINEACT][DELETEFLAG]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[FINEACT][POSTED]",TRUE);
	} else {
		if(update)
		{
			updateeditfilersrcsnokeys(MASTER->passkey->fileno,MASTER->passkey->keyno,mtnrsrc,TRUE);
		...
		if(numflds>0)
		{
			...
			if(!ADVEQLNRDsec(MASTER->passkey->fileno,MASTER->passkey->keyno,SCRNvirtualSubData,mtnrsrc))
			{
				...
				if(posted==FALSE && delflag==FALSE)
				{
					...
					{
						case 2:
							ADVupdatedist(MASTER->passkey->fileno,"ACCOUNT CODE",NULL,NULL,NULL,eyrnum,(-1),year,deftype1,"ENCUMBRANCES",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							break;
						case 1:
							ADVupdatedist(MASTER->passkey->fileno,"ACCOUNT CODE",NULL,NULL,NULL,eyrnum,(-1),year,deftype1,"APPROPRIATIONS",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							break;
						case 0:
							ADVupdatedist(MASTER->passkey->fileno,"ACCOUNT CODE",NULL,NULL,NULL,eyrnum,(-1),year,deftype1,"EXPENDITURES",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							break;
						default:
			...
			updatebrowse(update_list,mbl_fineact,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		} /* end if(numflds>0) */
		if(mbl_fineact!=NULL)
		{
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINEACT][ACCOUNT CODE]",deftype1);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINEACT][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINEACT][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINEACT][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINEACT][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINEACT][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
