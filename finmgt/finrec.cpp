...
/*lint -library */
#ifndef WIN32
#define __NAM__ "finrec.lnx"
#endif
#ifdef WIN32
#define __NAM__ "finrec.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rdaflds.h>
...
void main(int argc,char **argv)
{
	...
	if(findMaintainMaster(MASTER))
	{
		...
		if(banknum!=(-1))
		{
			if((depnum=OPNNRDsec("BNKREC","BNKDEP",TRUE,TRUE,FALSE))==(-1))
			{
				ERRORDIALOG("Security Access Denied","Security Access denied to the [BNKDEP] file or the file cannot otherwise be opened.\n",NULL,TRUE);
				ShutdownSecurity();
				RDAAPPMAINLOOP();
				return;
			}
