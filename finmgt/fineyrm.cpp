/* fineyrm.c - EXPENDITURE ACCOUNTS YEAR MAINTAIN */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern MakeBrowseList *mbl_fineyr;
extern void browse_fineyr(RDArsrc *,mtnstruct *);
...
void fineyrm(short,void *);
char editable=TRUE;

void fineyrm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_fineyr,mtn);
	if(mbl_fineyr!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_fineyr==NULL?TRUE:FALSE));
}
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINEYR][ACCOUNT CODE]",deftype1);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINEYR][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINEYR][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINEYR][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINEYR][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINEYR][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_fineyr,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_fineyr!=NULL) quit_record(mtnrsrc,mtn);
