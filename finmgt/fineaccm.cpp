/* fineaccm.c - Xpert Financial Expenditure Accounts Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern void browse_fineacc(RDArsrc *,mtnstruct *);
...
char editable=TRUE;
MakeBrowseList *mbl_fineacc=NULL;
void fineaccm(short,void *);
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINEACC][ACCOUNT CODE]",deftype);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINEACC][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINEACC][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINEACC][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINEACC][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINEACC][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
