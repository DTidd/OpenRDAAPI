...
extern short byrnum;
extern MakeBrowseList *mbl_finbact;
extern void browse_finbact(RDArsrc *,mtnstruct *);
...
void finbactm(short,void *);
char editable=TRUE;

void finbactm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_finbact,mtn);
	if(mbl_finbact!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_finbact==NULL?TRUE:FALSE));
}
static void set_appdefaults(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCSETSTRING(mtnrsrc,"[FINBACT][SOURCE USER]",USERLOGIN);
	FINDRSCSETSTRING(mtnrsrc,"[FINBACT][SOURCE MODULE]",MASTER->module);
	FINDRSCSETSTRING(mtnrsrc,"[FINBACT][SOURCE PROCESS]",MASTER->name);
	FINDRSCSETSTRING(mtnrsrc,"[FINBACT][ENTRY DATE]",date);
	FINDRSCSETSTRING(mtnrsrc,"[FINBACT][ENTRY TIME]",timex);
	if(update)
	{
...
static void seteditable(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCGETCHAR(mtnrsrc,"[FINBACT][POSTED]",&dist);
	if(dist)
	{
		if(update)
		{
		...
		FINDRSCSETEDITABLE(mtnrsrc,"[FINBACT][DELETEFLAG]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[FINBACT][POSTED]",TRUE);
	} else {
		if(update)
		{
			updateeditfilersrcsnokeys(MASTER->passkey->fileno,MASTER->passkey->keyno,mtnrsrc,TRUE);
		...
		if(numflds>0)
		{
			...
			if(!ADVEQLNRDsec(MASTER->passkey->fileno,MASTER->passkey->keyno,SCRNvirtualSubData,mtnrsrc))
			{
				...
				if(posted==FALSE && delflag==FALSE)
				{
					...
					{
						case 1:
							ADVupdatedist(MASTER->passkey->fileno,"ACCOUNT CODE",NULL,NULL,NULL,byrnum,(-1),year,deftype1,"CREDITS",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							break;
						case 0:
							ADVupdatedist(MASTER->passkey->fileno,"ACCOUNT CODE",NULL,NULL,NULL,byrnum,(-1),year,deftype1,"DEBITS",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							break;
						default:
			...
			updatebrowse(update_list,mbl_finbact,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		} /* end if(numflds>0) */
		if(mbl_finbact!=NULL)
		{
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINBACT][ACCOUNT CODE]",deftype1);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINBACT][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINBACT][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINBACT][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINBACT][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINBACT][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
