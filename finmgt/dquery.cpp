...
int dQueryCustomTableFunction(Wt::WWidget *parent,Wt::WWidget **pop)
{
...
void DisplayData()
{
	...
	while(!ef)
	{
		...
		else if(count<((CurrentPage-1)*NumberRows))
		{
			...
			FINDRSCSETSENSITIVE(MAINRSRC,stemp,TRUE);
			...
			memset(stemp,0,101);
			sprintf(stemp,"%08d:BANK IDENTIFICATION",display_count);
			FINDRSCSETSTRING(MAINRSRC,stemp,bankid);
			FINDRSCSETSENSITIVE(MAINRSRC,stemp,TRUE);
			...
			sprintf(stemp,"%08d:GRANT YEAR",display_count);
			FINDRSCSETINT(MAINRSRC,stemp,gy);
			if(AccountType!=2)
			{
				FINDRSCSETSENSITIVE(MAINRSRC,stemp,TRUE);
			}
			...
			memset(stemp,0,101);
			sprintf(stemp,"%08d:SOURCE DESCRIPTION",display_count);
			FINDRSCSETSTRING(MAINRSRC,stemp,SourceList[src]);
			FINDRSCSETSENSITIVE(MAINRSRC,stemp,TRUE);
			memset(stemp,0,101);
...
void GatherData()
{
	...
	{
		memset(stemp,0,101);
		sprintf(stemp,"%.4s-%.2s-%.2s",&FromAFFECTDate[6],&FromAFFECTDate[0],&FromAFFECTDate[3]);
		from_date_affected=stralloc(stemp);
	}
		...
		while(!ef)
		{
			...
			if(!delflag)
			{
				...
				if((pm==PM || Style==1) && (isEMPTY(REFERENCE_ID) || !RDAstrcmp(REFERENCE_ID,refid)) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
				{
					...
					if(DefinitionType==deftype && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0))))) 
					{
						read_record=FALSE;
				...
				if(!delflag)
				{
					...
					while(!ef2)
					{
						...
						if(!delflag)
						{
							...
							memset(stemp,0,101);
							sprintf(stemp,"%.4s-%.2s-%.2s",&adate[6],&adate[0],&adate[3]);
							if((pm==PM || Style==1) && fy==FiscalYear && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0))))) 
							{
				...
				while(!ef)
				{
					...
					if(!delflag)
					{
						...
						if((isEMPTY(REFERENCE_ID) || !RDAstrcmp(REFERENCE_ID,refid)) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
						{
							...
							memset(stemp,0,101);
							sprintf(stemp,"%.4s-%.2s-%.2s",&adate[6],&adate[0],&adate[3]);
							if(((pm+1)==PM || Style==1) && fy==FiscalYear && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0))))) 
							{
								FINDFLDGETSHORT(venvch,"ACCOUNT TYPE",&acctype);
								FINDFLDGETSHORT(venvch,"DEFINITION TYPE",&deftype);
								if(DefinitionType==deftype)
		...
		if((isEMPTY(SOURCE_MODULE) && isEMPTY(SOURCE_PROCESS)) || (!RDAstrcmp(SOURCE_MODULE,"PAYROLL") && !RDAstrcmp(SOURCE_PROCESS,"PAYVDST")))
		{
			if(AccountType==2 && (GrantYear==0 || GrantYear==(-1)))
			{
				...
				while(!ef)
				{
					...
					if((pm==PM || Style==1) && fy==FiscalYear && !delflag && DefinitionType==deftype  && (isEMPTY(REFERENCE_ID) || (!RDAstrcmp(REFERENCE_ID,refid))) && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0)))) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
					{
				...
				if((isEMPTY(SOURCE_MODULE) && isEMPTY(SOURCE_PROCESS)) || (!RDAstrcmp(SOURCE_MODULE,"PAYROLL") && !RDAstrcmp(SOURCE_PROCESS,"PAYCLCK")))
				{
					...
					while(!ef)
					{
						...
						if((pm==PM || Style==1) && fy==FiscalYear && !delflag && DefinitionType==deftype && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
						{
							...
							memset(stemp,0,101);
							sprintf(stemp,"%.4s-%.2s-%.2s",&adate[6],&adate[0],&adate[3]);
							if(!delflag && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0))))) 
							{
								read_record=FALSE;
								if(AccountType==2)
								{
					...
					while(!ef)
					{
						...
						if(fy==FiscalYear && (pm==PM || Style==1) && !delflag && (gy==GrantYear || GrantYear==(-1)))
						{
							...
							while(!ef2)
							{
								...
								if(!delflag && DefinitionType==deftype  && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0)))) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
								{
									read_record=FALSE;
				...
				while(!ef)
				{
					...
					if(!delflag && !RDAstrcmp(source_mod,"PAYROLL") && RDAstrcmp(source_proc,"PAYVDST") && (isEMPTY(REFERENCE_ID) || (!RDAstrcmp(REFERENCE_ID,refid))))
					{
						...
						if((pm==PM || Style==1) && !RDAstrcmp(acctcode,AccountCode))
						{
							...
							memset(stemp,0,101);
							sprintf(stemp,"%.4s-%.2s-%.2s",&adate[6],&adate[0],&adate[3]);
							if((isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0))))) 
							{
...
/* payments */
		...
		if(prmtpaid!=(-1))
		{
			...
			while(!ef)
			{
				...
				if(DefinitionType==deftype && (isEMPTY(REFERENCE_ID) || !RDAstrcmp(REFERENCE_ID,refid)) && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0)))) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
				{
					read_record=FALSE;
		...
		if(occrec!=(-1))
		{
			...
			while(!ef)
			{
				...
				if(!isEMPTY(REFERENCE_ID))
				{
					...
					memset(stemp,0,101);
					sprintf(stemp,"%04d",cy);
					refid=stralloc(stemp);
				}
				if(DefinitionType==deftype  && (isEMPTY(REFERENCE_ID) || !RDAstrcmp(REFERENCE_ID,refid)) && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0)))) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
				{
					read_record=FALSE;
			...
			if(utbtrn!=(-1))
			{
				...
				while(!ef)
				{
					...
					if(fy==FiscalYear && (pm==PM || Style==1) && !delflag)
					{
						...
						if(DefinitionType==deftype  && (isEMPTY(adate) || (((RDAstrcmp(stemp,date_affected))<=0) && (isEMPTY(from_date_affected) || ((RDAstrcmp(from_date_affected,stemp))<=0)))) && (isEMPTY(SOURCE_USER) || !RDAstrcmp(SOURCE_USER,su)) && (isEMPTY(DISTRIBUTED_DATE) || !RDAstrcmp(DISTRIBUTED_DATE,d_date)) && (isEMPTY(DISTRIBUTED_TIME) || TimeComparison(d_time)))
						{
							read_record=FALSE;
...
void ClearTableResources()
{
	...
	for(y=1;y<(NumberRows+1);++y)
	{
		...
		{
			memset(stemp,0,101);
			sprintf(stemp,"%08d:SOURCE DESCRIPTION",y);
			FINDRSCSETSENSITIVE(MAINRSRC,stemp,FALSE);
...
#endif
{
	...
	for(y=1;y<(NumberRows+1);++y)
	{
		if(fields!=NULL)
		{
			...
			if(!alreadyrsrc(stemp,MAINRSRC))
			{
				makefieldrsrc(MAINRSRC,stemp,PLAINTEXT,25,FALSE);
			}
		}
	}
