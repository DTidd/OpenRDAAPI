/* finraccm.c - Xpert Financial Revenue Accounts Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern void browse_finracc(RDArsrc *,mtnstruct *);
...
char editable=TRUE;
MakeBrowseList *mbl_finracc=NULL;
void finraccm(short,void *);

void finraccm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_finracc,mtn);
	if(mbl_finracc!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_finracc==NULL?TRUE:FALSE));
}
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINRACC][ACCOUNT CODE]",deftype);
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_finracc,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_finracc!=NULL) quit_record(mtnrsrc,mtn);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINRACC][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINRACC][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINRACC][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINRACC][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINRACC][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
