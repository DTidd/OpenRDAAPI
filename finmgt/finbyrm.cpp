/* finbyrm.c - Balance Sheet Accounts Year Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern MakeBrowseList *mbl_finbyr;
extern void browse_finbyr(RDArsrc *,mtnstruct *);
...
char editable=TRUE;
void finbyrm(short,void *);

void finbyrm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	if(fields!=NULL)
	{
		for(x=0,fldx=fields;x<nofields;++x,++fldx)
		{
			...
			if(fldx->type==DOLLARS && strncmp(fldx->name,"BEGINNING BALANCE",16))
			{
				edit_rsrc=FALSE;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_finbyr,mtn);
	if(mbl_finbyr!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_finbyr==NULL?TRUE:FALSE));
}
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINBYR][ACCOUNT CODE]",deftype1);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINBYR][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINBYR][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINBYR][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINBYR][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINBYR][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_finbyr,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_finbyr!=NULL) quit_record(mtnrsrc,mtn);
