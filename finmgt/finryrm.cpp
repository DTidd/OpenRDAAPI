/* finryrm.c - REVENUE ACCOUNTS YEAR MAINTAIN */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern MakeBrowseList *mbl_finryr;
extern void browse_finryr(RDArsrc *,mtnstruct *);
...
void finryrm(short,void *);
char editable=TRUE;

void finryrm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_finryr,mtn);
	if(mbl_finryr!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_finryr==NULL?TRUE:FALSE));
}
...
static void setDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[FINRYR][ACCOUNT CODE]",deftype1);
...
static void changeDEFtypecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[FINRYR][ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINRYR][ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[FINRYR][ACCOUNT CODE]",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINRYR][ACCOUNT CODE]",deftype2);
		updatersrc(mtnrsrc,"[FINRYR][ACCOUNT CODE]");
	}
	SETID(mtnrsrc,mtn);
}
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_finryr,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_finryr!=NULL) quit_record(mtnrsrc,mtn);
