/* finrjnlm.c - Receipt Journal Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern MakeBrowseList *mbl_finrjnl;
extern MaintainMaster *MASTER;
extern void browse_finrjnl(RDArsrc *,mtnstruct *);
...
void finrjnlm(short,void *);
char editable=TRUE;
	
void finrjnlm(short dowhich,void *targetkey)
{
	int which_type=0;
	char *name=NULL;
	...
	FINDRSCSETFUNC(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",acctcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",debcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",credcodecb,mtn);
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_finrjnl,mtn);
	if(mbl_finrjnl!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_finrjnl==NULL?TRUE:FALSE));
}
static short check_code(RDArsrc *mtnrsrc,mtnstruct *mtn,short acctwidget)
{
	...
	FINDRSCGETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",&acctcode1);
	FINDRSCGETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",&debcode1);
	FINDRSCGETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",&credcode1);
	switch(acctwidget)
	{
		case 0:
			...
			switch(accttype1)
			{
				...
				case EXP_ACCT:
					if((tmp=ADVCHECKACCT((-1),baccnum,debcode1,accttype1,deftype1,0,SCRNvirtualSubData,mtnrsrc))!=NULL)
					{
						...
						FINDRSCSETSTRING(mtnrsrc,"DEBIT CODE DESCRIPTION",NULL);
						return(FALSE);
					} else if((tmp=ADVGETBALREF(baccnum,debcode1,deftype1,SCRNvirtualSubData,mtnrsrc))!=NULL)
					{
						if(RDAstrcmp(tmp,"EXPENDITURE"))
						{
							WARNINGDIALOG("WARNING DIALOG SCREEN","INVALID DEBIT CODE!","The DEBIT CODE entered does not have the correct BALANCE SHEET REFERENCE TYPE of EXPENDITURE for ACCOUNT TYPE of EXPENDITURE." ,NULL,NULL,TRUE,NULL);
							Rfree(tmp);
							if(acctcode1!=NULL) Rfree(acctcode1);
							if(credcode1!=NULL) Rfree(credcode1);
							if(debcode1!=NULL) Rfree(debcode1);
			...
			switch(accttype1)
			{
				case REV_ACCT:
					if((tmp=ADVCHECKACCT((-1),baccnum,credcode1,accttype1,deftype1,0,SCRNvirtualSubData,mtnrsrc))!=NULL)
					{
						...
						FINDRSCSETSTRING(mtnrsrc,"CREDIT CODE DESCRIPTION",NULL);
						return(FALSE);
					} else if((tmp=ADVGETBALREF(baccnum,credcode1,deftype1,SCRNvirtualSubData,mtnrsrc))!=NULL)
					{
						if(RDAstrcmp(tmp,"REVENUE"))
						{
							WARNINGDIALOG("WARNING DIALOG SCREEN","INVALID CREDIT CODE!","The CREDIT CODE entered does not have the correct BALANCE SHEET REFERENCE TYPE of REVENUE for ACCOUNT TYPE of REVENUE.",NULL,NULL,TRUE,NULL);
							if(acctcode1!=NULL) Rfree(acctcode1);
							if(credcode1!=NULL) Rfree(credcode1);
							if(debcode1!=NULL) Rfree(debcode1);
							FINDRSCSETSTRING(mtnrsrc,"CREDIT CODE DESCRIPTION",NULL);
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	{
		FINDRSCGETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",&temp);
		if(temp!=NULL)
		{
			FINDRSCSETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",NULL);
			FINDFLDSETSTRING(MASTER->passkey->fileno,"DEFAULT DEBIT CODE",NULL);
			updatersrc(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]");
			Rfree(temp);
		}
		FINDRSCGETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",&temp);
		if(temp!=NULL)
		{
			FINDRSCSETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",NULL);
			FINDFLDSETSTRING(MASTER->passkey->fileno,"DEFAULT CREDIT CODE",NULL);
			updatersrc(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]");
			Rfree(temp);
		}
	}
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_finrjnl,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_finrjnl!=NULL) quit_record(mtnrsrc,mtn);
...
{
	readwidget(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]");
...
{
	readwidget(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]");
...
{
	readwidget(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]");
	...
	short restore_accounts)
{
	...
	FINDRSCGETACCTTYPE(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",&accttype1);
	FINDRSCGETDEFTYPE(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",&deftype1);
	accttype2=(short)selecteda;
	deftype2=(short)selectedd;
	if(accttype1!=accttype2 || deftype1!=deftype2)
	{
		FINDRSCSETSENSITIVE(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",TRUE);
		FINDRSCSETSENSITIVE(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",TRUE);
		if(!restore_accounts)
		{
			FINDRSCSETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",NULL);
			FINDRSCSETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",NULL);
			FINDRSCSETSTRING(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",NULL);
		}
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"DEBIT CODE DESCRIPTION",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CREDIT CODE DESCRIPTION",NULL);
		FINDRSCSETACCTTYPE(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",accttype2);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]",deftype2);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",deftype2);
		FINDRSCSETDEFTYPE(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",deftype2);
		if(selecteda==BAL_ACCT)
		{
			FINDRSCSETSENSITIVE(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",FALSE);
			FINDRSCSETSENSITIVE(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",FALSE);
			FINDRSCSETEDITABLE(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]",FALSE);
			FINDRSCSETEDITABLE(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]",FALSE);
		}
	}
	if(update_list)
	{
		updatersrc(mtnrsrc,"[FINRJRNL][DEFAULT ACCOUNT CODE]");
		updatersrc(mtnrsrc,"[FINRJRNL][DEFAULT DEBIT CODE]");
		updatersrc(mtnrsrc,"[FINRJRNL][DEFAULT CREDIT CODE]");
		updatersrc(mtnrsrc,"ACCOUNT CODE DESCRIPTION");
		updatersrc(mtnrsrc,"DEBIT CODE DESCRIPTION");
