/* posgratm.c - Xpert Gross Rate Definitions Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern MakeBrowseList *mbl_posgrat;
extern MaintainMaster *MASTER;
extern RDApayroll *PAYROLL_SETUP;
extern void browse_posgrat(RDArsrc *,RDATData *);
...
void posgratm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	if(PAYROLL_SETUP->manage_earnings==TRUE)
	{
		addbtnrsrc(mtnrsrc,"LOAD VALUE EARNINGS AMOUNT",TRUE,loadvalue,"[POSGRAT][EARINGS AMOUNT]");
		addbtnrsrc(mtnrsrc,"LOAD OPERATOR EARNINGS AMOUNT",TRUE,loadoperand,"[POSGRAT][EARNINGS AMOUNT]");
		addbtnrsrc(mtnrsrc,"LOAD GROUPER EARNINGS AMOUNT",TRUE,loadgrouper,"[POSGRAT][EARNINGS AMOUNT]");
		addbtnrsrc(mtnrsrc,"LOAD CONTROL EARNINGS AMOUNT",TRUE,loadcontrol,"[POSGRAT][EARNINGS AMOUNT]");
	}
	addbtnrsrc(mtnrsrc,"LOAD VALUE DEFAULT UNITS",TRUE,loadvalue,"[POSGRAT][DEFAULT UNITS]");
	addbtnrsrc(mtnrsrc,"LOAD OPERATOR DEFAULT UNITS",TRUE,loadoperand,"[POSGRAT][DEFAULT UNITS]");
	addbtnrsrc(mtnrsrc,"LOAD GROUPER DEFAULT UNITS",TRUE,loadgrouper,"[POSGRAT][DEFAULT UNITS]");
	addbtnrsrc(mtnrsrc,"LOAD CONTROL DEFAULT UNITS",TRUE,loadcontrol,"[POSGRAT][DEFAULT UNITS]");
	addbtnrsrc(mtnrsrc,"LOAD VALUE",TRUE,loadvalue,"[POSGRAT][RATE AMOUNT]");
	addbtnrsrc(mtnrsrc,"LOAD OPERATOR",TRUE,loadoperand,"[POSGRAT][RATE AMOUNT]");
	addbtnrsrc(mtnrsrc,"LOAD GROUPER",TRUE,loadgrouper,"[POSGRAT][RATE AMOUNT]");
	addbtnrsrc(mtnrsrc,"LOAD CONTROL",TRUE,loadcontrol,"[POSGRAT][RATE AMOUNT]");
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_posgrat,previous);
	...
	if(mbl_posgrat!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	if(!editable)
	{
		...
		if(PAYROLL_SETUP->manage_earnings==TRUE)
		{
			FINDRSCSETSENSITIVE(mtnrsrc,"LOAD VALUE EARNINGS AMOUNT",FALSE);
			FINDRSCSETSENSITIVE(mtnrsrc,"LOAD OPERATOR EARNINGS AMOUNT",FALSE);
			FINDRSCSETSENSITIVE(mtnrsrc,"LOAD GROUPER EARNINGS AMOUNT",FALSE);
			FINDRSCSETSENSITIVE(mtnrsrc,"LOAD CONTROL EARNINGS AMOUNT",FALSE);
		}
		FINDRSCSETSENSITIVE(mtnrsrc,"LOAD VALUE DEFAULT UNITS",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"LOAD OPERATOR DEFAULT UNITS",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"LOAD GROUPER DEFAULT UNITS",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"LOAD CONTROL DEFAULT UNITS",FALSE);
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_posgrat==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_posgrat,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_posgrat!=NULL) quit_record(mtnrsrc,previous);
