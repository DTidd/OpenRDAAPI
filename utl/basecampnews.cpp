...
/*lint -library */
#ifndef WIN32
#define __NAM__ "basecampnews.lnx"
#endif
#ifdef WIN32
#define __NAM__ "basecampnews.exe"
#endif
#include <app.hpp>
#include <powadd.hpp>
#include <misc.hpp>
...
void CreateCanvas()
{
	...
	if(RDAcurl)
	{
		curl_easy_setopt(RDAcurl,CURLOPT_URL,"http://rdasystems.basecamphq.com/feed/recent_items_rss");
#ifdef PRXY
		if(!isEMPTY(prxy))
		{
		...
		curl_easy_setopt(RDAcurl,CURLOPT_USERPWD,"16d7825849f2c7b500a4c86c57340ee801aa631a");
	...
	{
		RDA_News=(qt_QWidget *)qt_new_QWidget((qt_QWidget *)MainHBox,"BASECAMP NEWS",QT_WDestructiveClose|QT_WStyle_NoBorder);
		...
		memset(stemp,0,101);
		qc=qt_new_QString5("http://rdasystems.basecamphq.com/feed/recent_items_rss");
		...
		TE=qt_new_QTextBrowser((qt_QWidget *)hbox,"BASECAMP NEWS");
		...
		SetupWindowColorPalette((qt_QWidget *)RDA_News,"BASECAMP NEWS WINDOW");
...
#endif
{
	...
	RDA_SetEnv("NOTRANSACTIONS",stemp);
#endif
	if(InitializeSubsystems(argc,argv,"OLHELP","BASECAMP NEWS")) 
	{
        	return(1);
	}
#ifdef WIN32
	...
	temp=NULL;
	MainWindow=qt_new_QMainWindow((qt_QWidget *)NULL,"BASECAMP NEWS WINDOW",QT_WType_TopLevel|QT_WStyle_NormalBorder|QT_WStyle_Title|QT_WStyle_MinMax);
	if(News_W!=(-1) && News_H!=(-1))
	{
	...
	SetupWindowColorPalette(MainWindow,"BASECAMP NEWS WINDOW");
...
#ifdef RDA_64BITS
	sprintf(temp,"BASECAMP NEWS (64 BITS %s)",VERSION_DATE);
#endif
#ifndef RDA_64BITS
	sprintf(temp,"BASECAMP NEWS (32 BITS %s)",VERSION_DATE);
#endif
	...
	SetupWindowColorPalette((qt_QWidget *)MainHBox,"BASECAMP NEWS WINDOW");
	CreateCanvas();
