/* dfmtnnew.c - Define Maintain Master New Writes */
#include <cstdio>
...
extern APPlib *newwrtavl,*holdapplib;
extern void addholdvalues(APPlib *,char *,char *);
APPlib *makefieldvallistwrtmtn(PowerWrite *,int,int,short,char *,char *);
static void loadvaluenew(RDArsrc *,char *);
...
void getNewWrites(void);
extern PowerWrite *tempPrevWrites;
extern int tmpnumPrevWrites;
extern PowerWrite *tempNewWrites;
extern int tmpnumNewWrites;
extern APPlib *loadfilefieldlistA(RDArsrc *,char *,char *);

APPlib *makefieldvallistwrtmtn(PowerWrite *Write,int numWrites,int number,short dofields,char *wrtmodule,char *wrtfilename)
{
	...
	PowerWrite *W=NULL;
	DFincvir *a,*i;
	short addvf=FALSE,dumb=(-1);
	DFvirtual *d=NULL;
	RDArsrc *tmprsrc=NULL;

	tmprsrc=RDArsrcNEW(module,"TEMP");
	tmp=APPlibNEW();
	addholdvalues(tmp,wrtmodule,wrtfilename);
	addDFincvir(tmprsrc,MTNMSTR->module,MTNMSTR->mainfile,NULL,dumb);
	if(MTNMSTR->s!=NULL)
	{
		for(x=0,s=MTNMSTR->s;x<MTNMSTR->num;++x,++s)
		{
			addDFincvir(tmprsrc,s->module,s->filename,NULL,dumb);
		}
	}
	if(!isEMPTY(wrtmodule) && !isEMPTY(wrtfilename))
	{
		addDFincvir(tmprsrc,wrtmodule,wrtfilename,NULL,dumb);
	}
	...
	if(Write!=NULL)
	{
		for(x=0,W=Write;x<number;++x,++W)
		{
			addDFincvir(tmprsrc,W->module,W->filename,NULL,dumb);
		}
	}
	...
	if(virflist!=NULL)
	{
		...
		{
			d=getDFvirtual(MTNMSTR->module,virflist->libs[y]);
			if(d!=NULL)
			{
				addvf=TRUE;
				if(d->incvir!=NULL)
...
static void loadvaluenew(RDArsrc *r,char *fieldname)
{
	int x;
	LoadValue *load;
	...
	if(got_applib==TRUE && holdapplib!=NULL)
	{
		...
		holdapplib=makefieldvallistwrtmtn(tempNewWrites,tmpnumNewWrites,tmpnumNewWrites,TRUE,mod,fn);
		got_applib=TRUE;
		loadlist=CopyAPPlib(holdapplib);
	}
		...
		int type,void (*savefunc)(...),void (*quitfunctest)(...))
{
	...
	editrsrc=RDArsrcNEW(module,"MAINTAIN NEW POWER WRITE FIELDS");
	...
	if(got_applib==TRUE && holdapplib!=NULL)
	{
		...
		if(holdapplib!=NULL) freeapplib(holdapplib);
		holdapplib=makefieldvallistwrtmtn(tempNewWrites,tmpnumNewWrites,tmpnumNewWrites,TRUE,modx,filx);
		got_applib=TRUE;
		varnames=CopyAPPlib(holdapplib);
	}
	...
	addlstrsrc(editrsrc,"VARIABLE NAMES",&x,TRUE,NULL,varnames->numlibs,&varnames->libs,NULL);
	x=0;
	...
	addbtnrsrc(editrsrc,"LOAD VALUE",TRUE,loadvaluenew,"EXPRESSION");
...
static void makespaceimage(int pos)
{
	...
	++tmpnumNewWrites;
	new_fields=Rmalloc(tmpnumNewWrites*sizeof(PowerWrite));
	y=0;
	for(x=0,temp=tempNewWrites;x<(tmpnumNewWrites-1);++x,++temp)
	{
		temp1=new_fields+y;
		if(x==pos)
		{
	...
	for(x=0,temp1=tempNewWrites,temp=new_fields;x<tmpnumNewWrites;++x,++temp,++temp1)
	{
		temp1->fileno=temp->fileno;
		if(temp->module!=NULL)
		{
...
void getNewWrites(void)
{
	...
	{
		for(x=0,i=tempNewWrites;x<tmpnumNewWrites;++x,++i)
		{
			...
			addAPPlib(newwrtavl,tmp);
		}
	}
	if(tmp!=NULL) Rfree(tmp);
	if(newwrtavl->numlibs<1)
	{
		addAPPlib(newwrtavl,"No Add Write Lines Available.");
	}
}
...
static void savemimg(RDArsrc *mimgrsrc,RDArsrc *imgrsrc)
{
	...
	if(FINDRSCGETINT(imgrsrc,"NEW WRITE LIST",&value)) return;
	if(tempNewWrites==NULL) 
	{
		tempNewWrites=Rmalloc(sizeof(PowerWrite));
		tmpnumNewWrites=1;
	}
	s=tempNewWrites+value;
	readallwidgets(mimgrsrc);
	...
	getNewWrites();
	if(!FINDRSCLISTAPPlib(imgrsrc,"NEW WRITE LIST",value,newwrtavl))
		updatersrc(imgrsrc,"NEW WRITE LIST");
...
static void savemimgabove(RDArsrc *mimgrsrc,RDArsrc *imgrsrc)
{
	...
	if(FINDRSCGETINT(imgrsrc,"NEW WRITE LIST",&x)) return;
	makespaceimage(x);
...
static void savemimgbelow(RDArsrc *mimgrsrc,RDArsrc *imgrsrc)
{
	...
	if(FINDRSCGETINT(imgrsrc,"NEW WRITE LIST",&x)) return;
	makespaceimage(++x);
	FINDRSCSETINT(imgrsrc,"NEW WRITE LIST",x);
	savemimg(mimgrsrc,imgrsrc);
}
	...
	void (*savefunction)(...),void (*quitfunctiontest)(...))
{
	...
	mimgrsrc=RDArsrcNEW(module,"MAINTAIN NEW POWER WRITES");
	...
	addlstrsrc(mimgrsrc,"WRITE FIELDS",&x,TRUE,NULL,wrtfldavl->numlibs,
		&wrtfldavl->libs,NULL);
	addbtnrsrc(mimgrsrc,"LOAD VALUE",TRUE,loadvaluenew,"EXPRESSION");
...
void newwrtaddabove(RDArsrc *imgrsrc)
{
	int selected=0;

	got_applib=FALSE;
	if(newwrtavl->numlibs==1 && !RDAstrcmp(newwrtavl->libs[0],"Currently there are no Add Writes defined file this Power Add.")) return;
	if(FINDRSCGETINT(imgrsrc,"NEW WRITE LIST",&selected)) return;
	imagescreen(imgrsrc,++selected,NULL,NULL,NULL,0,NULL,savemimgabove,quitmimgabovetest);
}
void newwrtaddbelow(RDArsrc *imgrsrc)
{
	int selected=0;
...
void newwrtmove(RDArsrc *imgrsrc,short direction)
{
	int z;
	...
	if(tmpnumNewWrites<1) return;
	readallwidgets(imgrsrc);
	if(FINDRSCGETINT(imgrsrc,"NEW WRITE LIST",&value)) return;
	temp=tempNewWrites+value;
	if(direction)
	{
		if((temp==NULL) || (value >= (tmpnumNewWrites-1))) return;
		value++;
	} else {
		if((temp==NULL) || (value <=0)) return;
		value--;
	}
	temp2=tempNewWrites+value;
	...
	Rfree(temp1);
	getNewWrites();
	if(value>=newwrtavl->numlibs) value=(newwrtavl->numlibs-1);
	if(!FINDRSCLISTAPPlib(imgrsrc,"NEW WRITE LIST",value,newwrtavl))
		updatersrc(imgrsrc,"NEW WRITE LIST");
	changedvalues=TRUE;
}
void newwrtmoveup(RDArsrc *imgrsrc)
{
	newwrtmove(imgrsrc,0);
}
void newwrtmovedown(RDArsrc *imgrsrc)
{
	newwrtmove(imgrsrc,1);
}
void newwrtdelete(RDArsrc *imgrsrc)
{
	short x,y;
	int z;
	PowerWrite *temp,*temp1,*new_fields;
	...
	readallwidgets(imgrsrc);
	if(FINDRSCGETINT(imgrsrc,"NEW WRITE LIST",&value)) return;
	if(tmpnumNewWrites>1)
	{
		new_fields=Rmalloc((tmpnumNewWrites-1)*sizeof(PowerWrite));
		y=0;
		for(x=0,temp=tempNewWrites;x<tmpnumNewWrites;++x,++temp)
		{
			temp1=new_fields+y;
			if(x!=value)
			{
		...
		--tmpnumNewWrites;
		tempNewWrites=Rmalloc(tmpnumNewWrites*sizeof(PowerWrite));
		for(x=0,temp1=tempNewWrites,temp=new_fields;x<tmpnumNewWrites;++x,
			++temp,++temp1)
		{
			if(temp->module!=NULL)
			{
	...
	getNewWrites();
	if(value>=newwrtavl->numlibs) value=0;
	if(!FINDRSCLISTAPPlib(imgrsrc,"NEW WRITE LIST",value,newwrtavl))
		updatersrc(imgrsrc,"NEW WRITE LIST");
	changedvalues=TRUE;
}
void newwrtedit(RDArsrc *imgrsrc)
{
	int selected=0;
	PowerWrite *i;
...
void newwrtcopy(RDArsrc *imgrsrc)
{
	int selected=0;
	PowerWrite *i;
...
void savenewwrt(RDArsrc *mtnrsrc,MaintainMaster *MTNMSTR)
{
	int x=0,z=0;
	...
	{
		for(x=0,temp=MTNMSTR->NewWrites;x<MTNMSTR->numNewWrites;++x,++temp)
		{
			if(temp->module!=NULL) Rfree(temp->module);
			if(temp->filename!=NULL) Rfree(temp->filename);
			if(temp->expression!=NULL) Rfree(temp->expression);
		...
		Rfree(MTNMSTR->NewWrites);
		MTNMSTR->NewWrites=NULL;
		MTNMSTR->numNewWrites=0;
	}
	if(tempNewWrites!=NULL)
	{
		MTNMSTR->numNewWrites=tmpnumNewWrites;
		MTNMSTR->NewWrites=Rmalloc(tmpnumNewWrites*sizeof(PowerWrite));
		for(x=0,temp=tempNewWrites,temp1=MTNMSTR->NewWrites;x<tmpnumNewWrites;++x,++temp,
			++temp1)
		{
			if(temp->module!=NULL)
			{
...
void newgetprev(RDArsrc *m)
{
	int x=0,y=0,value=0,z=0;
	PowerWrite *i=NULL,*i1=NULL;
	PowerWriteField *a=NULL,*b=NULL;
	char *temp=NULL;
	
	if(FINDRSCGETINT(m,"NEW WRITE LIST",&value)) return;
	if(tempPrevWrites!=NULL)
	{
		if(tempNewWrites!=NULL)
		{
			tempNewWrites=Rrealloc(tempNewWrites,(tmpnumNewWrites+tmpnumPrevWrites)*sizeof(PowerWrite));
		} else {
			tempNewWrites=Rmalloc((tmpnumPrevWrites)*sizeof(PowerWrite));
			tmpnumNewWrites=0;
		}
		for(x=0,i=tempPrevWrites;x<tmpnumPrevWrites;++x,++i)
		{
			i1=tempNewWrites+(tmpnumNewWrites);
			i1->fileno=(-1);
			i1->module=stralloc(i->module);
			i1->filename=stralloc(i->filename);
			i1->expression=stralloc(i->expression);
			if(i->fields!=NULL)
			{
				i1->numflds=i->numflds;
				i1->fields=Rmalloc(i1->numflds*sizeof(PowerWriteField));
				for(y=0,a=i1->fields,b=i->fields;y<i1->numflds;++y,++a,++b)
				{
					a->copy_type=b->copy_type;
					a->to_name=stralloc(b->to_name);
					a->from_name=stralloc(b->from_name);
					if(a->copy_type==1 && !isEMPTY(a->from_name))
					{
						for(z=0,temp=a->from_name;z<RDAstrlen(a->from_name);++z,++temp)
						{
							if(!strncmp(temp,"PRE-YEAR",8))
							{
								temp+=8;
								z+=8;
							} else if(*temp=='-') *temp='+'; 
	...
	getNewWrites();
	if(!FINDRSCLISTAPPlib(m,"NEW WRITE LIST",value,newwrtavl))
		updatersrc(m,"NEW WRITE LIST");
}
