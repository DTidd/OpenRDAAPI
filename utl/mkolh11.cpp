/* OLHELP's - Make (OUROPENCALLS) Power Add */
#include <cstdio>
#include <mkadd.hpp>
#include <mkmsc.hpp>
...
/* QUIT WARNING DESCRIPTION */
	...
	sprintf(temp9,"%s",
		"");
	add=RDAPowerAddNEW(module,"OUROPENCALLS",temp1,"OUROPENCALLS","OUROPENCALLS DEFINE LIST","",3,temp2,temp3,temp4,temp5,temp6,temp7,temp8,temp9,0,NULL,0,NULL,0,NULL,0,NULL);
...
/* OUROPENCALLS wwwdoadd BUTTON  */
		...
		addAPPlib(args,"OUROPENCALLSC");
		x=addPowerAddButton(add,"wwwdoadd","","OUROPENCALLS","SECURITY","USERS","USERS KEY",temp1,1,0,args,0,NULL);
...
/* PASSWORD EDIT WIDGET */
		...
		if(writePowerAdd(defdir,add))
		{
			...
			sprintf(temp1,"POWER ADD WRITE ERROR: Module [OLHELP] Power Add [OUROPENCALLS], Can Not Save Power Add!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
...
/* QUIT WARNING DESCRIPTION */
	...
	sprintf(temp9,"%s",
		"");
	add=RDAPowerAddNEW(module,"OUROPENCALLSC",temp1,"OUROPENCALLS","OUROPENCALLS DEFINE LIST","",3,temp2,temp3,temp4,temp5,temp6,temp7,temp8,temp9,0,NULL,0,NULL,0,NULL,0,NULL);
	if(temp1!=NULL) Rfree(temp1);
	if(temp2!=NULL) Rfree(temp2);
	if(temp3!=NULL) Rfree(temp3);
	if(temp4!=NULL) Rfree(temp4);
...
/* OUROPENCALLS HTTP POST-GET BUTTON  */
		temp1=Rmalloc(54+1);
		sprintf(temp1,"%s",
		...
		temp2=Rmalloc(141+1);
		sprintf(temp2,"%s",
			"\"http://\"+(GENERIC_SETUP_VARIABLE(\"[UTILITIES][RDA SUPPORT IP ADDRESS]\"))+\":80/cgi-bin/ouropencalls.cgi\"");
		addAPPlib(args,temp2);
		if(temp2!=NULL) Rfree(temp2);
		x=addPowerAddButton(add,"HTTP POST-GET","","OUROPENCALLS","SECURITY","USERS","USERS KEY",temp1,1,1,args,0,NULL);
		if(args!=NULL) freeapplib(args);
		if(temp1!=NULL) Rfree(temp1);
		if(x!=(-1))
		{
...
/* OUROPENCALLS1 HTTP POST-GET BUTTON  */
		temp1=Rmalloc(55+1);
		sprintf(temp1,"%s",
		...
		addAPPlib(args,temp2);
		if(temp2!=NULL) Rfree(temp2);
		x=addPowerAddButton(add,"HTTP POST-GET","","OUROPENCALLS1","SECURITY","USERS","USERS KEY",temp1,1,1,args,0,NULL);
		if(args!=NULL) freeapplib(args);
		if(temp1!=NULL) Rfree(temp1);
		if(x!=(-1))
		{
...
/* PASSWORD EDIT WIDGET */
		...
		{
			if(temp1!=NULL) Rfree(temp1);
			temp1=Rmalloc(13+6+89+1);
			sprintf(temp1,"POWER ADD WRITE ERROR: Module [OLHELP] Power Add [OUROPENCALLSC], Can Not Save Power Add!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void MNU_OLHELP_OUROPENCALLS()
{
	RDAmenu *menu=NULL;
	char *defdir=NULL;

	menu=RDAmenuNEW("OLHELP OUROPENCALLS","doadd.EXT OLHELP OUROPENCALLS");
	if(menu!=NULL)
	{
...
static void SCN_OUROPENCALLS()
{
	...
	scrn=RDAscrnNEW("OLHELP","OUROPENCALLS");
	if(scrn!=NULL)
	{
		...
		if(writescrnbin(defdir,scrn))
		{
			if(temp1!=NULL) Rfree(temp1);
			temp1=Rmalloc(12+6+89+1);
			sprintf(temp1,"SCREEN WRITE ERROR: Module [OLHELP] Screen [OUROPENCALLS], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_OUROPENCALLS_DEFINE_LIST()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("OLHELP","OUROPENCALLS DEFINE LIST");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [OLHELP] Screen [OUROPENCALLS DEFINE LIST], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
void OUROPENCALLS()
{
	ADD_OUROPENCALLS();
	ADD_OUROPENCALLSC();
	MNU_OLHELP_OUROPENCALLS();
	SCN_OUROPENCALLS();
	SCN_OUROPENCALLS_DEFINE_LIST();
}
