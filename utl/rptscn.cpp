#ifndef WIN32
#define __NAM__ "rptscn.lnx"
#endif
#ifdef WIN32
#define __NAM__ "rptscn.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
/*CPP_OPART rptscn1 */
...
static void MAKE_RPTGEN_SCREENS(void);
...
static void printerrorlist(RDArsrc *parent,FILE *fp)
{
	...
	if(fp!=NULL)
	{
		...
		fprintf(fp,"                     XPERT MAKE RPTGEN SCREENS ERROR LIST          Date: %*s\n",8,date);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"MAKE RPTGEN SCREENS")) 
	{
		return;
	}
	MAKE_RPTGEN_SCREENS();
	if(argc>1)
	{
		...
		mainrsrc=RDArsrcNEW(module,"MAKE RPTGEN SCREENS");
		scnlist=makescnlist();
...
static void MAKE_RPTGEN_SCREENS()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;

	scrn=RDAscrnNEW("RPTGEN","MAKE RPTGEN SCREENS");
	if(scrn!=NULL)
	{
