...
#ifndef WIN32
#define __NAM__ "difnrd.lnx"
#endif
#ifdef WIN32
#define __NAM__ "difnrd.exe"
#endif
#include <app.hpp>
#include<mix.hpp>
#include<nrdp1.h>
...
static short CHANGE_FILE(char *,short,char *,short,NRDfield *,short,NRDkey *,char *);
...
static void print_header(FILE *fp,int *lines,int *pages,
	...
	*lines+=1;
	...
	if((RDAstrlen(module)+RDAstrlen(file)+14+15+lead_ws+tail_ws)>(line_length))
	{
		...
		*lines+=2;
		...
		fprintf(fp,"%*s %s Module %s File %*sDate: %s\n",
...
static void printfilemf(FILE *fp,char *modulename,char *filename)
{
	...
	if(doheaders)
	{
		...
		fprintf(fp,"Module [%s] File [%s]\n",modulename,filename);
		lines+=1;
	}
	if(ADVGETBIN(modulename,filename,&engine,&nofields,&fieldx,&nokeys,&keyx,&serverx))
	{
		fprintf(fp,"Can't Open Binary: Please check the file definition to resolve this error."); 
	}
	fprintf(fp,"Engine: [%s]  ",supported_engines[engine]);
	if(!isEMPTY(serverx))
	{
		fprintf(fp,"Server: [%s]",serverx);
	}
	fprintf(fp,"\n");
	++lines;
	if(doheaders)
	...
	if(nokeys>0)
	{
		...
		{
			fprintf(fp,"   Key Name: [%s] Number parts: [%d]\n",(keyx->name!=NULL ? key->name:""),key->numparts);
			++lines;
			if(doheaders)
			{
			if((lines)>58)
...
static short makepartlist()
{
	...
	{
		l=RDAstrlen(part->name)+9;
		s=Rmalloc(l);
		sprintf(s,"Name: [%s]",(part->name!=NULL ? 
			part->name:""));
...
static void makekeyfieldlist()
{
	...
	{
		l=RDAstrlen(keys->name)+29;
		s=Rmalloc(l);
		sprintf(s,"Name: [%s] Number parts [%d]",(keys->name!=NULL ?
...
static void makefieldlist()
{
	...
	{
		l=RDAstrlen(standardfieldtypes[fields->type])+RDAstrlen(fields->name)+34;
		s=Rmalloc(l);
		sprintf(s,"Name: [%s] Type: [%s] Length: [%d]",
...
/*
					...
					{
						case SCROLLEDTEXT:
							ERRORDIALOG("INVALID KEY TYPE!","There is a keypart with an invalid field type of SCROLLEDTEXT in the key definitions.  Faircom's Ctree does not allow for Variable Length Fields in its key definitions.  Check your definition and retry.  If problem persist call your installer.",NULL,FALSE);
							prterr("Error Faircom's Ctree Key Part [%s] of invalid type SCROLLEDTEXT in library [%s] definition [%s].",part->name,stemp,filename);
							return;
						case VARIABLETEXT:
							ERRORDIALOG("INVALID KEY TYPE!","There is a keypart with an invalid field type of VARIABLETEXT in the key definitions.  Faircom's Ctree does not allow for Variable Length Fields in its key definitions.  Check your definition and retry.  If problem persist call your installer.",NULL,FALSE);
	...
	} else if(engine==NRDInformixIsamEngine)
	{
		...
		for(x=0,field=datafile->fields;x<datafile->nofields;++x,++field)
		{
			if(field->len<1)
			{
				...
				sprintf(errorstring,"There is an incorrect Field Lenth on the Field [%s], Informix Isam database requires all fields have lengths greater than zero.  Check your definition and retry.  If problem persist call your installer.",(field->name==NULL?"":field->name));
...
static void savefile(RDArsrc *mtnfrsrc)
{
	...
	x=CHANGE_FILE(modulename,engine,filename,datafile->nofields,datafile->fields,datafile->nokeys,datafile->keys,serverx);
	if(x==(-1))
	{
...
static void setdbengines(RDArsrc *mtnrsrc)
{
	...
	if(!RDAstrcmp(dbengines->libs[selected],"Faircom's Ctree"))
	{
...
static short CHANGE_FILE(char *modulename,short engine,char *filename,short nofield,NRDfield *fields,short nokeys,NRDkey *keys,char *servery)
{
	...
	{
		ERRORDIALOG("OPEN FILE ERROR!","Couldn't open original file, file could be opended and/or locked by other processes.  Conversion failed.",NULL,FALSE);
			...
			if(tmpfileno!=(-1))
			{
				...
				if(tmprsrc!=NULL)
				{
					if(!ADVmakescrn(tmprsrc,TRUE))
					{
						...
						prterr("Error %s.",e);
/*
            					ERRORDIALOG("MAKESCRN FAILED",e,NULL,FALSE);
*/
                				if(e!=NULL) Rfree(e);
					}
				}
				...
				if(y!=(-1))
				{
					x=CRTNRD(modulename,engine,filename,nofield,fields,
						nokeys,keys,servery);
					if(x!=(-1))
					{
...
/* commented this out so wouldn't get two WARNING DIALOGUE SCREENS for same warning */
/*
								e=Rmalloc(130+RDAstrlen(tmprsrc->module)+RDAstrlen(tmprsrc->screen));
								sprintf(e,"The Make Screen function failed for the custom screen [%s] [%s]. Check to see the screen is available. If it is, call your installer.",tmprsrc->module,tmprsrc->screen);
               							WARNINGDIALOG("WARNING DIALOG SCREEN","MAKESCRN FAILED",e,NULL,NULL,FALSE,NULL);
...
*/
					...
					ERRORDIALOG("DELETE FILE ERROR!","Couldn't delete original file created, file could be opended and/or locked by other processes.  Conversion failed.",NULL,FALSE);
				...
				ERRORDIALOG("CREATE FILE ERROR!","Couldn't create temporary file.  Conversion failed.",NULL,FALSE);
			...
			ERRORDIALOG("DELETE FILE ERROR!","Couldn't initially delete temporary file created by previous processing, file could be opended and/or locked by other processes.  Conversion failed.",NULL,FALSE);
