...
#ifndef WIN32
#define __NAM__ "massie.lnx"
#endif
#ifdef WIN32
#define __NAM__ "massie.exe"
#endif
#include <app.hpp>
#ifdef WIN32
#include<cstdlib>
...
char RangeMod=FALSE,Which=FALSE;
	...
	if(modlst->numlibs<1)
	{
		...
		for(z=0;z<modlst->numlibs;++z)
		{
			...
			if(filelst->numlibs>0)
			{
				...
				{
					memset(stemp,0,101);
					sprintf(stemp,"%s-%s",filelst->libs[y],(Which==FALSE ? "EXPORT":"IMPORT"));
					args=APPlibNEW();
					addAPPlib(args,modlst->libs[z]);
					addAPPlib(args,stemp);
					Execute2Program("doie",args);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,"DATABASE","MASS IMPORT/EXPORT")) 
	{
		return;
	}
	mainrsrc=RDArsrcNEW("DATABASE","MASS IMPORT EXPORT");
