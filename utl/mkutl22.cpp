/* GUI's - Make (FIND CUSTOM INPUT FIELDS IN SCREEN DEFAULTS) */
#include <cstdio>
#include <mkmsc.hpp>

extern APPlib *errorlist;
static char *module="GUI";
void FIND_CUSTOMFIELD_SCREEN_DEFAULTS(void);
static void FIND_CUSTOM_INPUT_FIELDS_IN_SCREEN_DEFAULTS(void);

void FIND_CUSTOMFIELD_SCREEN_DEFAULTS()
{
	FIND_CUSTOM_INPUT_FIELDS_IN_SCREEN_DEFAULTS();
}
static void FIND_CUSTOM_INPUT_FIELDS_IN_SCREEN_DEFAULTS()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	...
	scrn=RDAscrnNEW("GUI","FIND CUSTOM INPUT FIELDS IN SCREEN DEFAULTS");
	if(scrn!=NULL)
	{
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			temp1=Rmalloc(35+3+89+1);
			sprintf(temp1,"SCREEN WRITE ERROR: Module [GUI] Screen [FIND CUSTOM INPUT FIELDS IN SCREENS], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
