/* GUI's - Make (FIND DATE REMOVE SCREEN DEFAULTS) Screen */
#include <cstdio>
#include <mkmsc.hpp>

extern APPlib *errorlist;
void FIND_DATE_REMOVE_SCREEN_DEFAULTS(void);

void FIND_DATE_REMOVE_SCREEN_DEFAULTS()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	...
	scrn=RDAscrnNEW("GUI","FIND DATE REMOVE SCREEN DEFAULTS");
	if(scrn!=NULL)
	{
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [GUI] Screen [FIND DATE REMOVE SCREEN DEFAULTS], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
