...
#ifndef WIN32
#define __NAM__ "crtie.lnx"
#endif
#ifdef WIN32
#define __NAM__ "crtie.exe"
#endif
#include <app.hpp>
#include <misc.hpp>
#include <gui.hpp>
...
void printdefinition(RDArsrc *prsrc,IEfile *IEdef)
{
	...
	if(outdevice!=NULL)
	{
		...
		RDA_fprintf(fp,"  Import File Name: [%s]  Type: [%s]\n",
			IEdef->ifile,
			(IEdef->type==0?"Flat Ascii":"Field Record Delimited"));
		if(IEdef->type==1)
		{
...
static void makefldlist1(void)
{
	...
	if(IMPDEF->fields!=NULL)
	{
		for(x=0,field=IMPDEF->fields;x<IMPDEF->num;++x,++field)
		{
			if(!IMPDEF->type)
			{
				l=RDAstrlen(standardfieldtypes[field->type])+RDAstrlen(field->name)+(field->use_quotes ? 3:2)+5+5+198;
				s=Rmalloc(l);
				...
				l=RDAstrlen(standardfieldtypes[field->type])+RDAstrlen(field->name)+(field->use_quotes ? 3:2)+(field->expression!=NULL ? 5:4)+5+5+70;
				s=Rmalloc(l);
...
void fielddefinition1(RDArsrc *mtnrsrc,char *name,short type,short start,short length,void (*savefunc)(...),void (*quitfunctest)(...),char *expression,char use_quotes,char *value_expression)
{
	...
	uquotes=use_quotes;
	addlstrsrc(fldrsrc,"USE QUOTES",&uquotes,TRUE,NULL,qtypes->numlibs,&qtypes->libs,NULL);
	if(!dowhich)
	{
...
void importdefinition1(RDArsrc *mainrsrc,char *defname,char *modx,char *filex,
	...
	char carriage_return,char linefeed,char skip_dups)
{
	...
	mtnrsrc=RDArsrcNEW(module,"MAINTAIN CREATE IMPORT EXPORT DEFINITION");
	if(flddefs!=NULL) freeapplib(flddefs);
	flddefs=APPlibNEW();
	...
	addstdrsrc(mtnrsrc,"CARRIAGE RETURN",BOOLNS,1,&carriage_return,TRUE);
	addstdrsrc(mtnrsrc,"LINE FEED",BOOLNS,1,&linefeed,TRUE);
	addstdrsrc(mtnrsrc,"SKIP DUPLICATES",BOOLNS,1,&skip_dups,TRUE);
	if(rangescrn)
	{
...
void setvaluesx(RDArsrc *mainrsrc)
{
	...
	if(RDAstrcmp(defsavl->libs[selected],"No Virtual Field Definitions"))
	{
...
static void newdef1(RDArsrc *mainrsrc)
{
	...
	importdefinition1(mainrsrc,IMPDEF->name,modlstx->libs[selectedm],
		NULL,NULL,NULL,NULL,NULL,0,0,FALSE,FALSE,FALSE,NULL,
		TRUE,TRUE,FALSE);
}
...
static void deletedeftest(RDArsrc *mainrsrc)
{
	...
	if(!RDAstrcmp(defsavl->libs[s],"No Virtual Field Definitions")) return;
...
#endif
{
	...
	if(modlstx!=NULL) freeapplib(modlstx);
	modlstx=APPlibNEW();
	addlstrsrc(mainrsrc,"MODULE LIST",&x,TRUE,getimpentries,
		modlstx->numlibs,&modlstx->libs,NULL);
	getdefinitionlist1(mainrsrc);
	x=0;
	if(defsavl!=NULL) freeapplib(defsavl);
	defsavl=APPlibNEW();
	addlstrsrc(mainrsrc,"DEFINITIONS AVAILABLE",&x,TRUE,setvaluesx,
