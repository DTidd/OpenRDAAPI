/*lint -library */
#ifndef WIN32
#define __NAM__ "datescn.lnx"
#endif
#ifdef WIN32
#define __NAM__ "datescn.exe"
#endif
#include <app.hpp>
...
static void ChangeScreenDefault_ContainsDate(RDA_PFILE *,RDAscrn *,char *,char *,char *,int *,int *,short);
...
static void checkheader(RDA_PFILE *fp,int *lines,int *pages,char *from_mod,char *to_mod,char dont_do_it)
{
	...
	if(*lines>60 || (*lines==0 && *pages==0))
	{
		...
		if(XPERT_SETUP->software_type==2)
		{
			if(dont_do_it==TRUE)
			{
				temp=stralloc("MBGUI SCREEN WIDGET DATE VALUES PREVIEW"); 
				printheader(fp,lines,pages,from_mod,to_mod,temp);
				if(temp!=NULL) Rfree(temp);
			} else {
				temp=stralloc("MBGUI SCREEN WIDGET DATE VALUES CHANGE"); 
				printheader(fp,lines,pages,from_mod,to_mod,temp);
				if(temp!=NULL) Rfree(temp);
			}
		} else {
			if(dont_do_it==TRUE)
			{
				temp=stralloc("XPERT SCREEN WIDGET DATE VALUES PREVIEW"); 
				printheader(fp,lines,pages,from_mod,to_mod,temp);
				if(temp!=NULL) Rfree(temp);
			} else {
				temp=stralloc("XPERT SCREEN WIDGET DATE VALUES CHANGE"); 
				printheader(fp,lines,pages,from_mod,to_mod,temp);
				if(temp!=NULL) Rfree(temp);
			}
		}
...
static void ChangeScreenDefault_ContainsDate(RDA_PFILE *fp1,RDAscrn *p,char *lib,char *from_mod,char *to_mod,int *lines,int *pages,short dont_do_it)
{
	int x=0;
	RDAwdgt *temp=NULL;
	...
	if(p!=NULL)
	{
		...
		for(x=0,temp=p->wdgts;x<(p->numwdgts-1);++x,++temp)
		{
			tempstring=ContainsADateField(temp->resource_name,TRUE);
			if(tempstring!=NULL)
			{
				if((temp->type==0) || (temp->type==15))
				{
					if(temp->cols==8)
					{
						if(*foundone==0) 
						{
							*foundone=1;
							...
							sprintf(string_temp,"SCREEN: [%s][%s]",
								(lib!=NULL?lib:""),
								(p->name!=NULL?p->name:""));
							RDA_fprintf(fp1,"%s\r\n",string_temp);
						...
						temp1=Rmalloc(RDAstrlen(temp->resource_name)+1);
						sprintf(temp1,"%s",(temp->resource_name!=NULL?temp->resource_name:""));
						temp2=Rmalloc(6+1);
						sprintf(temp2,"%.6d",(x+1));
						temp3=Rmalloc(RDAstrlen(tempstring)+30+1);
						sprintf(temp3,"(%s) date field and length of 8",tempstring);
				...
				if(temp->type==0)
				{
					if(temp->cols==8)
					{
						if(RDAstrstr(temp->resource_name,"DATE")
	)
						{
							if(*foundone==0) 
							{
							...
							sprintf(temp2,"%.6d",(x+1));
							temp3=Rmalloc(53+1);
							sprintf(temp3,"resource name has \"DATE\" in name and a length of 8");
							string_temp=RDA_FormatOutputDefaultDate(temp1,temp2,temp3);
							if(temp1!=NULL) Rfree(temp1);
							if(temp2!=NULL) Rfree(temp2);
							if(temp3!=NULL) Rfree(temp3);
...
*/
					for(x=0;findfile(liblist->libs[z],"*.SCN",&tmp,(int)x+1);++x) 
					{
						tmp[RDAstrlen(tmp)-4]=0;
						if(rng_mod==TRUE)
						{
					...
					if(modlst->libs!=NULL)
					{
						for(x=0;x<modlst->numlibs;++x)
						{
...
#ifndef WIN32
							sprintf(libx,"%s/%s.SCN",liblist->libs[z],modlst->libs[x]);
#endif
#ifdef WIN32
							sprintf(libx,"%s\\%s.SCN",liblist->libs[z],modlst->libs[x]);
#endif
							...
							if(file_def!=NULL)
							{
								for(y=0;y<file_def->numlibs;++y)
								{
									RDA_SCREEN=RDAscrnNEW(modlst->libs[x],file_def->libs[y]);
									if(diagapps)
									{
										prterr("Opening [%s][%s].\r\n",libx,file_def->libs[y]);TRACE;
									}
									if(getscrnbin(libx,RDA_SCREEN,FALSE)==0)
									{
										if(diagapps)
										{
											prterr("Found and Processing [%s][%s].\r\n",libx,file_def->libs[y]);TRACE;
										}
										ChangeScreenDefault_ContainsDate(fp1,RDA_SCREEN,libx,mod_from,mod_to,&lines,&pages,dont_do_it);		
									} else {
										if(diagapps)
										{
									...
									if(RDA_SCREEN!=NULL) free_scrn(RDA_SCREEN);
								}
								freeapplib(file_def);
							}
							if(libx!=NULL) Rfree(libx);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"FIND DATE CHANGE SCREEN LENGTHS")) 
	{
		return;
	}
	mainrsrc=RDArsrcNEW(module,"FIND DATE CHANGE SCREEN LENGTHS");
