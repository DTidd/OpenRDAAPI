...
/*lint -library */
#ifndef WIN32
#define __NAM__ "chartdat1.lnx"
#endif
#ifdef WIN32
#define __NAM__ "chartdat1.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <guip.hpp>
...
void DrawPieChart(qt_QCanvas *Canvas,RDAchart *chart,double total,int count)
{
	...
	for(i=0,e=chart->elements;i<chart->num;++i,++e)
	{
		if(e->valid)
		{
			...
			qt_QCanvasEllipse_drawShape((qt_QCanvasEllipse *)arc,(qt_QPainter *)painter);
	/*
			TRACE;
	...
	*/
			...
			qt_del_QPaintDevice((qt_QPaintDevice *)pd);
			...
			prterr("prox [%f] proy [%f] ",prox,proy);TRACE;
			...
			prterr("setX [%f] setY [%f] ",e->prox*width,e->proy*height);TRACE;
	...
	TRACE;
	qt_del_QPainter((qt_QPainter *)painter);
	qt_del_QFont(f);
}
...
void DrawElements(qt_QMainWindow *MainWindow,qt_QCanvas *Canvas,RDAchart *chart)
{
...
void DisplayChart(RDAchart *chart)
{
	...
	TRACE;
	DrawElements((qt_QMainWindow *)MainWindow,(qt_QCanvas *)Canvas,chart);
	TRACE;
	qt_QCanvasView_setCanvas((qt_QCanvasView *)CanvasView,(qt_QCanvas *)Canvas);
	qt_QWidget_update((qt_QWidget *)CanvasView);
	TRACE;
