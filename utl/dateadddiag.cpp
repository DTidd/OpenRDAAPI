...
/*lint -library */
#ifndef WIN32
#define __NAM__ "dateadddiag.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dateadddiag.exe"
#endif
#include <app.hpp>
#include <powadd.hpp>
/*ADDLIB add */
...
static short ContainsADateField(char *expression)
{
	...
	prterr("ContainsADate(): Checking string=\n[%s]\n",expression);TRACE;
...
static void POWERADD_ContainsDate(PowerAdd *p,char *lib)
{
	...
	if(p!=NULL)
	{
		if(p->subord!=NULL)
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->subord's",p->name);TRACE;
			for(x=0,addsub=p->subord;x<p->subnum;++x,++addsub)
			{
				if(!isEMPTY(addsub->expression))
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->subord [%s][%s]",p->name,addsub->module,addsub->filename);TRACE;
		...
		if(p->EditWdgts!=NULL)
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->EditWdgts",p->name);TRACE;
			for(x=0,AE=p->EditWdgts;x<p->numwdgts;++x,++AE)
			{
				prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->EditWdgts=[%s]",p->name,AE->name);TRACE;
				if(AE->type==DATES)
				{
				...
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->EditWdgts=[%s], AE->expression=\n[%s]\n",p->name,AE->name,AE->expression);TRACE;
				...
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->EditWdgts=[%s], AE->mgt_expression=\n[%s]\n",p->name,AE->name,AE->mgt_expression);TRACE;
				...
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->EditWdgts=[%s], AE->override_expression=\n[%s]\n",p->name,AE->name,AE->override_expression);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->save_error",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->save_error_desc",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->save_warning",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->save_warning_desc",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->quit_error",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->save_expression",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->quit_warning",p->name);TRACE;
		...
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->quit_warning_desc",p->name);TRACE;
		...
		if(p->Writes!=NULL)
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->Writes",p->name);TRACE;
			for(x=0,AW=p->Writes;x<p->numWrites;++x,++AW)
			{
				prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->Writes=[%s][%s]",p->name,AW->module,AW->filename);TRACE;
				if(!isEMPTY(AW->expression))
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->Writes=[%s][%s], AW->expression=\n[%s]\n",p->name,AW->module,AW->filename,AW->expression);TRACE;
				...
				if(AW->fields!=NULL)
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->Writes=[%s][%s] AWFfields\n",p->name,AW->module,AW->filename);TRACE;
					for(y=0,AWF=AW->fields;y<AW->numflds;++y,++AWF)
					{
						prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->Writes=[%s][%s], AWF->to_name=[%s]\n",p->name,AW->module,AW->filename,AWF->to_name);TRACE;
#ifdef XXXX
						if(AWF->copy_type==1)
						{
#endif /* XXXX */
						...
						{
							prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->Writes=[%s][%s], AWF->from->name=[%s]",p->name,AW->module,AW->filename,AWF->from_name);TRACE;
						...
						{
							prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s], p->Writes=[%s][%s], AWF->to_name=[%s]",p->name,AW->module,AW->filename,AWF->to_name);TRACE;
		...
		if(p->buttons!=NULL)
		{
			prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->buttons",p->name);TRACE;
			for(x=0,BTN=p->buttons;x<p->numbuttons;++x,++BTN)
			{
				prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] p->button->[%s]",p->name,BTN->buttonname);TRACE;
				if(!isEMPTY(BTN->expression))
				{
					prterr("POWERADD_ContainsDate(): Checking POWERADD->name=[%s] BTN->button=[%s] BTN->expression=\n[%s]\n",p->name,BTN->buttonname,BTN->expression);TRACE;
...
*/
				modlst=APPlibNEW();
				prterr("Checking the directory=[%s] for PowerAdd's (*.ADD)",dirx);TRACE;
				for(x=0;findfile(liblist->libs[z],"*.ADD",&tmp,(int)x+1);++x) 
				{
					prterr("Found POWERADD=[%s]",tmp);TRACE;
					tmp[RDAstrlen(tmp)-4]=0;
					if(rng_mod==TRUE)
					{
						prterr("Checking RANGE of POWERADD=[%s] against FROM=[%s] TO=[%s]",tmp,mod_from,mod_to);TRACE;
						if((RDAstrcmp(tmp,mod_from)>=0) &&
							(RDAstrcmp(tmp,mod_to)<=0))
						{
							TRACE;
					...
					for(x=0;x<modlst->numlibs;++x)
					{
						...
						if(file_def!=NULL)
						{
							for(y=0;y<file_def->numlibs;++y)
							{
								...
								prterr("Getting file_def->libs[%d]=[%s]",y,file_def->libs[y]);TRACE;
								if(!getPowerAdd(libx,POWERADD))
								{
									prterr("Checking POWERADD=[%s]",file_def->libs[y]);TRACE;
