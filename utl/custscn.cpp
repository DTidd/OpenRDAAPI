/*lint -library */
#ifndef WIN32
#define __NAM__ "custscn.lnx"
#endif
#ifdef WIN32
#define __NAM__ "custscn.exe"
#endif
#include <app.hpp>
#include <cnvscn.hpp>
/*ADDLIB cscn */
...
static void ChangeScreenDefault_ContainsCustom(RDA_PFILE *,RDAscrn *,char *,char *,char *,int *,int *);
...
static void ChangeScreenDefault_ContainsCustom(RDA_PFILE *fp1,RDAscrn *p,char *lib,char *from_mod,char *to_mod,int *lines,int *pages)
{
	int x=0;
	RDAwdgt *temp=NULL;
	...
	if(p!=NULL)
	{
		...
		for(x=0,temp=p->wdgts;x<(p->numwdgts-1);++x,++temp)
		{
			if(temp->type==15)
			{
				...
				temp3=Rmalloc(22+1);
				sprintf(temp3,"Custom Field in Screen");
			...
			tempstring=ContainsACustomField(temp->resource_name);
			if(tempstring!=NULL)
			{
				if(*foundone==0) 
				{
							...
							if(file_def!=NULL)
							{
								for(y=0;y<file_def->numlibs;++y)
								{
									...
									if(getscrnbin(libx,RDA_SCREEN,FALSE)==0)
									{
										...
										ChangeScreenDefault_ContainsCustom(fp1,RDA_SCREEN,libx,mod_from,mod_to,&lines,&pages);		
									} else {
										if(diagapps)
										{
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"FIND CUSTOM INPUT FIELDS IN SCREENS")) 
	{
		return;
	}
	mainrsrc=RDArsrcNEW(module,"FIND CUSTOM INPUT FIELDS IN SCREENS");
