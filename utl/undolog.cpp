/* undolog.c - Undo Transaction Log */
#ifndef WIN32
#define __NAM__ "undolog.lnx"
#endif
#ifdef WIN32
#define __NAM__ "undolog.exe"
#endif
#include <app.hpp>
#include <fcntl.h>
#ifdef UNIXWARE7
...
short CanUndoLog()
{
	...
	FINDFLDSETINT(dattrn,"TRANSACTION NUMBER",((INT_MAX)-1));
	ef=PRVNRD(dattrn,1);
	while(!ef)
	{
...
void UndoLog()
{
	...
	char kv1[9][121],kv2[9][121],last_npv=FALSE,*cdate=NULL,*ctime=NULL;
	int t_log=(-1),i=0;
	...
	if(!EQLNRD(datlog,1))
	{
		...
		FINDFLDSETCHAR(datlog,"UNDONE",TRUE);
		FINDFLDSETSTRING(datlog,"UNDONE BY",USERLOGIN);
		cdate=GETCURRENTDATE10();
		FINDFLDSETSTRING(datlog,"UNDONE DATE",cdate);
		ctime=GETCURRENTTIME();
		FINDFLDSETSTRING(datlog,"UNDONE TIME",ctime);
...
#endif
{
	...
	{
		memset(xtemp,0,1024);
		sprintf(xtemp,"The Log [%s] for [%s] number [%d] was found, but new values didn't match current database values, so there must exist newer logs that must be undone in reverse chronological order.",(TRN_MOD!=NULL ? TRN_MOD:""),(TRN_USERID!=NULL ? TRN_USERID:""),TRN_LOG);
		ERRORDIALOG("Unable to Undo Database Log (DATLOG)",xtemp,quitonerr,TRUE);
	} else {
		ShutdownSubsystems();
	}
}
