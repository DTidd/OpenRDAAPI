...
#ifndef WIN32
#define __NAM__ "htmlscn.lnx"
#endif
#ifdef WIN32
#define __NAM__ "htmlscn.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
/*CPP_OPART htmlscn1 */
...
RDArsrc *mrsrc=NULL;
extern void HTMLCreateWidgets(FILE *,RDAscrn *),HTMLsetrscnum(RDAscrn *);
...
static void makescrncode(RDAscrn *scrn,FILE *fp,char *dirname,char *libname,char *scrnname,char *cgifile)
{
	...
	{	
		...
		fprintf(fp,"<TITLE>XPERT %s %s</TITLE>\n",(libname!=NULL ? 
			libname:""),(scrnname!=NULL ? scrnname:""));
		fprintf(fp,"</HEAD>\n");
		fprintf(fp,"<HR>\n");
		fprintf(fp,"<BODY>\n");

		fprintf(fp,"<B><FONT SIZE=10><P ALIGN=\"CENTER\">XPERT %s %s</P>\n",(libname!=NULL ? libname:""),(scrnname!=NULL ? scrnname:""));
	...
	char *scrnname,char *cgifile)
{
	...
	{	
		...
		fprintf(fp,"push(@INC,\"/rda/data/test/cgi-bin/\");\n");
		fprintf(fp,"require \"cgi-lib.pl\";\n");
		fprintf(fp,"&ReadParse;\n");

		fprintf(fp,"$ENV{'LOGNAME'}=\"wwwrun\";\n");
...
/* Widget Values */
		...
		switch(RDAapi)
		{
			...
			case 1: /* doreport */
#ifdef WIN32
				fprintf(fp,"exec '%s/doreport.exe','%s','%s';\n",(prog_path!=NULL ? prog_path:""),(libname!=NULL ? libname:""),APIavl->libs[APIselected]);
#endif
#if defined(LINUX2_2) || defined(UBUNTU_OS)
				fprintf(fp,"exec '%s/doreport.lnx','%s','%s';\n",(prog_path!=NULL ? prog_path:""),(libname!=NULL ? libname:""),APIavl->libs[APIselected]);
#endif
#ifdef LINUX
				fprintf(fp,"exec '%s/doreport.lnx','%s','%s';\n",(prog_path!=NULL ? prog_path:""),(libname!=NULL ? libname:""),APIavl->libs[APIselected]);
#endif
#ifdef RISC6000
				fprintf(fp,"exec '%s/doreport.rsc','%s','%s';\n",(prog_path!=NULL ? prog_path:""),(libname!=NULL ? libname:""),APIavl->libs[APIselected]);
#endif
#ifdef UNIXWARE7
				fprintf(fp,"exec '%s/doreport.unw','%s','%s';\n",(prog_path!=NULL ? prog_path:""),(libname!=NULL ? libname:""),APIavl->libs[APIselected]);
#endif
#ifdef SCO50P
				fprintf(fp,"exec '%s/doreport.sco','%s','%s';\n",(prog_path!=NULL ? prog_path:""),(libname!=NULL ? libname:""),APIavl->libs[APIselected]);
#endif
				break;
			case 2: /* domenu */
				break;
...
static void GetDevicePrintRDAscrncode(RDAscrn *scrn)
{
	...
	prsrc=RDArsrcNEW("UTILITIES","HTML PRINT SCREEN");
...
#ifndef WIN32
	sprintf(defaultprinter,"html/%s.HTML",dashes);
#endif
#ifdef WIN32
	sprintf(defaultprinter,"html\\%s.HTML",dashes);
#endif
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,"UTILITIES","MAKE HTML SCREEN DEFINITION"))
	{
		ShutdownSubsystems();
		exit(1);
	}
	mrsrc=RDArsrcNEW("UTILITIES","MAKE HTML SCREEN DEFINITION");
	scr_libs=APPlibNEW();
	scr_defs=APPlibNEW();
	dir_libs=APPlibNEW();
