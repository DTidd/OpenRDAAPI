...
#ifndef WIN32
#define __NAM__ "doreportall.lnx"
#endif
#ifdef WIN32
#define __NAM__ "doreportall.exe"
#endif
#include <app.hpp>
...
void doreport(RDArsrc *mainrsrc)
{
	...
	ExecuteProgram("doreportall",args);
...
#endif
{
	...
	if(argc==4)
	{
		if(!RDAstrcmp(argv[1],"-a"))
		{
			...
			{
				if(!RDAstrcmp(argv[2],modlst->libs[y])) break;
			}
			if(y>=modlst->numlibs)
			{
				y=0;
			...
			{
				if(!RDAstrcmp(argv[3],rptavl->libs[x])) break;
			}
			if(x>=rptavl->numlibs)
			{
				x=0;
			...
			for(y=0;y<modlst->numlibs;++y)
			{
				if(!RDAstrcmp(argv[1],modlst->libs[y])) break;
			}
			if(y>=modlst->numlibs)
			{
				y=0;
