/* restclmlm.c - Real Estate Class Millage Master Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
MakeBrowseList *mbl_restclml=NULL;
extern MaintainMaster *MASTER;
void restclmlm(short,void *targetkey);
...
static APPlib *makefieldvallist()
{
	...
	addDFincvir(tmprsrc,module,"RESTRAN",NULL,dumb);
	addDFincvir(tmprsrc,module,"RESTPAID",NULL,dumb);
...
static void chkaccountcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]",&acctcode);
...
static void chkrevenuecodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]",&acctcode);
...
static void chkcashcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]",&acctcode);
...
static void setlists(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]",deftype);
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"REVENUE CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CASH CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]",deftype2);
		updatersrc(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]");
		updatersrc(mtnrsrc,"ACCOUNT CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]");
		updatersrc(mtnrsrc,"REVENUE CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]");
		updatersrc(mtnrsrc,"CASH CODE DESCRIPTION");
	}
	updateSCRNvirtuals(mtnrsrc);
}
...
void restclmlm(short dowhich,void *targetkey)
{
	...
	nofields=0;
	FINDRSCSETFUNC(mtnrsrc,"[RESTCLML][CLASS IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTCLML][MILLAGE IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTCLML][DEFAULT ACCOUNT CODE]",chkaccountcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTCLML][DEFAULT REVENUE CODE]",chkrevenuecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTCLML][DEFAULT CASH CODE]",chkcashcodecb,mtn);
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_restclml,MASTER->passkey->fileno,module,mtnrsrc);
		if(mbl_restclml!=NULL) quit_record(mtnrsrc,mtn);
...
static void quit_record(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	if(WindowCount==1 || mbl_restclml==NULL)
	{
		if(MASTER!=NULL) 
		{
...
static void getrecord(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTCLML][CLASS IDENTIFICATION]");
	readwidget(mtnrsrc,"[RESTCLML][MILLAGE IDENTIFICATION]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTCLML][CLASS IDENTIFICATION]",&temp);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"CLASS IDENTIFICATION",temp);
	if(temp!=NULL) Rfree(temp);
	FINDRSCGETSTRING(mtnrsrc,"[RESTCLML][MILLAGE IDENTIFICATION]",&temp);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"MILLAGE IDENTIFICATION",temp);
	if(temp!=NULL) Rfree(temp);
