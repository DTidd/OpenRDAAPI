/* restcml.c - Real Estate Class Millage Master Program*/
/*lint -library */
#ifndef WIN32
#define __NAM__ "restcml.lnx"
#endif
#ifdef WIN32
#define __NAM__ "restcml.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rdaflds.h>
...
extern MakeBrowseList *mbl_restclml;
extern void restclmlm(short,void *);
...
void browse_msr(RDArsrc *parent,mtnstruct *mtn)
{
	...
	mbl_restclml=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,
...
void main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,module,"MTN RESTCLML")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("RLSTMGT","MTN RESTCLML");
	if(findMaintainMaster(MASTER))
	{
		warnmessage=Rmalloc(300+RDAstrlen("RLSTMGT")+RDAstrlen("MTN RESTCLML"));
		sprintf(warnmessage,"Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN RESTCLML","RLSTMGT");
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_restclml!=NULL) mbl_restclml=NULL;
}
static void okfunc(MakeBrowseList *blist,void *targetkey)
{
	...
	{
		if(!run_func) restclmlm(0,targetkey);
			else ExecuteListFunc(run_func-1,targetkey,MASTER);
	} else restclmlm(0,targetkey);
}
