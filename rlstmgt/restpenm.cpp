/* restpenm.c - Real Estate Penalty Master Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
MakeBrowseList *mbl_restpen=NULL;
extern MaintainMaster *MASTER;
void restpenm(short,void *targetkey);
...
static void chkaccountcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]",&acctcode);
...
static void chkrevenuecodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]",&acctcode);
...
static void chkcashcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]",&acctcode);
...
static void setlists(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]",deftype);
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"REVENUE CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CASH CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]",deftype2);
		updatersrc(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]");
		updatersrc(mtnrsrc,"ACCOUNT CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]");
		updatersrc(mtnrsrc,"REVENUE CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]");
		updatersrc(mtnrsrc,"CASH CODE DESCRIPTION");
	}
	updateSCRNvirtuals(mtnrsrc);
}
...
void restpenm(short dowhich,void *targetkey)
{
	...
	nofields=0;
	FINDRSCSETFUNC(mtnrsrc,"[RESTPEN][CLASS IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTPEN][PENALTY IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTPEN][DEFAULT ACCOUNT CODE]",chkaccountcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTPEN][DEFAULT REVENUE CODE]",chkrevenuecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTPEN][DEFAULT CASH CODE]",chkcashcodecb,mtn);
	...
	short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_restpen,MASTER->passkey->fileno,module,mtnrsrc);
		if(mbl_restpen!=NULL) quit_record(mtnrsrc,mtn);
...
static void quit_record(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	if(WindowCount==1 || mbl_restpen==NULL)
	{
		if(MASTER!=NULL) 
		{
...
static void getrecord(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTPEN][CLASS IDENTIFICATION]");
	readwidget(mtnrsrc,"[RESTPEN][PENALTY IDENTIFICATION]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTPEN][CLASS IDENTIFICATION]",&temp);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"CLASS IDENTIFICATION",temp);
	if(temp!=NULL) Rfree(temp);
	FINDRSCGETSTRING(mtnrsrc,"[RESTPEN][PENALTY IDENTIFICATION]",&temp);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"PENALTY IDENTIFICATION",temp);
	if(temp!=NULL) Rfree(temp);
	if(ADVEQLNRDsec(MASTER->passkey->fileno,MASTER->passkey->keyno,
		SCRNvirtualSubData,mtnrsrc)) 
	{
