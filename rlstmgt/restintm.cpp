/* restintm.c - Real Estate Class Interest Master Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
MakeBrowseList *mbl_restint=NULL;
extern MaintainMaster *MASTER;
void restintm(short,void *targetkey);
...
static void chkaccountcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]",&acctcode);
...
static void chkrevenuecodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]",&acctcode);
...
static void chkcashcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]",&acctcode);
...
static void setlists(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]",deftype);
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"REVENUE CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CASH CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]",deftype2);
		updatersrc(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]");
		updatersrc(mtnrsrc,"ACCOUNT CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]");
		updatersrc(mtnrsrc,"REVENUE CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]");
		updatersrc(mtnrsrc,"CASH CODE DESCRIPTION");
	}
	updateSCRNvirtuals(mtnrsrc);
}
...
void restintm(short dowhich,void *targetkey)
{
	...
	nofields=0;
	FINDRSCSETFUNC(mtnrsrc,"[RESTINT][CLASS IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[TESTINT][INTEREST IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTINT][DEFAULT ACCOUNT CODE]",chkaccountcodecb,
		mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTINT][DEFAULT REVENUE CODE]",chkrevenuecodecb,
		mtn);
	FINDRSCSETFUNC(mtnrsrc,"[RESTINT][DEFAULT CASH CODE]",chkcashcodecb,mtn);
	...
	short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_restint,MASTER->passkey->fileno,module,mtnrsrc);
		if(mbl_restint!=NULL) quit_record(mtnrsrc,mtn);
...
static void quit_record(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	if(WindowCount==1 || mbl_restint==NULL)
	{
		if(MASTER!=NULL) 
		{
...
static void getrecord(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[RESTINT][CLASS IDENTIFICATION]");
	readwidget(mtnrsrc,"[RESTINT][INTEREST IDENTIFICATION]");
	FINDRSCGETSTRING(mtnrsrc,"[RESTINT][CLASS IDENTIFICATION]",&temp);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"CLASS IDENTIFICATION",temp);
	if(temp!=NULL) Rfree(temp);
	FINDRSCGETSTRING(mtnrsrc,"[RESTINT][INTEREST IDENTIFICATION]",&temp);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"INTEREST IDENTIFICATION",temp);
	if(temp!=NULL) Rfree(temp);
