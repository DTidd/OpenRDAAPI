/* restapen.c - Apply Penalties */
#ifndef WIN32
#define __NAM__ "restapen.lnx"
#endif
#ifdef WIN32
#define __NAM__ "restapen.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rptgen.h>
...
static short rstmst=(-1),rstcp=(-1),classnum=(-1);
...
static void applypenalties(RDArsrc *);
...
static short checktrantype(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	char compute_when=FALSE,deleteflag=FALSE;
	PassableStruct *p=NULL;
	...
	FINDFLDSETSTRING(rstcp,"PENALTY IDENTICATION",refid);
	p=Rmalloc(sizeof(PassableStruct));
	if(ADVEQLNRDsec(rstcp,1,ApplySubData,p))
	{
		KEYNRD(rstcp,1);
		if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
		...
		FINDFLDGETCHAR(rstcp,"DELETEFLAG",&deleteflag);
		if(deleteflag)
		{
			if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
			tmp2=Rmalloc(RDAstrlen(refid)+RDAstrlen(classid)+200);
			sprintf(tmp2,"The Reference Identification [%s] is found for Class [%s], but is marked for deletion in the Real Estate Class Penalty Master.",refid,classid);
			...
			FINDFLDGETCHAR(rstcp,"COMPUTE WHEN APPLIED",&compute_when);
			if(!compute_when)
			{
				if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
				tmp2=Rmalloc(RDAstrlen(refid)+RDAstrlen(classid)+200);
				sprintf(tmp2,"The Reference Identification [%s] is found for Class [%s], but is not to compute during APPLY PENALTIES in the Real Estate Class Penalty Master.",refid,classid);
	...
	if(dbsort!=NULL)
	{
		...
		while(!ef)
		{
			...
			if(!deleteflag && amount!=0.0)
			{
				...
				COPYFIELD(dbsort->fileno,rstmst,"OWNER IDENTIFICAITON");
				...
				ADVUpdateRealEstateTransactions(rstmst,(-1),
					rstrn,amount,ApplySubData,p);
				COUNT=TRUE;
				update_diag=TRUE;
			}
...
void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("RLSTMGT","APPLY PENALTIES SORT SEQUENCE");
	if(d!=NULL)
	{
...
void addsortrecord(DBsort *dbsrt,char *realestateid,char *ownerid,
	int TaxYear,char *penaltyid,double gross,short deftype,char *acctcode,
	char *revcode,char *cashcode,char *desc,PassableStruct *p)
{
	...
	FINDFLDSETSTRING(dbsrt->fileno,"REFERENCE IDENTIFICATION",penaltyid);
...
static void doexit_test(MakeBrowseList *blist)
{
	...
	if(dbsort!=NULL)
	{
		WARNINGDIALOG("WARNING DIALOG SCREEN","RECORDS NOT PROCESSED!","Are you sure that you want to quit without processing the Applied Penalties?",NULL,doexit,FALSE,blist);
	} else {
		doexit(blist);
	}
}
...
static void applypenalties(RDArsrc *mainrsrc)
{
	...
	char *penaltyid=NULL,*acctcode=NULL,*revcode=NULL,*cashcode=NULL;
		...
		if(!deleteflag && !testfile)
		{
			...
			if(!ADVEQLNRDsec(classnum,1,ApplySubData,p))
			{
				...
				if(!deleteflag && !testfile)
				{
					ZERNRD(rstcp);
					FINDFLDSETSTRING(rstcp,"CLASS IDENTIFICATION",classid);
					ef1=ADVGTENRDsec(rstcp,1,ApplySubData,p);
					while(!ef1)
					{
						FINDFLDGETSTRING(rstcp,"CLASS IDENTIFICATION",&classid1);
						FINDFLDGETCHAR(rstcp,"COMPUTE WHEN APPLIED",&compute_when);
						FINDFLDGETCHAR(rstcp,"DELETEFLAG",&deleteflag);
						if(RDAstrcmp(classid,classid1)) 
						{
							if(diagapps)
							{
								prterr("Skipped by Non-matching CLASS IDENTIFICATION in RESTPEN of [%s] to RESTMSTR.",classid1,classid);SEENRDRECORD(rstcp);SEENRDRECORD(rstmst);
							}
							break;
						}
						if(!deleteflag && compute_when)
						{
							FINDFLDGETSTRING(rstcp,"PENALTY IDENTIFICATION",&penaltyid);
							testfile=testfilerangersrc(rstcp,rs);
							if(!testfile)
							{
								...
								if(etest)
								{
									...
									if(x>=mainrsrc->num)
									{
										...
										{
											prterr("Skipped because PENALTY FORMULA empty.");SEENRDRECORD(rstcp);
										}
										if(AMOUNT!=0.0)
										{
											FINDFLDGETSHORT(rstcp,"DEFINITION TYPE",&deftype);
											FINDFLDGETSTRING(rstcp,"DEFAULT ACCOUNT CODE",&acctcode);
											FINDFLDGETSTRING(rstcp,"DEFAULT REVENUE CODE",&revcode);
											FINDFLDGETSTRING(rstcp,"DEFAULT CASH CODE",&cashcode);
											FINDFLDGETSTRING(rstcp,"NAME",&desc);
											addsortrecord(dbsort,realestateid,ownerid,TaxYear,penaltyid,AMOUNT,deftype,acctcode,revcode,cashcode,desc,p);
											if(acctcode!=NULL) Rfree(acctcode);
										...
										{
											prterr("Skipped by PENALITY FORMULA Evaluating to ZERO (0.00).");SEENRDRECORD(rstcp);
										}
									} else if(diagapps)
									{
							...
							{
								prterr("Skipped by Ranging on Class Penalty Master or deleted class penalty master.");
							}
						} else if(diagapps)
						{
							prterr("Skipped by finding a deleted Class Penalty.");SEENRDRECORD(rstcp);
						}
						CLEAR_SCREEN_VIRTUAL(mainrsrc);
						ef1=ADVNXTNRDsec(rstcp,1,ApplySubData,p);
					}
				} else if(diagapps)
				{
	...
	if(penaltyid!=NULL) Rfree(penaltyid);
	if(tmprsrc!=NULL)
	{
		killwindow(tmprsrc);
		free_rsrc(tmprsrc);
	...
	if(size_of_file>0)
	{
		switch(apply_type)
		{
			...
			case 1: /* Process Exceptions */
				...
				brsrc=RDArsrcNEW(module,"APPLY PENALTIES BROWSE");
				srsrc=RDArsrcNEW(module,"APPLY PENALTIES SEARCH BROWSE");
				drsrc=RDArsrcNEW(module,"APPLY PENALTIES DEFINE LIST");
		...
		ERRORDIALOG("Apply Penalties Error","No records selected using the given select criteria.  Please try again.",NULL,FALSE);
		CLSDBsort(dbsort);
		FreeDBsort(dbsort);
	}
}
...
void main(int argc,char **argv)
{
	...
	if((rstcp=APPOPNNRD(module,"RESTPEN",TRUE,FALSE))==(-1)) return;
	...
	addDFincvir(mainrsrc,module,"RESTPEN",NULL,rstcp);
	file2rangersrc(rstmst,mainrsrc);
	file2rangersrc(classnum,mainrsrc);
	file2rangersrc(rstcp,mainrsrc);
	...
	addbtnrsrc(mainrsrc,"SELECT",TRUE,applypenalties,NULL);
...
void applym(short dowhich,void *targetkey,DBsort *t)
{
	...
	mtnrsrc=RDArsrcNEW(module,"APPLY PENALTIES MAINTAIN SCREEN");
	addDFincvir(mtnrsrc,module,"RESTMSTR",NULL,rstmst);
	addDFincvir(mtnrsrc,module,"RESTCLAS",NULL,classnum);
	addDFincvir(mtnrsrc,module,"RESTPEN",NULL,rstcp);
	addDFincvir(mtnrsrc,"FINYEAR","FINYEAR",NULL,fsclyr);
...
/* do DEFINITION TYPE LIST */
	...
	if(rstcp!=(-1)) file2rsrc(rstcp,mtnrsrc,FALSE);
		...
		if(fsclyr!=(-1))
		{
			...
			if(ADVEQLNRDsec(fsclyr,1,ApplySubData,p))
			{
				...
				if(delflag)
				{
					...
					{
						warnmessage=Rmalloc(RDAstrlen(temp)+140);
						sprintf(warnmessage,"The FISCAY YEAR of [%d] entered is not active.  It exists as a Fiscal Year, but is presently set up in an inactive state.",yearid);
...
static void getsupportingfiles(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	COPYFIELD(dbsort->fileno,rstcp,"CLASS IDENTIFICATION");
	COPYFIELD(dbsort->fileno,rstcp,"PENALTY IDENTIFICATION");
	if(ADVEQLNRDsec(rstcp,1,ApplySubData,p)) KEYNRD(rstcp,1);
	filerecord2rsrc(rstcp,mtnrsrc);
	...
	FINDFLDGETSTRING(rstcp,"PENALTY FORMULA",&exprssn);
	AMOUNT=0.0;
	if(exprssn!=NULL)
	{
		AMOUNT=EVALUATEdbl(exprssn,ApplySubData,p);
