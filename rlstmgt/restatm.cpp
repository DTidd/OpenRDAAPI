/* restatm.c - Apply Tax Millage */
#ifndef WIN32
#define __NAM__ "restatm.lnx"
#endif
#ifdef WIN32
#define __NAM__ "restatm.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <ldval.h>
...
static short rstmst=(-1),rstnew=(-1),rstcm=(-1),classnum=(-1);
static short millnum=(-1),baccnum=(-1),raccnum=(-1),ryrnum=(-1),byrnum=(-1);
static short rstrn=(-1),fsclyr=(-1),COUNT=0;
static int apply_type=0,FiscalYear=0,xmonth=0;
double AMOUNT=0.0;
...
static void applytaxmillage_test(RDArsrc *);
static void applytaxmillage(RDArsrc *);
...
APPlib *makefieldvallist()
{
	...
	list=APPlibNEW();
	AddFileFields(list,rstnew);
	AddFileFields(list,classnum);
	AddFileFields(list,rstcm);
	AddFileFields(list,millnum);
	tmprsrc=RDArsrcNEW(module,"TEMP");
	addDFincvir(tmprsrc,module,"RESTMSTR",NULL,dumb);
	addDFincvir(tmprsrc,module,"RESTCLAS",NULL,dumb);
	addDFincvir(tmprsrc,module,"RESTCLML",NULL,dumb);
	addDFincvir(tmprsrc,module,"RESTMILL",NULL,dumb);
	addDFincvir(tmprsrc,module,"RESTNEW",NULL,dumb);
...
static short checktrantype(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	FINDFLDSETSTRING(rstcm,"MILLAGE IDENTICATION",refid);
	p=Rmalloc(sizeof(PassableStruct));
	p->r=mtnrsrc;
	p->a=dbsort;
	if(ADVEQLNRDsec(rstcm,1,ApplySubData,p))
	{
		KEYNRD(rstcm,1);
		if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
...
{
	short ef=FALSE,update_diag=FALSE,trantype=0;
	...
	if(dbsort!=NULL)
	{
		tmprsrc=diagscrn(dbsort->fileno,"DIAGNOSTIC SCREEN",module,
			"Appling Tax Millages",NULL);
		addDFincvir(tmprsrc,module,"RESTNEW",NULL,rstnew);
		...
		while(!ef)
		{
			...
			{
				COPYFIELD(dbsort->fileno,rstnew,"REAL ESTATE IDENTIFICATION");
				COPYFIELD(dbsort->fileno,rstnew,"OWNER IDENTIFICATION");
				if(ADVEQLNRDsec(rstnew,1,ApplySubData,p)) KEYNRD(rstnew,1);
				...
				ADVUpdateRealEstateTransactions(rstmst,rstnew,rstrn,amount,ApplySubData,p);
				COUNT=TRUE;
				update_diag=TRUE;
			}
...
void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("RLSTMGT","APPLY TAX MILLAGE SORT SEQUENCE");
	if(d!=NULL)
	{
...
void addsortrecord(DBsort *dbsrt,char *realestateid,
	char *ownerid,char *millageid,double gross,short deftype,char *acctcode,
	char *revcode,char *cashcode,char *desc,PassableStruct *p)
{
	...
	FINDFLDSETINT(dbsrt->fileno,"TAX YEAR",FiscalYear);
	FINDFLDSETSHORT(dbsrt->fileno,"MONTH",ymonth);
	FINDFLDSETSTRING(dbsrt->fileno,"REFERENCE IDENTIFICATION",millageid);
...
static void doexit_test(MakeBrowseList *blist)
{
	...
	if(dbsort!=NULL)
	{
		WARNINGDIALOG("WARNING DIALOG SCREEN","RECORDS NOT PROCESSED!","Are you sure that you want to quit without processing the Applied Tax Millages?",NULL,doexit,FALSE,blist);
	} else {
		doexit(blist);
	}
}
...
static void applytaxmillage_test(RDArsrc *mainrsrc)
{
	char flag=FALSE,*warnmessage=NULL;

	readallwidgets(mainrsrc);
	FINDRSCGETINT(mainrsrc,"TAX YEAR",&FiscalYear);
	ZERNRD(fsclyr);
	FINDFLDSETINT(fsclyr,"FISCAL YEAR",FiscalYear);
	if(ADVEQLNRDsec(fsclyr,1,SCRNvirtualSubData,mainrsrc))
	{
		KEYNRD(fsclyr,1);
		warnmessage=Rmalloc(4+170);
		sprintf(warnmessage,"The FISCAL YEAR of [%4d] entered is not valid.  It does not exist as a Fiscal Year.  The Tax Millage can not be applied to this Fiscal Year.",FiscalYear);
		WARNINGDIALOG("WARNING DIALOG SCREEN","INVALID FISCAL YEAR!",warnmessage,NULL,NULL,TRUE,NULL);
		prterr("The FISCAL YEAR of [%d] entered does not exist in the [FINMGT][FINYEAR] data file.  Can not apply the Tax Millage to this Fiscal Year.",FiscalYear);
		if(warnmessage!=NULL) Rfree(warnmessage);
		return;
	} else {
		FINDFLDGETCHAR(fsclyr,"DELETEFLAG",&flag);
		if(flag)
		{
			warnmessage=Rmalloc(4+180);
			sprintf(warnmessage,"The FISCAL YEAR of [%4d] entered is deleted.  It exists as a Fiscal Year, but is presently set up in a deleted state.  Can not apply the Tax Millage to this Fiscal Year.",FiscalYear);
			WARNINGDIALOG("WARNING DIALOG SCREEN","DELETED FISCAL YEAR!",warnmessage,NULL,NULL,TRUE,NULL);
			prterr("The FISCAL YEAR of [%d] entered is deleted.  It exist as a Fiscal Year, but is presently set up in a deleted state.  Can not apply the Tax Millage to this Fiscal Year.",FiscalYear);
			if(warnmessage!=NULL) Rfree(warnmessage);
			return;
		} else {
			FINDFLDGETCHAR(fsclyr,"ACTIVE",&flag);
			if(!flag)
			{
				warnmessage=Rmalloc(4+180);
				sprintf(warnmessage,"The TAX YEAR of [%4d] entered is not active.  It exists as a Fiscal Year, but is presently set up in an inactive state.  Cannot apply the Tax Millage to this Fiscal Year.",FiscalYear);
				WARNINGDIALOG("WARNING DIALOG SCREEN","INACTIVE TAX YEAR!",warnmessage,NULL,NULL,TRUE,NULL);
				prterr("The TAX YEAR of [%d] entered is not active.  It exists as a Fiscal Year, but is presently set up in an inactive state.  Cannot apply the Tax Millage to this Fiscal Year.",FiscalYear);
				if(warnmessage!=NULL) Rfree(warnmessage);
				return;
			}
		}
	}
	applytaxmillage(mainrsrc);
}
static void applytaxmillage(RDArsrc *mainrsrc)
{
	...
	char *millageid=NULL,*acctcode=NULL,*revcode=NULL,*cashcode=NULL;
	char deleteflag=FALSE,*sortname=NULL,*realestateid=NULL,*desc=NULL;
	char currentowner=FALSE;
	int x;
	long size_of_file=0;
	PassableStruct *p=NULL;
	RDAvirtual *v=NULL;
	...
	ReadRangeScreen(rstnew,rs);
	ReadRangeScreen(classnum,rs);
	ReadRangeScreen(rstcm,rs);
	ReadRangeScreen(millnum,rs);
	tmprsrc=diagscrn(rstnew,"DIAGNOSTIC SCREEN",module,
	...
	p->r=mainrsrc;
	p->a=dbsort;
	ef=ADVFRSNRDsec(rstnew,1,ApplySubData,p);
	while(!ef)
	{
		dodiag=FALSE;
		FINDFLDGETCHAR(rstnew,"DELETEFLAG",&deleteflag);
		FINDFLDGETCHAR(rstnew,"CURRENT OWNER",&currentowner);
		if(!deleteflag && currentowner)
		{
			testfile=testfilerangersrc(rstnew,rs);
		} else testfile=TRUE;
		if(!testfile)
		{
			FINDFLDGETSTRING(rstnew,"CLASS IDENTIFICATION",&classid);
			...
			if(!ADVEQLNRDsec(classnum,1,ApplySubData,p))
			{
				...
				{
					ZERNRD(rstmst);
					COPYFIELD(rstnew,rstmst,"REAL ESTATE IDENTIFICATION");
					COPYFIELD(rstnew,rstmst,"OWNER IDENTIFICATION");
					FINDFLDSETINT(rstmst,"TAX YEAR",FiscalYear);
					if(ADVEQLNRDsec(rstmst,1,ApplySubData,p)) KEYNRD(rstmst,1);
					ZERNRD(rstcm);
					FINDFLDSETSTRING(rstcm,"CLASS IDENTIFICATION",classid);
					ef1=ADVGTENRDsec(rstcm,1,ApplySubData,p);
					while(!ef1)
					{
						FINDFLDGETSTRING(rstcm,"CLASS IDENTIFICATION",&classid1);
						if(RDAstrcmp(classid,classid1)) break;
						FINDFLDGETCHAR(rstcm,"DELETEFLAG",&deleteflag);
						if(!deleteflag)
						{
							testfile=testfilerangersrc(rstcm,rs);
						} else testfile=TRUE;
						if(!testfile)
						{
							FINDFLDGETSTRING(rstcm,"MILLAGE IDENTIFICATION",&millageid);
							FINDFLDSETSTRING(millnum,"MILLAGE IDENTIFICATION",millageid);
							if(ADVEQLNRDsec(millnum,1,ApplySubData,p)) KEYNRD(millnum,1);
							FINDFLDGETCHAR(millnum,"DELETEFLAG",&deleteflag);
							if(!deleteflag)
							{
								testfile=testfilerangersrc(millnum,rs);
							} else testfile=TRUE;
							if(!testfile)
							{
								...
								if(etest)
								{
									...
									if(x>=mainrsrc->num)
									{
										...
										FINDFLDGETSHORT(rstcm,"DEFAULT DEFINITION TYPE",&deftype);
										FINDFLDGETSTRING(rstcm,"DEFAULT ACCOUNT CODE",&acctcode);
										FINDFLDGETSTRING(rstcm,"DEFAULT REVENUE CODE",&revcode);
										FINDFLDGETSTRING(rstcm,"DEFAULT CASH CODE",&cashcode);
										FINDFLDGETSTRING(millnum,"NAME",&desc);
										FINDFLDGETSTRING(rstnew,"REAL ESTATE IDENTIFICATION",&realestateid);
										FINDFLDGETSTRING(rstnew,"OWNER IDENTIFICATION",&ownerid);
										addsortrecord(dbsort,realestateid,ownerid,millageid,AMOUNT,deftype,acctcode,revcode,cashcode,desc,p);
							...
							{
								prterr("Skipped by Ranging on Class Millage Master or deleted class millage master.");
							}
						} else if(diagapps)
						{
							prterr("Skipped by finding a deleted Class Millage.");SEENRDRECORD(rstcm);
						}
						CLEAR_SCREEN_VIRTUAL(mainrsrc);
						ef1=ADVNXTNRDsec(rstcm,1,ApplySubData,p);
					}
				} else if(diagapps)
				{
		...
		{
			prterr("Skipped by Ranging on Real Estate Master or deleted real estate master.");KEYNRD(rstnew,1);SEENRDRECORD(rstnew);
		}
		if(tmprsrc!=NULL) update_diagnostic(tmprsrc,(dodiag ? TRUE:FALSE));
		ef=ADVNXTNRDsec(rstnew,1,ApplySubData,p);
	}
	if(p!=NULL) Rfree(p);
	if(ownerid!=NULL) Rfree(ownerid);
	if(classid!=NULL) Rfree(classid);
	...
	if(millageid!=NULL) Rfree(millageid);
	if(tmprsrc!=NULL)
	{
		killwindow(tmprsrc);
		free_rsrc(tmprsrc);
	...
	if(size_of_file>0)
	{
		switch(apply_type)
		{
			...
			case 1: /* Process Exceptions */
				...
				brsrc=RDArsrcNEW(module,"APPLY TAX MILLAGE BROWSE");
				srsrc=RDArsrcNEW(module,"APPLY TAX MILLAGE SEARCH BROWSE");
				drsrc=RDArsrcNEW(module,"APPLY TAX MILLAGE DEFINE LIST");
				addDFincvir(drsrc,module,"RESTMSTR",NULL,rstmst);
				addDFincvir(drsrc,module,"RESTNEW",NULL,rstnew);
				addDFincvir(drsrc,module,"RESTCLAS",NULL,classnum);
				addDFincvir(srsrc,module,"RESTMSTR",NULL,rstmst);
				addDFincvir(srsrc,module,"RESTNEW",NULL,rstnew);
		...
		ERRORDIALOG("Apply Tax Millage Error","No records selected using the given select criteria.  Please try again.",NULL,FALSE);
		CLSDBsort(dbsort);
		FreeDBsort(dbsort);
	}
}
...
void main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,module,"APPLY TAX MILLAGE")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	} else reportname=stralloc("APPLYTAX");
	fmgt=RDAfinmgtNEW();
	...
	if((rstcm=APPOPNNRD(module,"RESTCLML",TRUE,FALSE))==(-1)) return;
	if((rstrn=APPOPNNRD(module,"RESTRAN",TRUE,TRUE))==(-1)) return;
	if((rstnew=APPOPNNRD(module,"RESTNEW",TRUE,TRUE))==(-1)) return;
	if((classnum=APPOPNNRD(module,"RESTCLAS",TRUE,FALSE))==(-1)) return;
	if((millnum=APPOPNNRD(module,"RESTMILL",TRUE,FALSE))==(-1)) return;
	...
	makeapplytype();
	mainrsrc=RDArsrcNEW(module,"APPLY TAX MILLAGE");
	addDFincvir(mainrsrc,module,"RESTMSTR",NULL,rstmst);
	addDFincvir(mainrsrc,module,"RESTNEW",NULL,rstnew);
	addDFincvir(mainrsrc,module,"RESTCLAS",NULL,classnum);
	addDFincvir(mainrsrc,module,"RESTCLML",NULL,rstcm);
	addDFincvir(mainrsrc,module,"RESTMILL",NULL,millnum);
	addDFincvir(mainrsrc,"FINMGT","FINYEAR",NULL,fsclyr);
	file2rsrc(fsclyr,mainrsrc,FALSE);
	file2rangersrc(rstnew,mainrsrc);
	file2rangersrc(classnum,mainrsrc);
	file2rangersrc(rstcm,mainrsrc);
	file2rangersrc(millnum,mainrsrc);
	addlstrsrc(mainrsrc,"APPLY TYPES",&apply_type,TRUE,NULL,2,
		&applytype,NULL);
	xmonth=getcurrmonth();
	if(fmgt!=NULL)
	{
		FiscalYear=GETCURRENTFISCALYEAR(fmgt->month);
	}
	addlstrsrc(mainrsrc,"MONTHS",&xmonth,TRUE,NULL,12,&monthlist,NULL);
	addstdrsrc(mainrsrc,"TAX YEAR",LONGV,4,&FiscalYear,TRUE);
	...
	addbtnrsrc(mainrsrc,"SELECT",TRUE,applytaxmillage_test,NULL);
...
void applym(short dowhich,void *targetkey,DBsort *t)
{
	...
	mtnrsrc=RDArsrcNEW(module,"APPLY TAX MILLAGE MAINTAIN SCREEN");
	addDFincvir(mtnrsrc,module,"RESTMSTR",NULL,rstmst);
	addDFincvir(mtnrsrc,module,"RESTNEW",NULL,rstnew);
	addDFincvir(mtnrsrc,module,"RESTCLAS",NULL,classnum);
	addDFincvir(mtnrsrc,module,"RESTCLML",NULL,rstcm);
	addDFincvir(mtnrsrc,module,"RESTMILL",NULL,millnum);
...
/* do DEFINITION TYPE LIST */
	...
	if(rstnew!=(-1)) file2rsrc(rstnew,mtnrsrc,FALSE);
	if(classnum!=(-1)) file2rsrc(classnum,mtnrsrc,FALSE);
	if(rstcm!=(-1)) file2rsrc(rstcm,mtnrsrc,FALSE);
	if(millnum!=(-1)) file2rsrc(millnum,mtnrsrc,FALSE);
	...
	switch(dowhich)
	{
		...
		case 2:
			...
			FINDRSCGETINT(mtnrsrc,"TAX YEAR",&yearid);
			FINDRSCGETINT(mtnrsrc,"TRANSACTION NUMBER",&tranno);
			FINDRSCSETINT(mtnrsrc,"MONTHS",xmonth);
			...
			ymonth=(short)xmonth;
			FINDFLDSETSHORT(t->fileno,"MONTH",ymonth);
			if(realestateid!=NULL) Rfree(realestateid);
			if(ownerid!=NULL) Rfree(ownerid);
			break;
		default:
...
static void save_check(RDArsrc *mtnrsrc,short update_list,DBsort *dbsort)
{
	...
	if(!delflag)
	{
		...
		{
			FINDFLDGETINT(dbsort->fileno,"TAX YEAR",&yearid);
			FINDFLDSETINT(fsclyr,"TAX YEAR",yearid);
			if(ADVEQLNRDsec(fsclyr,1,ApplySubData,p))
			{
				KEYNRD(fsclyr,1);
				warnmessage=Rmalloc(RDAstrlen(temp)+137);
				sprintf(warnmessage,"The TAX YEAR of [%d] entered is not valid.  It does not exist as a Fiscal Year.",yearid);
				WARNINGDIALOG("WARNING DIALOG SCREEN","INVALID TAX YEAR!",warnmessage,NULL,NULL,TRUE,NULL);
				prterr("The TAX YEAR of [%d] entered does not exist in the [FINMGT][FINYEAR] data file.",yearid);
				if(warnmessage!=NULL) Rfree(warnmessage);
				return;
			} else {
				...
				if(delflag)
				{
					...
					sprintf(warnmessage,"The TAX YEAR of [%d] entered is deleted.  It exists as a Fiscal Year, but is presently set up in a deleted state.",yearid);
					WARNINGDIALOG("WARNING DIALOG SCREEN","DELETED TAX YEAR!",warnmessage,NULL,NULL,TRUE,NULL);
					prterr("The TAX YEAR of [%d] entered is deleted.  It exist as a Fiscal Year, but is presently set up in a deleted state.",yearid);
					if(warnmessage!=NULL) Rfree(warnmessage);
					return;
				} else {
					...
					{
						warnmessage=Rmalloc(RDAstrlen(temp)+140);
						sprintf(warnmessage,"The TAX YEAR of [%d] entered is not active.  It exists as a Fiscal Year, but is presently set up in an inactive state.",yearid);
						WARNINGDIALOG("WARNING DIALOG SCREEN","INACTIVE TAX YEAR!",warnmessage,NULL,NULL,TRUE,NULL);
						prterr("The TAX YEAR of [%d] entered is not active.  It exists as a Fiscal Year, but is presently set up in an inactive state.",yearid);
						if(warnmessage!=NULL) Rfree(warnmessage);
						return;
					}
				}
		...
		if(rstnew!=(-1))
		{
			...
			FINDFLDSETSTRING(rstnew,"REAL ESTATE IDENTIFICATION",temp);
			FINDFLDSETSTRING(rstnew,"OWNER IDENTIFICATION",ownid);
			if(ADVEQLNRDsec(rstnew,1,ApplySubData,p))
			{
				KEYNRD(rstnew,1);
				...
				FINDFLDGETCHAR(rstnew,"DELETEFLAG",&delflag);
				if(delflag)
				{
					warnmessage=Rmalloc(RDAstrlen(temp)+RDAstrlen(ownid)+160);
...
static void getsupportingfiles(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	COPYFIELD(dbsort->fileno,rstnew,"OWNER IDENTIFICATION");
	if(ADVEQLNRDsec(rstnew,1,ApplySubData,p)) KEYNRD(rstnew,1);
	filerecord2rsrc(rstnew,mtnrsrc);
	...
	COPYFIELD(dbsort->fileno,millnum,"MILLAGE IDENTIFICATION");
	if(ADVEQLNRDsec(millnum,1,ApplySubData,p)) KEYNRD(millnum,1);
	filerecord2rsrc(millnum,mtnrsrc);
	COPYFIELD(dbsort->fileno,rstcm,"CLASS IDENTIFICATION");
	COPYFIELD(dbsort->fileno,rstcm,"MILLAGE IDENTIFICATION");
	if(ADVEQLNRDsec(rstcm,1,ApplySubData,p)) KEYNRD(rstcm,1);
	filerecord2rsrc(rstcm,mtnrsrc);
	...
	FINDFLDGETSTRING(rstcm,"RATE EXPRESSION",&exprssn);
	AMOUNT=0.0;
	if(exprssn!=NULL)
	{
		AMOUNT=EVALUATEdbl(exprssn,ApplySubData,p);
