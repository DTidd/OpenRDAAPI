/* restmstr.c - RESTMSTR Browse */
/*lint -library */
#ifndef WIN32
#define __NAM__ "restmstr.lnx"
#endif
#ifdef WIN32
#define __NAM__ "restmstr.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rdaflds.h>
...
void main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,"RLSTMGT","MTN RESTMSTR"))
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew("RLSTMGT","MTN RESTMSTR");
	if(findMaintainMaster(MASTER))
	{
		warnmessage=Rmalloc(300+RDAstrlen("RLSTMGT")+RDAstrlen("MTN RESTMSTR"));
		sprintf(warnmessage,"Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN RESTMSTR","RLSTMGT");
