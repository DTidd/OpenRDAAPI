/* ARCHIVE's - Make (MTN ARCHWORK) Maintain Master */
#include <cstdio>
#include <mkmtn.hpp>
#include <mkmsc.hpp>
#include <mkdef.hpp>
...
static void MTN_MTN_ARCHWORK()
{
	...
	MTNMSTR=RDAMaintainMasterNEW(module,"MTN ARCHWORK",temp1,1,"ARCHWORK","ARCHWORK BROWSE","ARCHWORK DEFINE LIST","ARCHWORK SEARCH BROWSE","ARCHWORK MAINTAIN SCREEN",0,1,0,0,temp2,temp3,temp5,temp4,temp6,FALSE,FALSE,FALSE,"");
...
/* SECURITY USERS */
		...
		r=addMaintainMasterSupportingADV(MTNMSTR,"SECURITY","USERS",0,0,NULL,"USERS KEY",temp1,TRUE,"LOAD USER","SELECT USER IDENTIFICATION BROWSE","SELECT USER IDENTIFICATION DEFINE LIST","SELECT USER IDENTIFICATION SEARCH BROWSE",1,"ARCHIVE","ARCHWORK");
		if(temp1!=NULL) Rfree(temp1);
		if(r>0)
		{
			S=MTNMSTR->s+(r-1);
			addSupportingRDAconfld(S,"USER IDENTIFICATION","[ARCHWORK][USER IDENTIFICATION]",FALSE);
		}
...
/* ARCHIVE ARCHUSER */
		...
		sprintf(temp1,"%s",
			"");
		r=addMaintainMasterSubordinate(MTNMSTR,"ARCHIVE","ARCHUSER",0,"","","ARCHUSER KEY 1",temp1,FALSE);
		if(temp1!=NULL) Rfree(temp1);
		if(r>0)
		{
			SUBORD=MTNMSTR->subord+(r-1);
			addSubordinateRDAconfld(SUBORD,"WORK GROUP","[ARCHWORK][WORK GROUP]",FALSE);
			addSubordinateRDAconfld(SUBORD,"USER IDENTIFICATION","",FALSE);
		}
...
/* ARCHUSER mtnmst BUTTON */
		...
		addAPPlib(args,"MTN ARCHUSER");
		x=addMaintainMasterButton(MTNMSTR,"mtnmst","ARCHUSER","ARCHUSER","ARCHIVE","ARCHUSER","ARCHUSER KEY 1",temp1,0,0,args,0,NULL);
		if(temp1!=NULL) Rfree(temp1);
...
/* MTN ARCHIVES mtnmst BUTTON */
		...
		x=addMaintainMasterButton(MTNMSTR,"mtnmst","Archives","ARCHIVES","ARCHIVE","ARCHIVES","ARCHIVE KEY 1",temp1,0,0,args,0,NULL);
		if(temp1!=NULL) Rfree(temp1);
		if(args!=NULL) freeapplib(args);
		if(x!=(-1))
		{
...
/* BUTTONS Ending */
		...
		if(writeMaintainMaster(defdir,MTNMSTR))
		{
			...
			sprintf(temp1,"MAINTAIN MASTER WRITE ERROR: Module [ARCHIVE] Maintain Master [MTN ARCHWORK], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
...
static void MNU_ARCHIVE_MTN_ARCHWORK()
{
	RDAmenu *menu=NULL;
	char *defdir=NULL;

	menu=RDAmenuNEW("ARCHIVE MTN ARCHWORK","mtnmst.EXT ARCHIVE \"MTN ARCHWORK\"");
	if(menu!=NULL)
	{
...
static void SCN_ARCHWORK_BROWSE()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","ARCHWORK BROWSE");
	if(scrn!=NULL)
	{
		temp1=Rmalloc(23+1);
		sprintf(temp1,"%s",
		...
		ADVaddwdgt(scrn,3,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"ADD","Add","","",0,0,0,"((GENERIC_SETUP_VARIABLE(\"[ARCHIVE][ARCHIVE STYLE]\"))>0)",NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"SELECT","Select","","",0,0,36,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"ARCHIVES","Archives","","",0,0,30,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"ARCHUSER","Archuser","","",0,0,0,"((GENERIC_SETUP_VARIABLE(\"[ARCHIVE][ARCHIVE STYLE]\"))>0)",NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [ARCHWORK BROWSE], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_ARCHWORK_DEFINE_LIST()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","ARCHWORK DEFINE LIST");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [ARCHWORK DEFINE LIST], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_ARCHWORK_MAINTAIN_SCREEN()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","ARCHWORK MAINTAIN SCREEN");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,9,"[ARCHWORK][DELETEFLAG]","Delete this record","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,5,"","          Work Group/Folder:","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"PREVIOUS ARCHWORK KEY 1","Previous","","",0,0,2,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,0,"[ARCHWORK][WORK GROUP]","Work Group/Folder:","","",0,15,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"NEXT ARCHWORK KEY 1","Next","","",0,0,1,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,5,"","    Name:","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,0,"[ARCHWORK][NAME]","Name:","","",0,40,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,5,"","Creator/User Identification:","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"LOAD USER","Load User","","",0,0,1,NULL,NULL,"([LOGIN IDENTIFICATION]=[ARCHIVE][ARCHWORK][USER IDENTIFICATION])",NULL);
		ADVaddwdgt(scrn,6,"PREVIOUS LOAD USER","Previous Load User","","",0,0,2,NULL,NULL,"([LOGIN IDENTIFICATION]=[ARCHIVE][ARCHWORK][USER IDENTIFICATION])",NULL);
		ADVaddwdgt(scrn,0,"[ARCHWORK][USER IDENTIFICATION]","User Identification:","","",0,15,0,NULL,"([LOGIN IDENTIFICATION]=[ARCHIVE][ARCHWORK][USER IDENTIFICATION])",NULL,NULL);
		ADVaddwdgt(scrn,6,"NEXT LOAD USER","Next Load User","","",0,0,1,NULL,NULL,"([LOGIN IDENTIFICATION]=[ARCHIVE][ARCHWORK][USER IDENTIFICATION])",NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,5,"","                  Directory:","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,0,"[ARCHWORK][DIRECTORY]","Directory:","","",0,60,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,3,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		ADVaddwdgt(scrn,6,"ARCHUSER","Archuser","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"ARCHIVES","Archives","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,4,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,3,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"SAVE","Save","","",0,0,54,"((GENERIC_SETUP_VARIABLE(\"[ARCHIVE][ARCHIVE STYLE]\"))>0)",NULL,"([LOGIN IDENTIFICATION]=[ARCHIVE][ARCHWORK][USER IDENTIFICATION])",NULL);
		ADVaddwdgt(scrn,6,"SAVE NO UPDATE","Fast Save","","",0,0,23,"((GENERIC_SETUP_VARIABLE(\"[ARCHIVE][ARCHIVE STYLE]\"))>0)",NULL,"([LOGIN IDENTIFICATION]=[ARCHIVE][ARCHWORK][USER IDENTIFICATION])",NULL);
		ADVaddwdgt(scrn,6,"QUIT","Cancel","","",0,0,14,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"HELP","Help","","",0,0,11,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"DEFAULTS","Save Defaults","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [ARCHWORK MAINTAIN SCREEN], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_ARCHWORK_SEARCH_BROWSE()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","ARCHWORK SEARCH BROWSE");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,3,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,9,"RANGE ON [ARCHWORK][WORK GROUP]","Range on Work Group/Folder","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		ADVaddwdgt(scrn,6,"QUIT","Cancel","","",0,0,14,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,14,"ARCHWORK SEARCH BROWSE EXPRESSION","Expression","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"HELP","Help","","",0,0,11,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"DEFAULTS","Save Defaults","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,4,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [ARCHWORK SEARCH BROWSE], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_ARCHWORK_SEARCH_BROWSE_EXPRESSION()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","ARCHWORK SEARCH BROWSE EXPRESSION");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,5,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [ARCHWORK SEARCH BROWSE EXPRESSION], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_SELECT_USER_IDENTIFICATION_BROWSE()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","SELECT USER IDENTIFICATION BROWSE");
	if(scrn!=NULL)
	{
		temp1=Rmalloc(23+1);
		sprintf(temp1,"%s",
		...
		if(writescrnbin(defdir,scrn))
		{
			if(temp1!=NULL) Rfree(temp1);
			temp1=Rmalloc(33+7+89+1);
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [SELECT USER IDENTIFICATION BROWSE], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_SELECT_USER_IDENTIFICATION_DEFINE_LIST()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","SELECT USER IDENTIFICATION DEFINE LIST");
	if(scrn!=NULL)
	{
		ADVaddwdgt(scrn,1,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,5,"","Enter a position and length for each of the following:","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		if(writescrnbin(defdir,scrn))
		{
			...
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [SELECT USER IDENTIFICATION DEFINE LIST], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void SCN_SELECT_USER_IDENTIFICATION_SEARCH_BROWSE()
{
	RDAscrn *scrn=NULL;
	char *defdir=NULL;
	char *temp1=NULL,*temp2=NULL;
	...
	scrn=RDAscrnNEW("ARCHIVE","SELECT USER IDENTIFICATION SEARCH BROWSE");
	if(scrn!=NULL)
	{
		...
		ADVaddwdgt(scrn,6,"SELECT","Select","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"QUIT","Cancel","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,14,"SELECT USER IDENTIFICATION SEARCH BROWSE EXPRESSION","Expression","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"HELP","Help","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,6,"DEFAULTS","Save Defaults","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,2,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		ADVaddwdgt(scrn,4,"","","","",0,0,0,NULL,NULL,NULL,NULL);
		...
		{
			if(temp1!=NULL) Rfree(temp1);
			temp1=Rmalloc(40+7+89+1);
			sprintf(temp1,"SCREEN WRITE ERROR: Module [ARCHIVE] Screen [SELECT USER IDENTIFICATION SEARCH BROWSE], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void DEF_ARCHWORK_DEFINE_LIST()
{
	RDAdefault *def=NULL;
	char *defdir=NULL;
	char *temp1=NULL;

	def=RDAdefaultNEW("ARCHIVE","ARCHWORK DEFINE LIST");
	if(def!=NULL)
	{
		...
		addDefaultFieldSHORT(def,"WORK GROUP POSITION",TRUE,7,1);
		addDefaultFieldSHORT(def,"WORK GROUP LENGTH",TRUE,7,15);
		addDefaultFieldSHORT(def,"USER IDENTIFICATION POSITION",TRUE,7,5);
		addDefaultFieldSHORT(def,"USER IDENTIFICATION LENGTH",TRUE,7,15);
		addDefaultFieldSHORT(def,"NAME POSITION",TRUE,7,4);
		addDefaultFieldSHORT(def,"NAME LENGTH",TRUE,7,40);
		addDefaultFieldSHORT(def,"DIRECTORY POSITION",TRUE,7,0);
		addDefaultFieldSHORT(def,"DIRECTORY LENGTH",TRUE,7,40);
		...
		if(writedefaultbin(defdir,def))
		{
			...
			sprintf(temp1,"SCREEN DEFAULT WRITE ERROR: Module [ARCHIVE] Screen [ARCHWORK DEFINE LIST], Can Not Save Screen Defaults!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
static void MTN_MTN_ARCHWORK_SEARCH()
{
	char *defdir=NULL;
	int x=0;
	RDAsearch *S=NULL;
	...
	MTNMSTR=RDAMaintainMasterNEW(module,"MTN ARCHWORK.SEARCH",temp1,1,"ARCHWORK","ARCHWORK BROWSE","ARCHWORK DEFINE LIST","ARCHWORK SEARCH BROWSE","ARCHWORK MAINTAIN SCREEN",0,2,0,0,temp2,temp3,temp5,temp4,temp6,FALSE,FALSE,FALSE,"");
	if(temp1!=NULL) Rfree(temp1);
	if(temp2!=NULL) Rfree(temp2);
	if(temp3!=NULL) Rfree(temp3);
	if(temp4!=NULL) Rfree(temp4);
...
/* BUTTONS Ending */
		...
		if(writeMaintainMaster(defdir,MTNMSTR))
		{
			...
			sprintf(temp1,"MAINTAIN MTNMASTER WRITE ERROR: Module [ARCHIVE] Maintain Master [MTN ARCHWORK.SEARCH], Can Not Save Maintain Master!");
			prterr(temp1);
			if(errorlist!=NULL)
			{
				addERRlist(&errorlist,temp1);
...
{
	MTN_MTN_ARCHWORK();
	MNU_ARCHIVE_MTN_ARCHWORK();
	SCN_ARCHWORK_BROWSE();
	SCN_ARCHWORK_DEFINE_LIST();
	SCN_ARCHWORK_MAINTAIN_SCREEN();
	SCN_ARCHWORK_SEARCH_BROWSE();
	SCN_ARCHWORK_SEARCH_BROWSE_EXPRESSION();
	SCN_SELECT_USER_IDENTIFICATION_BROWSE();
	SCN_SELECT_USER_IDENTIFICATION_DEFINE_LIST();
	SCN_SELECT_USER_IDENTIFICATION_SEARCH_BROWSE();
	DEF_ARCHWORK_DEFINE_LIST();
	MTN_MTN_ARCHWORK_SEARCH();
}
