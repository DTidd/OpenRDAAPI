/* bftmdacm.c - Xpert Benefit Personnel Definition Type Maintain Screen */
#include <cstdio>
...
extern MakeBrowseList *mbl_bftmdac;
extern MaintainMaster *MTNMASTER;
extern void browse_bftmdac(RDArsrc *,mtnstruct *);
...
void bftmdacm(short dowhich,void *targetkey)
{
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_bftmdac,mtn);
	...
	if(mbl_bftmdac!=NULL) FINDRSCSETSENSITIVE(mtnrsrc,"BROWSE",FALSE);
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_bftmdac==NULL?TRUE:FALSE));
}
...
static void save_check(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	FINDRSCGETCHAR(mtnrsrc,"[BFTMDAC][DELETEFLAG]",&delflag);
	if(!delflag)
	{
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(ADVRECORDsec(MTNMASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_bftmdac,MTNMASTER->passkey->fileno,MTNMASTER->module,mtnrsrc);
		if(mbl_bftmdac!=NULL) quit_record(mtnrsrc,mtn);
...
static void getpreviouscb(RDArmem *member,mtnstruct *mtn)
{
	...
	readwidget(member->parent,"[BFTMDAC][PERSONNEL IDENTIFICATION]");
	readwidget(member->parent,"[BFTMDAC][DEDUCTION DESCRIPTION IDENTIFICATION]");
	FINDRSCGETSTRING(member->parent,"[BFTMDAC][PERSONNEL IDENTIFICATION]",&perid);
	FINDRSCGETSTRING(member->parent,"[BFTMDAC][DEDUCTION DESCRIPTION IDENTIFICATION]",&benefitid);
...
static void getrecordcb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[BFTMDAC][PERSONNEL IDENTIFICATION]");
	readwidget(mtnrsrc,"[BFTMDAC][DEDUCTION DESCRIPTION IDENTIFICATION]");
	FINDRSCGETSTRING(mtnrsrc,"[BFTMDAC][PERSONNEL IDENTIFICATION]",&perid);
	FINDRSCGETSTRING(mtnrsrc,"[BFTMDAC][DEDUCTION DESCRIPTION IDENTIFICATION]",&benefitid);
	FINDFLDSETSTRING(MTNMASTER->passkey->fileno,"PERSONNEL IDENTIFICATION",perid);
	FINDFLDSETSTRING(MTNMASTER->passkey->fileno,"DEDUCTION DESCRIPTION IDENTIFICATION",benefitid);
	if(perid!=NULL) Rfree(perid);
	if(benefitid!=NULL) Rfree(benefitid);
