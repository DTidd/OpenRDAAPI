/* cshaidc.c - Create Cash Drawer Receipt Identifications */
/*lint -library */
#ifndef WIN32
#define __NAM__ "cshaidc.lnx"
#endif
#ifdef WIN32
#define __NAM__ "cshaidc.exe"
#endif
#include <app.hpp>
...
void selectcshaidc(RDArsrc *mainrsrc)
{
	...
	for(x=fromnum;x<(tonum+1);++x)
	{
		...
		sprintf(idx,"%s%s%s",(pfix!=NULL ? pfix:""),Stemp,
			(sfix!=NULL ? sfix:""));
		ZERNRD(purids);
		FINDFLDSETSTRING(purids,"RECEIPT IDENTIFICATION",idx);
		LOCNRDFILE(purids);
...
void quitcshaidc(RDArsrc *mainrsrc)
{
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,
		"CREATE RECEIPT IDENTIFICATIONS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	if((purids=APPOPNNRD(module,"CSHID",TRUE,TRUE))==(-1)) return;
	mainrsrc=RDArsrcNEW(module,"CREATE RECEIPT IDENTIFICATIONS");
	...
	addbtnrsrc(mainrsrc,"SELECT",TRUE,selectcshaidc,NULL);
	addrfexrsrc(mainrsrc,"QUIT",TRUE,quitcshaidc,NULL);
	addbtnrsrc(mainrsrc,"HELP",TRUE,screenhelp,NULL);
	addbtnrsrc(mainrsrc,"PRINT RESOURCES",TRUE,printrsrcs,NULL);
	DefaultScreens(mainrsrc);
	APPmakescrn(mainrsrc,FALSE,quitcshaidc,NULL,TRUE);
	RDAAPPMAINLOOP();
}
