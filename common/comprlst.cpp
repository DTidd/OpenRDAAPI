...
#ifndef WIN32
#define __NAM__ "comprlst.lnx"
#endif
#ifdef WIN32
#define __NAM__ "comprlst.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static char *OWNERID=NULL,*REALID=NULL,*SUPID=NULL;
static int TAXYEAR=0;
...
static char use_estimates=FALSE,use_exemption_detail=FALSE,DO_AUDIT=FALSE;
...
static void quitfunc(RDArsrc *r)
{
	...
	{
		APPlib2SupportRequest("RLSTMGT","comprlst -> Compute Real Estate Taxes",errorlist);
	}
	if(errorlist!=NULL) freeapplib(errorlist);
	if(COMPUTE_DATE!=NULL) Rfree(COMPUTE_DATE);
/*
	if(SUPID!=NULL) Rfree(SUPID);
...
*/
	...
	{
		args=APPlibNEW();
		addAPPlib(args,"RLSTMGT");
		addAPPlib(args,"TRNAUDIT");
...
static void comptax(RDArsrc *mainrsrc)
{
	...
	char *milid=NULL,*errorstr=NULL,*tmp=NULL;
			...
			if(!testfilerangersrc(rstmst,rs))
			{
				...
				if(doit==TRUE)
				{
						...
						if(!delflag && !use_units && active && !testfilerangersrc(rstcml,rs) && !testfilerangersrc(rstcls,rs))
						{
							...
							if(use_exemptions==TRUE)
							{
								...
								while(!ef2)
								{
									...
									FINDFLDGETCHAR(rstmexp,"ACTIVE",&active);
									if(!delflag && !active)
									{
										amt=0.0;
										ZERNRD(rstmmexp);
										...
										FINDFLDSETDOUBLE(rstmmexp,"NET VALUE",tax_net_value);
										...
										if(!ADVEQLNRDsec(rstexp,1,ComputeRlstMgtSubData,diagrsrc))
										{
											...
											if(!delflag && active==TRUE && dit==TRUE)
											{
												...
												switch(exempt_type)
												{
													default:
													case 0:
														if(tax_net_value>=amt) 
														{
															tot_val_exmp+=amt; 
														} else {
															tot_val_exmp+=tax_net_value;
					amt=tax_net_value;
														}
														break;
													case 1:
					...
					amt=tax_net_value;
												...
												{
			tax_net_value=(tax_value*100)-tot_val_exmp;
...
/* default account type to typical value for completeness */
						...
						if(tax_value==0)
						{
							FINDFLDSETDOUBLE(rstran,"TAXABLE",act_val);
						} else {
						...
						FINDFLDSETSTRING(rstran,"STATEMENT DATE",COMPUTE_DATE);
						FINDFLDSETSTRING(rstran,"SOURCE USER",USERLOGIN);
						FINDFLDSETINT(rstran,"FISCAL YEAR",fiscalyear);
						if(XPERT_SETUP->software_type<2)
						{
							COPYFIELD(rstmst,rstran,"LAND VALUE");
							COPYFIELD(rstmst,rstran,"BUILDING AND IMPROVEMENT VALUE");
							COPYFIELD(rstmst,rstran,"SUPPLEMENT");
						}
						if(tax_amt!=0.0)
						{
						if(ADVWRTTRANSsec(rstran,1,"TRANSACTION NUMBER",NULL,ComputeRlstMgtSubData,diagrsrc))
						{
						} else {
							DID_STUFF=TRUE;
							DO_AUDIT=TRUE;
							if(XPERT_SETUP->software_type<2)
							{
								if(rwc)
								{
									...
									{
										ZERNRD(finryr);
										FINDFLDSETSHORT(finryr,"DEFINITION TYPE",deftype);
										FINDFLDSETINT(finryr,"FISCAL YEAR",fiscalyear);
										FINDFLDSETSTRING(finryr,"ACCOUNT CODE",acct);
										if(ADVEQLNRDsec(finryr,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finryr,1);
							
										ZERNRD(finracc);
										FINDFLDSETSHORT(finracc,"DEFINITION TYPE",deftype);
										FINDFLDSETSTRING(finracc,"ACCOUNT CODE",acct);
										if(ADVEQLNRDsec(finracc,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finracc,1);
										errorstr=ADVCHECKACCT2(finryr,finracc,acct,REV_ACCT,deftype,fiscalyear,1,SCRNvirtualSubData,diagrsrc);
										if(!isEMPTY(errorstr))
										{ 
											FINDFLDGETSTRING(rstran,"REFERENCE IDENTIFICATION",&milid);
											tmp=Rmalloc(RDAstrlen(errorstr)+RDAstrlen(classid)+RDAstrlen(milid)+280);
											sprintf(tmp,"CANNOT UPDATE REVENUE TO ACCOUNT CODE : Class [%-15s], Millage [%-15s] Reason: %s",(classid==NULL?"":classid),(milid!=NULL ? milid:""),errorstr);
											addAPPlib(errorlist,tmp);
											if(milid!=NULL) Rfree(milid);
											if(tmp!=NULL) Rfree(tmp);
										}
										if(errorstr!=NULL) Rfree(errorstr);
									}
									ZERNRD(finbyr);
									FINDFLDSETSHORT(finbyr,"DEFINITION TYPE",deftype);
									FINDFLDSETINT(finbyr,"FISCAL YEAR",fiscalyear);
									FINDFLDSETSTRING(finbyr,"ACCOUNT CODE",reccode);
									if(ADVEQLNRDsec(finbyr,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finbyr,1);
							
									ZERNRD(finbacc);
									FINDFLDSETSHORT(finbacc,"DEFINITION TYPE",deftype);
									FINDFLDSETSTRING(finbacc,"ACCOUNT CODE",reccode);
									if(ADVEQLNRDsec(finbacc,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finbacc,1);
									if(!isEMPTY(reccode))
									{
										errorstr=ADVCHECKACCT2(finbyr,finbacc,reccode,BAL_ACCT,deftype,fiscalyear,1,SCRNvirtualSubData,diagrsrc);
										if(!isEMPTY(errorstr))
										{ 
											FINDFLDGETSTRING(rstran,"REFERENCE IDENTIFICATION",&milid);
											tmp=Rmalloc(RDAstrlen(errorstr)+RDAstrlen(classid)+RDAstrlen(milid)+280);
											sprintf(tmp,"CANNOT UPDATE BALANCE SHEET ACCOUNT CODE : Class [%-15s], Millage [%-15s] Reason: %s",(classid==NULL?"":classid),(milid!=NULL ? milid:""),errorstr);
											addAPPlib(errorlist,tmp);
											if(milid!=NULL) Rfree(milid);
											if(tmp!=NULL) Rfree(tmp);
										}
									...
									FINDFLDSETSHORT(finbyr,"DEFINITION TYPE",deftype);
									FINDFLDSETINT(finbyr,"FISCAL YEAR",fiscalyear);
									FINDFLDSETSTRING(finbyr,"ACCOUNT CODE",revcode);
									if(ADVEQLNRDsec(finbyr,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finbyr,1);
							
									ZERNRD(finbacc);
									FINDFLDSETSHORT(finbacc,"DEFINITION TYPE",deftype);
									FINDFLDSETSTRING(finbacc,"ACCOUNT CODE",revcode);
									if(ADVEQLNRDsec(finbacc,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finbacc,1);
									if(!isEMPTY(revcode))
									{
										errorstr=ADVCHECKACCT2(finbyr,finbacc,revcode,BAL_ACCT,deftype,fiscalyear,1,SCRNvirtualSubData,diagrsrc);
										if(!isEMPTY(errorstr))
										{ 
											FINDFLDGETSTRING(rstran,"REFERENCE IDENTIFICATION",&milid);
											tmp=Rmalloc(RDAstrlen(errorstr)+RDAstrlen(classid)+RDAstrlen(milid)+280);
									...
									if(acctype==1)
									{
										ADVupdateundist(rstran,"ACCOUNT CODE","CASH CODE","REVENUE CODE",NULL,finryr,finbyr,fiscalyear,deftype,"REVENUE",FISCALPERIOD[fiscal_month],tax_amt,SCRNvirtualSubData,diagrsrc);
									} else {
										ADVupdateundist(rstran,NULL,"CASH CODE","REVENUE CODE",NULL,(-1),finbyr,fiscalyear,deftype,"REVENUE",FISCALPERIOD[fiscal_month],tax_amt,SCRNvirtualSubData,diagrsrc);
									}
								...
								if(defrev)
								{
									...
									FINDFLDSETSHORT(finbyr,"DEFINITION TYPE",deftype);
									FINDFLDSETINT(finbyr,"FISCAL YEAR",fiscalyear);
									FINDFLDSETSTRING(finbyr,"ACCOUNT CODE",defcode);
									if(ADVEQLNRDsec(finbyr,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finbyr,1);
							
									ZERNRD(finbacc);
									FINDFLDSETSHORT(finbacc,"DEFINITION TYPE",deftype);
									FINDFLDSETSTRING(finbacc,"ACCOUNT CODE",defcode);
									if(ADVEQLNRDsec(finbacc,1,SCRNvirtualSubData,diagrsrc)) KEYNRD(finbacc,1);
									if(!isEMPTY(defcode))
									{
										errorstr=ADVCHECKACCT2(finbyr,finbacc,defcode,BAL_ACCT,deftype,fiscalyear,1,SCRNvirtualSubData,diagrsrc);
										if(!isEMPTY(errorstr))
										{ 
											FINDFLDGETSTRING(rstran,"REFERENCE IDENTIFICATION",&milid);
											tmp=Rmalloc(RDAstrlen(errorstr)+RDAstrlen(classid)+RDAstrlen(milid)+280);
									...
									ADVupdateundist(rstran,NULL,"RECEIVABLE CODE","DEFERRED REVENUE",NULL,(-1),finbyr,fiscalyear,deftype,"REVENUE",FISCALPERIOD[fiscal_month],tax_amt,SCRNvirtualSubData,diagrsrc);
								}
							}
							}
							ZERNRD(rstmil);
							...
							UNLNRDFILE(rstmil);
							ZERNRD(rstmst);
							FINDFLDSETSTRING(rstmst,"REAL ESTATE IDENTIFICATION",realid);
							FINDFLDSETSTRING(rstmst,"OWNER IDENTIFICATION",ownid);
							FINDFLDSETINT(rstmst,"TAX YEAR",tax_year);
							LOCNRDFILE(rstmst);
							if(ADVEQLNRDsec(rstmst,1,ComputeRlstMgtSubData,diagrsrc))
							{
							} else {
								prev=RDATDataNEW(rstmst);
								if(transtype==0)
								{
									FINDFLDGETDOUBLE(rstmst,"TAX AMOUNT",&amt);
									amt+=tax_amt;
									FINDFLDSETDOUBLE(rstmst,"TAX AMOUNT",amt);
									FINDFLDGETDOUBLE(rstmst,"LAND VALUE",&lv);
									FINDFLDGETDOUBLE(rstmst,"BUILDING AND IMPROVEMENT VALUE",&bf);
									if((use_estimates==TRUE) && (lv==0.0) && (bf==0.0))
									{
										FINDFLDGETDOUBLE(rstmst,"ESTIMATED LAND VALUE",&lv);
										FINDFLDGETDOUBLE(rstmst,"ESTIMATED BUILDING AND IMPROVEMENT VALUE",&bf);
									} else 
									{
									    /* Do nothing  */
									} 
									...
									{
										 amt=0.0;
									}  else amt=(lv+bf)-tax_value;
				
									FINDFLDSETDOUBLE(rstmst,"TAX EXEMPTIONS",amt);
								} else {
									FINDFLDGETDOUBLE(rstmst,"OTHER AMOUNT",&amt);
									amt+=tax_amt;
									FINDFLDSETDOUBLE(rstmst,"OTHER AMOUNT",amt);
								}
								ADVWRTTRANSsec(rstmst,0,NULL,prev,ComputeRlstMgtSubData,diagrsrc);	
								if(prev!=NULL) FreeRDATData(prev);	
							}	
							UNLNRDFILE(rstmst);
...
#endif
{
	...
	OWNERID=RDA_GetEnv("OWNER_IDENTIFICATION");
	temp1=RDA_GetEnv("TAX_YEAR");
	if(temp1!=NULL) TAXYEAR=atoi(temp1);
		else TAXYEAR=(-1);
	SUPID=RDA_GetEnv("SUPPLEMENT");
	REALID=RDA_GetEnv("REAL_ESTATE_IDENTIFICATION");
...
/*lint +e611 */
	...
	gsv=RDAGenericSetupNew("RLSTMGT","CURRENT FISCAL YEAR");
	if(gsv!=NULL)
	{
		if(!getRDAGenericSetupbin(libx,gsv))
		{
	...
	if(gsv!=NULL) FreeRDAGenericSetup(gsv);
	gsv=RDAGenericSetupNew("RLSTMGT","CURRENT FISCAL MONTH");
	if(gsv!=NULL)
	{
		if(!getRDAGenericSetupbin(libx,gsv))
		{
			fiscal_month=*gsv->value.short_value;
		}
	}
	if(gsv!=NULL) FreeRDAGenericSetup(gsv);
	if(libx!=NULL) Rfree(libx);
	FINDRSCSETINT(mainrsrc,"FISCAL YEAR",fiscalyear);
	FINDRSCSETSHORT(mainrsrc,"FISCAL MONTH",fiscal_month);
	x=fiscal_month;
	FINDRSCSETINT(mainrsrc,"APPLYTAXES MONTHS",x);
	if(TAXYEAR!=(-1))
	{
		FINDRSCSETINT(mainrsrc,"TAX YEAR",TAXYEAR);
	}
	if(!isEMPTY(OWNERID))
	{
		FINDRSCSETCHAR(mainrsrc,"RANGE ON [RESTMSTR][OWNER IDENTIFICATION]",TRUE);
		FINDRSCSETSTRING(mainrsrc,"FROM [RESTMSTR][OWNER IDENTIFICATION]",OWNERID);
		FINDRSCSETSTRING(mainrsrc,"TO [RESTMSTR][OWNER IDENTIFICATION]",OWNERID);
	}
	if(!isEMPTY(SUPID))
	{
		FINDRSCSETCHAR(mainrsrc,"RANGE ON [RESTMSTR][SUPPLEMENT]",TRUE);
		FINDRSCSETSTRING(mainrsrc,"FROM [RESTMSTR][SUPPLEMENT]",SUPID);
		FINDRSCSETSTRING(mainrsrc,"TO [RESTMSTR][SUPPLEMENT]",SUPID);
	}
	if(!isEMPTY(REALID))
	{
		FINDRSCSETCHAR(mainrsrc,"RANGE ON [RESTMSTR][REAL ESTATE IDENTIFICATION]",TRUE);
		FINDRSCSETSTRING(mainrsrc,"FROM [RESTMSTR][REAL ESTATE IDENTIFICATION]",REALID);
		FINDRSCSETSTRING(mainrsrc,"TO [RESTMSTR][REAL ESTATE IDENTIFICATION]",REALID);
	}
	computeallSCRNvirtuals(mainrsrc);
	SetRangersrcsensitive(mainrsrc);
	if(!isEMPTY(OWNERID))
	{
		FINDRSCSETEDITABLE(mainrsrc,"RANGE ON [RESTMSTR][OWNER IDENTIFICATION]",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"FROM [RESTMSTR][OWNER IDENTIFICATION]",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"TO [RESTMSTR][OWNER IDENTIFICATION]",FALSE);
	}
	if(!isEMPTY(SUPID))
	{
		FINDRSCSETEDITABLE(mainrsrc,"RANGE ON [RESTMSTR][SUPPLEMENT]",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"FROM [RESTMSTR][SUPPLEMENT]",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"TO [RESTMSTR][SUPPLEMENT]",FALSE);
	}
	if(!isEMPTY(REALID))
	{
		FINDRSCSETEDITABLE(mainrsrc,"RANGE ON [RESTMSTR][REAL ESTATE IDENTIFICATION]",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"FROM [RESTMSTR][REAL ESTATE IDENTIFICATION]",FALSE);
		FINDRSCSETEDITABLE(mainrsrc,"TO [RESTMSTR][REAL ESTATE IDENTIFICATION]",FALSE);
	}
	...
	if(gsv!=NULL) FreeRDAGenericSetup(gsv);
	gsv=RDAGenericSetupNew("RLSTMGT","USE ESTIMATES");
	if(gsv!=NULL)
	{
		if(!getRDAGenericSetupbin(libx,gsv))
		{
			use_estimates=(*gsv->value.string_value==FALSE?FALSE:TRUE);
		} else {
			use_estimates=FALSE;
		}
	}
