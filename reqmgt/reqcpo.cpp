...
#ifndef WIN32
#define __NAM__ "reqcpo.lnx"
#endif
#ifdef WIN32
#define __NAM__ "reqcpo.exe"
#endif
#include <app.hpp>
...
static char approve_po=FALSE;
static int approve_type=0;
...
void select_backend(DBsort *dbsort)
{
	...
	char delflag=FALSE,*locid=NULL,*reqid=NULL,*item_unit=NULL;
	int itemno=0;
	...
	if(dbsort!=NULL)
	{
		...
		while(!ef)
		{
			...
			if(!deleteflag)
			{
				...
				if(ADVEQLNRDsec(reqitm,1,ReqCPOSubData,p)) KEYNRD(reqitm,1);
				ZERNRD(reqmst);
				...
				FINDFLDGETDOUBLE(dbsort->fileno,"TOTAL",&itemamt);
				FINDFLDGETDOUBLE(reqitm,"PERCENTAGE DISCOUNT",&percent);
				FINDFLDGETDOUBLE(dbsort->fileno,"UNIT RATE",&itemrate);
				FINDFLDGETSTRING(reqitm,"UNIT",&item_unit);

				FINDFLDSETINT(reqitm,"QUANTITY",qty);
				FINDFLDSETINT(purlin,"PERCENTAGE DISCOUNT",percent);
				FINDFLDSETDOUBLE(purlin,"OUTSTANDING LINE AMOUNT",itemamt);
				FINDFLDSETSTRING(purlin,"UNIT",item_unit);
				if(percent == 0.0) 
				{
				...
				*/
				FINDFLDSETDOUBLE(purlin,"COST",cost);
				FINDFLDSETDOUBLE(purlin,"OUTSTANDING LINE COST",cost);
				FINDFLDSETDOUBLE(purlin,"TOTAL",cost);
				...
				if(ADVEQLNRDsec(purmst,1,ReqCPOSubData,p))
				{
					...
					*/
					FINDFLDSETDOUBLE(purmst,"OUTSTANDING COST",cost);
					FINDFLDSETDOUBLE(purmst,"TOTAL LINE COST",cost);
					COPYFIELD(reqmst,purmst,"REMARKS 1");
					COPYFIELD(reqmst,purmst,"REMARKS 2");
					COPYFIELD(reqmst,purmst,"REMARKS 3");
					COPYFIELD(reqmst,purmst,"REMARKS 4");
					COPYFIELD(reqmst,purmst,"REMARKS 5");
					COPYFIELD(reqmst,purmst,"REMARKS 6");
					COPYFIELD(reqmst,purmst,"REMARKS 7");
					COPYFIELD(reqmst,purmst,"REMARKS 8");
					COPYFIELD(reqmst,purmst,"COMMENTS");
				} else {
					prev=RDATDataNEW(purmst);
					FINDFLDGETDOUBLE(purmst,"OUTSTANDING AMOUNT",&outamt);
					outamt+=total;
					outcost+=cost;
					FINDFLDSETDOUBLE(purmst,"OUTSTANDING AMOUNT",outamt);
					FINDFLDSETDOUBLE(purmst,"TOTAL LINE AMOUNT",outamt);
					FINDFLDSETDOUBLE(purmst,"OUTSTANDING COST",outcost);
					FINDFLDSETDOUBLE(purmst,"TOTAL LINE COST",outcost);
					COPYFIELD(reqmst,purmst,"REMARKS 1");
					COPYFIELD(reqmst,purmst,"REMARKS 2");
					COPYFIELD(reqmst,purmst,"REMARKS 3");
					COPYFIELD(reqmst,purmst,"REMARKS 4");
...
*/
							...
							*/
							FINDFLDSETDOUBLE(puracc,"COST",outcost);
							FINDFLDSETDOUBLE(puracc,"OUTSTANDING ACCOUNT COST",outcost);
...
/* Set Approval Right Here */
	if(approve_po == TRUE) 
	{ 
	FINDFLDSETINT(purmst,"APPROVE TYPE",(approve_type+1));
	}
...
#endif
{
	...
	if(gsv!=NULL)
	{
		...
		{
			approve_po=*gsv->value.string_value;
		} else {
			approve_po=FALSE;
		}
	}
	...
	if(gsv!=NULL)
	{
		if(!getRDAGenericSetupbin(libx,gsv))
		{
			approve_type=*gsv->value.string_value;
		} else {
			approve_type=0;
		}
	}
