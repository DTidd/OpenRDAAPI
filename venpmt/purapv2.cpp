...
static short check_code(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	char *paycode1=NULL,*dueto1=NULL,*clearfrom1=NULL,*clearcash1=NULL;
...
*/
	...
	double amount=0;
	char readdel=FALSE,trueflag=TRUE;
	...
	FINDFLDSETCHAR(dbsort->fileno,"PO VOUCHER",trueflag);
...
/*
	...
	if(accttype==BAL_ACCT)
	{
		FINDRSCGETSTRING(mtnrsrc,"DEBIT CODE",&temp);
		if(temp!=NULL)
		{
			FINDRSCSETSTRING(mtnrsrc,"DEBIT CODE",NULL);
			FINDFLDSETSTRING(dbsort->fileno,"DEBIT CODE",NULL);
				...
				if(readdel==FALSE)
				{
					...
					switch(accttype)
					{
						...
						case BAL_ACCT:
							...
							FINDFLDSETINT(byrnum,"FISCAL YEAR",procyrid);
							FINDFLDSETSHORT(byrnum,"DEFINITION TYPE",deftype);
							FINDFLDSETSTRING(byrnum,"ACCOUNT CODE",acctcode1);
							if(jentype!=NULL) { Rfree(jentype); jentype=NULL; }
							if(amount<0.0)
							{
								jentype=stralloc("CREDITS");
