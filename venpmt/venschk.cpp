...
#ifndef WIN32
#define __NAM__ "venschk.lnx"
#endif
#ifdef WIN32
#define __NAM__ "venschk.exe"
#endif
#include <app.hpp>
...
int start_check=1,change_check_date=0;
...
void quitfunc(RDArsrc *mainrsrc)
{
	...
	if(DID_STUFF)
	{
		...
		args=APPlibNEW();
		addAPPlib(args,"VENPMT");
		addAPPlib(args,"CHK CHECK NUM");
		Execute2Program("doreport",args);
		if(args!=NULL) freeapplib(args);
		args=APPlibNEW();
		addAPPlib(args,"VENPMT");
		addAPPlib(args,"VENCKREGISTER");
		Execute2Program("doreport",args);
		if(args!=NULL) freeapplib(args);
	}
	ShutdownSubsystems();
...
{
	char deleteflag=FALSE,sep=FALSE;
	NRDkey *key=NULL;
	NRDpart *part;
	int x;
...
void setcheck(RDArsrc *mainrsrc)
{
	...
	char verified=FALSE,electronic=FALSE,/**libx=NULL,*/bnkupd=FALSE;
	char default_check_detail=FALSE,sep=FALSE,printed=FALSE,distributed=FALSE;
	...
	while(!ef)
	{
		...
		FINDFLDGETCHAR(vchnum,"PRINTED",&printed);
		FINDFLDGETCHAR(vchnum,"VOID CHECK",&void_check);
		FINDFLDGETSHORT(vchnum,"APPROVED",&approved);
		if(verified && !cleared && !manual_voucher && !electronic && !deleteflag && !printed && !bnkupd && !void_check && ((use_approval==FALSE) || (approved==1)) && ((distributed==FALSE || use_affect_date==FALSE || affect_date_style==0)))
		{
		if(banknum!=(-1))
		{
	...
	if(SIZNRD(sortfile->fileno))
	{
		...
		if(sortfile2!=(-1))
		{
			...
			if(key!=NULL)
			{
				...
				/* Ignore body count if [VENMSR][DEFAULT CHECK DETAIL] is false */
				...
				while(!ef)
				{
						...
						RDAstrcmp(last_addrid,addrid) || RDAstrcmp(last_breakid,breakid))
					{
						...
						/* Ignore body count if [VENMSR][DEFAULT CHECK DETAIL] is false... */
						/* ...but, -don't- ignore body count if [VENINV][SEPARATE CHECK] is true! */
...
#endif
{
	...
	addstdrsrc(mainrsrc,"CHANGE CHECK DATE",BOOLNS,1,&change_check_date,TRUE);
	/*
	if(BANKids!=NULL) { freeapplib(BANKids); BANKids=NULL; }
	BANKids=APPlibNEW();
	if(banknum==(-1))
