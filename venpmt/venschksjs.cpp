...
#ifndef WIN32
#define __NAM__ "venschksjs.lnx"
#endif
#ifdef WIN32
#define __NAM__ "venschksjs.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <mix.h>
...
int voucher_count(RDArsrc *mainrsrc,short fileno,char *vendorid,char *addid,double *check_amount)
{
	int count=0;
	short ef;
	...
	while(!ef)
	{
		...
		if(!RDAstrcmp(venid,vendorid) && !RDAstrcmp(addid,addrid))
		{
			++count;
			FINDFLDGETDOUBLE(fileno,"AMOUNT",&amount);
			*check_amount+=amount; 
		} else {
			break;
		}
...
void setcheck(RDArsrc *mainrsrc)
{
	...
	while(!ef)
	{
		...
		if(verified && !cleared && !manual_voucher && !deleteflag)
		{
		if(banknum!=(-1))
		{
	...
	if(SIZNRD(sortfile->fileno))
	{
		...
		if(sortfile2!=(-1))
		{
			...
			if(key!=NULL)
			{
				...
				ven_count=voucher_count(mainrsrc,sortfile2,vendorid,addrid,&check_amount);
				...
				while(!ef)
				{
					...
					if(!ADVEQLNRDsec(vchnum,1,SCRNvirtualSubData,mainrsrc))
					{
						...
						FINDFLDSETINT(vchnum,"CHECK NUMBER",
							check_number);
						FINDFLDSETINT(vchnum,"VIRTUAL CHECK NUMBER",
							virt_chk_no);
