...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	FINDFLDGETCHAR(MASTER->passkey->fileno,"MANUAL VOUCHER",&manual);
	if(accttype==BAL_ACCT)
	{
		FINDRSCGETSTRING(mtnrsrc,"[VENVCH][ACCOUNT CODE]",&debcode);
		if(!isEMPTY(debcode))
		{
...
*/
					...
					switch(accttype)
					{
						...
						case BAL_ACCT:
							if(venpmt->use_fiscal_months==TRUE)
							{
								ADVupdateundist(MASTER->passkey->fileno,NULL,(dist==FALSE?"DEBIT CODE":NULL),(cleared==FALSE?"CREDIT CODE":NULL),"PAYABLE CODE",(-1),finbyr,procyrid,deftype,"CREDITS",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							} else {
								ADVupdateundist(MASTER->passkey->fileno,NULL,(dist==FALSE?"DEBIT CODE":NULL),(cleared==FALSE?"CREDIT CODE":NULL),"PAYABLE CODE",(-1),finbyr,procyrid,deftype,"CREDITS",CALENDAR_MONTHS[p_month],-amount,SCRNvirtualSubData,mtnrsrc);
							}
							break;
						default:
					...
					if(venpmt->use_venpmt_fund && !cleared && !manual)
					{
					if(venpmt->use_fiscal_months==TRUE)
					{
...
*/
				...
				switch(accttype)
				{
					...
					case BAL_ACCT:
						if(venpmt->use_fiscal_months==TRUE)
						{
							ADVupdateundist(MASTER->passkey->fileno,NULL,(dist==FALSE?"DEBIT CODE":NULL),(cleared==FALSE?"CREDIT CODE":NULL),"PAYABLE CODE",(-1),finbyr,procyrid,deftype,"DEBITS",FISCALPERIOD[p_month],amount,SCRNvirtualSubData,mtnrsrc);
						} else {
							ADVupdateundist(MASTER->passkey->fileno,NULL,(dist==FALSE?"DEBIT CODE":NULL),(cleared==FALSE?"CREDIT CODE":NULL),"PAYABLE CODE",(-1),finbyr,procyrid,deftype,"DEBITS",CALENDAR_MONTHS[p_month],amount,SCRNvirtualSubData,mtnrsrc);
						}
						break;
					default:
						prterr("Invalid Account Type [%d]",accttype);
...
static void set_appdefaults(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	if(accttype==BAL_ACCT)
	{
		FINDRSCSETEDITABLE(mtnrsrc,"[VENVCH][ACCOUNT CODE]",FALSE);
		FINDRSCSETSENSITIVE(mtnrsrc,"[VENVCH][ACCOUNT CODE]",FALSE);
	}
	if(update)
	{
...
static short check_code(RDArsrc *mtnrsrc,mtnstruct *mtn,short acctwidget)
{
	...
	char *acctcode=NULL,manual=FALSE,*tmp=NULL;
	...
	FINDRSCGETCHAR(mtnrsrc,"[VENVCH][MANUAL VOUCHER]",&manual);
	...
	switch(acctwidget)
	{
		...
		case 5: /* clearing due to */
			if(venpmt->use_venpmt_fund && accttype<2 && manual==FALSE)
			{
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn,short update)
{
	...
	if(accttype1!=accttype2 || deftype1!=deftype2)
	{
		...
		switch(selecteda)
		{
			case EXP_ACCT:
			case REV_ACCT:
				FINDRSCSETSENSITIVE(mtnrsrc,"[VENVCH][ACCOUNT CODE]",TRUE);
				FINDRSCSETEDITABLE(mtnrsrc,"[VENVCH][ACCOUNT CODE]",TRUE);
				break;
			case BAL_ACCT:
				FINDRSCSETSENSITIVE(mtnrsrc,"[VENVCH][ACCOUNT CODE]",FALSE);
