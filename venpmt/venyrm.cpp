/* venyrm.c - Vendor Payments Year Maintain */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
void venyrm(short,void *targetkey);
extern MakeBrowseList *mbl_venyr;
extern char editable;
extern void browse_venyr(RDArsrc *,RDATData *);
extern MaintainMaster *MASTER;
extern RDAvenpmt *venpmt;

void venyrm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	if(fields!=NULL)
	{
		...
		{
			if(fldx->type==LONGV) edit_rsrc=(short)(editable &&
				(strstr(fldx->name," NUMBER INVOICES")==NULL));
			else if(fldx->type==DOLLARS) edit_rsrc=(short)(editable &&
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_venyr,previous);
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update)
{
	...
	short veninv=(-1),venvch=(-1),fineyr=(-1),finryr=(-1),finbyr=(-1);
	short ef_vch=0,ef_inv=0;
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		if(deleteflag)
		{
			...
			veninv=OPNNRDsec("VENPMT","VENINV",TRUE,TRUE,FALSE);
			...
			if(veninv!=(-1))
			{
				...
				while(!ef_inv)
				{
					FINDFLDGETCHAR(veninv,"DELETEFLAG",&mdeleteflag);
					if(!mdeleteflag)
					{
						FINDFLDGETSTRING(veninv,"VENDOR IDENTIFICATION",&vendorid2);
							...
							&& yearid2==yearid1) 
						{
							...
							if(venvch!=(-1) && fineyr!=(-1) && finryr!=(-1) && finbyr!=(-1))
							{
								...
								while(!ef_vch)
								{
									FINDFLDGETCHAR(venvch,"DELETEFLAG",&pdeleteflag);
									if(!pdeleteflag)
									{
										FINDFLDGETSTRING(venvch,"VENDOR IDENTIFICATION",&vendorid3);
...
*/
		...
		updatebrowse(update,mbl_venyr,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
		if(mbl_venyr!=NULL)
		{
