/* dirdepmicr.c - Print Payroll Direct Deposit Slips */

#ifndef WIN32
#define __NAM__ "dirdepmicr.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdepmicr.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static short DD_NET_TYPE=0,check_signature=0,check_type=0;
...
static void printactualcheck(RDA_PFILE *fp)
{
	...
	RDA_fprintf(fp,"%-40sDir.Dep.#: %12d Date: %-10s\r\n",
	...
	RDA_fprintf(fp,"%-48s Total Gross: %8.02f %9s\r\n","",total_cur_gross/100,"");
	}
	RDA_fprintf(fp,"\r\n");
	...
	for(x=0;x<number_deductions;x+=3)
	{
		for(y=0;y<3;++y)
		{
			...
			if(!isEMPTY(temp) || cur!=0.0 || ytd!=0.0)
			{
				...
				RDA_fprintf(fp,"%-9s %7.02f %8s",(temp!=NULL
					 ? temp:""),cur/100,"");
				}
	...
	if(print_employer_deductions==FALSE)
	{
		...
		RDA_fprintf(fp,"%-27sTotal Employee Paid Deductions: %8.02f %9s\r\n","",
		...
		RDA_fprintf(fp,"%-22sEmployer: %8.02f %9s Employee: %8.02f %9s\r\n","",total_ecur/100,(total_cur-total_dirdep)/100,"");
		}
	}
	RDA_fprintf(fp,"\r\n");
...
if((XPERT_SETUP->LVEMGT==TRUE || XPERT_SETUP->LVSIMP==TRUE) && skip_leave==FALSE)
{
	...
	RDA_fprintf(fp,"Current Gross:%10.02f - Deductions:%10.02f = Net Direct Deposit:%10.02f\r\n",total_cur_gross/100,(total_cur-net)/100,net/100);
	RDA_fprintf(fp,"\r\n");
...
static void GatherHistory()
{
	...
	while(!ef)
	{
		...
		if(!deleteflag)
		{
			...
			if(diagapps)
			{
				fprintf(RDA_STDERR,"Personnel ID: %s Job Number: %d \r\n",empid,jobno);
			}
			ZERNRD(payjcsm);
...
#endif
{
	...
	mainrsrc=RDArsrcNEW(module,"PRINT DIRECT DEPOSIT MICR 8.5x11");
	...
	switch(startup_type)
	{
		...
		case  2:
			...
			break;
		case  3:
			/* MICR STUB DIRECT DEPOSIT */
			mainrsrc->scn->name=stralloc("PRINT PAYROLL DECORATED STUB MICR DIRECT DEPOSIT");
