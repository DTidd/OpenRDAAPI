/* netzero.c - Print Zero Net Pay Slips */
#ifndef WIN32
#define __NAM__ "netzero.lnx"
#endif
#ifdef WIN32
#define __NAM__ "netzero.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static short ADDRESS_OFFSET=0,netzero_type=0;
...
static void printactualcheck(RDA_PFILE *fp)
{
	...
	char *temp=NULL,*temp1=NULL,*temp2=NULL,*temp3=NULL;
	char *temp4=NULL;
	...
	RDA_fprintf(fp,"%-40sLocation:%-30s \r\n",
	...
	RDA_fprintf(fp,"%-40s           %12s Date: %-10s\r\n",
	...
	for(x=0;x<number_jobs;++x)
	{
		...
		total_ytd+=ytd;
		if(((cur_gross!=0.0 || ytd!=0.0)) || (!isEMPTY(temp) && show_zero_jobs))
		{
	...
	for(x=0;x<number_deductions;x+=3)
	{
		for(y=0;y<3;++y)
		{
			...
			if(!isEMPTY(temp) || cur!=0.0 || ytd!=0.0)
			{
				...
				RDA_fprintf(fp,"%-9s %7.02f %8.02f",(temp!=NULL
	...
	FINDFLDGETDOUBLE(sortfile->fileno,"CURRENT DIRECT DEPOSIT",&total_netzero);
	FINDFLDGETDOUBLE(sortfile->fileno,"YTD DIRECT DEPOSIT",&total_netzero_ytd);
	if(print_employer_deductions==FALSE)
	{
	RDA_fprintf(fp,"%-27sTotal Employee Paid Deductions: %8.02f %9.02f\r\n","",
		(total_cur-total_netzero)/100,(total_ytd-total_netzero_ytd)/100);
	} else {
	RDA_fprintf(fp,"%-22sEmployer: %8.02f %9.02f Employee: %8.02f %9.02f\r\n","",total_ecur/100,total_eytd/100,(total_cur-total_netzero)/100,(total_ytd-total_netzero_ytd)/100);
	}
	RDA_fprintf(fp,"\r\n");

	RDA_fprintf(fp,"Leave ID         Beg Earn  Used  Ending  Leave ID         Beg Earn  Used  Ending\r\n");
	...
	RDA_fprintf(fp,"Current Gross:%10.02f - Deductions:%10.02f = Net :%10.02f\r\n",total_cur_gross/100,(total_cur-net)/100,net/100);
	RDA_fprintf(fp,"\r\n");
	...
	if(netzero_type==5) fill_lines=15;
	for(f=0;f<(fill_lines-(new_lines-default_lines));++f)
	{
		RDA_fprintf(fp,"\r\n");
	}
...
static CheckRate *GatherCurrentCheckRate(char *empid,char *payid,int jobno)
{
	...
	while(!ef)
	{
		...
		if(!delflag)
		{
			...
			if(!RDAstrcmp(rate,"SALARY"))		
			{
				C->salary+=gross;
			 } else if(!RDAstrcmp(rate,"HOURLY") || !RDAstrcmp(rate,"HOURLY UNIT") || !RDAstrcmp(rate,"DAILY UNIT") || !RDAstrcmp(rate,"MISC SALARIES") || !RDAstrcmp(rate,"SUPPLEMENT") || !RDAstrcmp(rate,"MISC UNITS") || !RDAstrcmp(rate, "UNIT") || !RDAstrcmp(rate, "MISC UNIT") ||  !RDAstrcmp(rate,"DAILY") || !strncmp(rate,"MEETING",7)) /* Unit Rates */
			{
				C->units+=units;
				C->unit_amt+=gross;
...
static short AtLeastOneJob(char *empid,RangeScreen *rs,RDArsrc *mainrsrc)
{
	...
	short doit=FALSE;
	double camt=0.0,net=0.0,cnet=0.0,gross=0.0;
	RDAvirtual *v=NULL;
	CheckRate *C=NULL;
	...
	while(!ef)
	{
			 ...
			 && !RDAstrcmp(bankno,bankno1))
		{
			...
			if(doit==TRUE)
			{
				COPYFIELD(paymst,sortfile->fileno,"PAY DATE");
...
static void CreateSortFile(RangeScreen *rs,RDArsrc *mainrsrc)
{
	...
	diagrsrc=diagscrn(payppms,"DIAGNOSTIC SCREEN",module,
		"Selecting Net Zero Advice",NULL);
	addDFincvir(diagrsrc,"PRSNNL","PERDMG",NULL,perdmg);
	addDFincvir(diagrsrc,module,"PAYJMST",NULL,payjmst);
	addDFincvir(diagrsrc,"POSTRK","POSMSTR",NULL,posmst);
	...
	while(!ef)
	{
		...
		/* uncompute virtual fields */
		CLEAR_SCREEN_VIRTUAL(mainrsrc);
		FINDFLDGETDOUBLE(payppms,"NET",&net);
		if(!deleteflag && computed && net==0.0 && !testfilerangersrc(perdmg,rs) 
...
static void selectchecks(RDArsrc *r)
{
	...
	addDBfield(sortfile,"PAY DATE",DATES,10);
...
/* Key Info */
	addDBkey(sortfile,"PRINT NET ZERO KEY");
	...
	{	
		...
		rsrc=RDArsrcNEW("PAYROLL","PRINT NETZERO OUTPUT DEVICE SCREEN");
		...
		WARNINGDIALOG("WARNING DIALOG SCREEN","SELECT NETZERO ERROR","No Net Zero Advice were selected using the given criteria!",NULL,NULL,TRUE,NULL);
		prterr("Warning:  No Net Zero Advice were selected using the given criteria.");
		if(sortfile!=NULL)
		{
			CLSDBsort(sortfile);
			FreeDBsort(sortfile);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"PRINT NETZERO 8.5x11")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	if(gsv!=NULL)
	{
		if(!getRDAGenericSetupbin(libx,gsv))
		{
			netzero_type=*gsv->value.short_value;
		} else {
			netzero_type=FALSE;
		}
	}
	...
	mainrsrc=RDArsrcNEW(module,"PRINT NETZERO 8.5x11");
