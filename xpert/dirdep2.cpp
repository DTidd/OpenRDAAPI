...
#ifndef WIN32
#define __NAM__ "dirdep2.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdep2.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static void printactualcheck(RDA_PFILE *fp,char *outdevice)
{
	...
	SelectPrintType(outdevice,1,fp);
	...
	if(display_ssn)
	{
		RDA_fprintf(fp,"     SSN: %-11s Name: %-25s   DD#: %12d Period: %-10s %-10s Date: %-10s\r\n",
			(ssn!=NULL ? ssn:""),
		...
		RDA_fprintf(fp,"          %-11s Name: %-25s   DD#: %12d Period: %-10s %-10s Date: %-10s\r\n",
			(" "),
			(empname!=NULL ? empname:""),checkno,
			(paystarting!=NULL ? paystarting:""),
			(payending!=NULL ? payending:""),
	...
	for(x=0;x<number_jobs;++x)
	{
		...
		{
			RDA_fprintf(fp,"     %-15s  %10.02f  %10.02f  %10.02f  %8.02f  %10.02f  %8.02f  %10.02f  %12.02f\r\n",
		...
		{
			RDA_fprintf(fp,"     %-15s  %10.02f  %10.02f  %10.02f  %8.02f  %10.02f  %8.02f  %10.02f  %12s\r\n",
				(temp!=NULL ? temp:""),gross/100,adjust/100,
				regular/100,units,overtime/100,overunits,
				cur_gross/100,"");
		} else {
	...
	if(!outoforder)
	{
	RDA_fprintf(fp,"%-77s Total Gross: %10.02f %12.02f\r\n","",total_cur_gross/100,total_ytd/100);
	} else { 
	RDA_fprintf(fp,"%-77s Total Gross: %10.02f %12s\r\n","",total_cur_gross/100,"");
	}
	RDA_fprintf(fp,"\r\n");

	RDA_fprintf(fp,"     %-20s     Cur      YTD  %-20s     Cur      YTD  %-20s     Cur      YTD\r\n","Deduction","Deduction","Deduction");
	total_cur=0.0;
	total_ytd=0.0;
	for(x=0;x<number_deductions;x+=3)
	{
		for(y=0;y<3;++y)
		{
			...
			if(!isEMPTY(temp) || cur!=0.0 || ytd!=0.0)
			{
				...
				if(!outoforder)
				{
				RDA_fprintf(fp,"%-20s %7.02f %8.02f",
					(temp!=NULL?temp:""),
					cur/100,ytd/100);
				} else { 
				RDA_fprintf(fp,"%-20s %7.02f %8s",
					(temp!=NULL?temp:""),
					cur/100,"");
				}
			}
	...
	if(!outoforder)
	{
	RDA_fprintf(fp,"%-86sTotal Deductions: %7.02f %8.02f\r\n","",
		total_cur/100,total_ytd/100);
	} else { 
	RDA_fprintf(fp,"%-86sTotal Deductions: %7.02f %8s\r\n","",
		total_cur/100,"");
	}
	RDA_fprintf(fp,"\r\n");
	...
	if(did_leave==FALSE) /* Makes up for missing header line if no leave is printed */
	{
		RDA_fprintf(fp,"\r\n");
	}
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"%40sCurrent Gross:%10.02f - Deductions:%10.02f = Net Direct Deposit:%10.02f\r\n","",total_cur_gross/100,(total_cur-net)/100,net/100);
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	SelectPrintType(outdevice,0,fp);
	...
	for(f=0;f<(32-(new_lines-default_lines));++f)
	{
		RDA_fprintf(fp,"\r\n");
	}
...
{
	short ef=0,x=0;
	char *empid1=NULL,deleteflag=FALSE,active=FALSE,nos=FALSE,*empid=NULL;
...
static CheckRate *GatherCurrentCheckRate(char *empid,char *payid,int jobno)
{
	...
	while(!ef)
	{
		...
		if(!delflag)
		{
			...
			{
				C->salary+=gross;
			} else if(!RDAstrcmp(rate,"HOURLY") || !RDAstrcmp(rate,"HOURLY UNIT") || !RDAstrcmp(rate,"DAILY UNIT") || !RDAstrcmp(rate,"MISC SALARIES") || !RDAstrcmp(rate,"MISC UNITS") || !RDAstrcmp(rate, "MISC UNIT") ||  !RDAstrcmp(rate,"DAILY") || !strncmp(rate,"MEETING",7)) /* Unit Rates */
			{
				C->units+=units;
				C->unit_amt+=gross;
...
static void selectchecks(RDArsrc *r)
{
	...
	FINDRSCGETINT(r,"NUMBER JOBS",&number_jobs);
	FINDRSCGETINT(r,"NUMBER DEDUCTIONS",&number_deductions);
	FINDRSCGETINT(r,"NUMBER LVEMSTR",&number_lvemstr);
	FINDRSCGETCHAR(r,"SHOW ZERO JOBS",&show_zero_jobs);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"PRINT DIRECT DEPOSIT TRIFOLD")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"PRINT DIRECT DEPOSIT TRIFOLD");
	...
	addstdrsrc(mainrsrc,"NUMBER JOBS",LONGV,2,&number_jobs,TRUE);
	addstdrsrc(mainrsrc,"NUMBER DEDUCTIONS",LONGV,2,&number_deductions,TRUE);
	addstdrsrc(mainrsrc,"NUMBER LVEMSTR",LONGV,2,&number_lvemstr,TRUE);
	addstdrsrc(mainrsrc,"SHOW ZERO JOBS",BOOLNS,1,&show_zero_jobs,TRUE);
