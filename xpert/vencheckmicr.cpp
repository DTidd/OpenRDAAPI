/* Vendor Check */
#ifndef WIN32
#define __NAM__ "vencheckmicr.lnx"
#endif
#ifdef WIN32
#define __NAM__ "vencheckmicr.exe"
#endif
#include <app.hpp>
...
*/
	...
	/* The following non laser check types are being set to default to the 
	 * same placement as the high_spelled_amount option in case this 
	 * program is accidentally run or is used during transition
	 
		cch: high_spelled_amount
		clarke: high_spelled_amount
		glendale_union: high_spelled_amount
		VENCHECK-TAB: high_spelled_amount
		VENCHECK-ASCII: high_spelled_amount
	*/
...
int PrintMICRCheckForm(int print_type,RDA_PFILE *fp,int checknumber,char *checkdate,int amount, char *emp_name, char *emp_add1,char *emp_add2,char *emp_add3,char *output_device)
{
		...
		float decifloatx=0.0,decifloaty=0.0;
		...
		if(RDA_GetEnv("USE_DECIPOINTS")!=NULL)
		{
			use_decipoints=1;
		}
			...
			if(print_type==1)
			{
				...
				for(x=65;x>0;x--)
				{
					...
					{			
						...
						if(use_decipoints)
						{
							decifloatx=((micr_xpos[x])*0.72);
							if(micr_check_type==0)
							{
								decifloaty=((micr_bottom_ypos)*0.72);
								RDA_fprintf(fp,"%c&a%dH%c&a%dV%c",27,decifloatx,27,decifloaty,my_micr_xval[x]);
							}else if(micr_check_type==1) {
								decifloaty=((micr_middle_ypos)*0.72);
								RDA_fprintf(fp,"%c&a%dH%c&a%dV%c",27,decifloatx,27,decifloaty,my_micr_xval[x]);
							}
							decifloatx=0.0;
							decifloaty=0.0;
...
static void RepeatStub(short sfile,int checkno,int vcheckno,double total_amt,RDA_PFILE *fp,int startingrecordpos,int prtsubtotal)
{
	...
	double amt=0.0;
	int rpttrncounter=0;
	int prevcounter=0,prevcheckno=0,prevvcheckno=0;

	FINDFLDGETINT(sfile,"CHECK NUMBER",&prevcheckno);
	FINDFLDGETINT(sfile,"VIRTUAL CHECK NUMBER",&prevvcheckno);
	FINDFLDGETINT(sfile,"COUNTER",&prevcounter);
	...
	while(!ef)
	{
		...
		if(prtcheckdetail)
		{
			...
			{	
				RDA_fprintf(fp," %-10s  %-15s  %-16s  %-34s  %-33s     %10.02f\r\n",CenterText((invoicedate!=NULL ? invoicedate:""),10),(invid!=NULL ? invid:""),CenterText((creditcard!=NULL ? creditcard:stemp),16),(acc!=NULL ? acc:""),(desc!=NULL ? desc:""),amt/100);
			}else{
				RDA_fprintf(fp," %-10s  %-15s  %-16s  %-34s  %-33s     %10.02f\r\n",CenterText((invoicedate!=NULL ? invoicedate:""),10),(invid!=NULL ? invid:""),(poid!=NULL ? poid:""),(acc!=NULL ? acc:""),(desc!=NULL ? desc:""),amt/100);
			}
...
static void OutputChecks(short sfile)
{
	...
	if(SEC_USERS_FILENO!=(-1))
	{
		...
		{
			FINDFLDGETSTRING(SEC_USERS_FILENO,"VENPMT SIGNATURE FILENAME",&sig_filename);
			FINDFLDGETSTRING(SEC_USERS_FILENO,"VENPMT SIGNATURE PASSWORD",&sig_password);
		}
	}else{
	...
	if((usersigsec)&&(SIGNATURE_TYPE==4))
	{
	...
	{
		if(diagrptgen) { prterr("Loading PCL Macro 3002.\n"); }
	...
	{
		if(diagrptgen) { prterr("Loading PCL Macro 3003.\n"); }
	...
	{
		if(diagrptgen) { prterr("Loading PCL Macro 3000.\n"); }
...
/* negotiable check */
			...
			if((printstubform==1)&&(BODY_COUNT==15)&&(doublestub==0))
			{
				if(diagrptgen) { prterr("Using PCL Macro 3002.\n"); }
				SelectPrintTypeByName(output_device,"USE_PCLMACRO_3002",fp);
			}else if((printstubform==1)&&(BODY_COUNT==15)&&(doublestub==1))
			{
				if(diagrptgen) { prterr("Using PCL Macro 3003.\n"); }
				SelectPrintTypeByName(output_device,"USE_PCLMACRO_3003",fp);
			}else if((printstubform==1)&&(BODY_COUNT==46))
			{
				if(diagrptgen) { prterr("Using PCL Macro 3000.\n"); }
			...
			if((prtcheckdetail)&&((doublestub==0)||((doublestub==1)&&(use_full_micr_check==1))))
			{
				if(lastpage==1)
				{
					if((count!=0)&&(count<BODY_COUNT))
...
/* negotiable check */
	...
	if(use_full_micr_check)
	{
		...
		if((printstubform==1)&&(BODY_COUNT==15)&&(doublestub==0))
		{
			if(diagrptgen) { prterr("Delete PCL Macro 3002.\n"); }
			SelectPrintTypeByName(output_device,"DELETE_PCLMACRO_3002",fp);
		}else if((printstubform==1)&&(BODY_COUNT==15)&&(doublestub==1))
		{
			if(diagrptgen) { prterr("Delete PCL Macro 3003.\n"); }
			SelectPrintTypeByName(output_device,"DELETE_PCLMACRO_3003",fp);
		}else if((printstubform==1)&&(BODY_COUNT==46))
		{
			if(diagrptgen) { prterr("Delete PCL Macro 3000.\n"); }
			SelectPrintTypeByName(output_device,"DELETE_PCLMACRO_3000",fp);
		}
...
void setcheck(RDArsrc *mainrsrc)
{
	...
	while(!ef)
	{
		...
		if(verified && !manual_voucher && !electronic && !deleteflag && cnum!=0 && ((!cleared && re==FALSE) || (re==TRUE && select_reissued==TRUE)))
		{
		if(banknum!=(-1))
		{
