...
******************************************/
#ifndef WIN32
#define __NAM__ "dirdep1a.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdep1a.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static void printactualcheck()
{
	...
	char *temp5=NULL,*temp6=NULL,*temp=NULL;
	char *fed_title=NULL,*st_title=NULL,*fica_title=NULL,*local_title=NULL;
	char *med_title=NULL,*stret_title=NULL,*ret_title=NULL;
	APPlib *dedtitle;
	...
	double local_ytd=0.0,other_ret_ytd=0.0,state_ret_ytd=0.0;
	...
	FINDFLDGETSTRING(sortfile->fileno,"STATE RETIREMENT DEDUCTION TITLE",&stret_title);
	FINDFLDGETDOUBLE(sortfile->fileno,"STATE RETIREMENT W/H CURRENT",&state_ret);
	FINDFLDGETDOUBLE(sortfile->fileno,"STATE RETIREMENT W/H YTD",&state_ret_ytd);
	...
	sprintf(stemp,"BNKREC\\%s",(bankno!=NULL ? bankno:""));
#endif
	temp8=adddashes(stemp);
	RDAmkdir(temp8,00770,&ret_int);
#ifndef WIN32
	...
	sprintf(stemp,"BNKREC\\%s\\ETRS",(bankno!=NULL ? bankno:""));
#endif
	temp7=adddashes(stemp);
	RDAmkdir(temp7,00770,&ret_int);

	temp=Rmalloc(529);
	memset(temp,0,528);
	memset(stemp,0,101);
#ifndef WIN32
	sprintf(stemp,"BNKREC/%s/ETRS/%d.pdf",bankno,transno);
...
/* 	Rearranging deduction slots with Fed, FICA, State, Local, Medicare,
	...
	if((state_ret!=0) || (state_ret_ytd!=0))
	{
		if(stret_title!=NULL)	x=addAPPlib(dedtitle,stret_title);
		else			x=addAPPlib(dedtitle,"STATE RET");
		ded[x-1]=state_ret;
		ded_ytd[x-1]=state_ret_ytd;
	}
	if((other_ret!=0) || (other_ret_ytd!=0))
	{
		if(ret_title!=NULL)	x=addAPPlib(dedtitle,ret_title);
		else			x=addAPPlib(dedtitle,"OTHER RET");
		ded[x-1]=other_ret;
...
/*	Printing Transmittal Lines */
	...
	if(display_ssn==TRUE)
	{
	fprintf(fp,"       SSN:%-11s LOC:%-10s  Period End:%-10s Pay Date:%-10s\r\n",
		(ssn!=NULL ? ssn:""),(loc!=NULL ? loc:""),
		(payending!=NULL ? payending:""),trans_date);
	} else {
	fprintf(fp,"       LOC:%-10s %15s  Period End:%-10s Pay Date:%-10s\r\n",
		(loc!=NULL ? loc:""),"",
		(payending!=NULL ? payending:""),trans_date);
	}
	if(print_employer_deductions==TRUE)
	{
	fprintf(fp,"              Time      Gross        YTD Deduct    Current        YTD     District\r\n");
	} else {
	fprintf(fp,"              Time      Gross        YTD Deduct    Current        YTD\r\n");
	}
...
/*lint +e771*/
		...
		switch(x)
		{
			...
			{
				fprintf(fp,"       Sal:        %10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
				...
				if(print_employer_deductions==TRUE)
				{
					fprintf(fp," %10s",(temp6!=NULL ? temp6:""));
				}
			} } else  fprintf(fp,"       Sal:        %10s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""));*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")));
			fprintf(fp,"\r\n");
			...
			if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
			{
				fprintf(fp,"       Reg:%7s %10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					/*(temp5!=NULL ? temp5:""),*/
			...
			} } else  fprintf(fp,"       Reg:%7s %10s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					/*(temp5!=NULL ? temp5:""));*/
			...
			if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
			{
				fprintf(fp,"       Ovt:%7s %10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					/*(temp5!=NULL ? temp5:""),*/
			...
			} } else  fprintf(fp,"       Ovt:%7s %10s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					/*(temp5!=NULL ? temp5:""));*/
			...
			if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
			{
				fprintf(fp,"       Adj:        %10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					(dedtitle->libs[x]!=NULL ? dedtitle->libs[x]:""),
			...
			if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
			{
				fprintf(fp,"       Total Gross:%10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					(dedtitle->libs[x]!=NULL ? dedtitle->libs[x]:""),
			...
			} } else  fprintf(fp,"       Total Gross:%10s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""));*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")));
			fprintf(fp,"\r\n");
			...
			if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
			{
				fprintf(fp,"       Total Ded.: %10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					(dedtitle->libs[x]!=NULL ? dedtitle->libs[x]:""),
			...
			} } else  fprintf(fp,"       Total Ded.: %10s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""));*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")));
			fprintf(fp,"\r\n");
			...
			if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
			{
				fprintf(fp,"              Net: %10s %10s %-8s %8s %10s",
					(temp3!=NULL ? temp3:""),
					/*(temp4!=NULL ? temp4:""),*/
					(outoforder ? "":(temp4!=NULL ? temp4:"")),
					(dedtitle->libs[x]!=NULL ? dedtitle->libs[x]:""),
			...
			if(print_employer_deductions==TRUE)
			{
				...
				if((!isEMPTY(dedtitle->libs[x])) || (ded[x]!=0) || (ded_ytd[x]!=0) || (district[x]!=0))
				{
					fprintf(fp,"%8s Total Employer-Paid: %10s %-8s %8s %10s",
						"",(temp3!=NULL ? temp3:""),
					...
					fprintf(fp,"%8s Total Employer-Paid: %10s",
						"",(temp3!=NULL ? temp3:""));
				}
				fprintf(fp,"\r\n");
				break;
	...
	{
		fprintf(fp,"       ID:%-15s\r\n",emp_id);
		fprintf(fp,"       Leave      Begin Earned   Used  Ending     %-30s\r\n",emp_name);
	
		temp1=decimal(sickbb); temp2=decimal(sicke);
		temp3=decimal(sicku);  temp4=decimal(sickb);
		fprintf(fp,"       Sick Lve:%7s %6s %6s %7s     %-30s\r\n",
		...
		fprintf(fp,"       Vacation:%7s %6s %6s %7s     %-30s\r\n",
			(temp1!=NULL ? temp1:""), (temp2!=NULL ? temp2:""),
			(temp3!=NULL ? temp3:""), (temp4!=NULL ? temp4:""),
		...
		fprintf(fp,"       Personal:%7s %6s %6s %7s     %-30s\r\n",
			(temp1!=NULL ? temp1:""), (temp2!=NULL ? temp2:""),
			(temp3!=NULL ? temp3:""), (temp4!=NULL ? temp4:""),
	...
	while(!ef)
	{
		...
		if(!deleteflag)
		{
			...
			if(reduce_net==TRUE || print_employer_deductions==TRUE)
			{
				...
				} else if(!RDAstrcmp(type2,"STATE RETIREMENT"))
				{
...
/* Deduction Fields */
	...
	addDBfield(sortfile,"RETIREMENT MATCH",2,20);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"ARCHIVE PAYROLL TRANSMITTALS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"ARCHIVE DIRECT DEPOSIT TRANSMITTALS");
