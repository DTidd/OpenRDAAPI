...
#ifndef WIN32
#define __NAM__ "payvwcheckmicra.lnx"
#endif
#ifdef WIN32
#define __NAM__ "payvwcheckmicra.exe"
#endif
#include <app.hpp>
...
int PrintMICRCheckForm(int print_type,RDA_PFILE *fp,int checknumber,char *checkdate,int amount, char *emp_name, char *emp_add1,char *emp_add2,char *emp_add3,char *output_device)
{
		...
		if((print_type==1)||(print_type==3))
		{
			...
			*/
			...
			if(micr_check_type==0)
			{
				/*prterr("Sending Call For PCL Macro [%d].\n",(checkform+3200)); */
...
*/
	...
	memset(stemp,0,101);
	prterr("printstubform = %d BODY_COUNT = %d doublestub = %d", printstubform,BODY_COUNT,doublestub==0);
...
#ifndef WIN32
		if((access("forms/load_pclmacro_3012.pcl",R_OK))!=0)
		{
...
#ifndef WIN32
		if((access("forms/load_pclmacro_3013.pcl",R_OK))!=0)
		{
			prterr("Can not find PCL form [%d]\n",3013);
		}
#endif /* WIN32 */
...
#ifndef WIN32
		if((access("forms/load_pclmacro_3010.pcl",R_OK))!=0)
		{
			prterr("Can not find PCL form [%d]\n",3010);
		}
#endif /* WIN32 */
...
/* int x=0; */
	...
	while(!ef)
	{
		...
		if((close_page==1)||(close_page==2))
		{
			...
			{
				RDA_pclose(fp);
				/*InitializeArchivePage(&fp,vcheckno,++pagecount);*/
				InitializeArchivePage(&fp,checkno);
			}else{
				RDA_fprintf(fp,"\f");
			}
...
void setcheck(RDArsrc *mainrsrc)
{
	...
	*/
	...
	/* Assuming that only this type applies to VW ARCHIVE MICR */
	micr_check_type=0;
	...
	/* For Archive these values should be fixed */
	checkdate_xpos=1150;
	checkdate_ypos=2620;
	checknumber_xpos=2105;
	checknumber_ypos=2445;
	amount_xpos=1650;
	amount_ypos=2620;
	spelled_amount_xpos=60;
	spelled_amount_ypos=2490;
	payto_xpos=300;
	payto1_ypos=2740;
	payto2_ypos=2780;
	payto3_ypos=2820;
...
#endif
{
	...
	/*prterr("startup_type = %d",startup_type);*/
	switch(startup_type)
	{
		case  2:
	...
	mainrsrc=RDArsrcNEW(module,"ARCHIVE VENDOR WITHHOLDING CHECKS MICR");
	...
	*/
	...
	switch(startup_type)
	{
		...
		default:
			/* PLAIN TEXT CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE VENDOR WITHHOLDING PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+3;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  2:
			/* DECORATED STUB PLAIN CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE VENDOR WITHHOLDING DECORATED STUB PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+13;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  3:
			/* MICR STUB CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE VENDOR WITHHOLDING DECORATED STUB MICR CHECKS");
			w=mainrsrc->scn->wdgts+29;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  4:
			/* PLAIN TEXT DS CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE VENDOR WITHHOLDING PLAIN DOUBLE STUB CHECKS");
			w=mainrsrc->scn->wdgts+45;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  5:
			/* DS STUB CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE VENDOR WITHHOLDING DECORATED DOUBLE STUB CHECKS");
			w=mainrsrc->scn->wdgts+55;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  6:
			/* DS MICR STUB CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE VENDOR WITHHOLDING DECORATED DOUBLE STUB MICR CHECKS");
