...
#ifndef WIN32
#define __NAM__ "paycheckmicr.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paycheckmicr.exe"
#endif
#include <app.hpp>
...
if((XPERT_SETUP->LVEMGT==TRUE || XPERT_SETUP->LVSIMP==TRUE) && skip_leave==FALSE)
{
	...
	if((usersigsec)&&(check_signature==5))
	{
		SelectPrintTypeByName(output_device,"PUSH CURSOR",fp);
		SelectPrintTypeByName(output_device,"PAYROLL SIGNATURE",fp);
		SelectPrintTypeByName(output_device,"POP CURSOR",fp);
		SelectPrintTypeByName(output_device,"COMPRESSED TEXT",fp);
	} else if((usersigsec)&&(check_signature==4))
	{
		SelectPrintTypeByName(output_device,"PUSH CURSOR",fp);
		if(use_sign_locks)
		{
...
static void printchecks(RDArsrc *r,RDArsrc *mainrsrc)
{
	...
	if(!isEMPTY(output_device))
	{
		...
		if(micr_check_type==0)
		{
			...
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("LOAD_PCLMACRO_%04d",(checkform+3100)); }

			sprintf(stemp,"LOAD_PCLMACRO_%04d",(checkform+3300));
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("LOAD_PCLMACRO_%04d",(checkform+3300)); }

		}else if(micr_check_type==1) {
			sprintf(stemp,"LOAD_PCLMACRO_%04d",(checkform+3200));
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("LOAD_PCLMACRO_%04d",(checkform+3200)); }

			sprintf(stemp,"LOAD_PCLMACRO_%04d",(checkform+3400));
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("LOAD_PCLMACRO_%04d",(checkform+3400)); }
		}
	...
	if(use_full_micr_check)
	{
		...
		if(micr_check_type==0)
		{
			...
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("DELETE_PCLMACRO_%04d",(checkform+3100)); }

			sprintf(stemp,"DELETE_PCLMACRO_%04d",(checkform+3300));
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("DELETE_PCLMACRO_%04d",(checkform+3300)); }
		}else if(micr_check_type==1) {
			sprintf(stemp,"DELETE_PCLMACRO_%04d",(checkform+3200));
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("DELETE_PCLMACRO_%04d",(checkform+3200)); }

			sprintf(stemp,"DELETE_PCLMACRO_%04d",(checkform+3400));
			SelectPrintTypeByName(output_device,stemp,fp);
			if(diagrptgen) { prterr("DELETE_PCLMACRO_%04d",(checkform+3400)); }
		}
	}
...
/* Key Info */
	...
	if(diagrptgen)
	{
		prterr("micr_check_type [%d]   micr_bottom_ypos [%d]   micr_middle_ypos [%d]   micrtext_len [%d]   check_type [%d]\n",micr_check_type,micr_bottom_ypos,micr_middle_ypos,(strlen(micrtext)),check_type);
	}
	if((((micr_check_type==0)&&(micr_bottom_ypos!=0)||((micr_check_type==1)&&(micr_middle_ypos!=0))))&&((strlen(micrtext)!=0)&&(startup_type==3)))
	{
...
#endif
{
	...
	mainrsrc=RDArsrcNEW(module,"PRINT PAYROLL CHECKS MICR 8.5x11");
	...
	switch(startup_type)
	{
		...
		default:
			/* PLAIN TEXT CHECK */
			mainrsrc->scn->name=stralloc("PRINT PAYROLL PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+3;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  2:
			/* DECORATED STUB PLAIN CHECK */
			mainrsrc->scn->name=stralloc("PRINT PAYROLL DECORATED STUB PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+10;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  3:
			/* MICR STUB CHECK */
			mainrsrc->scn->name=stralloc("PRINT PAYROLL DECORATED STUB MICR CHECKS");
