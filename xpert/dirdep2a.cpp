...
#ifndef WIN32
#define __NAM__ "dirdep2a.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdep2a.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static char show_zero_jobs=FALSE,skip_leave=FALSE;
...
static void printactualcheck()
{
	...
	int checkno=0,x,y=0;
	char *check_date=NULL,outoforder=FALSE;
	...
	if(display_ssn==TRUE)
	{
	fprintf(fp,"SSN: %-11s Name: %-25s   DD#: %12d %-10s\r\n",
		(ssn!=NULL ? ssn:""),
		(empname!=NULL ? empname:""),checkno,
		(check_date!=NULL?check_date:""));
	} else {
	fprintf(fp,"Name: %-40s     DD#: %12d %-10s\r\n",
		(empname!=NULL ? empname:""),checkno,
		(check_date!=NULL?check_date:""));
	}
	...
	fprintf(fp,"Job Title           Gross  Adjust Regular Units Overtime Over  Current       YTD\r\n");
	for(x=0;x<number_jobs;++x)
	{
		...
		{
			fprintf(fp,"%-15s %9.02f %7.02f %7.02f %5.02f %8.02f %5.02f %7.02f %9\r\n",
				(temp!=NULL ? temp:""),gross/100,adjust/100,
				regular/100,units,overtime/100,overunits,
				cur_gross/100,"");
		} else {
	...
	fprintf(fp,"%-48s Total Gross: %8.02f %9s\r\n","",total_cur_gross/100,"");
	}
	fprintf(fp,"\r\n");
	...
	if(!outoforder)
	{
	fprintf(fp,"%-46sTotal Deductions: %7.02f %8.02f\r\n","",
		total_cur/100,total_ytd/100);
	} else {
	fprintf(fp,"%-46sTotal Deductions: %7.02f %8s\r\n","",
		total_cur/100,"");
	}
	fprintf(fp,"\r\n");
...
if((XPERT_SETUP->LVEMGT==TRUE || XPERT_SETUP->LVSIMP==TRUE) && skip_leave==FALSE)
{
	...
	for(f=0;f<(17-(new_lines-default_lines));++f)
	{
		fprintf(fp,"\r\n");
	}
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"ARCHIVE DIRECT DEPOSIT TRIFOLD")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"ARCHIVE DIRECT DEPOSIT TRIFOLD");
