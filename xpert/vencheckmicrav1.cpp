...
#define MODE_RWXRWX___ (00770)
#endif /* ifdef WIN32 */
static short DO_CHMOD(char *name)
{
...
static void MAKE_DIR(char *dirname)
{
	char *fname=NULL;
	int fd=(-1);
...
#ifdef WIN32
	fname=Rmalloc(RDAstrlen(CURRENTDIRECTORY)+RDAstrlen(dirname)+3);
	sprintf(fname,"%s\\%s",CURRENTDIRECTORY,dirname);
	fd=mkdir(fname);
#endif
#ifndef WIN32
	fname=Rmalloc(RDAstrlen(CURRENTDIRECTORY)+RDAstrlen(dirname)+3);
...
int SetCoordinateValues(short vals[13])
{
	...
	prterr("vals [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d] [%d]\n",vals[0],vals[1],vals[2],vals[3],vals[4],vals[5],vals[6],vals[7],myaddr,vals[9],vals[10],vals[11],vals[12]);
}
...
*/
	short high_spelled_amount [] = { 1320, 2436, 1738, 2436, 1950, 2436, 145, 2288, 475, 2587, 2637, 2687, 2737 };
	short low_spelled_amount [] = { 1320, 2486, 1738, 2486, 1950, 2486, 145, 2333, 473, 2585, 2635, 2685, 2735 };
	short high_address [] = { 1320, 2436, 1738, 2436, 1950, 2436, 149, 2287, 477, 2539, 2589, 2639, 2689 };
	short cch [] = { 1320, 2436, 1738, 2436, 1950, 2436, 147, 2285, 475, 2587, 2637, 2687, 2737 };
	short clarke [] = { 1320, 2436, 1738, 2436, 1950, 2436, 149, 2287, 477, 2539, 2589, 2639, 2689 };
	short glendale_union [] = { 1320, 2436, 1738, 2436, 1950, 2436, 149, 2287, 477, 2589, 2639, 2689, 2739 };

	//short high_spelled_amount [] = { 1320, 2436, 1738, 2436, 1950, 2436, 145, 2288, 240, 2587, 2637, 2687, 2737 };
	...
	switch(VENCHECK_TYPE)
	{
		...
		case 1:
			break;
		case 2:
			SetCoordinateValues(cch);
			prterr("Using default cch\n");
			break;
		case 3:
			...
			break;
		case 4:
			SetCoordinateValues(clarke);
			prterr("Using default clarke\n");
			break;
		case 5:
		...
		case 7:
			break;
		case 8:
			SetCoordinateValues(glendale_union);
			prterr("Using default glendale\n");
			break;
		default:
			break;
	}
...
int PrintMICRCheckForm(int print_type,RDA_PFILE *fp,int checknumber,char *checkdate,int amount, char *emp_name, char *emp_add1,char *emp_add2,char *emp_add3,char *output_device)
{
		...
		{
			RDA_fprintf(fp,"%c*p%dx%dY%s",48,spelled_amount_xpos,spelled_amount_ypos,spelled_amount);
			SelectPrintTypeByName(output_device,"USE DEFAULT PRIMARY FONT",fp);
			SelectPrintTypeByName(output_device,"NORMAL TEXT",fp);
		...
		if(((print_type==0)||(print_type==2))&&(!use_full_micr_check))
		{
			SelectPrintTypeByName(output_device,"USE DEFAULT PRIMARY FONT",fp);
			SelectPrintTypeByName(output_device,"NORMAL TEXT",fp);
			RDA_fprintf(fp,"%c*p%dx%dY%06d",48,checknumber_xpos,checknumber_ypos,checknumber);
		}
		if((print_type==1)||(print_type==3))
		{
		...
		{
			case 0:
				RDA_fprintf(fp,"%c*p%dx%dY%s",48,spelled_amount_xpos,spelled_amount_ypos,(spelled_amount!=NULL ? spelled_amount:""));
				RDA_fprintf(fp,"%c*p%dx%dY%s",48,checkdate_xpos,checkdate_ypos,(checkdate!=NULL ? checkdate:""));
				RDA_fprintf(fp,"%c*p%dx%dY%s",48,amount_xpos,amount_ypos,(tempstr!=NULL?tempstr:""));
				break;
			case 1:
			case 2:
				RDA_fprintf(fp,"%c*p%dx%dY%s",48,checkdate_xpos,checkdate_ypos,(checkdate!=NULL ? checkdate:""));
				...
				/* prterr("y = [%d]  \n",x); */
				RDA_fprintf(fp,"%c*p%dx%dY%s",48,amount_xpos,amount_ypos,stemp);
				break;
			default:
				RDA_fprintf(fp,"%c*p%dx%dY{%s}",48,checkdate_xpos,checkdate_ypos,(checkdate!=NULL ? checkdate:""));
				memset(stemp,0,101);
		...
		switch(security_font_type)
		{
			...
			case 11:
				...
				{
					RDA_fprintf(fp,"%c*p%dx%dY[%s]",48,payto_xpos,payto1_ypos,payto1);
				}
				SelectPrintTypeByName(output_device,"USE DEFAULT PRIMARY FONT",fp);
				SelectPrintTypeByName(output_device,"NORMAL TEXT",fp);
				if((RDAstrlen(payto2))!=0)
				{
					RDA_fprintf(fp,"%c*p%dx%dY%s",48,payto_xpos,payto2_ypos,payto2);
				}
				if((RDAstrlen(payto3))!=0)
				{
					RDA_fprintf(fp,"%c*p%dx%dY%s",48,payto_xpos,payto3_ypos,payto3);
				}
				if((RDAstrlen(payto4))!=0)
				{
					RDA_fprintf(fp,"%c*p%dx%dY%s",48,payto_xpos,payto4_ypos,payto4);
				}
				break;
			default:
				if((RDAstrlen(payto1))!=0)
			...
			if(print_type==1)
			{
				...
				for(x=65;x>0;x--)
				{
					...
					{			
						...
						{
							RDA_fprintf(fp,"%c*p%dx%dY%c",48,micr_xpos[x],micr_bottom_ypos,my_micr_xval[x]);
						}else if(micr_check_type==1) {
							RDA_fprintf(fp,"%c*p%dx%dY%c",48,micr_xpos[x],micr_middle_ypos,my_micr_xval[x]);
						}
					}
				}
			}
...
static void RepeatStub(short sfile,int checkno,int vcheckno,double total_amt,RDA_PFILE *fp)
{
	...
	if((checkno==vcheckno)&&(printstubform==1))
	{
		RDA_fprintf(fp,"\r\n");	
		RDA_fprintf(fp,"\r\n");	
...
/* negotiable check */
			...
			{
				RDA_pclose(fp);
				InitializeArchivePage(&fp,vcheckno);
			}else{
				RDA_fprintf(fp,"\f");
			}
			count=0;	
			...
			if(doublestub==0)
			{
				...
				if((lastpage==1)&&(use_full_micr_check==1))
				{
					//RDA_fprintf(fp,"\r\n");	
					if(printstubform==1)
					{
...
#endif
{
	...
	*/
	...
	mainrsrc=RDArsrcNEW(module,"VENDOR CHECKS MICR");
	...
	switch(startup_type)
	{
		...
		default:
			/* PLAIN TEXT CHECK */
			mainrsrc->scn->name=stralloc("VENDOR PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+3;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  2:
			/* DECORATED STUB PLAIN CHECK */
			mainrsrc->scn->name=stralloc("VENDOR DECORATED STUB PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+19;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  3:
			/* MICR STUB CHECK */
			mainrsrc->scn->name=stralloc("VENDOR DECORATED STUB MICR CHECKS");
			w=mainrsrc->scn->wdgts+41;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  4:
			/* PLAIN TEXT DS CHECK */
			mainrsrc->scn->name=stralloc("VENDOR PLAIN DOUBLE STUB CHECKS");
			w=mainrsrc->scn->wdgts+63;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  5:
			/* DS STUB CHECK */
			mainrsrc->scn->name=stralloc("VENDOR DECORATED DOUBLE STUB CHECKS");
			w=mainrsrc->scn->wdgts+79;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  6:
			/* DS MICR STUB CHECK */
			mainrsrc->scn->name=stralloc("VENDOR DECORATED DOUBLE STUB MICR CHECKS");
