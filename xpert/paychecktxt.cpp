...
#ifndef WIN32
#define __NAM__ "paychecktxt.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paychecktxt.exe"
#endif
#include <app.hpp>
...
static short formdecimal=FALSE,formleave=FALSE,jdtype=0;
static short PRINT_NUMBER_JOBS=DEFAULT_NUMBER_JOBS;
...
static char *message7=NULL,*message8=NULL,*message9=NULL,*filename=NULL;
static char *paystarting=NULL,*payending=NULL;
static char *bankno=NULL,*payid=NULL;
static char *defbankid=NULL;
static short DID_STUFF=0,check_signature=0;
...
static void printactualcheck()
{
	...
	fprintf(fp,"%-40sCheck No.: %12d Date: %-10s\n",
	...
	fprintf(fp,"Current Gross:%10.02f - Deductions:%10.02f = Net Check:%10.02f\n",
...
*/
	...
	fprintf(fp,"%*s%-40s\r\n",8,"",(empname!=NULL ? empname:""));
	fprintf(fp,"%*s%-40s\r\n",8,"",(emp_add1!=NULL ? emp_add1:""));
	fprintf(fp,"%*s%-40s\r\n",8,"",(emp_add2!=NULL ? emp_add2:""));
	fprintf(fp,"%*s%-40s\r\n\f",8,"",(emp_add3!=NULL ? emp_add3:""));
/*
	fprintf(fp,"\n\f");
*/
...
static void GatherLeave()
{
	short ef=0,x=0;
	char *empid1=NULL,deleteflag=FALSE,*empid=NULL,nos=FALSE,active=FALSE;
	double outlve=0.0,camt=0.0,beg=0.0,pearn=0.0,pused=0.0,earn=0.0,used=0.0;
	char *leaveid=NULL,*leaveid1=NULL;
	double lvemstrbal=0.0;
		...
		while(!ef)
		{
			...
			if(!deleteflag)
			{
				...
				if(ADVEQLNRDsec(lvedsc,1,SCRNvirtualSubData,diagrsrc)) 
				{
					...
					FINDFLDGETCHAR(lvemst,"ACTIVE",&active);
					FINDFLDGETCHAR(lvedsc,"NOT ON STUB",&nos);
					lvemstrbal=outlve+beg+pearn-pused+earn-used;
					if((nos==FALSE)  && (active || lvemstrbal!=0.0))
					{
					for(x=0;x<PRINT_NUMBER_LVEMSTR;++x)
					{
						sprintf(stemp1,"LEAVE ID %d",(x+1));
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"ARCHIVE PAYROLL CHECKS 8.5x11")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"PAYROLL CHECKS TEXT");
