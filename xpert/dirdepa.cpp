/* dirdep.c - Print Payroll Direct Deposit Slips */

#ifndef WIN32
#define __NAM__ "dirdepa.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdepa.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static char LEAVE_SHORT_DESC=FALSE, skip_leave=FALSE;
...
static char check_formfeed=FALSE;
...
static void printactualcheck()
{
	...
	char *temp=NULL,*temp1=NULL,*temp2=NULL,*temp3=NULL,*temp4;
	...
	double federaly=0.0,statey=0.0,ficay=0.0,medicarey=0.0;
	int checkno=0,x,y=0,skipped=0;
	char *check_date=NULL,emplr_paid=FALSE,outoforder=FALSE;
	int f=0,default_lines=0,new_lines=0;
	FILE *fp=NULL;
	int ret_int=0;
	...
	FINDFLDGETDOUBLE(sortfile->fileno,"FEDERAL YTD GROSS",&federaly);
	FINDFLDGETDOUBLE(sortfile->fileno,"STATE YTD GROSS",&statey);
	FINDFLDGETDOUBLE(sortfile->fileno,"FICA YTD GROSS",&ficay);
	FINDFLDGETDOUBLE(sortfile->fileno,"MEDICARE YTD GROSS",&medicarey);
#ifndef WIN32
...
#ifndef WIN32
	sprintf(stemp,"BNKREC/%s/ETRS",(bankno!=NULL ? bankno:""));
#endif
	...
	memset(stemp,0,101);
#ifndef WIN32
	sprintf(stemp,"BNKREC/%s/ETRS/%d.pdf",bankno,checkno);
#endif 
#ifdef WIN32
	sprintf(stemp,"BNKREC\\%s/ETRS\\%d.pdf",bankno,checkno);
#endif 
	...
	if(diagapps)
	{
		prterr("temp1 [%s] ",(temp1!=NULL ? temp1:""));TRACE;
	}
	...
	fprintf(fp,"%-40sLocation:%-15s %15s \r\n",
	...
	fprintf(fp,"Job Title           Gross  Adjust Regular Units OT Gross   OTU Current       YTD\r\n");
	for(x=0;x<number_jobs;++x)
	{
		...
		if((((cur_gross!=0.0 || ytd!=0.0) && !isEMPTY(temp)) || show_zero_jobs)  && outoforder==FALSE)
			{
			fprintf(fp,"%-15s %9.02f %7.02f %7.02f %5.02f %8.02f %5.02f %7.02f %9.02f\r\n",
			...
			{
			fprintf(fp,"%-15s %9.02f %7.02f %7.02f %5.02f %8.02f %5.02f %7.02f %9s\r\n",
				(temp!=NULL ? temp:""),gross/100,adjust/100,
				regular/100,units,overtime/100,overunits,
				cur_gross/100,"");
		} else {
	...
	for(x=0;x<skipped;++x) fprintf(fp,"\r\n");
	fprintf(fp,"%-48s Total Gross: %8.02f %9.02f\r\n","",total_cur_gross/100,total_ytd/100);
	fprintf(fp,"\r\n");

	fprintf(fp,"Deduction     Cur      YTD Deduction     Cur      YTD Deduction     Cur      YTD\r\n");
	total_cur=0.0;
	total_ytd=0.0;
	total_ecur=0.0;
	total_eytd=0.0;
	for(x=0;x<number_deductions;x+=3)
	{
		for(y=0;y<3;++y)
		{
			...
			if(!isEMPTY(temp) || cur!=0.0 || ytd!=0.0)
			{
				...
				if(!outoforder)
				{
				fprintf(fp,"%-9s %7.02f %8.02f",(temp!=NULL
					 ? temp:""),cur/100,ytd/100);
				} else { 
				fprintf(fp,"%-9s %7.02f %8s",(temp!=NULL
					 ? temp:""),cur/100,"");
				}
			}
			if(temp!=NULL) Rfree(temp);
	...
	if(print_employer_deductions==FALSE)
	{
		if(!outoforder)
		{	
		fprintf(fp,"%-27sTotal Employee Paid Deductions: %8.02f %9.02f\r\n","",
			(total_cur-total_dirdep)/100,(total_ytd-total_dirdep_ytd)/100);
		} else { 
		fprintf(fp,"%-27sTotal Employee Paid Deductions: %8.02f %9s\r\n","",
			(total_cur-total_dirdep)/100,"");
	}
	} else {
		if(!outoforder)
		{	
		fprintf(fp,"%-22sEmployer: %8.02f %9.02f Employee: %8.02f %9.02f\r\n","",total_ecur/100,total_eytd/100,(total_cur-total_dirdep)/100,(total_ytd-total_dirdep_ytd)/100);
		} else {
		fprintf(fp,"%-22sEmployer: %8.02f %9s Employee: %8.02f %9s\r\n","",total_ecur/100,(total_cur-total_dirdep)/100,"");
		}
	}
	fprintf(fp,"\r\n");
...
if((XPERT_SETUP->LVEMGT==TRUE || XPERT_SETUP->LVSIMP==TRUE) && skip_leave==FALSE)
{
	if(number_lvemstr==0)
	{
		...
		fprintf(fp,"Leave Desc       Beg Earn  Used  Ending  Leave Desc       Beg Earn  Used  Ending\r\n");
	}
	...
	fprintf(fp,"Current Gross:%10.02f - Deductions:%10.02f = Net Direct Deposit:%10.02f\r\n",total_cur_gross/100,(total_cur-net)/100,net/100);
	fprintf(fp,"\r\n");
	fprintf(fp,"YTD EARNINGS     Federal: %10.02f    State: %10.02f \r\n",federaly/100,statey/100,ficay/100,medicarey/100);
	fprintf(fp,"                    Fica: %10.02f Medicare: %10.02f \r\n",ficay/100,medicarey/100);
	fprintf(fp,"\r\n");
	for(f=1;f<PRINT_NUMBER_MESSAGE+1 && f<10 ;++f)
	{
		switch(f)
		{
			default:
	...
	default_lines=NUMBER_JOBS+(NUMBER_DEDUCTIONS/3)+(NUMBER_LVEMSTR/2);
	new_lines=number_jobs+(number_deductions/3)+(number_lvemstr/2);
	for(f=0;f<(13-(new_lines-default_lines));++f)
	{
		fprintf(fp,"\r\n");
	}
	fprintf(fp,"%16s%-40s\r\n","",(empname!=NULL ? empname:""));
	...
	while(!ef)
	{
		...
		if(!deleteflag && cyear==calyear)
		{
			...
			if(reduce_net==TRUE || (reduce_net==FALSE && print_employer_deductions==TRUE))
			{
				...
				if(reduce_net==TRUE)
				{
					if(!RDAstrcmp(dedtype,"FEDERAL"))
					{
						for(y=1;y<=12;++y)
						{
							sprintf(stemp2,"%s GROSS",FISCALPERIOD[y]);
							QAccFld(sortfile->fileno,"FEDERAL YTD GROSS",payjdcm,stemp2);
						}
					} else if(!RDAstrcmp(dedtype,"STATE"))
					{
						for(y=1;y<=12;++y)
						{
							sprintf(stemp2,"%s GROSS",FISCALPERIOD[y]);
							QAccFld(sortfile->fileno,"STATE YTD GROSS",payjdcm,stemp2);
						}
					} else if(!RDAstrcmp(dedtype,"FICA"))
					{
						for(y=1;y<=12;++y)
						{
							sprintf(stemp2,"%s GROSS",FISCALPERIOD[y]);
							QAccFld(sortfile->fileno,"FICA YTD GROSS",payjdcm,stemp2);
						}
					} else if(!RDAstrcmp(dedtype,"MEDICARE"))
					{
						for(y=1;y<=12;++y)
						{
							sprintf(stemp2,"%s GROSS",FISCALPERIOD[y]);
							QAccFld(sortfile->fileno,"MEDICARE YTD GROSS",payjdcm,stemp2);
						}
					}
				}
		...
		if(!deleteflag)
		{
			...
			if(reduce_net==TRUE || (reduce_net==FALSE && print_employer_deductions==TRUE))
			{
				...
				if(reduce_net==TRUE)
				{
					if(!RDAstrcmp(dedtype,"FEDERAL"))
					{
						QAccFld(sortfile->fileno,"FEDERAL YTD GROSS",payjdpm,"GROSS");
					} else if(!RDAstrcmp(dedtype,"STATE"))
					{
						QAccFld(sortfile->fileno,"STATE YTD GROSS",payjdpm,"GROSS");
					} else if(!RDAstrcmp(dedtype,"FICA"))
					{
						QAccFld(sortfile->fileno,"FICA YTD GROSS",payjdpm,"GROSS");
					} else if(!RDAstrcmp(dedtype,"MEDICARE"))
					{
						QAccFld(sortfile->fileno,"MEDICARE YTD GROSS",payjdpm,"GROSS");
					}
				}
...
/* Personnel Fields */
	...
	addDBfield(sortfile,"FEDERAL YTD GROSS",2,20);
	addDBfield(sortfile,"STATE YTD GROSS",2,20);
	addDBfield(sortfile,"FICA YTD GROSS",2,20);
	addDBfield(sortfile,"MEDICARE YTD GROSS",2,20);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"ARCHIVE DIRECT DEPOSIT 8.5x11")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"ARCHIVE DIRECT DEPOSIT 8.5X11");
