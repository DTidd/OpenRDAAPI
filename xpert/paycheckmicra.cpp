...
#ifndef WIN32
#define __NAM__ "paycheckmicra.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paycheckmicra.exe"
#endif
#include <app.hpp>
...
#define DEFAULT_NUMBER_JOBS	10
#define DEFAULT_NUMBER_DEDUCTIONS	36
#define DEFAULT_NUMBER_LVEMSTR	10
#define DEFAULT_NUMBER_MESSAGE	9
...
static short banknum=(-1),lvemst=(-1),lvedsc=(-1),paydtyp=(-1);
...
*/
	...
	short spelled_amount_high_low [] = { 1400, 2636, 1738, 2636, 1950, 2636, 60, 2488, 480, 2787, 2837, 2887, 2937 };
	short spelled_amount_higher_by_one [] = { 1400, 2586, 1738, 2586, 1950, 2586, 60, 2438, 480, 2737, 2787, 2837, 2887};
	short high_address_8x11 [] = { 1400, 2636, 1738, 2636, 1950, 2636, 60, 2488, 480, 2737, 2787, 2837, 2887 };

	switch(check_type)
	{
		case 0:   /* 8.5x11 RDA Standard  */
		case 1:   /* PAYCHECK  */
		case 2:   /* PAYCHECK-CCH  */
		case 3:   /* MBGUI Standard  */
		case 4:   /* MBGUI w/Personal  */
		case 5:   /* MBGUI w/Local YTD  */
		case 8:   /* 8.5x11 to TEXT  */
		default:
			if(high_low_gsv==2)
			{
				SetCoordinateValues(spelled_amount_higher_by_one);
			}else{
				SetCoordinateValues(spelled_amount_high_low);
			}
			break;
		...
		case 6:   /* 8.5x11 Trifold  */
			/* Probably needs values to be used with middle check type in bnkacc */
			/* Not known if this is actually in use to build preset values */
			/* Use high/low value as default for now. */
			SetCoordinateValues(spelled_amount_high_low);
			break;
	}
...
int PrintMICRCheckForm(int print_type,RDA_PFILE *fp,int checknumber,char *checkdate,int amount, char *emp_name, char *emp_add1,char *emp_add2,char *emp_add3,char *output_device)
{
		...
		{
			case 0:
				/*RDA_fprintf(fp,"%c240%d%c%c241%d%c%s",27,spelled_amount_xpos,spelled_amount_ypos,(spelled_amount!=NULL ? spelled_amount:""));*/
			...
			case 2:
				...
				break;
			default:
				/*prterr("%c240%d%c%c241%d%c{%s}",27,checkdate_xpos,27,27,checkdate_ypos,27,(checkdate!=NULL ? checkdate:""));*/
				RDA_fprintf(fp,"%c240%d%c%c241%d%c{%s}",27,checkdate_xpos,27,27,checkdate_ypos,27,(checkdate!=NULL ? checkdate:""));
				memset(stemp,0,101);
#ifdef WIN32
		...
		switch(security_font_type)
		{
			...
			case 11:
				...
				{
					/*RDA_fprintf(fp,"%c240%d%c%c241%d%c[%s]",27,payto_xpos,payto1_ypos,payto1);*/
		...
		if((print_type==1)||(print_type==3))
		{
			...
			if(print_type==1)
			{
				...
				for(x=65;x>0;x--)
				{
					/* prterr(" micr_xpos = %d and micr_xval = %c ", micr_xpos[x],micr_xval[x]); */
...
static void InitializeArchivePage(RDA_PFILE **fp, int checkno)
{
	char *temp1,*temp5=NULL, *temp6=NULL;
	char temp[512];
	int ret_int=0;
	...
	sprintf(temp,"viewpdf.lnx --dont-view --dont-delete -o \"%s/%s\"",CURRENTDIRECTORY,temp1);
#endif 
#ifdef WIN32
	sprintf(temp,"viewpdf.exe --dont-view --dont-delete -o \"%s/%s\"",CURRENTDIRECTORY,temp1);
#endif 

	if(diagrptgen) { prterr("POPEN: [%s]\n",temp); }
...
/*  Moved from printchecks()  function  */
...
{
	char *ssn=NULL,*empid=NULL,*loc=NULL,*emp_name=NULL,*sfs=NULL; /* state filing status */
	...
	if(RDAstrlen(emp_name)>25) emp_name[25]=0;
	...
	RDA_fprintf(fp,"%7s%-40s       Check Number:    %12d     Check Date:   %-10s\r\n","",
...
if((XPERT_SETUP->LVEMGT==TRUE || XPERT_SETUP->LVSIMP==TRUE) && skip_leave==FALSE)
{
				...
				|| used!=0.0 || beg!=0.0)
			{
				...
				switch(formleave)
				{
					...
					case 0:
						...
						//RDA_fprintf(fp,"A%2s%23s  '%11s' '%4s' '%5s' '%7s'","",
						//RDA_fprintf(fp,"'%-21s' '%9s'  '%2s'  '%3s'  '%5s'",
					...
					case 1:
						/*if(RDAstrlen(temp)>12) temp[12]=0;*/
						 /*RDA_fprintf(fp,"%021s  %012s %04s %05s %08s","", */
						RDA_fprintf(fp,"%-23s %11s  %6s  %6s  %7s",
							(temp!=NULL ? temp:""),
							(temp1!=NULL ? temp1:""),
							(temp2!=NULL ? temp2:""),
	...
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"%53sCurrent Gross: %10.02f  -  Deductions: %10.02f  =  Net Check: %10.02f\r\n","",total_cur_gross/100,total_cur/100,net/100);
	RDA_fprintf(fp,"\r\n");
	...
	if(total_cur==0 && net==0) /* no deductions */
	{
		...
		{
			PrintMICRCheckForm(2,fp,checkno,check_date,total_cur_gross,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}else if((use_full_micr_check==0)&&(printvoid)) 
		{
			PrintMICRCheckForm(0,fp,checkno,check_date,total_cur_gross,emp_name,emp_add1,emp_add2,emp_add3,output_device); 

		}else if((use_full_micr_check==1)&&(printvoid))
		{
			PrintMICRCheckForm(3,fp,checkno,check_date,total_cur_gross,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}else if((use_full_micr_check==1)&&(printvoid)) 
		{
			PrintMICRCheckForm(1,fp,checkno,check_date,total_cur_gross,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}
	} else {
		...
		if((use_full_micr_check==0)&&(printvoid))
		{
			PrintMICRCheckForm(2,fp,checkno,check_date,net,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}else if(use_full_micr_check==0) 
		{
			PrintMICRCheckForm(0,fp,checkno,check_date,net,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}else if((use_full_micr_check==1)&&(printvoid))
		{
			PrintMICRCheckForm(3,fp,checkno,check_date,net,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}else if(use_full_micr_check==1) 
		{
			PrintMICRCheckForm(1,fp,checkno,check_date,net,emp_name,emp_add1,emp_add2,emp_add3,output_device); 
		}
	}
...
static void printchecks(RDArsrc *r,RDArsrc *mainrsrc)
{
	...
	addDFincvir(diagrsrc,"BNKREC","BNKACC",NULL,banknum);
...
static CheckRate *GatherCurrentCheckRate(char *empid,char *payid,int jobno)
{
	...
	if(payjprm_keyno<1)
	{
		prterr("ERROR: Key Not Found - [PAYROLL][PAYJPRM] missing [RDA PAYJPRM KEY]");
		/*TRACE;*/
		payjprm_keyno=1; 
	}
...
static short AtLeastOneJob(char *empid,RangeScreen *rs,RDArsrc *mainrsrc)
{
	...
	if(payjpms_keyno<1)
	{
		prterr("ERROR: Key Not Found - [PAYROLL][PAYJPMS] missing [RDA PAYJPMS KEY]");
		/*TRACE;*/
		payjpms_keyno=1; 
	}
...
/* Key Info */
	...
	/* Set MICR Values */
	FINDFLDSETSTRING(banknum,"BANK IDENTIFICATION",bankno);
	...
	micr_check_type = 0;
	if((((micr_check_type==0)&&(micr_bottom_ypos!=0)||((micr_check_type==1)&&(micr_middle_ypos!=0))))&&((strlen(micrtext)!=0))&&(check_type==10))
	{
	...
	{	
		...
		addrfcbrsrc(rsrc,"PRINT",TRUE,printchecks,NULL);
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"PRINT PAYROLL CHECKS MICR 8.5x11")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	if(gsv!=NULL)
	{
		if(!getRDAGenericSetupbin(libx,gsv))
		{
			high_low_gsv=*gsv->value.short_value;
		} else {
			high_low_gsv=0;
		}
	}
	...
	if((check_signature==1) || (check_signature==2) || (check_signature==3))
	{
		ERRORDIALOG("Invalid Signature Types","DIMM Signature Types \"1:1\", \"1:2\", and \"A:1\" Must Be Updated To The New Format.  \nPlease Contact RDA Support.\n",NULL,TRUE);
		RDAAPPMAINLOOP();
		ShutdownSubsystems();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"ARCHIVE PAYROLL CHECKS MICR 8.5x11");
	...
	switch(startup_type)
	{
		...
		default:
			/* PLAIN TEXT CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE PAYROLL PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+3;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  2:
			/* DECORATED STUB PLAIN CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE PAYROLL DECORATED STUB PLAIN CHECKS");
			w=mainrsrc->scn->wdgts+10;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  3:
			/* MICR STUB CHECK */
			mainrsrc->scn->name=stralloc("ARCHIVE PAYROLL DECORATED STUB MICR CHECKS");
