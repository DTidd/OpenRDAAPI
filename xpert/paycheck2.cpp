...
#ifndef WIN32
#define __NAM__ "paycheck2.lnx"
#endif
#ifdef WIN32
#define __NAM__ "paycheck2.exe"
#endif
#include <app.hpp>
...
static void printactualcheck(RDA_PFILE *fp,char *outdevice)
{
	...
	if(display_ssn)
	{
		RDA_fprintf(fp,"     SSN: %-11s Name: %-25s Check: %12d Period: %-10s %-10s Date: %-10s\r\n",
			(ssn!=NULL ? ssn:""),
		...
		RDA_fprintf(fp,"          %-11s Name: %-25s Check: %12d Period: %-10s %-10s Date: %-10s\r\n",
			(" "),
			(empname!=NULL ? empname:""),checkno,
			(paystarting!=NULL ? paystarting:""),
			(payending!=NULL ? payending:""),
...
/* Insert newline if there are two leave IDs printed in a line or if there are an odd number of leave IDs printed and the loop is about to end */
			if((y%2) || ((!(y%2)) && (x==(NUMBER_LVEMSTR-1))))
			{
				RDA_fprintf(fp,"\r\n");
			}
			y++;
	...
	if(y<(NUMBER_LVEMSTR-1))
	{
		for(x=0;x<(NUMBER_LVEMSTR-1-y);x+=2)
		{
			RDA_fprintf(fp,"\r\n");
		}
	}
	...
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"%49sCurrent Gross:%10.02f - Deductions:%10.02f = Net Check:%10.02f\r\n","",total_cur_gross/100,total_cur/100,net/100);
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	...
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"%40s%-10s %12d\r\n","",(check_date!=NULL ? check_date:""),checkno);
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
	if(net>0)
	{
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"PRINT PAYROLL CHECKS TRIFOLD")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"PRINT PAYROLL CHECKS TRIFOLD");
