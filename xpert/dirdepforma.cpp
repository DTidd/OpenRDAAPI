/* dirdepforma.c - Print Payroll Direct Deposit Slips */

#ifndef WIN32
#define __NAM__ "dirdepforma.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdepforma.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
/*
	...
	short defaultsetting [] ={ 1320, 2436, 1738, 2436, 1950, 2436, 149, 2287, 530, 2904, 2954, 3004, 3124 };
*/
...
int PrintMICRCheckForm(int print_type,RDA_PFILE *fp,int checknumber,char *checkdate,int amount, char *empname, char *emp_add1,char *emp_add2,char *emp_add3,char *output_device)
{
		...
		*/
		...
		if(((print_type==0)||(print_type==2))&&(!use_full_micr_check))
		{
			...
			// do not print direct deposit number
			RDA_fprintf(fp,"\r\n");
		}
		if((print_type==1)||(print_type==3))
		{
		...
		//prterr("payto_xpos = %d", payto_xpos);
	/*
	prterr ("payto_xpos= %d, payto1_ypos= %d, payto2_ypos= %d, payto3_ypos= %d, payto4_ypos= %d",payto_xpos,payto1_ypos,payto2_ypos,payto3_ypos,payto4_ypos);
	*/
		switch(security_font_type)
		{
			...
			default:
				...
				prterr ("payto_xpos= %d, payto1_ypos= %d, payto2_ypos= %d, payto3_ypos= %d, payto4_ypos= %d",payto_xpos,payto1_ypos,payto2_ypos,payto3_ypos,payto4_ypos);
				*/
					//RDA_fprintf(fp,"%c240%d%c%c241%d%c%s",27,payto_xpos,27,27,payto1_ypos,27,"");
					RDA_fprintf(fp,"%c240%d%c%c241%d%c%s",27,payto_xpos,27,27,payto1_ypos,27,payto1);
				}
				/*SelectPrintTypeByName(output_device,"USE DEFAULT PRIMARY FONT",fp);*/
				SelectPrintTypeByName(output_device,"NORMAL TEXT",fp);
...
static void printactualcheck(RDA_PFILE *fp)
{
	...
	/*
	prterr("amount_xpos = %d , amount_ypos = %d", amount_xpos,amount_ypos);
	*/
...
if((XPERT_SETUP->LVEMGT==TRUE || XPERT_SETUP->LVSIMP==TRUE) && skip_leave==FALSE)
{
	...
	if(printstubform!=1)
	{
		RDA_fprintf(fp,"%53sYTD EARNINGS     Federal: %10.02f    State: %10.02f \r\n","",federaly/100,statey/100,ficay/100,medicarey/100);
		RDA_fprintf(fp,"%53s                    Fica: %10.02f Medicare: %10.02f \r\n","",ficay/100,medicarey/100);
		RDA_fprintf(fp,"\r\n");
	}
...
*/
	...
	*/
	...
	/*mainrsrc=RDArsrcNEW(module,"PRINT DIRECT DEPOSIT FORM 8.5x11");*/
	mainrsrc=RDArsrcNEW(module,"ARCHIVE PAYROLL DIRECT DEPOSIT FORM 8.5x11");
	...
	switch(startup_type)
	{
		...
		default:
			...
			mainrsrc->scn->name=stralloc("ARCHIVE PAYROLL PLAIN DIRECT DEPOSIT");
			w=mainrsrc->scn->wdgts+3;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  2:
			/* DECORATED STUB PLAIN DIRECT DEPOSIT */
			mainrsrc->scn->name=stralloc("ARCHIVE PAYROLL DECORATED STUB PLAIN DIRECT DEPOSIT");
			w=mainrsrc->scn->wdgts+10;
			if(w->expression!=NULL) Rfree(w->expression);
			break;
		case  3:
			/* FORM STUB DIRECT DEPOSIT */
			mainrsrc->scn->name=stralloc("ARCHIVE PAYROLL DECORATED STUB FORM DIRECT DEPOSIT");
