...
#ifndef WIN32
#define __NAM__ "dirdeptxt.lnx"
#endif
#ifdef WIN32
#define __NAM__ "dirdeptxt.exe"
#endif
#include <app.hpp>
#include <mix.hpp>
#include <fin.hpp>
...
static short PRINT_NUMBER_MESSAGE=NUMBER_MESSAGE,jdtype=0;
...
static short formleave=FALSE;
static RDArsrc *diagrsrc=NULL;
static char *message1=NULL,*message2=NULL,*message3=NULL,*filename=NULL;
...
static void printactualcheck()
{
	...
	for(x=0;x<number_jobs;++x)
	{
		...
		{
			fprintf(fp,"%-15s %9.02f %7.02f %7.02f %5.02f %8.02f %5.02f %7.02f\r\n",
				(temp!=NULL ? temp:""),gross/100,adjust/100,
				regular/100,units,overtime/100,overunits,
				cur_gross/100);
	...
	if(!outoforder)
	{
		...
		fprintf(fp,"%-27sEmployer: %7.02f %8s Employee: %7.02f %8s\r\n","",total_ecur/100,"",total_cur/100,"");
		}
	}
	fprintf(fp,"\r\n");
...
static void printchecks()
{
	...
	if((fp=fopen(filename,"w"))!=NULL)
	{
		...
		prterr("Error:  Failed to open file [%s].",(filename!=NULL ? filename:""));
		return;
	}
...
static CheckRate *GatherCurrentCheckRate(char *empid,char *payid,int jobno)
{
	...
	while(!ef)
	{
		...
		if(!delflag)
		{
			...
			{
				C->salary+=gross;
			 } else if(!RDAstrcmp(rate,"HOURLY") || !RDAstrcmp(rate,"HOURLY UNIT") || !RDAstrcmp(rate,"DAILY UNIT") || !RDAstrcmp(rate,"MISC SALARIES") || !RDAstrcmp(rate,"SUPPLEMENT") || !RDAstrcmp(rate,"MISC UNITS") || !RDAstrcmp(rate, "MISC UNIT") ||  !RDAstrcmp(rate,"DAILY") || !strncmp(rate,"MEETING",7)) /* Unit Rates */
			{
				C->units+=units;
				C->unit_amt+=gross;
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"PRINT DIRECT DEPOSIT 8.5x11")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"DIRECT DEPOSIT TEXT");
	...
	addstdrsrc(mainrsrc,"FILENAME",PLAINTEXT,40,NULL,TRUE);
