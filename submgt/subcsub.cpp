...
#ifndef WIN32
#define __NAM__ "subcsub.lnx"
#endif
#ifdef WIN32
#define __NAM__ "subcsub.exe"
#endif
#include <app.hpp>
...
static void browse_empabtr(RDArsrc *mtnrsrc)
{
	...
	file2rangersrc(subposmstr,mainrsrc);
	...
	addlstrsrc(mainrsrc,"CLASS IDENTIFICATIONS",&start,TRUE,NULL,classlist->numlibs,&classlist->libs,NULL);
...
static void makesubvalues(void)
{
	...
	if(classlist==NULL)
	{
		classlist=MakeNRDlistAllSTRING(subcls,module,
			"Creating Class Identification List",1,
			"CLASS IDENTIFICATION");
		if(classlist->numlibs<1)
		{
			addAPPlib(classlist,"No Class Identifications Available");
		}
	}
}
static void getempabtr(void)
{
	MTNMASTER=MaintainMasterNew(module,"EMPLOYEE ABSENTEE CONTACT SUBSTITUTES");
	makesubvalues();
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"CONTACT SUBSTITUTES")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
...
static void empabtrm(short dowhich,void *targetkey)
{
	...
	file2rangersrc(subposmstr,mtnrsrc);
	...
	addlstrsrc(mtnrsrc,"CLASS IDENTIFICATIONS",&start,TRUE,NULL,classlist->numlibs,&classlist->libs,NULL);
