/* subloc.c - Xpert Position Addenda Browse */
/*lint -library */
#ifndef WIN32
#define __NAM__ "subloc.lnx"
#endif
#ifdef WIN32
#define __NAM__ "subloc.exe"
#endif
#include <app.hpp>
...
static void sublocm(short,void *);
static void doexit(MakeBrowseList *),okfunc(MakeBrowseList *,void *);
static void new_record(RDArsrc *,MakeBrowseList *);
static short filenum=(-1),msrnum=(-1),dmgnum=(-1),locnum=(-1);
static char editable=TRUE;
static char *module="SUBMGT";
static MakeBrowseList *mbl_subloc=NULL;
static MaintainMaster *MTNMASTER=NULL;
static void browse_subloc(RDArsrc *,mtnstruct *);
...
static void browse_subloc(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	addDFincvir(mainrsrc,"POSTRK","POSLMST",NULL,locnum);
	addDFincvir(searchrsrc,module,"SUBLOC",NULL,filenum);
	addDFincvir(searchrsrc,"PRSNNL","PERDMG",NULL,dmgnum);
	addDFincvir(searchrsrc,"POSTRK","POSLMST",NULL,locnum);
	addDFincvir(definelist,module,"SUBLOC",NULL,filenum);
	addDFincvir(definelist,"PRSNNL","PERDMG",NULL,dmgnum);
	addDFincvir(definelist,"POSTRK","POSLMST",NULL,locnum);
	...
	if(locnum!=(-1))
	{
		...
		if(EQLNRDsec(locnum,1)) 
		{
			...
			file2rsrc(locnum,mainrsrc,FALSE);
			filerecord2rsrc(locnum,mainrsrc);
		}
	}
	...
	mbl_subloc=ADVPbrowse(filenum,MTNMASTER->passkey->keyno,
		mainrsrc,searchrsrc,definelist,doexit,okfunc,
		new_record,NULL,FALSE,NULL,0,0,2,0,MTNMASTER->passkey);
}
int main(int argc,char **argv)
{
	if(InitializeSubsystems(argc,argv,module,"MTN SUBLOC")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	if((filenum=APPOPNNRDEDITABLE(module,"SUBLOC",&editable))==(-1)) return;
	...
	MTNMASTER=MaintainMasterNew(module,"MTN SUBLOC");
	if(findMaintainMaster(MTNMASTER))
	{
		...
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN SUBLOC",module);
		doexit(NULL);
	} else {
...
static void sublocm(short dowhich,void *targetkey)
{
	...
	FINDRSCSETFUNC(mtnrsrc,"[SUBLOC][PERSONNEL IDENTIFICATION]",getrecordcb,
		mtn);
	FINDRSCSETFUNC(mtnrsrc,"[SUBLOC][LOCATION IDENTIFICATION]",getrecordcb,
		mtn);
	...
	addDFincvir(mtnrsrc,module,"SUBLOC",NULL,filenum);
	...
	if(mbl_subloc==NULL) 
	{
		addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_subloc,mtn);
	}
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,mtn,(mbl_subloc==NULL?TRUE:FALSE));
}
...
static void save_check(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	char *perid1=NULL,*locid1=NULL;
	short return_value=FALSE;

	readallwidgets(mtnrsrc);
	FINDRSCGETCHAR(mtnrsrc,"[SUBLOC][DELETEFLAG]",&delflag);
	FINDRSCGETSTRING(mtnrsrc,"[SUBLOC][PERSONNEL IDENTIFICATION]",&perid1);
	FINDRSCGETSTRING(mtnrsrc,"[SUBLOC][LOCATION IDENTIFICATION]",&locid1);
	if(!delflag)
	{
...
static void save_record(RDArsrc *mtnrsrc,mtnstruct *mtn,short update_list)
{
	...
	if(RECORDsec(filenum))
	{
		...
		updatebrowse(update_list,mbl_subloc,filenum,module,mtnrsrc);
		if(mbl_subloc!=NULL) quit_record(mtnrsrc,mtn);
...
static void getrecordcb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDFLDSETSTRING(filenum,"LOCATION IDENTIFICATION",locid1);
