/* ubcsub1.c - Substitution Managment's Contact Substitutes */
#include <cstdio>
		...
		while(!ef)
		{
			...
			if(active && !deleteflag)
			{
				...
				if(!EQLNRDsec(subposmstr,1))
				{
					...
					if(!deleteflag && !RDAstrcmp(class_identification,classid2))
					{
...
static void makeDBsortfile(DBsort *dbsort)
{
	...
	if(FIELDS!=NULL)
	{
		...
		addDBfield(dbsort,"STARTING DATE",DATES,10);
		addDBfield(dbsort,"ENDING DATE",DATES,10);
	}
...
void select_subs(RDArsrc *eabsmtnrsrc)
{
	...
	int subjobno=0,transno=(-1),jobno=(-1),selclass=0;
	...
	empabtrrsrc=eabsmtnrsrc;
        FINDRSCGETINT(empabtrrsrc,"CLASS IDENTIFICATIONS",&selclass);
	...
	FINDFLDGETSTRING(poslmst,"LOCATION IDENTIFICATION",&locid);
        if(RDAstrcmp(classlist->libs[selclass],"No Class Identifications Available"))
        {
                class_identification=stralloc(classlist->libs[selclass]);
        } else class_identification=NULL;
	...
	tmprsrc=diagscrn(submst,"DIAGNOSTIC SCREEN",module,
	...
	addDFincvir(tmprsrc,"POSTRK","POSMSTR",NULL,subposmstr);
		...
		if(!deleteflag && RDAstrcmp(perid,subperid) && !testfile)
		{
			...
			for(ef2=GTENRDsec(subjobmst,1);!ef2;ef2=NXTNRDsec(subjobmst,1))
			{
				...
				if(!deleteflag && !testfile)
				{
					...
					{
						FINDFLDGETCHAR(subposmstr,"DELETEFLAG",&deleteflag);
						testfile=testfilerangersrc(subposmstr,rs);
						if(!deleteflag && !testfile)
						{
							FINDFLDGETSTRING(subposmstr,"CLASS IDENTIFICATION",&classid2);
							if(!RDAstrcmp(class_identification,classid2))
							{
...
*/
								...
								if(diagapps)
								{
								prterr("Skipped because CLASS IDENTIFICATION not for correct type for substitute.");KEYNRD(subposmstr,1);SEENRDRECORD(subposmstr);
								}
#endif
							}
						} else {
							...
							if(diagapps)
							{
							prterr("Skipped by Ranging on Position Identification Master or a deleted Position Identification Master Record.");KEYNRD(subposmstr,1);SEENRDRECORD(subposmstr);
							}
#endif
						}
					} else {
	...
	if(size_of_file>0)
	{
		MTNMASTER1=MaintainMasterNew(module,"CONTACT SUBSTITUTES");
