...
/*
	memset(stemp,0,101);
	sprintf(stemp,"ACCOUNT CODE MINUS 1 PART VIR");
	...
	d->num=3;
	d->type=PLAINTEXT;
	d->length=accminus1_longest;
	d->when=0;
	d->avail2rsrc=TRUE;
	if(tmplabel5!=NULL)
	{
		d->expression=stralloc(tmpemp);
		Rfree(tmpemp);
	} else d->expression=NULL;
	memset(temp,0,15001);
	sprintf(temp,"Represents Account Code minus last account part from the Account Structure representing all account types and definitions using the Virtual Fields instead of the actual database files.");
...
*/
	...
	char *tmplabel2=NULL,*tmplabel3=NULL,*tmplabel4=NULL;
/*
	char *tmplabel5=NULL;
*/
	int field_length=0;
...
/*
			...
			if(part!=NULL)
			{
				if(RDAstrlen(part->name)>part->length)
				{
					length=RDAstrlen(part->name);
				} else {
					length=part->length;
				}
				if(length>longest_length) longest_length=length;
				memset(temp,0,15001);
				sprintf(temp,"IF([DEFINITION TYPE]=%d) THEN RETURN_VALUE=[ACCOUNT CODE][1,%d]",z,(length+1));
				if_count+=1;
				if(tmpexp!=NULL)
				{
					tmpexp=Rrealloc(tmpexp,
			...
			if(EXPENDITURE_ACCOUNT->codetype!=NULL)
			{
					...
					y<EXPENDITURE_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL && acc->num>x)
					{
						...
						{
							sprintf(temp,"IF([FINMGT][FINEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINEACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,part->length,x+1);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([FINMGT][FINEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINEACC][ACCOUNT CODE][1,%d,%d]+\"%s\"+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(pad_string!=NULL?pad_string:""),(x+1));
							} else {
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([FINMGT][FINEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINEACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,part->length);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([FINMGT][FINEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINEACC][ACCOUNT CODE][1,%d,%d]+\"%s\"\n",y,(x+1),acc_count,endlength,(pad_string!=NULL?pad_string:""));
							} else {
			...
			if(REVENUE_ACCOUNT->codetype!=NULL)
			{
					...
					y<REVENUE_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL && acc->num>x)
					{
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([FINMGT][FINRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINRACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,part->length,x+1);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([FINMGT][FINRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINRACC][ACCOUNT CODE][1,%d,%d]+\"%s\"+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(pad_string!=NULL?pad_string:""),(x+1));
							} else {
								sprintf(temp,"IF([FINMGT][FINRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINRACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(x+1));
							}
							if(pad_string!=NULL) Rfree(pad_string);
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([FINMGT][FINRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINRACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,part->length);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([FINMGT][FINRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINRACC][ACCOUNT CODE][1,%d,%d]+\"%s\"\n",y,(x+1),acc_count,endlength,(pad_string!=NULL?pad_string:""));
							} else {
								sprintf(temp,"IF([FINMGT][FINRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINRACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,endlength);
							}
							if(pad_string!=NULL) Rfree(pad_string);
			...
			if(BALANCE_SHEET_ACCOUNT->codetype!=NULL)
			{
					...
					y<BALANCE_SHEET_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL && acc->num>x)
					{
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINBACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,part->length,x+1);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINBACC][ACCOUNT CODE][1,%d,%d]+\"%s\"+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(pad_string!=NULL?pad_string:""),(x+1));
							} else {
								sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINBACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,x+1);
							}
							if(pad_string!=NULL) Rfree(pad_string);
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINBACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,part->length);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINBACC][ACCOUNT CODE][1,%d,%d]+\"%s\"\n",y,(x+1),acc_count,endlength,(pad_string!=NULL?pad_string:""));
							} else {
								sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[FINMGT][FINBACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,endlength);
							}
							if(pad_string!=NULL) Rfree(pad_string);
			...
			if(BALANCE_SHEET_ACCOUNT->num>0)
			{
					...
					y<BALANCE_SHEET_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL && acc->num>0)
					{
						...
						memset(temp,0,15001);
/*
						sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN RETURN_VALUE=[ACCOUNT CODE][%d,%d]\n",y,acc_count,part->length);
*/
						...
						memset(temp,0,15001);
/*
						sprintf(temp,"IF([FINMGT][FINBACC][DEFINITION TYPE]=%d) THEN RETURN_VALUE=[ACCOUNT CODE][1,%d]\n",y,acc_count);
*/
...
/*
	memset(temp,0,15001);
	sprintf(temp,"RETURN_VALUE=\"\"");
	tmplabel5=Rrealloc(tmplabel5,label5_count+RDAstrlen(temp)+16);
	sprintf(&tmplabel5[label5_count],"ELSE %s FI FI FI\n",temp);
*/
...
/*
	...
	if(tmplabel5!=NULL)
	{
		d->expression=stralloc(tmplabel5);
		Rfree(tmplabel5);
	} else d->expression=NULL;
	memset(temp,0,15001);
	sprintf(temp,"Represents Account Code minus last account part from the Account Structure representing all account types and definitions using the Virtual Fields instead of the actual database files.");
			...
			for(x=0;x<no_parts;++x)
			{
					...
					y<EXPENDITURE_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL)
					{
						...
						for(z=0,p=acc->acct;z<x;++z,++p) 
							acc_count+=p->length+1;
						sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN RETURN_VALUE=[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d] ",y,acc_count,part->length);
						if_count+=1;
						if(tmpexp!=NULL)
						{
							tmpexp=Rrealloc(tmpexp,
			...
			for(x=0;x<no_parts;++x)
			{
					...
					y<REVENUE_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL)
					{
						...
						for(z=0,p=acc->acct;z<x;++z,++p) 
							acc_count+=p->length+1;
						sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN RETURN_VALUE=[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d] ",y,acc_count,part->length);
						if_count+=1;
						if(tmpexp!=NULL)
						{
							tmpexp=Rrealloc(tmpexp,
			...
			if(EXPENDITURE_ACCOUNT->codetype!=NULL)
			{
					...
					y<EXPENDITURE_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL && acc->num>x)
					{
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,part->length,x+1);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d]+\"%s\"+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(pad_string!=NULL?pad_string:""),(x+1));
							} else {
								sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(x+1));
							}
							if(pad_string!=NULL) Rfree(pad_string);
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,part->length);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d]+\"%s\"\n",y,(x+1),acc_count,endlength,(pad_string!=NULL?pad_string:""));
							} else {
								sprintf(temp,"IF([BUDPREP][BUDEACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[BUDPREP][BUDEACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,endlength);
							}
							if(pad_string!=NULL) Rfree(pad_string);
			...
			if(REVENUE_ACCOUNT->codetype!=NULL)
			{
					...
					y<REVENUE_ACCOUNT->num;++y,++acc)
				{
					if(acc->acct!=NULL && acc->num>x)
					{
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,part->length,x+1);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d]+\"%s\"+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(pad_string!=NULL?pad_string:""),(x+1));
							} else {
								sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d]+\" \"+[FINMGT][FINCOA][DESCRIPTION][%d,1,40]\n",y,acc_count,endlength,(x+1));
							}
							if(pad_string!=NULL) Rfree(pad_string);
						...
						if(endlength>=part->length || endlength==0)
						{
							sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,part->length);
						} else {
							pad_string=padstr((part->length-endlength),'0');
							if(!isEMPTY(pad_string))
							{
								sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d]+\"%s\"\n",y,(x+1),acc_count,endlength,(pad_string!=NULL?pad_string:""));
							} else {
								sprintf(temp,"IF([BUDPREP][BUDRACC][DEFINITION TYPE]=%d) THEN\n\t\tRETURN_VALUE=[FINMGT][FINCOA][DESCRIPTION][%d,1,40]+\" \"+[BUDPREP][BUDRACC][ACCOUNT CODE][1,%d,%d]\n",y,(x+1),acc_count,endlength);
							}
							if(pad_string!=NULL) Rfree(pad_string);
