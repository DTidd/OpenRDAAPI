/* miscintm.c - Misc Billing Interest Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
extern short filenum,raccnum,baccnum;
...
MakeBrowseList *mbl_miscint=NULL;
extern MaintainMaster *MASTER;
void miscintm(short,void *targetkey);
...
static APPlib *makefieldvallist()
{
	...
	if(libx!=NULL) Rfree(libx);
	addAPPlibNoDuplicates(tmp,"[PAYMENT DATE]");
...
static void chkaccountcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(filenum,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]",&acctcode);
	FINDFLDSETSTRING(filenum,"DEFAULT ACCOUNT CODE",acctcode);
...
static short chkaccountcode(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FIELDCOPY(filenum,"DEFAULT DEFINITION TYPE",raccnum,"DEFINITION TYPE");
	FINDFLDGETSTRING(filenum,"DEFAULT ACCOUNT CODE",&acctcode);
...
static void chkrevenuecodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(filenum,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]",&acctcode);
	FINDFLDSETSTRING(filenum,"DEFAULT REVENUE CODE",acctcode);
	if(acctcode!=NULL) Rfree(acctcode);
...
static short chkrevenuecode(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FIELDCOPY(filenum,"DEFAULT DEFINITION TYPE",baccnum,"DEFINITION TYPE");
	FINDFLDGETSTRING(filenum,"DEFAULT REVENUE CODE",&acctcode);
...
static void chkcashcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(filenum,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]",&acctcode);
	FINDFLDSETSTRING(filenum,"DEFAULT CASH CODE",acctcode);
	if(acctcode!=NULL) Rfree(acctcode);
...
static short chkcashcode(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FIELDCOPY(filenum,"DEFAULT DEFINITION TYPE",baccnum,"DEFINITION TYPE");
	FINDFLDGETSTRING(filenum,"DEFAULT CASH CODE",&acctcode);
	FINDFLDSETSTRING(baccnum,"ACCOUNT CODE",acctcode);
	if(ADVEQLNRDsec(baccnum,1,SCRNvirtualSubData,mtnrsrc))
	{
		KEYNRD(baccnum,1);
...
static void setlists(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDFLDGETSHORT(filenum,"DEFAULT DEFINITION TYPE",&deftype);
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]",deftype);
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"REVENUE CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CASH CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]",deftype2);
		updatersrc(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]");
		updatersrc(mtnrsrc,"ACCOUNT CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]");
		updatersrc(mtnrsrc,"REVENUE CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]");
		updatersrc(mtnrsrc,"CASH CODE DESCRIPTION");
	}
	updateSCRNvirtuals(mtnrsrc);
}
...
void miscintm(short dowhich,void *targetkey)
{
	...
	FINDRSCSETFUNC(mtnrsrc,"[MISCINT][INTEREST IDENTIFICATION]",getrecord,mtn);
	nofields=0;
	FINDRSCSETFUNC(mtnrsrc,"[MISCINT][DEFAULT ACCOUNT CODE]",chkaccountcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[MISCINT][DEFAULT REVENUE CODE]",chkrevenuecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[MISCINT][DEFAULT CASH CODE]",chkcashcodecb,mtn);
	...
	addrfcbrsrc(mtnrsrc,"QUIT",TRUE,quit_recordcb,mtn);
...
static void readscreen(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDFLDSETSHORT(filenum,"DEFAULT DEFINITION TYPE",deftype);
}
	...
	short update_list)
{
	...
	FINDRSCGETCHAR(mtnrsrc,"[MISCINT][DELETEFLAG]",&delflag);
	if(!delflag)
	{
	...
	short update_list)
{
	...
	if(ADVRECORDsec(filenum,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_miscint,filenum,module,mtnrsrc);
		if(mbl_miscint!=NULL) quit_record(mtnrsrc,mtn);
...
static void getrecord(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[MISCINT][INTEREST IDENTIFICATION]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCINT][INTEREST IDENTIFICATION]",&intid);
	FINDFLDSETSTRING(filenum,"INTEREST IDENTIFICATION",intid);
