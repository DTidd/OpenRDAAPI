/* miscint.c - Misc Billing Interest Master Program*/
/*lint -library */
#ifndef WIN32
#define __NAM__ "miscint.lnx"
#endif
#ifdef WIN32
#define __NAM__ "miscint.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rdaflds.h>
...
extern MakeBrowseList *mbl_miscint;
extern void miscintm(short,void *);
static void doexit(MakeBrowseList *),okfunc(MakeBrowseList *,void *);
static void new_record(RDArsrc *,MakeBrowseList *);
short filenum=0,raccnum=0,baccnum=0;
char editable=TRUE;
...
void browse_msr(RDArsrc *parent,mtnstruct *mtn)
{
	...
	mbl_miscint=ADVPbrowse(filenum,MASTER->passkey->keyno,mainrsrc,
		searchrsrc,definelist,doexit,okfunc,new_record,
		NULL,FALSE,NULL,0,0,2,0,MASTER->passkey);
}
void main(int argc,char **argv)
{
	if(InitializeSubsystems(argc,argv,module,"MTN MISCINT")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	MASTER=MaintainMasterNew(module,"MTN MISCINT");
	if(findMaintainMaster(MASTER))
	{
		ERRORDIALOG("MAINTAIN MASTER NOT FOUND.","The Maintain Master function wasn't found.",NULL,TRUE); 
		prterr("Error Maintain Master Definition not found, this process cannot be executed without it's definition [%s] in the [%s.MTN] library.","MTN MISCINT",module);
		doexit(NULL);
	} else {
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_miscint!=NULL) mbl_miscint=NULL;
}
static void okfunc(MakeBrowseList *blist,void *targetkey)
{
	...
	{
		if(!run_func) miscintm(0,targetkey);
			else ExecuteListFunc(run_func-1,targetkey,MASTER);
	} else miscintm(0,targetkey);
}
