/* miscpenm.c - Misc Billing Penalty Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <alloc.h>
...
MakeBrowseList *mbl_miscpen=NULL;
extern MaintainMaster *MASTER;
void miscpenm(short,void *targetkey);
...
static void chkaccountcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]",&acctcode);
...
static void chkrevenuecodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]",&acctcode);
...
static void chkcashcodecb(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	d=x;
	FINDFLDSETSHORT(MASTER->passkey->fileno,"DEFAULT DEFINITION TYPE",d);
	readwidget(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]",&acctcode);
...
static void setlists(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	FINDRSCSETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]",deftype);
	FINDRSCSETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]",deftype);
...
static void changeDEFtype(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]");
	FINDRSCGETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]",&deftype1);
	FINDRSCGETINT(mtnrsrc,"DEFINITION TYPES",&seldeftype2);
	deftype2=(short)seldeftype2;
	if(deftype1!=deftype2)
	{
		FINDRSCSETSTRING(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"ACCOUNT CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"REVENUE CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]",deftype2);
		FINDRSCSETSTRING(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]",NULL);
		FINDRSCSETSTRING(mtnrsrc,"CASH CODE DESCRIPTION",NULL);
		FINDRSCSETDEFTYPE(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]",deftype2);
		updatersrc(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]");
		updatersrc(mtnrsrc,"ACCOUNT CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]");
		updatersrc(mtnrsrc,"REVENUE CODE DESCRIPTION");
		updatersrc(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]");
		updatersrc(mtnrsrc,"CASH CODE DESCRIPTION");
	}
	updateSCRNvirtuals(mtnrsrc);
}
void miscpenm(short dowhich,void *targetkey)
{
	...
	nofields=0;
	FINDRSCSETFUNC(mtnrsrc,"[MISCPEN][PENALTY IDENTIFICATION]",getrecord,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[MISCPEN][DEFAULT ACCOUNT CODE]",chkaccountcodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[MISCPEN][DEFAULT REVENUE CODE]",chkrevenuecodecb,mtn);
	FINDRSCSETFUNC(mtnrsrc,"[MISCPEN][DEFAULT CASH CODE]",chkcashcodecb,mtn);
	...
	short update_list)
{
	...
	FINDRSCGETCHAR(mtnrsrc,"[MISCPEN][DELETEFLAG]",&delflag);
	if(!delflag)
	{
	...
	short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_miscpen,MASTER->passkey->fileno,module,mtnrsrc);
		if(mbl_miscpen!=NULL) quit_record(mtnrsrc,mtn);
...
static void getrecord(RDArsrc *mtnrsrc,mtnstruct *mtn)
{
	...
	readwidget(mtnrsrc,"[MISCPEN][PENALTY IDENTIFICATION]");
	FINDRSCGETSTRING(mtnrsrc,"[MISCPEN][PENALTY IDENTIFICATION]",&pen);
	FINDFLDSETSTRING(MASTER->passkey->fileno,"PENALTY IDENTIFICATION",pen);
