/* miscaint.c - Apply Interest */
#ifndef WIN32
#define __NAM__ "miscaint.lnx"
#endif
#ifdef WIN32
#define __NAM__ "miscaint.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rptgen.h>
...
APPlib *makefieldvallist()
{
	...
	addDFincvir(tmprsrc,module,"MISCINT",NULL,dumb);
	...
	if(ADVEQLNRDsec(rstci,1,ApplySubData,p))
	{
		...
		if(deleteflag)
		{
			...
			if(!compute_when)
			{
				if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
				tmp2=Rmalloc(RDAstrlen(refid)+200);
				sprintf(tmp2,"The Reference Identification [%s] is found, but is not to compute during APPLY INTEREST in the Misc Billing Interest Master.",refid);
...
void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("MISCBILL","APPLY INTEREST SORT SEQUENCE");
	if(d!=NULL)
	{
				...
				if(!testfile)
				{
					...
					if(etest)
					{
						...
						if(x>=mainrsrc->num)
						{
							...
							dodiag=TRUE;
							FINDFLDGETSTRING(rstci,"INTEREST FORMULA",
								&exprssn);
							AMOUNT=0.0;
							if(exprssn!=NULL)
							{
...
void main(int argc,char **argv)
{
	...
	if((rstci=APPOPNNRD(module,"MISCINT",TRUE,FALSE))==(-1)) return;
	...
	addDFincvir(mainrsrc,module,"MISCINT",NULL,rstci);
...
void applym(short dowhich,void *targetkey,DBsort *t)
{
	...
	addDFincvir(mtnrsrc,module,"MISCINT",NULL,rstci);
