/* miscmstrm.c - MISCMSTR Maintain Screen */
#include <stdio.h>
#include <rdaglob.h>
#include <rdaflds.h>
#include <mix.h>
...
void mtnutlmstr(short dowhich,void *targetkey)
{
	...
	FINDRSCSETEDITABLE(mtnrsrc,"[MISCMSTR][UNIT AMOUNT]",FALSE);
	FINDRSCSETEDITABLE(mtnrsrc,"[MISCMSTR][RATE AMOUNT]",FALSE);
	FINDRSCSETEDITABLE(mtnrsrc,"[MISCMSTR][INTEREST AMOUNT]",FALSE);
	FINDRSCSETEDITABLE(mtnrsrc,"[MISCMSTR][PENALTY AMOUNT]",FALSE);
	FINDRSCSETEDITABLE(mtnrsrc,"[MISCMSTR][PAID AMOUNT]",FALSE);
	if(!editable)
	{
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	int tran=0,tran1=0,yearid=0,yearid1=0,yearid2=0,fyear=0;
	RDArsrc *tmprsrc=NULL;
	RDATData *prev=NULL;
	double units=0.0,amount=0.0;
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		if(deleteflag)
		{
			FINDFLDGETSTRING(MASTER->passkey->fileno,"MISC IDENTIFICATION",&realestateid);
			FINDFLDGETSTRING(MASTER->passkey->fileno,"CLASS IDENTIFICATION",&classid);
			...
			trannum=OPNNRDsec(MASTER->module,"MISCTRAN",TRUE,TRUE,FALSE);
			paidnum=OPNNRDsec(MASTER->module,"MISCPAY",TRUE,TRUE,FALSE);
			...
			addDFincvir(tmprsrc,MASTER->module,"MISCTRAN",NULL,trannum);
			addDFincvir(tmprsrc,MASTER->module,"MISCPAY",NULL,paidnum);
			...
			{
				ZERNRD(trannum);
				FINDFLDSETSTRING(trannum,"MISC IDENTIFICATION",realestateid);
				FINDFLDSETSTRING(trannum,"CLASS IDENTIFICATION",classid);
				...
				while(!ef_tran)
				{
					FINDFLDGETSTRING(trannum,"MISC IDENTIFICATION",&realestateid1);
					FINDFLDGETSTRING(trannum,"CLASS IDENTIFICATION",&classid1);
					FINDFLDGETSTRING(trannum,"OWNER IDENTIFICATION",&ownid1);
					FINDFLDGETINT(trannum,"TAX YEAR",&yearid1);
					if(RDAstrcmp(realestateid,realestateid1) || RDAstrcmp(ownid,ownid1) || RDAstrcmp(classid,classid1) || yearid!=yearid1) break;
					...
					if(!tdeleteflag)
					{
						...
						FINDFLDGETDOUBLE(trannum,"UNIT AMOUNT",&units);
						ADVUpdateMiscBillingTransactions(MASTER->passkey->fileno,trannum,-units,-amount,SCRNvirtualSubData,mtnrsrc);
						...
						{
							ZERNRD(paidnum);
							FINDFLDSETSTRING(paidnum,"MISC IDENTIFICATION",realestateid);
							FINDFLDSETSTRING(paidnum,"CLASS IDENTIFICATION",classid);
							...
							while(!ef_paid)
							{
								...
								if(RDAstrcmp(realestateid1,realestateid2) || RDAstrcmp(ownid1,ownid2) || RDAstrcmp(classid1,classid2) || yearid1!=yearid2 || tran!=tran1) break;
								...
								if(!vdeleteflag)
								{
									...
									if(dist==FALSE)
									{
										...
										if(!type)
										{
											ADVupdateundist(trannum,NULL,"CASH CODE","REVENUE CODE",NULL,ryrnum,byrnum,fyear,deftype,"REVENUE",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,tmprsrc);
										} else {
											ADVupdateundist(trannum,"ACCOUNT CODE","CASH CODE","REVENUE CODE",NULL,ryrnum,byrnum,fyear,deftype,"REVENUE",FISCALPERIOD[p_month],-amount,SCRNvirtualSubData,tmprsrc);
										}
									}
									ADVUpdateMiscBillingPayments(taxdwr,MASTER->passkey->fileno,trannum,paidnum,-amount,SCRNvirtualSubData,tmprsrc);
