/* miscapen.c - Apply Penalties */
#ifndef WIN32
#define __NAM__ "miscapen.lnx"
#endif
#ifdef WIN32
#define __NAM__ "miscapen.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rptgen.h>
...
APPlib *makefieldvallist()
{
	...
	addDFincvir(tmprsrc,module,"MISCPEN",NULL,dumb);
...
static short checktrantype(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	FINDFLDSETSTRING(rstci,"PENALTY IDENTICATION",refid);
	p=Rmalloc(sizeof(PassableStruct));
	p->r=mtnrsrc;
	p->a=dbsort;
	if(ADVEQLNRDsec(rstci,1,ApplySubData,p))
	{
		...
		if(deleteflag)
		{
			...
			if(!compute_when)
			{
				if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
				tmp2=Rmalloc(RDAstrlen(refid)+200);
				sprintf(tmp2,"The Reference Identification [%s] is found, but is not to compute during APPLY PENALTY in the Misc Billing Interest Master.",refid);
...
{
	short ef=FALSE,update_diag=FALSE,trantype=1;
...
void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("MISCBILL","APPLY PENALTY SORT SEQUENCE");
	if(d!=NULL)
	{
	...
	while(!ef)
	{
		...
		if(!testfile)
		{
			...
			while(!ef1)
			{
				...
				if(!testfile)
				{
					FINDFLDGETSTRING(rstci,"PENALTY IDENTIFICATION",&interestid);
					...
					if(etest)
					{
						...
						if(x>=mainrsrc->num)
						{
							...
							dodiag=TRUE;
							FINDFLDGETSTRING(rstci,"PENALTY FORMULA",
								&exprssn);
							AMOUNT=0.0;
							if(exprssn!=NULL)
							{
							...
							{
								prterr("Skipped because PENALTY FORMULA Evaluated to ZERO ($0.00).");SEENRDRECORD(rstci);
							}
						} else if(diagapps)
						{
	...
	if(size_of_file>0)
	{
		switch(apply_type)
		{
			...
			case 1: /* Process Exceptions */
				...
				brsrc=RDArsrcNEW(module,"APPLY PENALTY BROWSE");
				srsrc=RDArsrcNEW(module,"APPLY PENALTY SEARCH BROWSE");
				drsrc=RDArsrcNEW(module,"APPLY PENALTY DEFINE LIST");
...
void main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,module,"APPLY PENALTIES")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	} else reportname=stralloc("APPLYPEN");
	fmgt=RDAfinmgtNEW();
	...
	if((rstci=APPOPNNRD(module,"MISCPEN",TRUE,FALSE))==(-1)) return;
	...
	makeapplytype();
	mainrsrc=RDArsrcNEW(module,"APPLY PENALTIES");
	addDFincvir(mainrsrc,module,"MISCMSTR",NULL,pptmstr);
	addDFincvir(mainrsrc,module,"MISCPEN",NULL,rstci);
...
void applym(short dowhich,void *targetkey,DBsort *t)
{
	...
	mtnrsrc=RDArsrcNEW(module,"APPLY PENALTY MAINTAIN SCREEN");
	addDFincvir(mtnrsrc,module,"MISCMSTR",NULL,pptmstr);
	addDFincvir(mtnrsrc,module,"MISCPEN",NULL,rstci);
...
static void getsupportingfiles(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	p->r=mtnrsrc;
	p->a=dbsort;
	COPYFIELD(dbsort->fileno,rstci,"PENALTY IDENTIFICATION");
	...
	FINDFLDGETSTRING(rstci,"PENALTY FORMULA",&exprssn);
	AMOUNT=0.0;
	if(exprssn!=NULL)
	{
		AMOUNT=EVALUATEdbl(exprssn,ApplySubData,p);
