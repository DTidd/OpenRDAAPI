/* miscarat.c - Apply Class Rates */
#ifndef WIN32
#define __NAM__ "miscarat.lnx"
#endif
#ifdef WIN32
#define __NAM__ "miscarat.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rptgen.h>
...
static char *module="MISCBILL",**applytype=NULL,*reportname=NULL;
static char *apply_expression=NULL,**monthlist=NULL;
static short pptmstr=(-1),rstci=(-1);
static short COUNT=FALSE;
static short baccnum=(-1),raccnum=(-1),ryrnum=(-1),byrnum=(-1);
static short miscclas=(-1),rstrn=(-1),fsclyr=(-1);
...
APPlib *makefieldvallist()
{
	...
	list=APPlibNEW();
	AddFileFields(list,pptmstr);
	AddFileFields(list,miscclas);
	...
	sprintf(libx,"%s/rda/MISCBILL.VIR",CURRENTDIRECTORY);
...
static void changetrantype(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	readwidget(mtnrsrc,"[MISCMSTR][CLASS IDENTIFICATION]");
...
static short checktrantype(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	if(ADVEQLNRDsec(rstci,1,ApplySubData,p))
	{
		KEYNRD(rstci,1);
		if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
		tmp2=Rmalloc(RDAstrlen(refid)+200);
		sprintf(tmp2,"The Reference Identification [%s] is not found in the Misc Billing Interest Master.",refid);
		...
		if(deleteflag)
		{
			if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
			tmp2=Rmalloc(RDAstrlen(refid)+200);
			sprintf(tmp2,"The Reference Identification [%s] is found, but is marked for deletion in the Misc Billing Interest Master.",refid);
			...
			if(!compute_when)
			{
				if(tmp2!=NULL) { Rfree(tmp2); tmp2=NULL; }
				tmp2=Rmalloc(RDAstrlen(refid)+200);
				sprintf(tmp2,"The Reference Identification [%s] is found, but is not to compute during APPLY RATE in the Misc Billing Interest Master.",refid);
...
void apply_backend(DBsort *dbsort)
{
	...
	if(dbsort!=NULL)
	{
		...
		addDFincvir(tmprsrc,module,"MISCMSTR",NULL,pptmstr);
		addDFincvir(tmprsrc,module,"MISCCLAS",NULL,miscclas);
		addDFincvir(tmprsrc,module,"MISCTRAN",NULL,rstrn);
		...
		while(!ef)
		{
			...
			if(!deleteflag && amount!=0.0)
			{
				...
				COPYFIELD(dbsort->fileno,miscclas,"CLASS IDENTIFICATION");
				if(ADVEQLNRDsec(miscclas,1,ApplySubData,p)) KEYNRD(miscclas,1);
				COPYFIELD(dbsort->fileno,pptmstr,"MISC IDENTIFICATION");
				COPYFIELD(dbsort->fileno,pptmstr,"CLASS IDENTIFICATION");
				COPYFIELD(dbsort->fileno,pptmstr,"OWNER IDENTIFICATION");
				if(ADVEQLNRDsec(pptmstr,1,ApplySubData,p)) KEYNRD(pptmstr,1);
				COPYFIELD(dbsort->fileno,rstrn,"MISC IDENTIFICATION");
				COPYFIELD(dbsort->fileno,rstrn,"CLASS IDENTIFICATION");
				...
				ADVUpdateMiscBillingTransactions(pptmstr,rstrn,0.0,amount,ApplySubData,p);
...
void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("MISCBILL","APPLY RATE SORT SEQUENCE");
	if(d!=NULL)
	{
	...
	if(fields!=NULL)
	{
		...
		{
			if(strncmp(field->name,"UPDATED",7) &&
				RDAstrcmp(field->name,"OUTSTANDING AMOUNT") &&
...
void addsortrecord(DBsort *dbsrt,char *classid,char *realestateid,
	char *ownerid,int TaxYear,
	char *interestid,double gross,short deftype,char *acctcode,
	char *revcode,char *cashcode,char *desc,PassableStruct *p)
{
	...
	FINDFLDSETSTRING(dbsrt->fileno,"MISC IDENTIFICATION",realestateid);
	FINDFLDSETSTRING(dbsrt->fileno,"CLASS IDENTIFICATION",classid);
...
static void applyinterest(RDArsrc *mainrsrc)
{
	...
	short deftype=0;
	short testfile1=FALSE;
	char *classid=NULL,*ownerid=NULL,*exprssn=NULL;
	...
	ReadRangeScreen(pptmstr,rs);
	ReadRangeScreen(miscclas,rs);
	ReadRangeScreen(rstci,rs);
	tmprsrc=diagscrn(pptmstr,"DIAGNOSTIC SCREEN",module,
		"Selecting Misc Billing.....",NULL);
	if(tmprsrc!=NULL)
	{
		if(!APPmakescrn(tmprsrc,TRUE,NULL,NULL,FALSE))
			ForceWindowUpdate(tmprsrc);
	...
	p->a=dbsort;
	ZERNRD(pptmstr);
	ef=ADVFRSNRDsec(pptmstr,1,ApplySubData,p);
	while(!ef)
	{
		dodiag=FALSE;
		FINDFLDGETCHAR(pptmstr,"DELETEFLAG",&deleteflag);
		testfile1=FALSE;
		if(!deleteflag)
		{
			testfile=testfilerangersrc(pptmstr,rs);
			COPYFIELD(pptmstr,miscclas,"CLASS IDENTIFICATION");
			if(ADVEQLNRDsec(miscclas,1,ApplySubData,p)) KEYNRD(miscclas,1);
			testfile1=testfilerangersrc(miscclas,rs);
		} else testfile=TRUE;
		if(!testfile && !testfile1)
		{
			FINDFLDGETSTRING(pptmstr,"MISC IDENTIFICATION",
				&realestateid);
			FINDFLDGETSTRING(pptmstr,"CLASS IDENTIFICATION",&classid);
			FINDFLDGETSTRING(pptmstr,"OWNER IDENTIFICATION",&ownerid);
			FINDFLDGETINT(pptmstr,"FISCAL YEAR",&fyear);
			ZERNRD(rstci);
			...
			while(!ef1)
			{
				...
				if(!testfile)
				{
					FINDFLDGETSTRING(rstci,"RATE IDENTIFICATION",&interestid);
					if(!isEMPTY(apply_expression))
					{
						etest=EVALUATEbol(apply_expression,ApplySubData,p);
					} else etest=TRUE;
					if(etest)
					{
						for(x=0,v=mainrsrc->virtual;x<mainrsrc->num;
							++x,++v)
						{
						...
						if(x>=mainrsrc->num)
						{
							...
							dodiag=TRUE;
							FINDFLDGETSTRING(rstci,"RATE FORMULA",
								&exprssn);
							AMOUNT=0.0;
							if(exprssn!=NULL)
							{
							...
							if(AMOUNT!=0.0)
							{
								...
								addsortrecord(dbsort,classid,realestateid,ownerid,fyear,interestid,AMOUNT,deftype,acctcode,revcode,cashcode,desc,p);
								if(acctcode!=NULL) Rfree(acctcode);
							...
							{
								prterr("Skipped because RATE FORMULA Evaluated to ZERO ($0.00).");SEENRDRECORD(rstci);
							}
						} else if(diagapps)
						{
		...
		{
			prterr("Skipped by Ranging on Misc Billing Master or deleted personal property master.");KEYNRD(pptmstr,1);SEENRDRECORD(pptmstr);
		}
		if(tmprsrc!=NULL) update_diagnostic(tmprsrc,(dodiag ? TRUE:FALSE));
		ef=ADVNXTNRDsec(pptmstr,1,ApplySubData,p);
	}
	if(p!=NULL) Rfree(p);
	...
	if(size_of_file>0)
	{
		switch(apply_type)
		{
			...
			case 1: /* Process Exceptions */
				...
				brsrc=RDArsrcNEW(module,"APPLY RATE BROWSE");
				srsrc=RDArsrcNEW(module,"APPLY RATE SEARCH BROWSE");
				drsrc=RDArsrcNEW(module,"APPLY RATE DEFINE LIST");
				addDFincvir(drsrc,module,"MISCMSTR",NULL,pptmstr);
				addDFincvir(drsrc,module,"MISCCLAS",NULL,miscclas);
				addDFincvir(srsrc,module,"MISCMSTR",NULL,pptmstr);
				addDFincvir(srsrc,module,"MISCCLAS",NULL,miscclas);
...
void quitselect(RDArsrc *mainrsrc)
{
	if(COUNT)
	{
		RUNREPORT("MISCBILL",reportname,NULL,NULL,TRUE,TRUE,NULL);
	}
	if(mainrsrc!=NULL) free_rsrc(mainrsrc);
	if(pptmstr!=(-1)) CLSNRD(pptmstr);
...
void main(int argc,char **argv)
{
	...
	if(InitializeSubsystems(argc,argv,module,"APPLY CLASS RATES")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	} else reportname=stralloc("APPLYRATE");
	fmgt=RDAfinmgtNEW();
	...
	if((pptmstr=APPOPNNRD(module,"MISCMSTR",TRUE,TRUE))==(-1)) return;
	if((miscclas=APPOPNNRD(module,"MISCCLAS",TRUE,FALSE))==(-1)) return;
	if((rstci=APPOPNNRD(module,"MISCRATE",TRUE,FALSE))==(-1)) return;
	if((rstrn=APPOPNNRD(module,"MISCTRAN",TRUE,TRUE))==(-1)) return;
	...
	makeapplytype();
	mainrsrc=RDArsrcNEW(module,"APPLY RATE");
	addDFincvir(mainrsrc,module,"MISCMSTR",NULL,pptmstr);
	addDFincvir(mainrsrc,module,"MISCCLAS",NULL,miscclas);
	addDFincvir(mainrsrc,module,"MISCRATE",NULL,rstci);
	file2rangersrc(pptmstr,mainrsrc);
	file2rangersrc(miscclas,mainrsrc);
...
{
	char *realestateid=NULL,*ownerid=NULL,*classid=NULL;
	...
	FINDRSCGETSTRING(mtnrsrc,"MISC IDENTIFICATION",&realestateid);
	FINDRSCGETSTRING(mtnrsrc,"CLASS IDENTIFICATION",&classid);
	FINDRSCGETSTRING(mtnrsrc,"OWNER IDENTIFICATION",&ownerid);
	FINDRSCGETINT(mtnrsrc,"TAX YEAR",&yearid);
	FINDRSCGETINT(mtnrsrc,"TRANSACTION NUMBER",&tranno);
	FINDFLDSETSTRING(dbsort->fileno,"MISC IDENTIFICATION",realestateid);
	FINDFLDSETSTRING(dbsort->fileno,"CLASS IDENTIFICATION",classid);
...
{
	char *realestateid=NULL,*classid=NULL,*ownerid=NULL;
	int yearid=0,tranno=0,which_type=0;
	short x=0;
	PassableStruct *p=NULL;
...
{
	char *classid=NULL,*realestateid=NULL,*ownerid=NULL;
	...
	mtnrsrc=RDArsrcNEW(module,"APPLY RATE MAINTAIN SCREEN");
	addDFincvir(mtnrsrc,module,"MISCMSTR",NULL,pptmstr);
	addDFincvir(mtnrsrc,module,"MISCCLAS",NULL,miscclas);
	addDFincvir(mtnrsrc,module,"MISCRATE",NULL,rstci);
...
/* do DEFINITION TYPE LIST */
	...
	if(pptmstr!=(-1)) file2rsrc(pptmstr,mtnrsrc,FALSE);
	if(miscclas!=(-1)) file2rsrc(miscclas,mtnrsrc,FALSE);
	if(rstci!=(-1)) file2rsrc(rstci,mtnrsrc,FALSE);
	if(fsclyr!=(-1)) file2rsrc(fsclyr,mtnrsrc,FALSE);
	FINDRSCSETFUNC(mtnrsrc,"MISC IDENTIFICATION",getfile,t);
	FINDRSCSETFUNC(mtnrsrc,"CLASS IDENTIFICATION",getfile,t);
	...
	switch(dowhich)
	{
		case 1: /* this option isn't used */
			break;
		case 2:
			FINDRSCGETSTRING(mtnrsrc,"MISC IDENTIFICATION",
				&realestateid);
			FINDRSCGETSTRING(mtnrsrc,"CLASS IDENTIFICATION",
				&classid);
			FINDRSCGETSTRING(mtnrsrc,"OWNER IDENTIFICATION",&ownerid);
			FINDRSCGETINT(mtnrsrc,"TAX YEAR",&yearid);
			FINDRSCGETINT(mtnrsrc,"TRANSACTION NUMBER",&tranno);
			FINDRSCSETSHORT(mtnrsrc,"MONTHS",xmonth);
			FINDFLDSETSTRING(t->fileno,"MISC IDENTIFICATION",
				realestateid);
			FINDFLDSETSTRING(t->fileno,"CLASS IDENTIFICATION",
				classid);
...
static void save_check(RDArsrc *mtnrsrc,short update_list,DBsort *dbsort)
{
	...
	if(!delflag)
	{
		...
		if(pptmstr!=(-1))
		{
			FINDFLDGETSTRING(dbsort->fileno,"MISC IDENTIFICATION",&temp);
			FINDFLDGETSTRING(dbsort->fileno,"CLASS IDENTIFICATION",&classid);
			FINDFLDGETSTRING(dbsort->fileno,"OWNER IDENTIFICATION",&ownid);
			FINDFLDSETSTRING(pptmstr,"MISC IDENTIFICATION",temp);
			FINDFLDSETSTRING(pptmstr,"CLASS IDENTIFICATION",classid);
			FINDFLDSETSTRING(pptmstr,"OWNER IDENTIFICATION",ownid);
			if(ADVEQLNRDsec(pptmstr,1,ApplySubData,p))
			{
				KEYNRD(pptmstr,1);
				warnmessage=Rmalloc(RDAstrlen(classid)+RDAstrlen(temp)+200);
				sprintf(warnmessage,"The IDENTIFICATION of (CLASS/MISC) [%s][%s] entered is not valid.  It does not exist as a Misc Billing Master.",(classid!=NULL ? classid:""),(temp==NULL?"":temp));
				WARNINGDIALOG("WARNING DIALOG SCREEN","INVALID MISC IDENTIFICATION!",warnmessage,NULL,NULL,TRUE,NULL);
				prterr("The CLASS IDENTIFICATION [%s] MISC IDENTIFICATION of [%s] entered does not exist in the [MISCBILL][MISCMSTR] data file.",(classid!=NULL ? classid:""),(temp==NULL ? "":temp));
				if(temp!=NULL) Rfree(temp); 
				...
				FINDFLDGETCHAR(pptmstr,"DELETEFLAG",&delflag);
				if(delflag)
				{
					warnmessage=Rmalloc(RDAstrlen(classid)+RDAstrlen(temp)+140);
					sprintf(warnmessage,"The IDENTIFICATION (CLASS/MISC) of [%s][%s] entered is deleted.  It exists as a Misc Billing, but is presently set up in a deleted state.",(classid!=NULL ? classid:""),(temp==NULL ? "":temp));
					WARNINGDIALOG("WARNING DIALOG SCREEN","DELETED MISC IDENTIFICATION!",warnmessage,NULL,NULL,TRUE,NULL);
					prterr("The MISC IDENTIFICATION of [%s] entered is deleted.  It exist as a Misc Billing, but is presently set up in a deleted state.",(temp==NULL?"":temp));
					if(temp!=NULL) Rfree(temp); 
					if(classid!=NULL) Rfree(classid);
					if(ownid!=NULL) Rfree(ownid);
					if(warnmessage!=NULL) Rfree(warnmessage);
...
static void save_record(RDArsrc *mtnrsrc,short update_list,DBsort *dbsort)
{
	...
	if(update_list==TRUE)
	{
		if(mbl_trans->mainrsrc!=NULL)
		{
			...
			tmprsrc=diagscrn(dbsort->fileno,"DIAGNOSTIC SCREEN","MISCBILL","Browse Diagnostic",NULL);
			if(tmprsrc!=NULL)
			{
...
static void getsupportingfiles(RDArsrc *mtnrsrc,DBsort *dbsort)
{
	...
	COPYFIELD(dbsort->fileno,miscclas,"CLASS IDENTIFICATION");
	if(ADVEQLNRDsec(miscclas,1,ApplySubData,p)) KEYNRD(miscclas,1);
	filerecord2rsrc(miscclas,mtnrsrc);
	COPYFIELD(dbsort->fileno,rstci,"RATE IDENTIFICATION");
	if(ADVEQLNRDsec(rstci,1,ApplySubData,p)) KEYNRD(rstci,1);
	filerecord2rsrc(rstci,mtnrsrc);
	COPYFIELD(dbsort->fileno,pptmstr,"MISC IDENTIFICATION");
	COPYFIELD(dbsort->fileno,pptmstr,"CLASS IDENTIFICATION");
	COPYFIELD(dbsort->fileno,pptmstr,"OWNER IDENTIFICATION");
	COPYFIELD(dbsort->fileno,pptmstr,"TAX YEAR");
	if(ADVEQLNRDsec(pptmstr,1,ApplySubData,p)) KEYNRD(pptmstr,1);
	filerecord2rsrc(pptmstr,mtnrsrc);
	COPYFIELD(dbsort->fileno,fsclyr,"TAX YEAR");
	...
	FINDFLDGETSTRING(rstci,"RATE FORMULA",&exprssn);
	AMOUNT=0.0;
	if(exprssn!=NULL)
	{
		AMOUNT=EVALUATEdbl(exprssn,ApplySubData,p);
