...
#ifndef WIN32
#define __NAM__ "mkmtncd.lnx"
#endif
#ifdef WIN32
#define __NAM__ "mkmtncd.exe"
#endif
#include <app.hpp>
#include <rptgen.hpp>
#include <mkmtn.hpp>
...
static void printMaintainMastercode(RDArsrc *prsrc,MaintainMaster *MTNMASTER)
{
	...
	if(MTNMASTER!=NULL)
	{
		...
		if(outdevice!=NULL)
		{
			...
			if(FINDRSCGETINT(mrsrc,"MAINTAIN MASTERS AVAILABLE",&s))
			{
				return;
			}
#ifndef WIN32
...
static void setvalues(RDArsrc *mainrsrc)
{
	...
	if(FINDRSCGETINT(mainrsrc,"MAINTAIN MASTERS AVAILABLE",&selected)) return;
...
static void doliblist(RDArsrc *mainrsrc)
{
	...
	FINDRSCGETINT(mainrsrc,"MAINTAIN MASTERS AVAILABLE",&s);
	freeapplib(mtnsavl);
	...
	if(!FINDRSCLISTAPPlib(mainrsrc,"MAINTAIN MASTERS AVAILABLE",s,mtnsavl))
	{
		updatersrc(mainrsrc,"MAINTAIN MASTERS AVAILABLE");
	}
	setvalues(mainrsrc);
}
...
#endif
{
	...
	if(argc>2)
	{
		if(argc>3)
		{
			if(InitializeSubsystems(argc,argv,argv[2],"MAKE MAINTAIN MASTER CODE"))
			{
				RDAAPPMAINLOOP();
				return;
			}
		...
		{
			if(InitializeSubsystems(argc,argv,argv[1],"MAKE MAINTAIN MASTER CODE"))
			{
				RDAAPPMAINLOOP();
				return;
			}
		...
		if(MTNMASTER!=NULL)
		{
			makeMaintainMastercode(MTNMASTER,stdout,dirname,libname,mtnname);
		}
		if(MTNMASTER!=NULL) Rfree(MTNMASTER);
		if(dirname!=NULL) Rfree(dirname);
		if(libname!=NULL) Rfree(libname);
		...
		x=0;
		addlstrsrc(mrsrc,"MAINTAIN MASTERS AVAILABLE",&x,TRUE,setvalues,mtnsavl->numlibs,&mtnsavl->libs,NULL);
		...
		doliblist(mrsrc);
		FINDRSCLISTAPPlib(mrsrc,"MAINTAIN MASTERS AVAILABLE",s,mtnsavl);
		setvalues(mrsrc);
