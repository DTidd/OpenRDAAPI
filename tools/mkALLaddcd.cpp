/* mkALLaddcd.c - Generate Power Add C Code for speed of development*/
#ifndef WIN32
#define __NAM__ "mkALLaddcd.lnx"
#endif
#ifdef WIN32
#define __NAM__ "mkALLaddcd.exe"
#endif
#include <app.hpp>
#include <rptgen.hpp>
#include <mkadd.hpp>
...
static void makeAllPowerAddcode(PowerAdd *add,char *outdevice,char *dirname,char *libname,char *addname)
{
	FILE *fp=NULL;
	FILE *s_fp=NULL; /* load screens list file */
	FILE *sc_fp=NULL; /* load screens code file */
	char *temp1=NULL;
	int x=0;
	RDAaddsub *subord=NULL;
	char *dirx=NULL,*libx=NULL;

	if(!RDAstrcmp(outdevice,"STDOUT"))
	{
		fp = stdout;
	}
	else
	...
	if(fp!=NULL)
	{
		...
		/* Pack main screens */
		if(!isEMPTY(add->addscreen))
		{
			pack_add_screen(fp,dirname,libname,add->addscreen,1,1,1);
		}
		if(!isEMPTY(add->define_list))
		{
			pack_screen(fp,dirname,libname,add->define_list,0,0,0);
		}
		/* Make default menu and pack it */
		makeDefaultAddMenu(add);
		if(temp1!=NULL) Rfree(temp1);
		temp1=Rmalloc(40+RDAstrlen(add->module)+RDAstrlen(add->name)+1);
		sprintf(temp1,"%s %s",add->module,add->name);
		...
		if(add->subord!=NULL)
		{
			for(x=0,subord=add->subord;x<add->subnum;++x,++subord)
			{
				...
				if(!isEMPTY(subord->browse_screen))
				{
#ifndef WIN32
					if(temp1!=NULL) Rfree(temp1);
					temp1=Rmalloc(40+RDAstrlen(subord->browse_screen)+1);
					sprintf(temp1,"grep -q \"%s\" loadscreens.list", subord->browse_screen);
					if(system_command_retvalue(temp1)!=0)
					{
#endif /* !WIN32 */
						fprintf(s_fp,"%s\n", subord->browse_screen);
						pack_screen(sc_fp,dirname,libname,subord->browse_screen,0,0,0);
#ifndef WIN32
					}
					if(temp1!=NULL) Rfree(temp1);
#endif /* !WIN32 */
				}
				if(!isEMPTY(subord->define_list))
				{
#ifndef WIN32
					if(temp1!=NULL) Rfree(temp1);
				        temp1=Rmalloc(40+RDAstrlen(subord->define_list)+1);
					sprintf(temp1,"grep -q \"%s\" loadscreens.list", subord->define_list);
					if(system_command_retvalue(temp1)!=0)
					{
#endif /* !WIN32 */
						fprintf(s_fp,"%s\n", subord->define_list);
						pack_screen(sc_fp,dirname,libname,subord->define_list,0,0,0);
#ifndef WIN32
					}
					if(temp1!=NULL) Rfree(temp1);
#endif /* !WIN32 */
				}
				if(!isEMPTY(subord->search_browse))
				{
#ifndef WIN32
					if(temp1!=NULL) Rfree(temp1);
				        temp1=Rmalloc(40+RDAstrlen(subord->search_browse)+1);
					sprintf(temp1,"grep -q \"%s\" loadscreens.list", subord->search_browse);
					if(system_command_retvalue(temp1)!=0)
					{
#endif /* !WIN32 */
						fprintf(s_fp,"%s\n", subord->search_browse);
						pack_screen(sc_fp,dirname,libname,subord->search_browse,1,1,0);
#ifndef WIN32
					}
					if(temp1!=NULL) Rfree(temp1);
#endif /* !WIN32 */
		...
		prterr("Error PrintMaintainMaster popen call for output device [%s] failed.",(outdevice!=NULL?outdevice:""));
...
static void printPowerAddcode(RDArsrc *prsrc,PowerAdd *add)
{
	...
	if(add!=NULL)
	{
		...
		if(outdevice!=NULL)
		{
			...
			makeAllPowerAddcode(add,outdevice,dirname,libname,addname);
			if(outdevice!=NULL) Rfree(outdevice);
...
#endif
{
	...
	if(argc>2)
	{
		...
		if(add!=NULL)
		{
			makeAllPowerAddcode(add,"STDOUT",dirname,libname,addname);
		}
		if(add!=NULL) Rfree(add);
		if(dirname!=NULL) Rfree(dirname);
		if(libname!=NULL) Rfree(libname);
