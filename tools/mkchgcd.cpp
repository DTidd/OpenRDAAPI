/* mkchgcd.c - Generate Change File C Code for speed of development*/
#ifndef WIN32
#define __NAM__ "mkchgcd.lnx"
#endif
#ifdef WIN32
#define __NAM__ "mkchgcd.exe"
#endif
#include <app.hpp>
#include <rptgen.hpp>
#include <mkchg.hpp>
...
#endif
{
	...
	if(argc>2)
	{
		...
		if(CHGDEF!=NULL)
		{
			makechgcode(CHGDEF,stdout,dirname,libname,chgname);
		}
		if(CHGDEF!=NULL) FreeCHGstruct(CHGDEF);
		if(dirname!=NULL) Rfree(dirname);
		if(libname!=NULL) Rfree(libname);
