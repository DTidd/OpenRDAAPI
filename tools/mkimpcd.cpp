/* mkimpcd.c - Generate Import/Export C Code for speed of development*/
#ifndef WIN32
#define __NAM__ "mkimpcd.lnx"
#endif
#ifdef WIN32
#define __NAM__ "mkimpcd.exe"
#endif
#include <app.hpp>
#include <rptgen.hpp>
#include <mkimp.hpp>
...
#endif
{
	...
	if(argc>2)
	{
		...
		if(IMPEXP!=NULL)
		{
			makeIMPEXPcode(IMPEXP,stdout,libname,name);
			free_import(IMPEXP);
		}
		if(libname!=NULL) Rfree(libname);
		if(name!=NULL) Rfree(name);
