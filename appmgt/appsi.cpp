/* appsi.c - Supervisor Interview */
/*lint -library */
#ifndef WIN32
#define __NAM__ "appsi.lnx"
#endif
#ifdef WIN32
#define __NAM__ "appsi.exe"
#endif
#include <app.hpp>
...
static char GetAllSup(RDArsrc *tmprsrc,DBsort *dbsort)
{
	short ef=FALSE;
	char *appid=NULL,*lappid=NULL,*posid=NULL,*lposid=NULL,retval=TRUE;
	char deleteflag=FALSE;
	int appno=0,lappno=0;
	...
	FINDFLDSETSTRING(apppos,"APPLICANT IDENTIFICATION",appid);
	FINDFLDSETINT(apppos,"APPLICATION NUMBER",appno);
	ef=ADVGTENRDsec(apppos,1,SCRNvirtualSubData,tmprsrc);
	while(!ef)
	{
		FINDFLDGETSTRING(apppos,"APPLICANT IDENTIFICATION",&lappid);
		FINDFLDGETINT(apppos,"APPLICATION NUMBER",&lappno);
		if(RDAstrcmp(appid,lappid) || appno!=lappno) break;
		FINDFLDGETCHAR(apppos,"DELETEFLAG",&deleteflag);
		if(!deleteflag)
		{
			FINDFLDGETSTRING(apppos,"POSITION IDENTIFICATION",&lposid);
			if(!RDAstrcmp(posid,lposid)) retval=TRUE;
				else retval=FALSE;
		}
		if(!retval) break;
		ef=ADVNXTNRDsec(apppos,1,SCRNvirtualSubData,tmprsrc);
	}
	if(appid!=NULL) Rfree(appid);
...
static void interview_backend(DBsort *dbsort)
{
	...
	char withdraw=FALSE,*interview=NULL;
	char pers_int=FALSE,rec2hire=FALSE,setallsup=FALSE;
	RDArsrc *tmprsrc=NULL;
	RDATData *prev=NULL;
	...
	if(dbsort!=NULL)
	{
		...
		while(!ef)
		{
			...
			FINDFLDGETSTRING(dbsort->fileno,"SUPERVISOR INTERVIEW",&interview);
			FINDFLDGETCHAR(dbsort->fileno,"RECOMMEND TO HIRE",&rec2hire);
			FINDFLDGETCHAR(dbsort->fileno,"SUPERVISOR INTERVIEWED",&pers_int);
			if(!deleteflag && (pers_int || withdraw || rec2hire))
			{
				COPYFIELD(dbsort->fileno,appcnt,"APPLICANT IDENTIFICATION");
					...
					tmprsrc))
				{
						...
						if(pers_int)
						{
							setallsup=GetAllSup(tmprsrc,dbsort);
							FINDFLDSETCHAR(appcat,
								"ALL SUPERVISORS INTERVIEWED",setallsup);
						}
						if(withdraw)
						{
							FINDFLDSETCHAR(appcat,"WITHDRAW APPLICANT",
								withdraw);
							curdate=GETCURRENTDATE10();
							curtime=GETCURRENTTIME();
							FINDFLDSETSTRING(appcat,"WITHDRAWN BY",
								USERLOGIN);
							FINDFLDSETSTRING(appcat,"WITHDRAWN DATE",
								curdate);
							FINDFLDSETSTRING(appcat,"WITHDRAWN TIME",
								curtime);
							if(curdate!=NULL) Rfree(curdate);
							if(curtime!=NULL) Rfree(curtime);
						}
						ADVWRTTRANSsec(appcat,0,NULL,prev,SCRNvirtualSubData,
							tmprsrc);
					...
					tmprsrc))
				{
					...
					FINDFLDSETSTRING(apppos,"SUPERVISOR INTERVIEW",interview);
					FINDFLDSETCHAR(apppos,"SUPERVISOR INTERVIEWED",pers_int);
					FINDFLDSETCHAR(apppos,"RECOMMEND TO HIRE",rec2hire);
					if(rec2hire)
					{
					...
					curdate=GETCURRENTDATE10();
					curtime=GETCURRENTTIME();
					if(rec2hire && !AppMgt->use_administrative_interview)
					{
						FINDFLDSETCHAR(apppos,"APPROVED TO HIRE",rec2hire);
					...
					if(pers_int)
					{
						FINDFLDSETSTRING(apppos,"SUPERVISOR INTERVIEWED BY",USERLOGIN);
						FINDFLDSETSTRING(apppos,"SUPERVISOR INTERVIEWED DATE",curdate);
						FINDFLDSETSTRING(apppos,"SUPERVISOR INTERVIEWED TIME",curtime);
					}
					if(curdate!=NULL) Rfree(curdate);
					if(curtime!=NULL) Rfree(curtime);
...
static void interviewm(void *targetkey,DBsort *t)
{
	...
	mtnrsrc=RDArsrcNEW(module,
		"SUPERVISOR INTERVIEW MAINTAIN SCREEN");
...
static void makeDBsortfile(DBsort *dbsrt)
{
	...
	addDBfilefield(dbsrt,apppos,"SUPERVISOR INTERVIEW");
...
static void addsortrecord(DBsort *dbsrt)
{
	...
	COPYFIELD(apppos,dbsrt->fileno,"SUPERVISOR INTERVIEW");
...
{
	DBsort *dbsort=NULL;
	char *sortname=NULL,deleteflag=FALSE,sup2int=FALSE;
	char *exprssn=NULL,withdraw=FALSE,pappt=FALSE,supint=FALSE,hired=FALSE,*e=NULL;
	...
	while(!ef)
	{
		...
		FINDFLDGETCHAR(apppos,"SUPERVISOR INTERVIEWED",&supint);
		FINDFLDGETCHAR(apppos,"SUPERVISOR APPOINTMENT VERIFIED",&pappt);
		FINDFLDGETCHAR(apppos,"HIRED",&hired);
		/*
		if(!deleteflag && sup2int && !supint && pappt)
		*/
		if(!deleteflag && !hired)
		{
			if(!testfilerangersrc(apppos,rs))
			{
				COPYFIELD(apppos,posmst,
					"POSITION IDENTIFICATION");
				if(!ADVEQLNRDsec(posmst,1,
					SCRNvirtualSubData,tmprsrc))
				{
					FINDFLDGETCHAR(posmst,
						"DELETEFLAG",&deleteflag);
					if(!deleteflag &&
						!testfilerangersrc(posmst,rs))
					{
	...
	if(SIZNRD(dbsort->fileno))
	{
		brsrc=RDArsrcNEW(module,
			"SUPERVISOR INTERVIEW BROWSE");
		srsrc=RDArsrcNEW(module,
			"SUPERVISOR INTERVIEW SEARCH BROWSE");
		drsrc=RDArsrcNEW(module,
			"SUPERVISOR INTERVIEW DEFINE LIST");
...
#endif
{
	...
	} else reportname=stralloc("SUPINT");
	...
	AppMgt=RDAappmgtNEW();
	if(GetAppMgtSetup(AppMgt)==(-1)) return;
	mainrsrc=RDArsrcNEW(module,"SUPERVISOR INTERVIEWS");
