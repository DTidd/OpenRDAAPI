/* apppos.c  Application Management Personal Documents Master Program */
/*lint -library */
#ifndef WIN32
#define __NAM__ "apppos.lnx"
#endif
#ifdef WIN32
#define __NAM__ "apppos.exe"
#endif
#include <app.hpp>
...
static short posmst=(-1);
static char editable=TRUE;
static MakeBrowseList *mbl_apppos=NULL;
static void browse_apppos(RDArsrc *,RDATData *);
static MaintainMaster *MASTER=NULL;
static void appposm(short,void *);
...
static void browse_apppos(RDArsrc *mtnrsrc,RDATData *previous)
{
	...
	mbl_apppos=ADVPbrowse(MASTER->passkey->fileno,MASTER->passkey->keyno,mainrsrc,
...
int main(int argc,char **argv)
{
	...
	if(findMaintainMaster(MASTER))
	{
		...
		if((filenum=APPOPNNRDEDITABLE(MASTER->module,"APPPOS",&editable))==(-1))
		{
			doexit(NULL);
			return;
		}
		...
		posmst=APPReturnSupportingFileno(MASTER,"POSTRK","POSMSTR",1);
...
static void doexit(MakeBrowseList *blist)
{
	...
	if(mbl_apppos!=NULL) mbl_apppos=NULL;
}
...
static void okfunc_position(MakeBrowseList *b,void *targetkey)
{
	...
	FINDFLDSETSTRING(MASTER->passkey->fileno,"POSITION IDENTIFICATION",posid);
	updatersrc(b->funcargs,"[APPPOS][POSITION IDENTIFICATION]");
	if(posid!=NULL) Rfree(posid);
	y=FINDRSC(b->funcargs,"[APPPOS][POSITION IDENTIFICATION]");
	if(y!=(-1))
	{
...
static short select_position(MakeBrowseList *blist)
{
	char delflag=FALSE;
...
void browse_positions(RDArsrc *mtnrsrc)
{
	...
	mainrsrc=RDArsrcNEW(MASTER->module,"SELECT POSITION BROWSE");
	searchrsrc=RDArsrcNEW(MASTER->module,"SELECT POSITION SEARCH BROWSE");
	definelist=RDArsrcNEW(MASTER->module,"SELECT POSITION DEFINE LIST");
	ZERNRD(posmst);
	...
	ADVSbrowse(posmst,1,mainrsrc,searchrsrc,definelist,
		NULL,NULL,select_position,doexit_position,okfunc_position,NULL,
		mtnrsrc,TRUE,NULL,2,2,2,0);
}
static void appposm(short dowhich,void *targetkey)
{
	char *name=NULL;
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_apppos,previous);
	if(mbl_apppos!=NULL)
	{
	...
	APPmakescrn(mtnrsrc,FALSE,quit_record,previous,(mbl_apppos==NULL?TRUE:FALSE));
}
...
static void save_record(RDArsrc *mtnrsrc,RDATData *previous,short update_list)
{
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		updatebrowse(update_list,mbl_apppos,MASTER->passkey->fileno,MASTER->module,mtnrsrc);
  		if(mbl_apppos!=NULL) quit_record(mtnrsrc,previous);
