/* appssa.c - Schedule Supervisor Appointments */
/*lint -library */
#ifndef WIN32
#define __NAM__ "appssa.lnx"
#endif
#ifdef WIN32
#define __NAM__ "appssa.exe"
#endif
#include <app.hpp>
...
static void schedule_backend(DBsort *dbsort)
{
	...
	char *apptdate=NULL,*appttime=NULL,withdraw=FALSE;
	RDArsrc *tmprsrc=NULL;
	RDATData *prev=NULL;
		...
		while(!ef)
		{
			...
			if(!deleteflag && (!isEMPTY(apptdate) && !isEMPTY(appttime)))
			{
				...
				if(!ADVEQLNRDsec(apppos,1,SCRNvirtualSubData,tmprsrc))
				{
					...
					COPYFIELD(dbsort->fileno,apppos,"SUPERVISOR INTERVIEW");
					FINDFLDSETSTRING(apppos,"SUPERVISOR APPOINTMENT DATE",
						apptdate);
					FINDFLDSETSTRING(apppos,"SUPERVISOR APPOINTMENT TIME",
						appttime);
					if(!isEMPTY(apptdate) && !isEMPTY(appttime))
					{
						curdate=GETCURRENTDATE10();
						curtime=GETCURRENTTIME();
						FINDFLDSETSTRING(apppos,"SUPERVISOR APPOINTMENT VERIFIED BY",USERLOGIN);
						FINDFLDSETSTRING(apppos,"SUPERVISOR APPOINTMENT VERIFIED DATE",curdate);
						FINDFLDSETSTRING(apppos,"SUPERVISOR APPOINTMENT VERIFIED TIME",curtime);

						/*
						FINDFLDSETCHAR(apppos,"SUPERVISOR INTERVIEWED",TRUE);
			...
			if(!deleteflag && withdraw)
			{
				...
				if(!ADVEQLNRDsec(appcat,1,SCRNvirtualSubData,tmprsrc))
				{
					...
					curdate=GETCURRENTDATE10();
					curtime=GETCURRENTTIME();
					FINDFLDSETSTRING(apppos,"WITHDRAWN BY",USERLOGIN);
					FINDFLDSETSTRING(apppos,"WITHDRAWN DATE",curdate);
					FINDFLDSETSTRING(apppos,"WITHDRAWN TIME",curtime);
					if(curdate!=NULL) Rfree(curdate);
					if(curtime!=NULL) Rfree(curtime);
...
static void schedulem(void *targetkey,DBsort *t)
{
	...
	mtnrsrc=RDArsrcNEW(module,
		"SCHEDULE SUPERVISOR APPOINTMENT MAINTAIN SCREEN");
...
static void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("APPMGT","SUPERVISOR APPOINTMENT SCHEDULE SORT SEQUENCE");
	if(d!=NULL)
	{
	...
	addDBfield(dbsrt,"SUPERVISOR INTERVIEW",SCROLLEDTEXT,400);
        addDBfield(dbsrt,"NAME",PLAINTEXT,40);
	addDBfield(dbsrt,"WORK PHONE",PHONE,12);
	addDBfield(dbsrt,"HOME PHONE",PHONE,12);
...
static void selectschedule(RDArsrc *mainrsrc)
{
	...
	char *exprssn=NULL,withdraw=FALSE,piv=FALSE,*e=NULL;
				...
				if(!ADVEQLNRDsec(posmst,1,SCRNvirtualSubData,tmprsrc))
				{
					...
					if(!deleteflag && !testfilerangersrc(posmst,rs))
					{
						...
						if(!ADVEQLNRDsec(appcat,1,SCRNvirtualSubData,tmprsrc))
						{
							...
							FINDFLDGETCHAR(appcat,"WITHDRAW APPLICANT",&withdraw);
							FINDFLDGETCHAR(appcat,"PERSONNEL INTERVIEW VERIFIED",
								&piv);
							/*
							if(!deleteflag && !withdraw && 
								((!AppMgt->use_personnel_interview) || 
								(AppMgt->use_personnel_interview && piv)))
							*/
	...
	if(SIZNRD(dbsort->fileno))
	{
		brsrc=RDArsrcNEW(module,
			"SCHEDULE SUPERVISOR APPOINTMENT BROWSE");
		srsrc=RDArsrcNEW(module,
			"SCHEDULE SUPERVISOR APPOINTMENT SEARCH BROWSE");
		drsrc=RDArsrcNEW(module,
			"SCHEDULE SUPERVISOR APPOINTMENT DEFINE LIST");
...
#endif
{
	...
	} else reportname=stralloc("SUPSCH");
	mainrsrc=RDArsrcNEW(module,"SCHEDULE SUPERVISOR APPOINTMENTS");
