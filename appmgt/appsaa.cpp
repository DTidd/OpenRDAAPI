/* appsaa.c - Schedule Administrative Appointments */
/*lint -library */
#ifndef WIN32
#define __NAM__ "appsaa.lnx"
#endif
#ifdef WIN32
#define __NAM__ "appsaa.exe"
#endif
#include <app.hpp>
		...
		while(!ef)
		{
				...
				|| withdraw))
			{
					...
					tmprsrc))
				{
					...
					FINDFLDSETSTRING(appcat,"ADMINISTRATOR APPOINTMENT DATE",
						apptdate);
					FINDFLDSETSTRING(appcat,"ADMINISTRATOR APPOINTMENT TIME",
						appttime);
					COPYFIELD(dbsort->fileno,appcat,"ADMINISTRATOR INTERVIEW");
					...
					{
						FINDFLDSETSTRING(appcat,"ADMINISTRATOR APPOINTMENT VERIFIED BY",USERLOGIN);
						FINDFLDSETSTRING(appcat,"ADMINISTRATOR APPOINTMENT VERIFIED DATE",curdate);
						FINDFLDSETSTRING(appcat,"ADMINISTRATOR APPOINTMENT VERIFIED TIME",curtime);
					}

					if(!isEMPTY(apptdate) && !isEMPTY(appttime))
					{
					    	/*
						FINDFLDSETCHAR(appcat,"ADMINISTRATOR INTERVIEWED",TRUE);
...
static void schedulem(void *targetkey,DBsort *t)
{
	...
	mtnrsrc=RDArsrcNEW(module,
		"SCHEDULE ADMINISTRATOR APPOINTMENT MAINTAIN SCREEN");
...
static void makeDBsortfile(DBsort *dbsrt)
{
	...
	d=getDFvirtual("APPMGT",
		"ADMINISTRATOR APPOINTMENT SCHEDULE SORT SEQUENCE");
	if(d!=NULL)
	{
	...
	addDBfilefield(dbsrt,appcat,"ADMINISTRATOR INTERVIEW");
...
{
	DBsort *dbsort=NULL;
	char *sortname=NULL,deleteflag=FALSE,sup2int=FALSE,pappt=FALSE,hired=FALSE,*e=NULL;
	char *exprssn=NULL,withdraw=FALSE,rec2hire=FALSE,apphire=FALSE;
	...
	while(!ef)
	{
		...
		FINDFLDGETCHAR(apppos,"APPROVED TO HIRE",&apphire);
		FINDFLDGETCHAR(apppos,"HIRED",&hired);
		/*
		if(!deleteflag && sup2int && rec2hire && !apphire)
		    */
		if(!deleteflag && !hired)
		{
					...
					SCRNvirtualSubData,tmprsrc))
				{
						...
						!testfilerangersrc(posmst,rs))
					{
						...
						if(!ADVEQLNRDsec(appcat,1,SCRNvirtualSubData,tmprsrc))
						{
							...
							FINDFLDGETCHAR(appcat,"ADMINISTRATOR APPOINTMENT VERIFIED",&pappt);
							FINDFLDGETCHAR(appcat,"ADMINISTRATOR INTERVIEWED",
								&sup2int);
	...
	if(SIZNRD(dbsort->fileno))
	{
		brsrc=RDArsrcNEW(module,
			"SCHEDULE ADMINISTRATOR APPOINTMENT BROWSE");
		srsrc=RDArsrcNEW(module,
			"SCHEDULE ADMINISTRATOR APPOINTMENT SEARCH BROWSE");
		drsrc=RDArsrcNEW(module,
			"SCHEDULE ADMINISTRATOR APPOINTMENT DEFINE LIST");
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,
		"SCHEDULE ADMINISTRATOR APPOINTMENTS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	} else reportname=stralloc("ADMSCH");
	...
	mainrsrc=RDArsrcNEW(module,"SCHEDULE ADMINISTRATOR APPOINTMENTS");
