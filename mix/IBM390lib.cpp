...
#include <mix.hpp>
 ...
 /* Powers of 10 */
double power_of_ten[32] = { 1.0, 10.0, 100.0, 1000.0, 10000.0,
    ...
    int    ndec )
{
 ...
 if ( ndec != 0 ) {
    out_num = out_num / power_of_ten[ndec];
 }
...
/* prterr("Evaluating number [%d] to be packed with fsign of [%i].",perlnum,((fsign==TRUE) ? TRUE:FALSE)); */

 if (perlnum >= 0) {
    perl_absval = perlnum;   signum = ((fsign==TRUE) ? 0x0F : 0x0C);
 } else {
 ...
 if (ndec == 0) {
    ...
    sprintf(digits, "%031.0f", perl_absval * power_of_ten[ndec]);
 }
...
#endif
 if (perlnum >= 0) {
    perl_absval = perlnum;   signum = 0xF0; /* Changed from 0xC0 to agree with the Rockbridge export file */
 } else {
 ...
 * in IBM390lib.hpp (a2e_table, etc.).
 *-------------------------------------------------------*/
void  CF_fcs_xlate
  ( char  *outstring,
    char  *instring,
