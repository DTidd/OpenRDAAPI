...
struct PowerAddSubordPassableAs
{
	PowerAdd *POWERADD;
	int *addsub_offset;
} 
typedef PowerAddSubordPassableAs PowerAddSubordPassableA
...
short xgetPowerAdd(char *libname,PowerAdd *POWERADD,int line,char *file)
{
	...
	x=BINgetshort(bin);
	if(x!=(-1686) && x!=(-1687) && x!=(-1688) && x!=(-1689) && x!=(-1690) && x!=(-1691) && x!=(-1692) && x!=(-1693) && x!=(-1694) && x!=(-1695) && x!=(-1696) && x!=(-1697) && x!=(-1698) && x!=(-1699) && x!=(-1701) && x!=(-1700))
	{
...
char *xgetPowerAdddesc(char *libname,char *name,int line,char *file)
{
	...
	x=BINgetshort(bin);
	if(x!=(-1686) && x!=(-1687) && x!=(-1688) && x!=(-1689) && x!=(-1690) && x!=(-1691) && x!=(-1692) && x!=(-1693) && x!=(-1694) && x!=(-1696) && x!=(-1697) && x!=(-1698) && x!=(-1699) && x!=(-1701) && x!=(-1700))
	{
...
AddEditWdgt *xFINDEDITWDGT(PowerAdd *POWERADD,char *name,int line,char *file)
{
	...
	if(diagadd)
	{
		prterr("DIAG FINDEDITWDGT at line [%d] program [%s] ",line,file);
	}
	if(POWERADD!=NULL)
	{
		if(POWERADD->EditWdgts!=NULL)
...
*/
			...
			if(BL->list!=NULL)
			{
				if(BL->list->key!=NULL && BL->list->num>0)
				{
					...
					if(EQLNRDKEYsec(fileno,keyno,comptarget))
					{
						KEYNRD(fileno,keyno);
						return_value=FALSE;	
					} else {
