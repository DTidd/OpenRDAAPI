/* prtapo.c - Print Actual Purchase Orders */

#ifndef WIN32
#define __NAM__ "prtapo.lnx"
#endif
#ifdef WIN32
#define __NAM__ "prtapo.exe"
#endif
#include <app.hpp>
...
static void SetPrintedFlag(short sfile)
{
	...
	while(!ef)
	{
		...
		FINDFLDSETCHAR(purmst,"PRINTED",TRUE);	
		FINDFLDSETSTRING(purmst,"PRINTED BY",USERLOGIN);	
		FINDFLDSETSTRING(purmst,"PRINTED DATE",date);	
		FINDFLDSETSTRING(purmst,"PRINTED TIME",timex);	
...
static void quitprint(RDArsrc *r)
{
	if(sortfile!=NULL)
	{
...
/* PAGE HEADERS */
	switch(address_position)
	{
		...
		case 0:
		/*
			RDA_fprintf(fp,"                           P U R C H A S E  O R D E R               Page: %6d\r\n",page_no);
		*/
		...
		*/
		...
		case 3:
			RDA_fprintf(fp,"\r\n");
		/*
			RDA_fprintf(fp,"                           P U R C H A S E  O R D E R                           \r\n");
		*/
			RDA_fprintf(fp,"\r\n");
			RDA_fprintf(fp,"                                                                Page: %d\r\n",page_no);
			RDA_fprintf(fp,"%*sP.O.: %-10.10s\r\n",(PAGE_WIDTH-6-10),"",(poid1!=NULL ? poid1:""));
	...
	/*
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"QNTY      ITEM ID      DESCRIPTION                          UNIT PRICE    AMOUNT\r\n");
	RDA_fprintf(fp,"---- ---- ------------ ------------------------------------ ---------- ---------\r\n");
	*/
	RDA_fprintf(fp,"\r\n");
	RDA_fprintf(fp,"\r\n");
...
static void printactualpos(RDA_PFILE *fp,APPlib *imagelines)
{
	...
	if(imagelines->numlibs>0)
	{
		...
		if((po_signature)&&(usersigsec)&&(using_viewpdf))
		{
			SelectPrintTypeByName(outdevice,"PUSH CURSOR",fp);
			if(PO_SIG_XY!=NULL) 
			{
			...
			SelectPrintTypeByName(outdevice,"POP CURSOR",fp);
		}else if((po_signature)&&(usersigsec))
		{
			SelectPrintTypeByName(outdevice,"PUSH CURSOR",fp);
...
static void createoutput(RDA_PFILE *fp)
{
	...
	char *purmst_remarks1=NULL,*purmst_remarks2=NULL,*purlin_unit=NULL;
...
static void printpos(RDArsrc *r)
{
	RDA_PFILE *fp=NULL;
	RDApdev *pdev=NULL;
	RDApvar *pvar;
...
#endif 
#ifdef WIN32
	outdevice=stralloc("viewpdf.exe");
#endif 
	if(!isEMPTY(outdevice))
	{
		if(!strncmp(outdevice,"viewpdf",7) || !strncmp(outdevice,"VIEWPDF",7))
		{
			using_viewpdf=1;
		}
		...
		if((!isEMPTY(filename))&&(!isEMPTY(password)))
		{
			...
			prterr("file: [%s]    pass: [%s]\n",filename,password);
		}

		if((usersigsec)&&(using_viewpdf))
		{
			y=0;
			memset(stemp,0,101);
	...
	{
		SelectPrintTypeByName(outdevice,"DELETE_FONT_200",fp);
		SelectPrintTypeByName(outdevice,"DELETE_FONT_201",fp);
	}
	*/
...
static void selectpos(RDArsrc *r)
{
	...
	*/
	...
	addDBfield(sortfile,"PURSIT TERMS",1,40);
...
/* Key Info */
	...
	*/
		...
		/*
		rsrc=RDArsrcNEW("PURORD","PRINT PURCHASE ORDERS OUTPUT DEVICE SCREEN");
...
#endif
{
	...
	if(InitializeSubsystems(argc,argv,module,"PRINT PURCHASE ORDERS")) 
	{
		RDAAPPMAINLOOP();
		return;
	}
	...
	mainrsrc=RDArsrcNEW(module,"PRINT PURCHASE ORDERS");
