/* lvetran.c - Xpert Leave Transaction Program */
/*lint -library */
#ifndef WIN32
#define __NAM__ "lvetran.lnx"
#endif
#ifdef WIN32
#define __NAM__ "lvetran.exe"
#endif
#include <app.h>
#include <rdaglob.h>
#include <rdaflds.h>
...
short dscnum=(-1),mstnum=(-1),lvetyp=(-1),lvejob=(-1);
char editable=TRUE;
MakeBrowseList *mbl_tran=NULL;
MaintainMaster *MASTER=NULL;
void browse_lvetran(RDArsrc *,RDATData *);
...
static void setlvedesc(RDArsrc *,RDATData *);
...
void lvemstm(short dowhich,void *targetkey)
{
	...
	addrfkwrsrc(mtnrsrc,"BROWSE",TRUE,browse_lvetran,previous);
...
static void set_appdefaults(RDArsrc *mtnrsrc,RDATData *previous,short update)
{
	...
	FINDRSCSETSTRING(mtnrsrc,"[LVETRAN][ENTRY DATE]",date);
	FINDRSCSETSTRING(mtnrsrc,"[LVETRAN][ENTRY TIME]",timex);
	FINDRSCSETSTRING(mtnrsrc,"[LVETRAN][SOURCE USER]",USERLOGIN);
	if(update)
	{
...
{
	short numflds=0,y=0,earning=0;
	char *rcddesc=NULL,*errordesc=NULL,*temp=NULL;
	...
	if(ADVRECORDsec(MASTER->passkey->fileno,SCRNvirtualSubData,mtnrsrc))
	{
		...
		if(numflds>0)
		{
			...
			if(!ADVEQLNRDsec(MASTER->passkey->fileno,MASTER->passkey->keyno,SCRNvirtualSubData,mtnrsrc))
			{
				...
				FINDFLDGETSHORT(MASTER->passkey->fileno,"TYPE",&earning);
				if(!updated && !delflag)
				{
					...
					ZERNRD(mstnum);
					FINDFLDSETSTRING(mstnum,"PERSONNEL IDENTIFICATION",perid1);
					FINDFLDSETSTRING(mstnum,"LEAVE DESCRIPTION IDENTIFICATION",lvedid1);
					LOCNRDFILE(mstnum);
					if(ADVEQLNRDsec(mstnum,1,SCRNvirtualSubData,mtnrsrc))
					{
						if(errordesc!=NULL) Rfree(errordesc);
						errordesc=Rmalloc(RDAstrlen(MODULENAME(mstnum))+RDAstrlen(FILENAME(mstnum))+102);
						sprintf(errordesc,"Security failed on the file [%s][%s], this supporting record that you attempted to update was not saved.",MODULENAME(mstnum),FILENAME(mstnum));
						...
						prev=RDATDataNEW(mstnum);
						if(earning==0)
						{
							FINDFLDGETDOUBLE(mstnum,"UNPROCESSED EARNINGS",&initamt);
							initamt-=amount;
							FINDFLDSETDOUBLE(mstnum,"UNPROCESSED EARNINGS",initamt);
						} else if(earning==1)
						{
							FINDFLDGETDOUBLE(mstnum,"UNPROCESSED USED",&initamt);
							initamt-=amount;
							FINDFLDSETDOUBLE(mstnum,"UNPROCESSED USED",initamt);
						}
						if(ADVWRTTRANSsec(mstnum,0,NULL,prev,SCRNvirtualSubData,mtnrsrc))
						{
							if(errordesc!=NULL) Rfree(errordesc);
							errordesc=Rmalloc(RDAstrlen(MODULENAME(mstnum))+RDAstrlen(FILENAME(mstnum))+100);
							sprintf(errordesc,"Write failed to the file [%s][%s], this supporting record that you attempted to update was not saved.",MODULENAME(mstnum),FILENAME(mstnum));
					...
					FINDFLDSETSTRING(lvejob,"LEAVE DESCRIPTION IDENTIFICATION",lvedid1);
					FINDFLDSETINT(lvejob,"JOB NUMBER",jobno);
					LOCNRDFILE(lvejob);
					if(ADVEQLNRDsec(lvejob,1,SCRNvirtualSubData,mtnrsrc))
					{
						if(errordesc!=NULL) Rfree(errordesc);
						errordesc=Rmalloc(RDAstrlen(MODULENAME(lvejob))+RDAstrlen(FILENAME(lvejob))+102);
						sprintf(errordesc,"Security failed on the file [%s][%s], this supporting record that you attempted to update was not saved.",MODULENAME(lvejob),FILENAME(lvejob));
						ERRORDIALOG("SECURITY ACCESS DENIED!",errordesc,NULL,FALSE);
						prterr("Error Security Doesn't Allow Save to Supporting Record: [%s]",errordesc);
						if(errordesc!=NULL) Rfree(errordesc);
					} else {
						...
						if(earning==0)
						{
							FINDFLDGETDOUBLE(lvejob,"UNPROCESSED EARNINGS",&initamt);
							initamt-=amount;
							FINDFLDSETDOUBLE(lvejob,"UNPROCESSED EARNINGS",initamt);
						} else if(earning==1)
						{
							FINDFLDGETDOUBLE(lvejob,"UNPROCESSED USED",&initamt);
							initamt-=amount;
							FINDFLDSETDOUBLE(lvejob,"UNPROCESSED USED",initamt);
						}
						if(ADVWRTTRANSsec(lvejob,0,NULL,prev,SCRNvirtualSubData,mtnrsrc))
						{
							if(errordesc!=NULL) Rfree(errordesc);
							errordesc=Rmalloc(RDAstrlen(MODULENAME(lvejob))+RDAstrlen(FILENAME(lvejob))+100);
							sprintf(errordesc,"Write failed to the file [%s][%s], this supporting record that you attempted to update was not saved.",MODULENAME(lvejob),FILENAME(lvejob));
							ERRORDIALOG("WRITE ERROR OCCURRED!",errordesc,NULL,FALSE);
							prterr("Error Write Failed to Supporting Record: [%s]",errordesc);
							if(errordesc!=NULL) Rfree(errordesc);
						}
...
void browse_lvetran(RDArsrc *mtnrsrc,RDATData *previous)
{
...
void main(int argc,char **argv)
{
	...
	if(findMaintainMaster(MASTER))
	{
		...
		SetSupportingWritable(MASTER,MASTER->module,"LVEMSTR",1,TRUE,&editable);
		SetSupportingWritable(MASTER,MASTER->module,"LVEJOB",1,TRUE,&editable);
		if(!OPEN_MASTER_FILES(MASTER))
		{
			doexit(NULL);
			return;
		}
		lvejob=APPReturnSupportingFileno(MASTER,MASTER->module,"LVEJOB",1);
		mstnum=APPReturnSupportingFileno(MASTER,MASTER->module,"LVEMSTR",1);
...
{
	char deleteflag=FALSE,suv=FALSE;

	FINDRSCGETCHAR(mtnrsrc,"[LVETRAN][SOURCE USER VERIFIED]",&suv);
	FINDRSCGETCHAR(mtnrsrc,"[LVETRAN][DELETEFLAG]",&deleteflag);
	if(suv && !deleteflag)
	{
		if(update)
		{
		...
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DELETEFLAG]",TRUE);
	} else {
		if(update)
		{
			updateeditfilersrcsnokeys(MASTER->passkey->fileno,MASTER->passkey->keyno,mtnrsrc,TRUE);
	...
	FINDRSCGETCHAR(mtnrsrc,"[LVETRAN][DOCK COMPUTED]",&suv);
	if(!suv)
	{
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPUTED]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPUTED BY]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPUTED DATE]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPUTED TIME]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK VALUE]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK UNITS]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCKED AMOUNT]",TRUE);
	}
	FINDRSCGETCHAR(mtnrsrc,"[LVETRAN][DOCK COMPLETED]",&suv);
	if(!suv)
	{
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPLETED]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPLETED BY]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPLETED DATE]",TRUE);
		FINDRSCSETEDITABLE(mtnrsrc,"[LVETRAN][DOCK COMPLETED TIME]",TRUE);
	}
}
